...
, which are here referred to as "NOTIFY(SOA)", are sent
The original DNS notifications
...
[RFC7477] [RFC7583] [RFC8078] [RFC8901]
, and
,
found in
...
, including
,
. DNS-specific terminology can be
...
This is addressed by parameterizing the lookup with the name of the child. The parent operator
may then announce the notification endpoint in a delegation-specific way by publishing it at a
child-specific name. (A catch-all endpoint may be indicated by wildcarding.)

The solution specified here is thus for the parent operator to publish the address where someone
listens for notifications, in a child-specific way (see Section 3). Potential senders can easily
determine the name of the parent and then look up that information (see Section 4.1).
...
for (see the "Resource Record (RR) TYPEs" registry at
parameters/>
indicating an updated CDS or CDNSKEY record set.

). For now, only CDS and CSYNC are supported values, with the former
...
described in this document, and values 128-255 are Reserved for Private Use. Other values
are currently unassigned. Future assignments are maintained in the registry created in
Section 6.2.
...
The transport port number on the target host of the notification service. This is a 16-bit

unsigned integer in network byte order. Records with value 0 are ignored by consumers.
...
service of listening for generalized notifications of the specified type. This name
...
and port number that correspond to that DSYNC record, using conventional DNS transport
[RFC1035]
.
...
Parent operators participating in the discovery scheme for the purpose of delegation
maintenance notifications
Section 2 under the _dsync subdomain of the parent zone, as described in the following
subsections.

 publish endpoint information using the record type defined in
...
MUST NOT
 ...
 be more than one DSYNC record for each combination of RRtype and Scheme. It

RECOMMENDED

 that zones containing DSYNC records be secured with DNSSEC.

For practical purposes, the parent operator
MAY
...
delegations, or if the parent operator wants to relay notifications to some other party, a default
...
NOTIFY messages for delegation maintenance
with the qtype field replaced as appropriate.

MUST

 be formatted as described in
...
initiate the same DNS lookups and verifications for DNSSEC bootstrapping
maintenance
...
designated party) is listening for CSYNC notifications.
 ...
 inefficiency may be similarly treated, with the child sending a
...
For example, assume that subsub.sub.child.example is delegated from example (and
...
4.1.
 ...
 send a suitable notification to one of the endpoints discovered as described in Section

A NOTIFY message can only carry information about changes concerning one child zone. When
there are changes to several child zones, the sender
 send a separate notification for each
one.
...
Therefore, it is
until a consistent public view of the pertinent records could be ensured.

RECOMMENDED

 that the child would delay sending notifications to the recipient

4.2.1.  Timeouts and Error Handling

NOTIFY messages are expected to elicit a response from the recipient (
no response is received, senders
[RFC1996]
...
SHOULD
 ...
 employ the same logic as for SOA notifications
...
In order to learn about such failures, senders
[RFC9567]
a report query with an appropriate extended DNS error (EDE) code as described in
(The prohibition of this option in queries (
and thus does not cover NOTIFY messages.)

[RFC9567], Section 6.1

MAY

 include an EDNS0 Report-Channel option

) only applies to resolver queries

[RFC8914]
.

When including this EDNS0 option, the second label (QTYPE) of the report query name is equal
to the qtype received in the NOTIFY message. Its agent domain
 be subordinate or equal to
one of the NS hostnames, as listed in the child's delegation in the parent zone. This is to prevent
malicious senders from causing the NOTIFY recipient to send unsolicited report queries to
unrelated third parties.

MUST

For example, when receiving a NOTIFY(CDS) message for "example.com" with agent domain
"errors.ns1.example.net", and when the requested DS update is found to break the delegation,
then the following report query with EDE code 6 (DNSSEC Bogus) may be made (preferably over
TCP):
...
This opens up the possibility of having an arbitrary party (e.g., a service separate from a
nameserver) send the notifications, enabling this functionality even before the emergence of
built-in support in nameserver software.

4.3.  Processing of NOTIFY Messages for Delegation Maintenance

The following algorithm applies to NOTIFY(CDS) and NOTIFY(CSYNC) processing.

NOTIFY messages carrying notification payloads (records) for more than one child zone
...
Acknowledge receipt by sending a NOTIFY response as described in
...
processing party
 report any errors occurring during CDS/CDNSKEY/CSYNC
processing by sending a report query with an appropriate EDE code as described in
[RFC8914]

. Reporting may be done asynchronously (outside of the NOTIFY transaction).
...
a week). If a CDS/CDNSKEY/CSYNC change is later detected through scanning (without
having received a notification), the NOTIFY-related state
default scanning schedule for this child.

 be cleared, reverting to the
...
apply; a low-frequency scanning schedule
MAY

 thus be used by default in such cases.

2.

Do not act upon the notification. To prevent retries, recipients
...
security and authentication requirements (e.g.,
chosen to apply.

) that the parent registry/registrar has
...
However, when the outgoing query occurs via encrypted transport, some amplification is
possible, both with respect to bandwidth and computational burden. In this case, the usual
principle of bounding the work applies, even under unforeseen events.

MUST

 implement rate limiting for notification processing. It is
Therefore, receivers
...
Requests to register additional entries
...
needs to have sufficient information to identify what the point is being used for. A scheme
number may have exactly one mnemonic.
...
, IANA has registered the following entries to the "Underscored and Globally
...
, and
Delegation Trust Maintenance" RFC 7344 DOI 10.17487/RFC7344
,
2014,

<https://www.rfc-editor.org/info/rfc7344>

G. Barwood "Automating DNSSEC
...
M. Larson "DNS Error Reporting" RFC 9567 DOI 10.17487/
...
Thomassen, P.
Authenticated Signals from the Zone's Operator" RFC 9615 DOI 10.17487/
RFC9615

N. Wisiol "Automatic DNSSEC Bootstrapping Using
...
Appendix A.  Efficiency and Convergence Issues in DNS
Scanning
...
convergence time for secondary servers when a DNS zone had been updated in the primary
server. The basic idea was to augment the original "pull" mechanism (a periodic SOA query)
with a "push" mechanism (a NOTIFY) for a common case that was otherwise very inefficient (due
to either slow convergence or wasteful and overly frequent scanning of the primary for
changes).
...
[RFC1996]
frequently checking for new versions of a zone and infrequent checks by replacing scheduled
scanning with the more efficient NOTIFY mechanism.

 addressed the optimization of the time-and-cost trade-off between a secondary server
...
Only a very small number of the delegations will have updated a CDS or CDNSKEY record in
between two scanning runs. However, frequent scanning for CDS and CDNSKEY records is costly,
and infrequent scanning causes slower convergence (i.e., delay until the DS RRset is updated).
...
The authors acknowledge the contributions and reviews of the following individuals (in order of
their first contribution or review):
Guðmundsson Paul Wouters Brian Dickson Warren Kumari Geoff Huston Patrick Mevzek Tim

Joe Abley Mark Andrews Christian Elmerot Ólafur
,
,
...
Wicinski Q Misell Stefan Ubbink Matthijs Mekking Kevin P. Fleming Nicolai Leymann
,
Giuseppe Fioccola Peter Yee Tony Li Paul Wouters Roman Danyliw Peter van Dijk John
Scudder Éric Vyncke
