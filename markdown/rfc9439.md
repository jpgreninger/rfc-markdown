...
Section 14.2 of [RFC7285]
the "ALTO Cost Metrics" registry (
...
generic "routingcost" metric (
metrics are defined in the base protocol.

) as a systematic mechanism to allow

); no latency- or bandwidth-related
...
Engineering (TE) performance metrics commonly defined in OSPF
[RFC5305] [RFC8570]
...
. Deriving ALTO cost performance

[RFC3630] [RFC7471]

[RFC8571]

[RFC7971] [FlowDirector]
.
...
[RFC3630]
Maximum (Link) Bandwidth as defined in
...
(Link) Bandwidth and Unreserved Bandwidth as defined in
...
The seventh metric in Table 1 (the estimated TCP-flow throughput metric) provides an estimation
of the bandwidth of a TCP flow, using TCP throughput modeling, to support use cases of adaptive
applications
[RFC9000]
future. For example, QUIC-related metrics
for measuring such metrics is more mature (e.g., see

 can be considered when the methodology
[QUIC-THROUGHPUT-TESTING]

. Note that other transport-specific metrics can be defined in the
...
; this document provides a definition for it.
...
Section 14.2 of [RFC7285]
the context of ALTO. This document follows the guidelines defined in
...
representation of each metric in Table 1, this document conforms to
 but specifies
...
metric in Table 1 will be specified as non-negative (>= 0); Hop Count is specified to be an integer.
 ...
 for JSON data type specifications. In particular,
...
performance metrics, and for guidance only. A fine-grained framework as specified in
...
resources.
 ...
 announce multiple CostType entries with the same "cost-
...
"estimation". The "cost-source" field of the "cost-context" field
 be one that is registered in
...
"target" or "committed" values. For an "sla" metric, it is
...
information from routing protocols (e.g., see
...
), and measurement frameworks (e.g., IPPM), with corresponding operational issues.
...
"estimation".
 ...
 assume that the value of "cost-source" is the most generic source, i.e.,
...
, instead of a single value. A statistical operator is applied to the

[Prometheus]

Hence, this document extends the general ASCII alphanumeric cost metric strings, formally
specified as the CostMetric type defined in
...
U+003A), if the statistical operator string exists. The total length of the cost metric string
MUST
NOT

 exceed 32, as required by

[RFC7285]
.

The statistical operator string
...
The instantaneous observation value of the metric from the most recent sample (i.e., the
...
Gives the percentile specified by the
 be a non-negative JSON number in the

percentile, with the letter 'p' followed by a number:
number following the letter 'p'. The number
...
for a given percentile, it is
 that the common form be used; that is, instead of
...
operator, the metric
 ...
 be interpreted as the default statistical operator in the definition of the base

 be considered the median value.
...
[RFC7285]
 ...
 limits the overall cost metric identifier to 32 characters. The cost metric
...
dropped before reaching the destination (pkt.dropped), and the number of network hops that the
...
To specify the temporal and spatial aggregated delay of a stream of packets

from the specified source to the specified destination. The base semantics of the metric is the
Unidirectional Delay metric as defined in
, but instead of
...
delay metric is provided in
. The spatial aggregation level is specified in the query
context, e.g., provider-defined identifier (PID) to PID, or endpoint to endpoint, where the PID
is as defined in

Section 5.1 of [RFC7285]
...
This metric could be used as a cost metric constraint attribute or as a returned cost metric
...
Many networks provide delay-related parameters in their application-level SLAs. It is
 that the "parameters" field of an "sla" one-way delay metric include a link
RECOMMENDED
...
"estimation": The exact estimation method is outside the scope of this document. There can be
...
Link Delay metrics for OSPF
 that the "parameters" field of an "estimation" one-way delay metric include the
RECOMMENDED
following information: (1) the RFC defining the routing protocol metrics (e.g., see
 for
...
routing link metrics, as
, and
Link Delay.

 provide Min/Max Unidirectional
...
the IPPM metric defined in the IPPM "Performance Metrics" registry
<https://
www.iana.org/assignments/performance-metrics/OWDelay_Active_IP-UDP-Poisson-
Payload250B_RFC8912sec7_Seconds_95Percentile>
(i.e., IPPM OWDelay* metrics). The statistical operator of the ALTO metric
...
MUST
 ...
 be non-negative. The unit is

4.2.3.  Intended Semantics and Use

Intended Semantics:

To specify temporal and spatial aggregated round-trip delay between the
...
definition of the end-to-end round-trip delay metric is provided in
...
Note that it is possible for a client to query two one-way delay (delay-ow) items and then
...
See the "estimation" entry in Section 4.1.4. For estimation by aggregation of

routing protocol link metrics, the aggregation should include all links from the source to the
 be
destination and then back to the source; for estimation using IPPM, the IPPM metric
MUST
round-trip delay (i.e., IPPM RTDelay* metrics). The statistical operator of the ALTO metric
MUST

 be consistent with the IPPM statistical property (e.g., 95th percentile).
...
To specify temporal and spatial aggregated delay variation (also called
...
query context (e.g., PID to PID, or endpoint to endpoint).
 ...
 allows the specification of a generic
...
sum of the link delay variations; for estimation using IPPM, the IPPM metric
MUST
variation (i.e., IPPM OWPDV* metrics). The statistical operator of the ALTO metric
...
MUST
 ...
 be non-negative. The value

4.4.3.  Intended Semantics and Use

Intended Semantics:

To specify the temporal and spatial aggregated one-way packet loss rate
from the specified source and the specified destination. The base semantics of the metric is
the Unidirectional Link Loss metric as defined in
, but
...
Typically, the packet loss rate does not have a nominal value, although some
...
correlated loss rates. For estimation using IPPM, the IPPM metric
IPPM OWLoss* metrics). The statistical operator of the ALTO metric
...
The hop count (hopcount) metric is mentioned in
...
To specify the number of hops in the path from the specified source to the
...
The exact estimation method is outside the scope of this document. An example of

estimating hop count values is by importing from IGP routing protocols. It is
that the "parameters" field of an "estimation" hop count define the meaning of a hop.
...
This section introduces three metrics related to throughput and bandwidth. Given a specified
source and a specified destination, these metrics reflect the volume of traffic that the network
can carry from the source to the destination.
...
To give the throughput of a congestion control conforming TCP flow from

the specified source to the specified destination. The throughput
only an estimation, and the estimation is designed only for bulk flows.
...
Typically, TCP throughput does not have a nominal value and
...
Typically, TCP throughput does not have an SLA value and
...
The exact estimation method is outside the scope of this document. It is

RECOMMENDED
the following information: (1) the congestion control algorithm and (2) the estimation

 that the "parameters" field of an "estimation" TCP throughput metric include
...
RECOMMENDED
 ...
 that the "parameters" field (object) include a
methodology. To specify (1), it is
field named "congestion-control-algorithm", which provides a URI for the specification of the
algorithm; for example, for an ALTO server to provide estimation of the throughput of a
CUBIC congestion control flow, its "parameters" field includes the "congestion-control-
; for an ongoing congestion
algorithm" field, with value being set to the URI for
control algorithm such as BBR, a link to its specification can be added. To specify (2), the
"parameters" field includes as many details as possible; for example, for the TCP Cubic
throughout estimation, the "parameters" field specifies that the throughput is estimated by
setting _C_ to 0.4, and the equation in
alternative, the methodology may be based on the NUM model
described in
document.

. The exact specification of the "parameters" field is outside the scope of this

, Figure 8 is applied; as an
 or the model
...
The metric value type is a single 'JSONNumber' type value that is non-negative. The unit of
measurement is bytes per second.
...
To specify temporal and spatial residual bandwidth from the specified
source to the specified destination. The base semantics of the metric is the Unidirectional
, but instead of
Residual Bandwidth metric as defined in
specifying the residual bandwidth for a link, it is the residual bandwidth of the path from the
source to the destination. Hence, it is the minimal residual bandwidth among all links from
the source to the destination. When the max statistical operator is defined for the metric, it
typically provides the minimum of the link capacities along the path, as the default value of
the residual bandwidth of a link is its link capacity
. The
spatial aggregation unit is specified in the query context (e.g., PID to PID, or endpoint to
endpoint).

[RFC8571] [RFC8570] [RFC7471]

[RFC7471]

, and

The default statistical operator for residual bandwidth is the current instantaneous sample;
that is, the default is assumed to be "cur".
...
See the "estimation" entry in Section 4.1.4. The current ("cur") residual bandwidth

of a path is the minimal residual bandwidth of all links on the path.
...
To specify temporal and spatial available bandwidth from the specified

source to the specified destination. The base semantics of the metric is the Unidirectional
Available Bandwidth metric as defined in
specifying the available bandwidth for a link, it is the available bandwidth of the path from
the source to the destination. Hence, it is the minimal available bandwidth among all links
from the source to the destination. The spatial aggregation unit is specified in the query
...
The default statistical operator for available bandwidth is the current instantaneous sample;
that is, the default is assumed to be "cur".
...
See the "estimation" entry in Section 4.1.4. The current ("cur") available

bandwidth of a path is the minimum of the available bandwidth of all links on the path.
...
The exact measurement infrastructure, measurement conditions, and computation algorithms
can vary between different networks and are outside the scope of this document. Both the ALTO
server and the ALTO clients, however, need to be cognizant of the operational issues discussed in
the following subsections.

Also, the performance metrics specified in this document are similar in that they may use similar
data sources and have similar issues in their calculation. Hence, this document specifies issues
that the performance metrics might have in common and also discusses challenges regarding the
computation of ALTO performance metrics (Section 6.4).

6.1.  Source Considerations

The addition of the "cost-source" field solves a key issue: an ALTO server needs data sources to
compute the cost metrics described in this document, and an ALTO client needs to know the data
sources to better interpret the values.

To avoid information that is too fine grained, this document introduces "cost-source" to indicate
only the high-level types of data sources: "estimation", "nominal", or "sla", where "estimation" is a
type of measurement data source, "nominal" is a type of static configuration, and "sla" is a type
that is based more on policy.

For example, for "estimation", the ALTO server may use log servers or the Operations,
Administration, and Maintenance (OAM) system as its data source, as recommended by
[RFC7971]
routing systems as the data sources.

. In particular, the cost metrics defined in this document can be computed using

6.2.  Metric Timestamp Considerations

Despite the introduction of the additional "cost-context" information, the metrics do not have a
field to indicate the timestamps of the data used to compute the metrics. To indicate this
attribute, the ALTO server
of the data used to compute the performance metrics.

 return an HTTP Last-Modified value to indicate the freshness

SHOULD

If the ALTO client obtains updates through an incremental update mechanism
client
approximated by the receiving time.

 assume that the metric is computed using a snapshot at the time that is
...
One potential issue introduced by the optional "cost-source" field is backward compatibility.
Consider the case where an IRD defines two "cost-type" entries with the same "cost-mode" and
"cost-metric", but one with "cost-source" being "estimation" and the other being "sla". In such a
case, an ALTO client that is not aware of the extension will not be able to distinguish between
...
these two types. A similar issue can arise even with a single "cost-type" whose "cost-source" is
"sla": an ALTO client that is not aware of this extension will ignore this field and instead consider
the metric estimation.

To address the backward-compatibility issue, if a "cost-metric" is "routingcost" and the metric
contains a "cost-context" field, then it
the information as invalid.

 be "estimation"; if it is not, the client
...
The metric values exposed by an ALTO server may result from additional processing of
measurements from data sources to compute exposed metrics. This may involve data processing
tasks such as aggregating the results across multiple systems, removing outliers, and creating
additional statistics. The computation of ALTO performance metrics can present two challenges.

6.4.1.  Configuration Parameter Considerations

Performance metrics often depend on configuration parameters, and exposing such
configuration parameters can help an ALTO client to better understand the exposed metrics. In
particular, an ALTO server may be configured to compute a TE metric (e.g., packet loss rate) at
fixed intervals, say every T seconds. To expose this information, the ALTO server may provide the
client with two pieces of additional information: (1) when the metrics were last computed and (2)
when the metrics will be updated (i.e., the validity period of the exposed metric values). The
ALTO server can expose these two pieces of information by using the HTTP response headers
Last-Modified and Expires.

6.4.2.  Aggregation Computation Considerations

An ALTO server may not be able to measure the performance metrics to be exposed. The basic
issue is that the "source" information can often be link-level information. For example, routing
protocols often measure and report only per-link loss and not end-to-end loss; similarly, routing
protocols report link-level available bandwidth and not end-to-end available bandwidth. The
ALTO server then needs to aggregate these data to provide an abstract and unified view that can
be more useful to applications. The server should be aware that different metrics may use
different aggregation computations. For example, the end-to-end latency of a path is the sum of
the latencies of the links on the path; the end-to-end available bandwidth of a path is the
minimum of the available bandwidth of the links on the path; in contrast, aggregating loss values
is complicated by the potential for correlated loss events on different links in the path.

7.  Security Considerations

The properties defined in this document present no security considerations beyond those in
[RFC7285]
.
Section 15 of the base ALTO specification

However, concerns addressed in Sections 15.1, 15.2, and 15.3 of
importance. Indeed, TE performance is highly sensitive ISP information; therefore, sharing TE
metric values in numerical mode requires full mutual confidence between the entities managing
the ALTO server and the ALTO client. ALTO servers will most likely distribute numerical TE
...
performance to ALTO clients under strict and formal mutual trust agreements. On the other
hand, ALTO clients must be cognizant of the risks attached to such information that they would
have acquired outside formal conditions of mutual trust.

To mitigate confidentiality risks during information transport of TE performance metrics, the
operator should address the risk of ALTO information being leaked to malicious clients or third
parties through such attacks as person-in-the-middle (PITM) attacks. As specified in Section
("Protection Strategies")
transmitting an ALTO information resource containing sensitive TE performance metrics.
Section
...
, the ALTO server should authenticate ALTO clients when

8.3.5 ("Authentication and Encryption")

 support the "https" URI scheme

 specifies that ALTO server
...
IANA created and now maintains the "ALTO Cost Metrics" registry, as listed in
14.2
has added the following entries to the "ALTO Cost Metrics" registry.

, Table 3. This registry is located at
...
IANA has created the "ALTO Cost Source Types" registry. This registry serves two purposes. First,
it ensures the uniqueness of identifiers referring to ALTO cost source types. Second, it provides
references to particular semantics of allocated cost source types to be applied by both ALTO
servers and applications utilizing ALTO clients.
...
A new ALTO cost source type can be added after IETF Review
documentation regarding the new ALTO cost source type and its security considerations has been
provided. The RFC(s) documenting the new cost source type should be detailed enough to
provide guidance to both ALTO service providers and applications utilizing ALTO clients as to
how values of the registered ALTO cost source type should be interpreted. Updates and deletions
of ALTO cost source types follow the same procedure.

, to ensure that proper

[RFC8126]

Registered ALTO address type identifiers
in Section 3.1. Identifiers are to be recorded and displayed as strings.

MUST

 conform to the syntactical requirements specified

Requests to add a new value to the registry
...
ALTO cost source types carry with them semantics to guide their usage by
...
ALTO endpoint property should be interpreted.

Security Considerations:

ALTO cost source types expose information to ALTO clients. ALTO

service providers should be made aware of the security ramifications related to the exposure
...
Values in nominal cases (Section 3.1)

Section 7

sla

Values reflecting Service Level
...
Values by estimation (Section 3.1)
...
IANA "Performance Metrics" <https://www.iana.org/assignments/performance-
metrics/>
...
Pujol, E. Poese, I. Zerwas, J. Smaragdakis, G.
Hyper-Giants' Traffic at Scale" ACM CoNEXT '19
...
Ros-Giralt, J. Bohara, A. Yellamraju, S. Harper Langston, M. Lethin, R. Jiang, Y.
,
,
,
Tassiulas, L. Li, J. Tan, Y.
M. Veeraraghavan "On the Bottleneck Structure
,
of Congestion-Controlled Networks" Proceedings of the ACM on Measurement
and Analysis of Computing Systems, Vol. 3, No. 3, Article No. 59, pp. 1-31 DOI
10.1145/3366707
.
...
Volz, J.
,
(Talk)" SREcon15 Europe
, May 2015.

 and
,

B. Rabenstein "Prometheus: A Next-Generation Monitoring System

,

,

Zhang, J. Gao, K. Yang, YR.
Model-Based Throughput Prediction for Reactive Flows in DC Networks" IEEE/
ACM Transactions on Networking, Volume 28, Issue 601, pp. 2475-2488
December 2020,

<https://dl.acm.org/doi/10.1109/TNET.2020.3016838>

J. Bi "Prophet: Toward Fast, Error-Tolerant
...
Corre, K. "Framework for QUIC Throughput Testing" Work in
...
Progress Internet-Draft, draft-corre-quic-throughput-testing-00
2021,
testing-00>

<https://datatracker.ietf.org/doc/html/draft-corre-quic-throughput-
...
Layer Traffic Optimization (ALTO) Deployment Considerations" RFC 7971 DOI
10.17487/RFC7971

S. Previdi "Application-
,
,
...
thorough AD reviews and comments. We thank
,
Gao Geng Li Lili Liu Danny Alex Lachos Perez
comments. We thank
Benjamin Kaduk Erik Kline Francesca Palombini Lars Eggert Martin
,
Vigoureux Murray Kucherawy Roman Danyliw Zaheduzzaman Sarker
 for
, and
,
discussions and comments that improved this document.

Christian Amsüss Elwyn Davies Haizhou Du Kai
, and
,
