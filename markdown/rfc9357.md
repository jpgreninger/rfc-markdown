...
RFC 9357
Label Switched Path (LSP) Object Flag Extension for
Stateful PCE
...
As defined in
, the length of the Flag field is 12 bits, and all of the bits have already
...
This indicates the length of the value portion in bytes. It
...
significant as bit zero, where each bit represents one LSP flag (for operation, feature, or state).
 use the minimal amount of space needed to encode the
The LSP Extended Flags field
 be set to zero on transmission
flag bits. Currently, no bits are assigned. Unassigned bits
and
...
particular flag
 ...
 be set to zero on transmission. Implementations that do not understand any
...
by the implementation, it

 act as if the bits beyond the length were not set.
...
Following the model provided in
...
, we provide the following advice for
...
cases in which the LSP-EXTENDED-FLAG TLV is missing when it
...
. Future documents that define bits
...
LSP-EXTENDED-FLAG TLV will take that into consideration.
 ...
 be ignored. It is expected that future documents that define bits in the
...
of the LSP-EXTENDED-FLAG TLV. New values are assigned by Standards Action
...
followed, then the flags will be protected by TLS.
 ...
 recommends that the stateful PCEP extension be authenticated and

[RFC8253] [PCEPS-TLS1.3]
[RFC9325]

. Assuming that the recommendation is
...
Binding Label/Segment Identifier (SID) in PCE-based Networks." Work in
...
F. Qin "PCEP Extension for SR-MPLS Entropy
, and
,
Label Position" Work in Progress Internet-Draft, draft-peng-pce-entropy-label-
,
position-08
pce-entropy-label-position-08>

<https://datatracker.ietf.org/doc/html/draft-peng-
...
R. Housley "PCEPS with TLS 1.3" Work in Progress
...
Internet-Draft, draft-dhody-pce-pceps-tls13-01
datatracker.ietf.org/doc/html/draft-dhody-pce-pceps-tls13-01>
...
useful.
 ...
 use the same approach for the PCE-CAP-FLAGS sub-TLV and are found to be
...
Loa Andersson Adrian Farrel Aijun Wang
