...
Separation Protocol (LISP)
 context to perform packet encapsulation. The
mechanism is totally transparent to Ingress and Egress Tunnel Routers (xTRs) not supporting or
not using such functionality. The architecture of LISP is described in
...
, which is the initial experimental specification that describes
...
the LISP-specific header as specified in
permissible combinations of the flags when the V-bit is set to 1 are described in
...
LISP-specific header has the following encoding:
 ...
 and shown in the example in Figure 1. All
...
) is reported here as an example in Figure 2. This memo does
Map Record (specified in
...
This packet format is backward compatible with xTRs that do not support Map-Versioning, since
...
one,
 ...
 silently drop the message and an appropriate log action
...
for the Null Map-Version as explained in at the end of Section 6.1).
 ...
 consist of an increment of the older version number (the only exception is
...
is basically a serial number on which the arithmetic described in
...
expressed on 12 bits, the following steps
strictly define their order:

 be performed (in the same order shown below) to
...
be configured.
 ...
 be set to the Null Map-Version value (0x000), because the Null Map-Version
...
mappings in the EID-to-RLOC Database.
 ...
 automatically generate and assign Map-Version numbers to
...
EID-Prefix referred to by the Map Record
 contain any Map-Version numbers (V-bit set
...
updating a Map-Version number because of a change in the mapping, if the next value is 0, then
 be incremented by 2 (i.e., set to 1 (0x001), which is the next valid
...
Map-Version number. A check on this version number
can arise:

 be done, where the following cases
...
meaning that the Map-Version number of its mapping is the correct one, the packet carries a
 be silently dropped
version number that is not considered valid. Therefore, the packet
and an appropriate log action
SHOULD
...
; however, the
 be informed that a newer mapping is available, respecting rate-
...
ITR sending the packet
. This is done with a Map-Request message sent
limitation policies described in
back to the ITR, as specified in
...
attack. According to the rate-limitation policy defined in
messages, after 10 retries, Map-Requests are sent every 30 seconds; if after the first 10 retries
the Dest Map-Version number in the packets is not updated, the ETR
SHOULD
...
MAY
 ...
 be more restrictive. If the Record TTL of the previous mapping has

The rule in the third case
 be silently dropped right
already expired, all packets arriving with an old Map-Version
...
EID-to-RLOC Map-Cache for the source EID, then a check
cases can arise:
...
RLOC Map-Cache a mapping that is stale and needs to be updated. A Map-Request
...
recommendation, which are outside the scope of this document.
 ...
 be silently dropped. Operators can configure exceptions to this
...
situation can arise.
 ...
 be ignored. See Appendix A.1 for an example of when this
...
random Map-Versions. The Map-Requests are rate limited as described in
...
The specifications in the present memo are relatively conservative in the sense that, in several
...
The same problem can, however, arise without Map-Versioning, for instance, if the two ITRs of
Domain A send different Locator-Status-Bits. In this case, either the traffic is disrupted if ETR B
does not verify reachability or if ETR B will start sending Map-Requests to confirm each change
in reachability.

So far, LISP does not provide any specific synchronization mechanism but assumes that
synchronization is provided by configuring the different xTRs consistently. The same applies for
Map-Versioning. If in the future any synchronization mechanism is provided, Map-Versioning
will take advantage of it automatically, since it is included in the Map Record format, as
...
Iannone, L. Saucez, D.
,
(LISP) Map-Versioning" RFC 6834 DOI 10.17487/RFC6834
www.rfc-editor.org/info/rfc6834>
...
. LISP interworking defines three techniques to allow communication LISP
...
A Proxy-ETR does not have any mapping, since it just decapsulates packets arriving from the
...
) but without actually turning it off.
