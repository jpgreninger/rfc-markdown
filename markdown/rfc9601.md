...
RFC 9601
Propagating Explicit Congestion Notification across
...
[RFC3168]
 ...
 consistent for all forms of IP-in-IP tunnel.
...
Problem 1: It was intended to include cases where one or more shim headers sit between the IP
headers. Many tunnelling implementers have interpreted the scope of
intended, but it is ambiguous. Therefore, this specification updates
...
been unreasonable to add such a requirement to
...
Digging to arbitrary depths to find an inner IP header within an encapsulation is strictly a
...
optimization. The guidelines in
violation to be benign.

 give the conditions for this layering
...
. Nonetheless, propagation of ECN between IP
...
Therefore, this specification updates
the end of the section:

Section 4.3 of [RFC6040]

 by inserting the following text at
...
[RFC3168]
 be configured to zero the outer ECN field in all of the following cases:
...
[RFC6040] [RFC4301]
,
propagation of the ECN field (
[RFC3168]

, or the full functionality mode of
...
with the definition of the 6-bit Diffserv field in
...
" makes compliant implementations non-

There might be concern that the above "
MUST NOT
...
[RFC3260]
been non-compliant with the definition of the 6-bit Diffserv field in
...
sufficient to claim compliance with
 because it would not meet the aim of introducing
...
The following requirements update
...
, which omitted handling of the ECN field during

[RFC6040]

If a tunnel ingress fragments a packet, it
same value as it would have set if it had not fragmented the packet.

MUST

 set the outer ECN field of all the fragments to the
...
tunnel egress can reassemble the fragments
...
During reassembly of outer fragments, the packet
...
Capable Transport (Not-ECT) and other ECN codepoints.
 ...
 be discarded if the ECN fields of the

MUST

•

If there is mix of ECT(0) and ECT(1) outer fragments, then the reassembled packet
set to ECT(1).
...
[RFC3168]
 ...
 originally defined ECT(0) and ECT(1) as equivalent, but

Reasoning:
 to make ECT(1) available for congestion marking differences. The
been updated by
rule is independent of the current experimental use of ECT(1) for Low Latency, Low Loss, and
Scalable throughput (L4S)
. The rule is compatible with Pre-Congestion Notification
, which uses 2 levels of congestion severity, with the ranking of severity from
(PCN)
...
the avoidance of doubt, the updated scope of
 is defined in Section 3 and is not limited
to this list.
...
[RFC3931]
Layer Two Tunneling Protocol (L2TP), specifically L2TPv2
,
...
[RFC2784]
 ...
 and Network Virtualization using GRE

•

GPRS Tunnelling Protocol (GTP), specifically GTPv1
...
[RFC7348]
 ...
 and Generic Protocol Extensions
...
and/or outer. This specification applies to the cases where there is an inner and outer IP header
 gives guidance on how to design propagation of
as described in Section 3. Otherwise,
...
control, it is
 that implementations of encapsulation and decapsulation comply
with
requirement to comply with

 that their specifications are updated to add a
...
. However, there is no need for the present specification to update PPTP because

[RFC2637]

NVGRE is not under the change control of the IETF, but it has been documented in an
Informational RFC
from the initial specification of GRE
updates GRE in Section 6.1.2 below is also intended to update NVGRE.

. NVGRE is a specific use case of GRE (it re-purposes the key field
 as a Virtual Subnet ID). Therefore, the text that
...
The specification of CAPWAP already specifies
negotiation. Without modification, the CAPWAP specification already interworks with the
 in
backward-compatible updates to

 ECN propagation and ECN capability
...
LISP made the ECN propagation procedures in
has since been updated by
...
, but the changes are backwards compatible, so there is still
...
VXLAN is not under the change control of the IETF, but it has been documented in an
Informational RFC. It is
...
documented under IETF change control and it does require compliance with
 ...
 as updated by the present specification. In contrast, VXLAN-GPE is being
[RFC6040]
.

 that VXLAN implementations comply with
...
. A proposal has been made for the processing of ECN when handling transport
 ...
 has been defined as a shim-based encapsulation to
...
The specification of Geneve already refers to

[RFC6040]

 for ECN encapsulation.

 already explains that a tunnel that encapsulates an IP header within

Section 3.1.11 of [RFC8085]
a UDP/IP datagram needs to follow
outer IP headers. Section 3 of the present specification updates
...
[RFC8085]
 ...
 when propagating the ECN field between inner and
 to clarify that its scope

[RFC6040]

[RFC6040]

The requirements in Section 4 update
[RFC8085]
guidelines in
...
, and hence also indirectly update the UDP usage

[RFC6040]

Section 9.5 of [RFC9329]
...
The L2TP terminology used here is defined in
...
[RFC3931]
 ...
 is used as a shim header between any packet-switched network (PSN) header
...
are therefore within the scope of

, as updated by Section 3.
...
Implementation of the ECN extension to L2TPv2 and L2TPv3 defined in Section 6.1.1.2 is
[RFC8087]
RECOMMENDED
L2TP tunnel becomes the bottleneck for an end-to-end traffic flow.

 in order to provide the benefits of ECN

 whenever a node within an

6.1.1.1.  Safe Configuration of a "Non-ECN" Ingress LCCE

The following text is appended to both
an update to the base L2TPv2 and L2TPv3 specifications:

Section 5.3 of [RFC2661]

 and

Section 4.5 of [RFC3931]
...
it clears the outer IP ECN field to 0b00 when the outer PSN header is IP (v4 or v6).
 ...
 follow the configuration requirements in Section 4 of RFC 9601 to ensure
...
between inner and outer IP headers
extension of L2TP

 be independent of any configuration of the Diffserv
...
egress LCCE supports ECN propagation as defined in
...
(clearing the outer IP ECN field to 0b00).
 ...
 requires an ingress LCCE to check that the

 or the full functionality mode of

 or one of its compatible
...
[RFC5129]
ECN propagation specification will need to be followed, e.g.,
[RFC9599]
specification exists for ECN propagation by a particular PSN,
...
MAY
 ...
 be present in the Start-Control-Connection-Request (SCCRQ) and Start-Control-
MAY

This AVP
Connection-Reply (SCCRP) message types. This AVP
...
the IETF Vendor ID of 0.
 ...
 be hidden (the H-bit is set to 0 or 1) and
...
any sessions created by that control connection, the tunnel initiator will then use the
 to clear the ECN field of the outer IP header to 0b00.
...
The GRE terminology used here is defined in
shim header between IP headers. Sometimes, the GRE shim header encapsulates an L2 header,
[RFC6040]
which might in turn encapsulate an IP header. Therefore, GRE is within the scope of
updated by Section 3.

. GRE is often used as a tightly coupled
...
tunnelled over GRE using IP as the delivery protocol (outer header).
 ...
 as updated by the present specification is
...
protocols, such as Next Hop Resolution Protocol (NHRP)
[RFC5944]
[RFC7296]
...
will propagate the ECN field as defined in
[RFC4301]
(
...
Section 3 of [RFC2784]
 ...
 as an update to the base GRE

The operator of a GRE tunnel ingress
Section 4 of RFC 9601 when the outer delivery protocol is IP (v4 or v6).

 follow the configuration requirements in
...
header between the two.
 ...
 provides a way to tunnel IPv6 over an IPv4 network with a UDP-based shim
...
defined in
 or one of its compatible predecessors (
functionality mode of

 zero the ECN field in the outer IPv6 header.
...
[RFC7450]
 ...
 is a tightly coupled shim header that encapsulates an IP packet and is

AMT
encapsulated within a UDP/IP datagram. Therefore, AMT is within the scope of
updated by Section 3.
...
, an AMT relay and gateway will follow the rules for propagation of the
...
Before encapsulating any data packets,
egress AMT gateway supports ECN propagation as defined in
predecessors (
...
means it has to clear the outer ECN field to zero
 ...
 requires an ingress AMT relay to check that the

 or one of its compatible

). If the egress gateway
...
When the E flag is set to 1, it indicates that the sender of the message supports
...
An AMT gateway that does not support
to 1.

[RFC6040] MUST NOT

 set the E flag of its Request Message

An AMT gateway that supports
...
datagrams (i.e., it copies the IP ECN field from inner to outer header).
 ...
 ECN propagation, it will store the ECN capability of the

 to propagate the ECN field when encapsulating

 use the normal mode of
...
If the discovered AMT relay does not support
...
[RFC6040]
 ...
 ECN propagation, it will ignore the E

If the AMT relay does not support
expected to configure it to comply with the safety provisions set out in Section 6.1.4.1.

 ECN propagation, the network operator is still
...
[RFC9599]
 ...
 apply equally to the scope defined for
...
Briscoe, B. Moncaster, T.
Notification (PCN) States in the IP Header Using a Single Diffserv Codepoint
(DSCP)" RFC 6660 DOI 10.17487/RFC6660
info/rfc6660>

M. Menth "Encoding Three Pre-Congestion
...
J. Kaippallimalil "Guidelines for Adding Congestion Notification

Briscoe, B.
to Protocols that Encapsulate IP" RFC 9599 DOI 10.17487/RFC9599
,
...
Briscoe, B. "A Test for IP-ECN Propagation by a Remote Tunnel Endpoint"
Technical Report, TR-BB-2023-003 DOI 10.48550/arXiv.2311.16825
2023,
...
Work in Progress Internet-Draft, draft-ietf-intarea-tunnels-13
<https://datatracker.ietf.org/doc/html/draft-ietf-intarea-tunnels-13>
...
U. Elzur "Generic Protocol Extension for
...
Luciani, J. Katz, D. Piscitello, D. Cole, B.
,
,
Resolution Protocol (NHRP)" RFC 2332 DOI 10.17487/RFC2332
,
...
Grossman, D. "New Terminology and Clarifications for Diffserv" RFC 3260 DOI
10.17487/RFC3260
...
Calhoun, P. Luo, W. McPherson, D.
, and
Protocol (L2TP) Differentiated Services Extension" RFC 3308 DOI 10.17487/
RFC3308

<https://www.rfc-editor.org/info/rfc3308>

K. Peirce "Layer Two Tunneling
...
Muhanna, A. Khalil, M. Gundavelli, S.
, and
Encapsulation (GRE) Key Option for Proxy Mobile IPv6" RFC 5845 DOI 10.17487/
<https://www.rfc-editor.org/info/rfc5845>
RFC5845

K. Leung "Generic Routing
...
Perkins, C., Ed. "IP Mobility Support for IPv4, Revised" RFC 5944 DOI 10.17487/
RFC5944
...
Steffann, S. van Beijnum, I.
Tunnel Mechanisms" RFC 7059 DOI 10.17487/RFC7059
...
R. van Rein "A Comparison of IPv6-over-IPv4
...
Konstantynowicz, M., Ed. Heron, G., Ed. Schatzmayr, R.
"Keyed IPv6 Tunnel" RFC 8159 DOI 10.17487/RFC8159
www.rfc-editor.org/info/rfc8159>
...
B. Briscoe, Ed. "The Explicit Congestion Notification (ECN)
De Schepper, K.
...
Eastlake 3rd, D. Briscoe, B. Zhuang, S. Malis, A.
...
Congestion Notification (ECN) and Congestion Feedback Using the Network
Service Header (NSH) and IPFIX" Work in Progress Internet-Draft, draft-ietf-sfc-
nsh-ecn-support-13
ietf-sfc-nsh-ecn-support-13>
...
Thanks to
and its applicability. Thanks also to
Praveen Balasubramanian Joe Touch Mohamed Boucadair David Black Jake Holland Sri
,
[RFC7059]
Gundavelli Gorry Fairhurst
helped to identify a number of tunnelling protocols to include within the scope of this document.

 for initial discussions on the need for ECN propagation in L2TP
Carlos Pignataro Tom Herbert Ignacio Goyret Alia Atlas
...
,
 ...
 was part-funded by the Research Council of Norway through the TimeIn project for

Bob Briscoe
early drafts, and he was funded by Apple Inc. for later draft versions (from -17). The views
