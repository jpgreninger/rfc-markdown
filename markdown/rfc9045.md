...
X.509 Public Key Infrastructure Certiﬁcate Request
Message Format (CRMF)

Abstract

This document updates the cryptographic algorithm requirements for the Password-Based
Message Authentication Code in the Internet X.509 Public Key Infrastructure Certiﬁcate Request
Message Format (CRMF) speciﬁed in RFC 4211.
...
This document updates the cryptographic algorithm requirements for the Password-Based
Message Authentication Code (MAC) in the Internet X.509 Public Key Infrastructure Certiﬁcate
Request Message Format (CRMF)
appropriate in 2005; however, these algorithms are no longer considered the best choices:

. The algorithms speciﬁed in
...
[HMAC] [SHS]
 ...
 is not broken yet, but there are much stronger alternatives
...
.
 ...
 provides 56 bits of security, which is no longer considered secure
...
This update speciﬁes algorithms that are more appropriate today.

CRMF is deﬁned using Abstract Syntax Notation One (ASN.1)
...
.
 ...
 speciﬁes the proof-of-possession (POP) processing. This section is

Section 7.1 of

OLD:

algId identiﬁes the algorithm used to compute the MAC value. All implementations
...
.
Section 7.1 of [RFC8018]

 also support PBMAC1 as presented in
...
Section 4.4 of [RFC4211]
compute a symmetric key from the password and a MAC algorithm. This section speciﬁes
algorithm requirements for the one-way function and the MAC algorithm.

 speciﬁes a Password-Based MAC that relies on a one-way function to
...
password, the correct check value cannot be computed. The algorithm computes the
one-way function multiple times in order to slow down any dictionary attacks against
the password value. The password used to compute this MAC
 be used for
any other purpose.
...
owf identiﬁes the algorithm and associated parameters used to compute the key used in
the MAC process. All implementations
...
iterationCount identiﬁes the number of times the hash is applied during the key
computation process. The iterationCount
MUST
suggest using values as high as 1000 iterations as the minimum value. The trade oﬀ here
is between protection of the password from attacks and the time spent by the server
processing all of the diﬀerent iterations in deriving passwords. Hashing is generally
considered a cheap operation but this may not be true with all hash functions in the
future.

 be a minimum of 100. Many people
...
iterationCount
10,000
and the time spent by the server processing the iterations. As part of that trade-oﬀ, an
iteration count smaller than 10,000 can be used when automated generation produces
shared secrets with high entropy.

. There is a trade-oﬀ between protection of the password from attacks

 be as large as server performance will allow, typically at least

 be a minimum of 100; however, the
...
mac identiﬁes the algorithm and associated parameters of the MAC function to be used.
All implementations
SHOULD
...
For convenience, the identiﬁers for these two algorithms are repeated here.

The ASN.1 algorithm identiﬁer for HMAC-SHA256 is deﬁned in
...
When this object identiﬁer is used in the ASN.1 algorithm identiﬁer, the parameters
 contain a type of NULL as speciﬁed in
present. When present, the parameters
MUST
...
The ASN.1 algorithm identiﬁer for AES-GMAC
[RFC9044]
:

[AES] [GMAC]

 with a 128-bit key is deﬁned in
...
any number of bits between 8 and (2^64)-1, but it
 be a multiple of 8 bits. Within the scope
of any GMAC key, the nonce value
 be unique. A nonce value of 12 octets can be processed
more eﬃciently, so that length for the nonce value is
...
The GMACParameters length parameter ﬁeld tells the size of the message authentication code in
octets. GMAC supports lengths between 12 and 16 octets, inclusive. However, for use with CRMF,
the maximum length of 16 octets
...
calculation is available at very low cost
, which reduces the protection provided by a high
...
entropy of only 27 bits
strings. For example, assuming an alphabet of 60 characters, a randomly chosen password with
10 characters oﬀers 59 bits of entropy, and 20 characters oﬀers 118 bits of entropy. Using a one-
time password also increases the security of the MAC, assuming that the integrity-protected
transaction will complete before the attacker is able to learn the password with an oﬄine attack.

. Higher entropy can be achieved by using randomly generated
...
[RFC8018]
 ...
 for security considerations related to PBMAC1.
...
[SHS]
 ...
 for security considerations related to HMAC-SHA256.
...
[GMAC]
 ...
 for security considerations related to AES-GMAC.
...
For this reason, the algorithm requirements for CRMF are updated by this speciﬁcation.
...
Schaad, J. "Internet X.509 Public Key Infrastructure Certiﬁcate Request Message
,
...
,
,
Dell'Amico, M. Michiardi, P.
Analysis" DOI 10.1109/INFCOM.2010.5461951
doi.org/10.1109/INFCOM.2010.5461951>
...
Y. Roudier "Password Strength: An Empirical
...
,
Pathirana, A. Halgamuge, M.
Maximize the Mining Proﬁt: Using Data from 119 Bitcoin Mining Hardware
Setups" International Conference on Advances in Business Management and
Information Technology, pp. 1-14

A. Syed "Energy Eﬃcient Bitcoin Mining to
...
National Institute of Standards and Technology "Transitioning the Use of
,
Cryptographic Algorithms and Key Lengths" NIST Special Publication
, March 2019,
800-131Ar2 DOI 10.6028/NIST.SP.800-131Ar2
doi.org/10.6028/NIST.SP.800-131Ar2>
...
National Institute of Standards and Technology "NIST Withdraws Outdated Data
Encryption Standard"
news/2005/06/nist-withdraws-outdated-data-encryption-standard>
...
Many thanks to
Hans Aschauer Hendrik Brockhaus Quynh Dang Roman Danyliw Lars Eggert
,
Tomas Gustavsson Jonathan Hammell Tim Hollebeek Ben Kaduk Erik Kline Lijun Liao Mike
,
Ounsworth Francesca Palombini Tim Polk Ines Robles Mike StJohns
careful review and improvements.
