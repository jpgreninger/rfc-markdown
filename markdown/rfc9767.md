...
authorization to a piece of software (the client) and conveying the results and artifacts of that
delegation to the software. This extension defines methods for resource servers (RSs) to connect
with authorization servers (ASs) in an interoperable fashion.
...
access rights the access token is approved for.
 ...
 defines distinct roles for the authorization server (AS) and
...
trailing backslash \ to indicate line wrapping for long values, as per
...
[GNAP]
 ...
 focuses on the relationship between the client and the AS.
...
JSON Web Token (JWT)
 ...
 standard format are provided for each item in the model.
...
If structured tokens like those in
...
signature to validate and identify an individual token.
 ...
 are used, the value of the token might not be stored by

[JWT]

2.1.2.  Issuer

The access token is issued by the AS as defined in
...
. The AS will need to identify itself in
...
In the payload of a JSON Web Token
the iss claim.

[JWT]

 or a token introspection response, this corresponds to
...
[JWT]
 ...
 or token introspection response, this corresponds to
...
, this corresponds to the cnf (confirmation) claim. In a
...
[JWT]
nbf claim in the payload of a JSON Web Token
...
This identifier is not usually exposed to the client instance using the token, because the client
...
This corresponds to the client_id claim in the payload of a JSON Web Token
...
request can be continued using the API defined in
...
Since the grant itself does not need to be identified in any of the protocol messages, GNAP does
...
When an access token is used for the grant continuation API defined in
continuation access token), the token management API defined in
management access token), or the RS-facing API defined in Section 3 (the resource server
management access token), the AS
...
access tokens are also opaque to the RS.
 ...
 separate these access tokens from other access tokens
...
grant response in
. The client instance is given token management access
tokens only as part of the manage field of the grant response in
...
MUST
 ...
 fail. When introspection is used, the AS
...
uses token introspection internally, the AS
from those issued for use at an external RS.

 differentiate these AS-specific access tokens

 return an active value of true for AS-
...
Some token formats, such as Macaroons
...
In all places where the token format is listed explicitly, it
the "GNAP Token Formats" registry Section 5.3.

MUST

 be one of the registered values in
...
The discovery response is a JSON document
following fields:

[RFC8259]

 consisting of a single JSON object with the

grant_request_endpoint (string):
. This URL

The location of the AS's grant request endpoint defined by
 be the same URL used by client instances in support of
...
A list of token formats supported by this AS. The
 be registered in the "GNAP Token Formats" registry per Section 5.3.
...
indicates that the AS does not support this feature. See Section 3.4.
 ...
 if the AS supports dynamic resource registration. An absent value
...
by
 ...
 be registered in the "GNAP Key Proofing Methods" registry established
MUST
[GNAP] OPTIONAL
.
...
Unless otherwise specified, the RS
methods defined in
...
.
 ...
 protect its calls to the AS using any of the signature

MAY

 present its keys by reference or by value in a similar fashion to a client instance

The RS
calling the AS in the core protocol of GNAP, as described in
...
The means by which an RS's keys are made known to the AS are out of the scope of this
specification. The AS
arbitrary keys in a trust-on-first-use model.

 require an RS to preregister its keys, or it could allow calls from

MAY

 issue access tokens, called "resource server management access tokens", to the RS to

MAY

The AS
protect the RS-facing API endpoints. If such tokens are issued, the RS
...
MUST
 ...
 be registered in the "GNAP Key Proofing Methods" registry.
...
Client1RSASInstance234RFC 9767
...
MUST
 ...
 validate the access token value and determine if the token is active. The
...
MAY
 ...
 include any additional fields defined in an access token response and
...
, to the AS's resource registration endpoint along with information about what
...
The list of token formats that the RS is able to
 be registered in the "GNAP Token Formats" registry per
process. The values in this array
Section 5.3. If the field is omitted, the token format is at the discretion of the AS. If the AS does
not support any of the requested token formats, the AS
.
OPTIONAL

 return an error to the RS.
...
set. If the AS does not support token introspection for this RS, the AS
the RS.
...
REQUIRED
 ...
 make this handle available to a client instance as part of a discovery

Section 9.1 of [GNAP]

 or as documentation to client software
...
If a resource was previously registered, the AS
in previous responses.

MAY

 return the same resource reference value as
...
access rights requested by the client instance.
 ...
 be combined with any other additional
...
endpoint, request structure, and response structure as a client instance's request.
 ...
 that allows for the RS to generate its own
...
Client1RS1ASRS2Instance23456RFC 9767
...
making a token request as described in
in the "existing_access_token" field.

 and presenting the existing access token's value

[GNAP]

Since the RS is acting as a client instance, the RS
client field and sign the request just as any client instance would, as described in Section 3.2.
The AS
 determine that the token being presented is appropriate for use at the RS making
the token chaining request.

 identify itself with its own key in the
...
JSON Web Token, signed with JWS
...
JSON Web Token, encrypted with JWE
...
are to be made through the Specification Required registration policy
...
results are appropriate for the request being made. For reference-style tokens or tokens that are
...
The proofing should be validated independently on each request to the RS, particularly as
...
Furthermore, such patterns could lead to the client software and RS depending on certain
...
When the client instance receives information about the protecting AS from an RS, it can be used
to derive information about the resources being protected without releasing the resources
themselves. For example, if a medical record is protected by a personal AS, an untrusted client
could call an RS to discover the location of the AS protecting the record. Since the AS is tied
strongly to a single RO, the untrusted and unauthorized client software can gain information
about the resource being protected without accessing the record itself.
...
Richer, J., Ed.
(GNAP)" RFC 9635 DOI 10.17487/RFC9635
editor.org/info/rfc9635>
...
Biscuit "Biscuit Authorization" <https://www.biscuitsec.org/>
,
...
Birgisson, A. Politz, J. G. Erlingsson, U. Taly, A. Vrable, M.
,
,
"Macaroons: Cookies with Contextual Caveats for Decentralized Authorization
in the Cloud" NDSS Symposium 2014 DOI 10.14722/ndss.2014.23212
, February
2014,
...
Lemmer-Webber, C., Ed.
Linked Data v0.3" W3C Draft Community Group Report
w3c-ccg.github.io/zcap-spec/>
...
M. Sporny, Ed. "Authorization Capabilities for
...
The editors would like to thank the following individuals for their reviews, feedback,
implementations, and contributions:
Aaron Parecki Adrian Gropper Andrii Deinega Annabelle
,
Backman Dmitry Barinov Fabien Imbault Florian Helmschmidt George Fletcher Justin Richer
,
,
Kathleen Moriarty Leif Johansson Mike Varley Nat Sakimura Takahiko Kawasaki
,
, and
Sheffer
.
...
Additionally, the editors want to acknowledge the immense contributions of
the content of this document. We thank him for his insight, input, and hard work, without which
