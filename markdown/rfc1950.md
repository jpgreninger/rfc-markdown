...
8.1 Preset dictionaries ..... 8
8.2 The Adler-32 algorithm ..... 8
9 Appendix: Sample code ..... 8
...
Version 3.1 was the first public release of this specification. In version 3.2, some terminology was changed and the Adler-32 sample code was rewritten for clarity. In version 3.3, the support for a preset dictionary was introduced, and the specification was converted to RFC style.
...
![](https://cdn.mathpix.com/cropped/38a3fb08-e806-4017-ac8b-4813c0f3b1b2-04.jpg?height=126&width=373&top_left_y=457&top_left_x=339)
...
![](https://cdn.mathpix.com/cropped/38a3fb08-e806-4017-ac8b-4813c0f3b1b2-04.jpg?height=183&width=482&top_left_y=1912&top_left_x=339)
(if FLG.FDICT set)
![](https://cdn.mathpix.com/cropped/38a3fb08-e806-4017-ac8b-4813c0f3b1b2-04.jpg?height=177&width=663&top_left_y=2202&top_left_x=339)
...
This identifies the compression method used in the file. $\mathrm{CM}=8$ denotes the "deflate" compression method with a window size up to 32 K . This is the method used by gzip and PNG (see references [1] and [2] in Chapter 3, below, for the reference documents). CM = 15 is reserved. It might be used in a future version of this specification to indicate the presence of an extra field before the compressed data.

## CINFO (Compression info)

For CM = 8, CINFO is the base-2 logarithm of the LZ77 window size, minus eight (CINFO=7 indicates a 32 K window size). Values of CINFO above 7 are not allowed in this version of the specification. CINFO is not defined in this specification for CM not equal to 8 .
...
These flags are available for use by specific compression methods. The "deflate" method ( $\mathrm{CM}=8$ ) sets these flags as follows:
...
Adler-32 is composed of two sums accumulated per byte: s1 is the sum of all bytes, s2 is the sum of all s1 values. Both sums are done modulo 65521. s1 is initialized to 1, s2 to zero. The Adler-32 checksum is stored as $\mathrm{s} 2 * 65536+\mathrm{s} 1$ in most-significant-byte first (network) order.
...
The modulo on unsigned long accumulators can be delayed for 5552 bytes, so the modulo operation time is negligible. If the bytes are $\mathrm{a}, \mathrm{b}, \mathrm{c}$, the second sum is $3 \mathrm{a}+2 \mathrm{~b}+\mathrm{c}+3$, and so is position and order sensitive, unlike the first sum, which is just a checksum. That 65521 is prime is important to avoid a possible large class of two-byte errors that leave the check unchanged. (The Fletcher checksum uses 255, which is not prime and which also makes the Fletcher check insensitive to single byte changes $0<->255$.)
