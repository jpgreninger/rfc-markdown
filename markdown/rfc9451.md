...
Packet and Behavior in the Network Service Header
(NSH)
...
Setting this bit indicates an OAM packet (see

[RFC6291]

). The actual format and
...
MUST
 ...
 be set for OAM packets and

MUST NOT

 be set for non-OAM packets.
...
procedures
 ...
 discard packets with O bit set, but
...
enabling this behavior. The configurable parameter
...
MUST
 ...
 be set for NSH OAM packets and
...
elements
 support a configuration parameter to filter valid Next Protocol values
...
(MPLS), or 0x05 (Ethernet).
 ...
 discard any NSH packets with the O bit set and Next Protocol set to
...
MAY
 or a flow identifier

[RFC9263]

 include optional Context Headers (e.g., a subscriber

) that are used to influence the

An NSH OAM packet
inner packet. The processing of the SFC OAM data (including the order)
specified in the relevant OAM or Context Header specification.

 include SFC OAM data in both Context Headers and the
...
SHOULD
 ...
 discard packets with the O bit set but
...
OAM functions prior to enabling this behavior. The configurable parameter
MUST
disabled by default.
...
Section 9 of [RFC8924]
.

Section 8 of [RFC8300]

. Additional security considerations related to SFC OAM are

Section 6 of

 and

Any data included in an NSH OAM packet
...
,
Policy Identifier Context Headers in the Network Service Header (NSH)" RFC
...
Jim Guichard Greg Mirsky Joel Halpern Christian Jacquenet Dirk von-Hugo Carlos
, and
...
Barry Leiba
 ...
 for the art directorate review and
