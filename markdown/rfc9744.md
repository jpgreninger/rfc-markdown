...
RFC 9744
EVPN Virtual Private Wire Service (VPWS) Flexible
Cross-Connect (FXC) Service
...
[RFC8214]
defined in
. It delivers this P2P service between a pair of Attachment Circuits (ACs),
where an AC on a PE can represent a port, a VLAN on a port, or a group of VLANs on a port. It
also leverages multi-homing and fast convergence capabilities of
...
plane upon node or link failure
...
. Furthermore, the use of EVPN BGP constructs
...
above requirements. Furthermore,
...
[RFC8214]
 ...
 is itself based on extensions to EVPN
...
In
, a PE signals an AC indirectly by first associating that AC to a VPWS service tunnel
...
"mass withdrawal" message associated with the failed ES per
, a PE device can quickly
...
For single-homed endpoints, it is acceptable not to signal each AC in BGP because, in the event of
...
VLAN bundle service interface described in
3.2.1.

. This solution is described in Section
...
VID. 24-bit VPWS service instance identifiers
identifiers representing normalized VIDs

 be set to that VID. When a double normalized VID is used, the lower 12 bits of the
 be set to the inner VID, while the higher 12 bits are set to the outer

 as well as 12-bit VPWS service instance
...
VIDs to the local VID)
egress interface itself.

 be performed either as part of the VID table lookup or at the
...
Since the VID lookup (single or double) needs to be performed at the disposition PE, VID
normalization
 be completed prior to MPLS encapsulation on the ingress PE. This requires
...
, a unique value identifying the service is signaled in the context of each PE's EVI.

In
The 32-bit Ethernet Tag ID field
 be set to this VPWS service instance identifier value.
...
point-to-point (P2P) EVPN-VPWS service tunnel between a pair of PEs for a specific set of ACs.
 ...
 be aware of the VLANs through configuration. There should ideally be a single
...
normalized VID lookup in that table identifies the egress interface.
 ...
 in the disposition direction to forward packets to their
...
representing the EVPN-VPWS service tunnel. This route is sent with a Route Target (RT) that
...
defined in
 with two new flags (outlined in Section 4) that indicate: 1) the
...
check and
 generate an alarm if it detects inconsistencies, but it will not disrupt the VPWS
service.
...
It should be noted that in this mode of operation, the PE sends a single Ethernet A-D per-EVI
...
ACs
 ...
 be locally switched within the PE.
...
with the baseline EVPN principles described in
, where locally switched preference is
...
The V field defined in Section 4 is
...
. However, if transmitted, its value may indicate an
...
This EC is sent with Ethernet A-D per-EVI route per Section 3 and
Active and All-Active redundancy modes.

SHOULD

 be sent for both Single-
...
The following bits in the Control Flags are defined; the remaining bits
sending and

 be ignored when receiving this community.
...
The M and V fields are
OPTIONAL
and is used for error notifications.

. The M field is ignored at reception for forwarding purposes
...
In this scenario, PE1 and PE2 advertise an Ethernet A-D per EVI route for each normalized VID
...
, and upon such failure detection, the switch over procedure to the
...
Bashandy, A., Ed. Filsfils, C.
Convergence" Work in Progress Internet-Draft, draft-ietf-rtgwg-bgp-pic-21
,
<https://datatracker.ietf.org/doc/html/draft-ietf-rtgwg-bgp-pic-21>
July 2024,

P. Mohapatra "BGP Prefix Independent
, and
,
...
S. Bryant "An Architecture for Multi-Segment Pseudowire

Bocci, M.
Emulation Edge-to-Edge" RFC 5659 DOI 10.17487/RFC5659
