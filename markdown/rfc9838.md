...
1.  Introduction and Overview
 ...
 called G-IKEv2, which accommodates
This document presents an extension to IKEv2
...
(GSA) defined in Multicast Group Security Architecture
...
[RFC4046]
Security (MSEC) Group Key Management Architecture"
Domain of Interpretation"
using IKEv1
management use cases can benefit from the simplicity, increased robustness, and cryptographic
improvements of IKEv2 (see

[RFC3740]
[RFC5374]
. G-IKEv2 replaces "The Group

 (since deprecated by IKEv2). When G-IKEv2 is used, group key

, which defines a similar group key management protocol
...
for multicast
and skips many of its details. The notation and conventions from
describing G-IKEv2 payloads and exchanges.

[RFC5374]

. This document defines an extension to the IKEv2 protocol
...
The following key terms are used throughout this document (mostly borrowed from
[RFC5374]
...
A group management method defined in
...
[RFC7296]
G-IKEv2 is an extension to the IKEv2 protocol
secure policy, and keys download from the GCKS to GMs.

 that provides group authorization,

2.1.  G-IKEv2 Integration into the IKEv2 Protocol

G-IKEv2 is compatible with most IKEv2 extensions defined so far (see Section 6 for details). In
particular, it is assumed that, if necessary, the IKE_INTERMEDIATE exchanges
...
As an IKEv2 extension, G-IKEv2
transport for the IKE SA used for registration (which is unicast), as defined in TCP Encapsulation
of IKEv2 and IPsec
 also use UDP port 848, the same as Group Domain of
Interpretation (GDOI)
the IKE header distinguishes the G-IKEv2 protocol from the GDOI protocol

, because they serve a similar function. The version number in

 use the IKEv2 ports (500, 4500). G-IKEv2
...
MAY
 ...
 describes how IKEv2 supports paths with NATs. The G-IKEv2
...
described in this section regarding the IKE SA
MUST NOT
GCKS used UDP port 848 for the IKE_SA_INIT exchange, they will operate the same as if they had
used UDP port 500.

 depend on the port used to create the initial IKE SA. For example, if the GM and the

 be honored. The behavior of GMs and GCKS
...
Payloads defined as part of other IKEv2 extensions
 also be included in these messages.
...
Exchange
exchange, called IKE_SA_INIT, is defined in IKEv2
...
transform. See Section 4.5.4 for details.
 ...
 are used. Each exchange consists of request/response pairs. The first
[RFC7296]

. This exchange negotiates

[RFC9242]

The second exchange, called GSA_AUTH, is similar to the IKEv2 IKE_AUTH exchange
...
) and is referred to as such throughout this document. However, it is
...
and this SA can be deleted at any time.
 ...
 to use this IKE SA for the purpose of creating unicast Child SAs between the GM
...
) can also be used with the GSA_AUTH exchange.
...
the GCKS to provide any additional policy necessary by group senders.
 ...
 include a Notify payload status type of GROUP_SENDER, which enables
...
Section 2.21.2 of [RFC7296]

Possible errors should be handled in accordance with
. In addition to
...
or wants to leave the group for some reason. The GM
and the Notify type NO_PROPOSAL_CHOSEN or REGISTRATION_FAILED as shown below. In this
case, the GCKS

 remove the GM from the group denoted in IDg.

 notify the GCKS by sending IDg
...
group the GM wishes to join. A GM intending to emit data traffic
 send a GROUP_SENDER
...
defined in Table 2. Other Transform Types
the GCKS. The Security Parameter Index (SPI) length of each Proposal in an SAg is set to zero,
and thus the SPI field is empty. The GCKS
MUST NOT
payload.

) SAs may be included into SAg. Proposals for

 use SPI length and SPI fields in the SAg

 be included as they will be ignored by
...
particular, the restriction from
 that Authenticated Encryption with
...
Proposal for one protocol, then it
...
Proposal Num field.
 ...
 only use transforms from one of the Proposals with the
...
Although the SAg payload is optional, it is
the GSA_AUTH request to allow the GCKS to select an appropriate policy.

RECOMMENDED

 that the GM include this payload into

MAY

 also indicate the support for IPcomp by including one or more the

A GM
IPCOMP_SUPPORTED notifications along with the SAg payload in the request. The Compression
Parameter Index (CPI) in these notifications is set to zero and

 be ignored by the GCKS.
...
or one GW policy (although at least one TEK or KEK policy
GSA payload is not acceptable to the GM, it
SHOULD
...
and it is ready to send or receive packets matching the TEK policy.
 ...
 be Present). If the policy in the

 notify the GCKS by initiating a
...
receive back the data it sends, then it

 install this SA only in the outgoing direction.

 install this SA only in the
...
MUST
 ...
 be checked by a GM against any previously received Message ID for this group. If
...
attribute
it is less than the previously received number, it should be considered stale and
...
MUST
 ...
 install the Rekey SA only in the inbound direction.

Once a GM successfully registers to the group, it
group (policy, keys) that it might have as a result of a previous registration with a new one.

 replace any information related to this

MUST

Once a GM has received the GIKE_UPDATE policy during a registration, the IKE SA
closed. By convention, the GCKS closes the IKE SA; the GM
MAY
...
described in
 consider itself
GIKE_UPDATE policy is received during the registration process, the GM
excluded from the group. To continue participating in the group, the GM needs to re-register.

. If for some reason the IKE SA is closed and no

 be
 close it. The GCKS

Section 1.3.2 of [RFC7296]
...
in the KD TEK. The GW policy
 also be included to provide the Activation Time Delay (ATD)
and/or Deactivation Time Delay (DTD) (Section 4.4.3.1.1) to specify activation and deactivation
delays for SAs generated from the TEKs. If the GM has indicated that it is a sender of data traffic
...
[RFC5374]
 ...
 defines two modes of operation for
...
If a GM includes an SAg in its GSA_AUTH or GSA_REGISTRATION request, the GCKS may evaluate
...
the group policy as defined below.
 ...
 return a NO_PROPOSAL_CHOSEN notification. Alternatively, the GCKS can change
...
then the GCKS
 ...
 close the IKE SA to save resources after a short period of time.
...
Depending on its policy, the GCKS
...
would provide reliable keys delivery) and the GSA_REKEY for the rest of the GMs.
 ...
 combine these two methods. For example, the GCKS may
...
the windowing mechanism described in
...
in the case of LKH) and/or creates new Data-Security SAs. The GM_SENDER_ID attribute in the
 be part of the Rekey Exchange, as
Key Download payload (defined in Section 4.5.3.3)
...
Section 2.3 of [RFC7296] MUST NOT
...
MAY
 ...
 be included to instruct the GM to delete existing SAs. See Section 4.6 for

MUST

 be included to authenticate the GSA_REKEY message if the

The AUTH payload
authentication method is based on public key signatures and
...
During GM registration, the GCKS sends the authentication key in the KD payload, the AUTH_KEY
...
key sent during the GM registration. If implicit authentication is used, then AUTH_KEY
be sent to GMs.
...
fields must be properly filled in; see Signature Authentication in
...
a lack of response from the GMs. This has the following implications.
 ...
 can be used to perform fragmentation of large GSA_REKEY
...
negotiated support of IKE fragmentation in IKE_SA_INIT, then it
...
Section 2.5.1 of [RFC7383]

 contains

•

The Path MTU (PMTU) mechanism, defined in
to lack of GSA_REKEY response messages.

Section 2.5.2 of [RFC7383]

, cannot be used due

MUST

The calculation of authentication data
 be applied to whole messages only before possible
...
before verifying its authenticity as if it were received unfragmented. The RESERVED field in the
 be set to the value of the RESERVED field in the
reconstructed Encrypted Payload header
...
included in the previous rekey message. The first message sent over a new Rekey SA
 use a
...
policy. The (encrypted) retransmitted messages
...
be substantially skewed for the GMs that would receive different copies of the messages.
 ...
 be bitwise identical and should be sent

MUST

GCKS may also include one or several GSA_NEXT_SPI attributes specifying SPIs for the
...
exchange. Then the GM verifies the Message ID and processes the GSA and KD payloads. The GM
...
in the GSA_INITIAL_MESSAGE_ID attribute. Note that if the GSA_INITIAL_MESSAGE_ID attribute
 assume zero as the first expected Message ID. The
is not received for the Rekey SA, the GM
...
ID value (for example, when no counter-mode is employed for other Data-Security SAs), it
MUST
 initiate a re-registration to the GCKS in order to obtain
NOT
a Sender-ID value (along with the current group policy).

 install the Data-Security SAs. It
...
old one (identified by the Protocol and SPI fields in the Delete payload)
 be silently deleted
after waiting the DEACTIVATION_TIME_DELAY interval regardless of its expiration time.

 be silently deleted after waiting the

MUST

MUST

Section 4.4.2.1 of [RFC4301]
...
replacement SAs, assuming the GCKS policy has created them. A GM
registration request if a Rekey SA is about to expire and not yet replaced with a new one.

 initiate a registration to the GCKS.
...
Because this is a normal IKEv2 exchange, the HDR is treated as defined in IKEv2
...
accomplished by sending the Delete Payload described in
GSA_REKEY pseudo-exchange as shown below.

Section 3.11 of [RFC7296]
...
remove the Rekey SA and all the Data-Security SAs, the GCKS sends a Delete payload with an SPI
...
GSA_REKEY pseudo-exchange or in the GSA_INBAND_REKEY exchange. In this situation, the GM
 re-register if it wants to continue participating in this group. The registration is performed
MUST
as described in Section 2.3. It is
...
However, after excluding a GM from the group, the GCKS
MAY
with this GM (if any) if the credentials of this GM are revoked.

 that a GM waits some randomly chosen time

 immediately delete the unicast SA

RECOMMENDED

2.5.  Counter-Based Modes of Operation

Several counter-based modes of operation have been specified for ESP (e.g., AES-CTR
AES-GCM
[RFC4543]
). These counter-based modes require that no two
...
Security SA specifying the use of a counter-based mode of operation. A GCKS
...
the group sender. A group sender
 request as many Sender-ID values matching the number
of encryption modules in which it will be installing the TEKs in the outbound direction.
Alternatively, a group sender
...
TEK expires).
 ...
 request more than one Sender-ID and use them serially. This
...
When the group policy includes a counter-based mode of operation, a GCKS should use the
...
A GCKS maintains a Sender-ID counter, which records the Sender-IDs that have been
allocated. Sender-IDs are allocated sequentially with zero as the first allocated value.

2.

Each time a Sender-ID is allocated, the current value of the counter is saved and allocated to
the group sender. The Sender-ID counter is then incremented in preparation for the next
allocation.

3.

When the GCKS specifies a counter-based mode of operation in the Data-Security SA, a group
...
A GCKS allocates new Sender-ID values for each registration operation by a group sender,
...
When the Sender-ID counter maintained by the GCKS reaches its final Sender-ID value, no
more Sender-ID values can be distributed. Before distributing any new Sender-ID values, the
GCKS
 exclude all GMs from the group as described in Section 2.4.3. This will result in
...
from the GWP_SENDER_ID_BITS attributes, then the GM
 treat this as a fatal error and
...
IPsec, replay protection is not always possible to achieve (see
particular, if there are many group senders for a Data-Security SA, then each of them will
independently increment the Sequence Number field in the ESP header (see
Section 2.2 of
[RFC4303]
), thus making it impossible for the group receivers to
...
GCKS
 create several Data-Security SAs with the same traffic selectors allowing only a single
...
Section 6.1 of [RFC3740]

Section 2.5 of [RFC4302]
...
MUST
 ...
 include the Sequence Numbers transform in the GSA payload for every Data-

When a Data-Security SA has a single sender, the GCKS
frequently enough so that the 32-bit sequence numbers do not wrap.

MUST

 be configured to rekey the SA

2.7.  Encryption Transforms with Implicit IV

The "Transform Type 1 - Encryption Algorithm Transform IDs" IANA registry
...
Sequence Numbers for IV construction
MUST
NOT

 for details). It requires anti-replay service to be enabled for an

 be used for any G-IKEv2 SA (both unicast and multicast).

 be used. In any case, such transforms
...
SHOULD
 be less than the size of the key for the Encryption Algorithm transform for the Rekey SA and

Each KWK is associated with a key wrap algorithm specified in the Key Wrap Algorithm
transform. The size of these KWKs is determined by the key wrap algorithm used, but it
NOT
...
The GCKS always sends group SA keys encrypted with the

GSK_w.

An LKH key management method:

The GCKS provides each GM with an individual key at the
...
keys
 be included in a G-IKEv2 rekey message, which changes group membership. This
...
members that are denied access to the group, then two sequential G-IKEv2 rekey messages
 be sent by the GCKS. The first G-IKEv2 rekey message creates a
changing the group KEK
...
again changing the KEK allows complete forward access control. A G-IKEv2 rekey message
NOT
...
If other methods of using LKH or other group management algorithms are added to G-IKEv2,
 remove the above restrictions requiring multiple G-IKEv2 rekey messages,
those methods
...
MUST
 ...
 contain the same keying material encrypted using different KWKs. The GM in
...
Depending on the new Key Path, the GM should do the following actions to be prepared for
future key updates:

•

If the new Key Path is empty, then no actions are needed. This may happen if no WRAP_KEY
attributes from the received message were used.

•

If the new Key Path is non-empty and it ends with the default KWK (GSK_w), then the whole
...
For a Data-Security SA, the keys are taken in accordance to the third bullet from
[RFC7296]
the leftmost bits of the keying material and the integrity key (if any)
remaining bits.

. In particular, for the ESP and AH SAs, the encryption key (if any)
...
The G-IKEv2 header (Section 4.1), IDg payload, and SAg payload reuse the IKEv2 format for the
...
G-IKEv2 uses the same IKE header format as specified in
. The Major
Version is 2 and the Minor Version is 0, as in IKEv2. IKE SA Initiator's SPI, IKE SA Responder's
SPI, Flags, Message ID, and Length are as specified in

Section 3.1 of [RFC7296]
...
Section 3.5 of [RFC7296]
The payload is constructed by using the IKEv2 Identification Payload (
ID type ID_KEY_ID
ID_IPV6_ADDR
expected to be used. The Payload Type for the IDg payload is fifty (50).

 be supported. ID types ID_IPV4_ADDR, ID_FQDN, ID_RFC822_ADDR, and
 be supported. ID types ID_DER_ASN1_DN and ID_DER_ASN1_GN are not
...
). The Payload Type for SAg payloads is thirty-three
...
employed security protocol, a group SA policy may be either a Rekey SA (GSA KEK) policy or a
Data-Security (GSA TEK) SA policy. GSA payload may contain zero or one GSA KEK policy, zero or
more GSA TEK policies, and zero or one GW policy, where either one GSA KEK or one GSA TEK
policy
...
Selectors.
 ...
 distribute both ESP and AH policies for the same set of Traffic
...
Security Protocol Identifiers" registry in
(GIKE_UPDATE) for Rekey SA and 2 (AH) or 3 (ESP) for Data-Security SAs.

. The valid values for this field are 6
...
these substructures is defined in IKEv2 (

Section 3.13.1 of [RFC7296]

).

For the Rekey SA (with the GIKE_UPDATE protocol), the destination traffic selectors
define a single multicast IP address, an IP protocol (assumed to be UDP), and a single port the
GSA_REKEY messages will be destined to. In this case, the source traffic selector
define a single IP address, an IP protocol (assumed to be UDP), and a single port the
 define a
GSA_REKEY messages will be originated from. The source traffic selector
...
If the Data-Security SAs are created in tunnel mode, then it
address preservation (see Multicast Extensions to the Security Architecture
encapsulation of ESP packets
for the multicast Data-Security SAs.

. UDP
 cannot be specified in G-IKEv2 and thus is not used
...
). The "Last Substruc" field in each Transform
...
this purpose, the transforms defined in
 are used. In addition, new Transform Types
...
optional Transform Type
 ...
 be present in the group SA policy substructure.
...
If the AEAD encryption algorithm is used, then INTEG transform either

MUST NOT

 contain value NONE; otherwise, it
...
ability for the GMs to correctly decrypt them and verify their ICV. In this case, the GCKS
NOT

 include the AUTH_KEY attribute into the KD payload. Additionally, the AUTH payload
...
case, the GCKS
 include the AUTH_KEY attribute containing the public key into the KD
...
contains DER-encoded ASN.1 object AlgorithmIdentifier, which specifies the signature
algorithm and the hash function that the GCKS will use for authentication. The
AlgorithmIdentifier object is defined in
...
In the case of the Digital Signature Transform ID, the GCKS
Algorithm Identifier attribute in the Group Controller Authentication Method transform. In
this case, the AUTH payload in the GIKE_UPDATE messages
Signature authentication method (value 14) and be formatted as defined in
[RFC7427]
MUST
...
Digital Signature Transform ID and

. The AlgorithmIdentifier ASN.1 object in the AUTH payload
...
the Group Controller Authentication Method transform
it may only appear in the registration exchange (either GSA_AUTH or GSA_REGISTRATION).

 change as a result of rekey operations. This means that

 appear in the rekey messages;
...
[RFC5649]
The key wrap algorithm defined in
respectively. This key wrap algorithm is designed for use with AES block cipher.

 with a 128-bit, 192-bit, and 256-bit key,

KW_ARX:

The ARX-KW-8-2-4-GX key wrap algorithm defined in
designed for use with Chacha20 stream cipher.

[ARX-KW]

. This key wrap algorithm is
...
The Sequence Numbers (SNs) Transform Type is defined in
...
Sequential Numbers" that correspond to non-ESN and ESN cases from AH
[RFC4303]

. This transform describes
...
Transform ID "32-bit Sequential Numbers"
multicast Data-Security SAs utilizing protocols ESP or AH.

SHOULD

 be used by the GCKS for single-sender
...
[RFC4302]
 ...
 are defined in such a way that high-order 32 bits of
...
algorithm for doing this described in AH
...
bit Sequential Numbers" Transform ID
utilizing protocols ESP or AH.

 be used for multicast Data-Security SAs
...
MUST
 ...
 be used by the GCKS in case of multi-sender multicast Data-Security SAs
...
if replay protection is not needed (e.g., it is done on the application level).
 ...
 also use this Transform ID for single-sender multicast Data-Security SAs
...
The attributes follow the format defined in IKEv2 (
...
seconds. When the lifetime expires, the group SA and all associated keys
GCKS may delete the SA at any time before the end of the validity period.

 be deleted. The

MUST

A single attribute of this type
multicast rekey is employed by the GCKS. This attribute
(via the GSA_INBAND_REKEY exchange) is employed by the GCKS for the GM.

 be included into any group SA policy substructure if
SHOULD NOT

MUST

 be used if inband rekey
...
MUST NOT
 ...
 be used if inband rekey (via the GSA_INBAND_REKEY exchange) is
...
Multiple attributes of this type
MAY
used in the replacement group SA.

 be included, meaning that any of the supplied SPIs can be

MAY

 store these values. Later on, if the GM starts receiving messages with one of these

The GM
SPIs without seeing a rekey message over the current Rekey SA, then it may be used as an
indication that the rekey message got lost on its way to this GM. In this case, the GM
...
Protocol (1 octet):
 ...
 be zero. This value is reserved (see Section 9) and is never used for any security
...
The attributes follow the format defined in the IKEv2 (
column defines what attribute format is allowed: Type/Length/Value (TLV) or Type/Value (TV).
...
to GMs: Activation Time Delay (ATD) and Deactivation Time Delay (DTD).
 ...
 specifies a key rollover method that requires two values be provided
...
The GWP_DTD attribute (2) allows the GCKS to set the DTD for previously distributed SAs. The
...
specifying the delay in seconds. These attributes are
...
in Figure 1 of Using Counter Modes with ESP and AH to Protect Group Traffic
 and as
specified in Section 2.5.2. Guidance for a GCKS choosing the value is provided in
[RFC6054]

. This value is applied to each Sender-ID value distributed in the KD payload.
...
MUST
 ...
 include this attribute if there are more than one senders in the group and any of
...
Protocol Identifiers" registry in
...
Value Group Key Bag Attributes
...
attribute is formatted according to Section 4.5.4 with a precondition that the Key ID field
always be zero. The size of the keying material
to be taken from this keying material (see Section 3.4) for the corresponding SA.

 be equal to the total size of the keys needed
...
If the key bag is for a Rekey SA (GIKE_UPDATE protocol), then exactly one SA_KEY attribute
be present in the GSA_AUTH, GSA_REGISTRATION, and GSA_INBAND_REKEY exchanges. In the
GSA_REKEY pseudo-exchange, at least one SA_KEY attribute
attributes

 be present (depending on the key management method employed by the GCKS).
...
time the GM registers to the group (in either GSA_AUTH or GSA_REGISTRATION exchanges).
 ...
 be sent in GSA_REKEY messages and

 only be sent via unicast SA at the
...
hierarchy (e.g., LKH). This attribute
GSA_INBAND_REKEY exchange) is employed by the GCKS for the GM.

 be used if inband rekey (via the
...
data
 ...
 be zero. The algorithm associated with the key is defined by the Key Wrap

 be equal to the key size for this key wrap algorithm.

MUST NOT

MUST

Multiple instances of the WRAP_KEY attributes

MAY

 be present in the key bag.
...
MUST
 ...
 be represented as DER-encoded ASN.1 object SubjectPublicKeyInfo, defined in

If digital signatures are used for the GSA_REKEY message authentication, then the content of
...
algorithm field of the SubjectPublicKeyInfo object, then the parameters field
 include
the RSASSA-PSS-params structure.

. The algorithm field inside the SubjectPublicKeyInfo object
...
In case of implicit authentication, the AUTH_KEY Attribute is not used and
(see Section 2.4.1).
...
Multiple instances of the AUTH_KEY attributes
...
exclusive use of a GM. One or more of these attributes
 be sent by the GCKS if the GM
...
the GCKS
instances of the GM_SENDER_ID attribute. The GCKS
the GM (e.g., if it is running out of Sender-IDs), but it

 provide it with the requested number of Sender-IDs by sending multiple
MAY

 send fewer values than requested by
...
in the GSA_INBAND_REKEY exchange).
 ...
 appear in the rekey operations (in the GSA_REKEY pseudo-exchange or
...
ChaCha
 ...
 defines key wrapping using AES, and Key Wrapping Constructions using SipHash and

 define key wrapping using Chacha20.

[ARX-KW]

MUST

 be negotiated in the IKE_SA_INIT exchange so that the

In G-IKEv2, the key wrap algorithm
GCKS is able to send encrypted keys to the GM in the GSA_AUTH exchange. In addition, if the
GCKS plans to use the multicast Rekey SA for group rekey, then it
 specify the key wrap
...
included in the GSA_AUTH request, then it
 indicate which key wrap algorithms are
MUST
...
G-IKEv2 uses the same Notify payload as specified in
...
content of the Notification Data field
 ...
 be zero. There is no data associated with this notification and the
...
Size fields in the Notify payload
 be zero. There is no data associated with this notification
and the content of the Notification Data field
...
Notification Data field
 ...
 be zero. There is no data associated with this notification and the content of the
...
data includes a count of how many Sender-ID values the GM desires. The count
long and contain the big-endian representation of the number of requested Sender-IDs. The
Protocol ID and SPI Size fields in the Notify payload
...
GSA_AUTH GSA_REGISTRATION GSA_REKEY Notes
...
The GWP_SENDER_ID_BITS attribute
...
GM_SENDER_ID attribute
the GM requested more Sender-IDs), and it

 be present in the former case (and more

 be present in the latter case.

 be present. At least one

 be present if the GCKS policy includes at least
...
For a Data-Security SA, exactly one SA_KEY attribute
exactly one SA_KEY attribute
GSA_REGISTRATION exchange. In the GSA_REKEY pseudo-exchange, at least one SA_KEY
attribute

 be present and more of these attributes

 be present in the GSA_AUTH and the

 be present. For a Rekey SA,
...
The WRAP_KEY attribute
that relies on a key tree (like LKH).

MUST

 be present if the GCKS employs a key management method

MUST

 be present in the GSA_AUTH and GSA_REGISTRATION

The AUTH_KEY attribute
exchanges if the GCKS employs an authentication method of rekey operations based on
digital signatures and
 be present if implicit authentication is employed. The
AUTH_KEY attribute
 be present in the GSA_REKEY pseudo-exchange if the GCKS
...
into the registration request. Otherwise, it
GM_SENDER_ID attribute
the GM requested more Sender-IDs), and it
...
defined in
authentication
with and can use IKEv2 Redirect Mechanism
[RFC5723]
defined in

. G-IKEv2 is also compatible with Multiple Key Exchanges in the IKEv2 framework, as
[RFC9370]
.

 can be used to establish registration IKE SA, as well as EAP-only

 and secure password authentication

 and IKEv2 Session Resumption

. G-IKEv2 is compatible
...
Using implicit IV for counter-based encryption modes in ESP is defined in
...
. However, the use of PPKs leaves the initial IKE SA susceptible to quantum
for IKEv2
...
then decrypt it (i.e., Store Now Decrypt Later attack). See

Section 6 of [RFC8784]

 for details.

While the group keys are protected with PPK and thus are immune to QC, GCKS
implementations that care about other data sent over initial IKE SA
[RFC9867]
extensions that protect even initial IKE SA against QC (like
...
Aggregation and fragmentation mode for ESP is defined in
...
increasing sequence numbers in the incoming packets. Thus, it is impossible to use this mode for
...
Few extensions were defined for the GDOI protocol
Security Services
[RFC8263]
documents, if needed.

[RFC6407]
 or the GDOI GROUPKEY-PUSH Acknowledgement Message
. It is expected that these extensions will be redefined for G-IKEv2 in separate

[RFC8052]

, like GDOI Support for IEC 62351
...
, including authentication, confidentiality,

Section 5 of [RFC7296]
...
authentication, so the GM cannot be sure that the message came from the GCKS. For this reason,
using implicit authentication is
parties.

 unless used with a small group of trusted
...
Message ID number less than or equal to the last received value
...
Exchange Version 2 (IKEv2) Parameters" registry group
Expert Review, and Private Use are as defined in
...
IANA has created the "Transform Type 13 - Key Wrap Algorithm Transform IDs" registry.
...
IANA has created the "Transform Type 14 - Group Controller Authentication Method
Transform IDs" registry. The registration policy for this registry is Expert Review
...
IANA has created the "Group SA Attributes" registry. The registration policy for this registry
is Expert Review
...
IANA has created the "Group-Wide Policy Attributes" registry. The registration policy for this
...
IANA has created the "Group Key Bag Attributes" registry. The registration policy for this
registry is Expert Review
...
IANA has created the "Member Key Bag Attributes" registry. The registration policy for this
registry is Expert Review
...
In the "IKEv2 Exchange Types" registry, IANA has updated the references for the following
...
In the "IKEv2 Payload Types" registry, IANA has listed this document as a reference for the
following entries:
...
In the "IKEv2 Payload Types" registry, IANA has updated the definition of Payload Type 33
...
In the "Transform Type Values" registry, IANA has made the following changes:

◦
Registered "Key Wrap Algorithm (KWA)" and "Group Controller Authentication Method
(GCAUTH)".
◦
Updated the "Used In" column for values 1 and 3 and listed this document as an additional
reference.
...
In the "IKEv2 Transform Attribute Types" registry, IANA has added the following entry:
...
In the "Transform Type 5 - Sequence Numbers Transform IDs" registry, IANA has added the
following entry:
...
32-bit Unspecified Numbers

Table 22

7.

In the "IKEv2 Notify Message Error Types" registry, IANA has made the following changes:

Registered "REGISTRATION_FAILED".
◦
◦
Updated the references for "INVALID_GROUP_ID" and "AUTHORIZATION_FAILED" to point
to this document.

Value

Notify Message Error Type
...
document, IANA has renamed it as follows:

 registered the Notify type 16429 in the "IKEv2
...
In the "IKEv2 Security Protocol Identifiers" registry, IANA has added the following entry:
...
B. Weis "Using Counter Modes with Encapsulating Security

McGrew, D.
Payload (ESP) and Authentication Header (AH) to Protect Group Traffic" RFC
6054 DOI 10.17487/RFC6054
,
rfc6054>
...
Smyslov, V. "Renaming the Extended Sequence Numbers (ESN) Transform Type
in the Internet Key Exchange Protocol Version 2 (IKEv2)" RFC 9827 DOI
10.17487/RFC9827
...
Shinichi, S. "ARX-KW, a family of key wrapping constructions using SipHash
and ChaCha" Cryptology ePrint Archive, Paper 2020/059
eprint.iacr.org/2020/059.pdf>
...
Rowles, S. Yeung, A. Tran, P.
Work in Progress Internet-Draft, draft-yeung-g-ikev2-07
<https://datatracker.ietf.org/doc/html/draft-yeung-g-ikev2-07>
...
Y. Nir "Group Key Management using IKEv2"
...
J. Lotspiech "Revocation and Tracing Schemes for

Naor, D. Naor, M.
,
Stateless Receivers" Advances in Cryptology - CRYPTO 2001, Lecture Notes in
Computer Science, vol. 2139, pp. 41-62 DOI 10.1007/3-540-44647-8_3
,
...
A. Sherman "Key Establishment in Large Dynamic Groups

McGrew, D.
Using One-Way Function Trees" IEEE Transactions on Software Engineering,
vol. 29, no. 5, pp. 444-458 DOI 10.1109/TSE.2003.1199073
<https://
pdfs.semanticscholar.org/d24c/7b41f7bcc2b6690e1b4d80eaf8c3e1cc5ee5.pdf>
...
B. Weis "The Multicast Group Security Architecture" RFC 3740
,
...
(MSEC) Group Key Management Architecture" RFC 4046 DOI 10.17487/RFC4046
,
April 2005,

F. Lindholm "Multicast Security
,
...
J. Viega "The Use of Galois Message Authentication Code

McGrew, D.
(GMAC) in IPsec ESP and AH" RFC 4543 DOI 10.17487/RFC4543
,
...
D. Ignjatic "Multicast Extensions to the Security
...
M. Dworkin "Advanced Encryption Standard (AES) Key Wrap
, September 2009,
...
Eronen, P. Tschofenig, H.
, and
Authentication in IKEv2" RFC 5998 DOI 10.17487/RFC5998
,
...
Y. Sheffer "An Extension for EAP-Only
...
Kivinen, T. "Secure Password Framework for Internet Key Exchange Version 2
(IKEv2)" RFC 6467 DOI 10.17487/RFC6467
editor.org/info/rfc6467>
...
H. Falk "Group Domain of Interpretation (GDOI)
Weis, B. Seewald, M.
Protocol Support for IEC 62351 Security Services" RFC 8052 DOI 10.17487/
,
RFC8052

<https://www.rfc-editor.org/info/rfc8052>
...
Weis, B. Mangla, U. Karl, T.
Interpretation (GDOI) GROUPKEY-PUSH Acknowledgement Message" RFC 8263
DOI 10.17487/RFC8263
rfc8263>

N. Maheshwari "Group Domain of
...
Tjhai, CJ. Tomlinson, M. Bartlett, G. Fluhrer, S. Van Geest, D. Garcia-Morchon,
O.
V. Smyslov "Multiple Key Exchanges in the Internet Key Exchange
Protocol Version 2 (IKEv2)" RFC 9370 DOI 10.17487/RFC9370
www.rfc-editor.org/info/rfc9370>
...
Smyslov, V. "Mixing Preshared Keys in the IKE_INTERMEDIATE and
CREATE_CHILD_SA Exchanges of the Internet Key Exchange Protocol Version 2
(IKEv2) for Post-Quantum Security" RFC 9867 DOI 10.17487/RFC9867
November 2025,
...
GMs. This section clarifies how the LKH architecture is used with G-IKEv2.
 ...
 describes the LKH architecture and how a GCKS uses LKH to exclude
...
From these attributes, the GM A will construct the Key Path K_sa1->1->3->7->GSK_w. Since it ends
...
The authors thank
group key management and providing the basis behind the protocol.
Amjad Inamdar
the document.
...
Jing Xiang
 ...
 for first exploring the use of IKEv2 for

Mike Sullenberger

 and

 were instrumental in helping resolve many issues in several draft versions of
...
Tero Kivinen Daniel Migault Gorry Fairhurst Robert Sparks Russ
 for their careful reviews and valuable proposals for improving the
...
The following individuals made substantial contributions to earlier draft versions of this
document.
