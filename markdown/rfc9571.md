...
RFC 9571
Extension of RFC 6374-Based Performance
...
[RFC6374]
(OAM) protocol for use with MPLS Transport Profile (MPLS-TP)
...
[RFC7190]
 ...
 may also lead to synchronization issues.
...
packets within a batch are marked with the SFL
...
packets. The format of the Query and Response packets is described in Section 9.
 ...
 corresponding to that batch. The
...
used
 to ensure that the packet that was being used
...
Any system that needs to measure delay

MUST

 be able to measure loss.

2.

Any system that is to measure delay
...
MAY
 ...
 start at any point during a loss measurement batch, subject to

4.

A delay measurement interval
enclosing loss batch completes.

MUST

 be short enough that it will complete before the
...
Reserved, and Differentiated Services (DS) fields are as defined in
. The
...
creating or modifying the buckets according to the intervals in the Query message), then the
 respond with 0 packets in each bucket until it has been configured for a full
responder
...
and DS fields are as defined in
...
[RFC9341]
 ...
 is the concept of a one-way delay measurement in which the average
...
The Version, Flags, Control Code, Message Length, QTF, RTF, RPTF, Session Identifier, and DS fields
are as defined in
...
the duration of a marked batch of packets but the interval between a pair of packets taking a
...
The MPLS label stack is exactly the same as that used for the user data service packets being
...
(Optional) The SFL TLV specified in Section 9.1
...
the MPLS OAM handler. This TLV is required if messages are sent over UDP
. This
TLV
known that this information is not needed by the responder.

[RFC7876]
 be included unless, by some method outside the scope of this document, it is

 be carried if needed. It is used to provide
...
(Optional) The Return Information
the response to the Querier. This is not needed if the response is requested in band and the
MPLS construct being measured is a point-to-point LSP, but it otherwise
Return Address TLV is defined in
[RFC7876]
.
...
, and the optional UDP Return Object is defined in
 ...
 be carried if needed. It allows the responder send
...
The
 SFL TLV is shown in Figure 6. This contains the SFL that was carried in the label
...
The security considerations documented in
[RFC5920]
...
) are applicable to this protocol.
...
An attacker that manages to corrupt the
 responder is unable to detect. However, the network
measurements in a way that the
...
normal MPLS network security procedures make this type of attack extremely unlikely.
 ...
 SFL TLV in Section 9.1 could disrupt the
...
As per the IANA considerations in
...
S. Bryant "Retiring TLVs from the Associated Channel Header of

Farrel, A.
the MPLS Generic Associated Channel" RFC 7026 DOI 10.17487/RFC7026
September 2013,
...
Andersson, L.
IANA Registries to a New Registry" RFC 7214 DOI 10.17487/RFC7214
,
<https://www.rfc-editor.org/info/rfc7214>

C. Pignataro "Moving Generic Associated Channel (G-ACh)
...
Measurement for MPLS Networks" RFC 7876 DOI 10.17487/RFC7876
<https://www.rfc-editor.org/info/rfc7876>

S. Soni "UDP Return Path for Packet Loss and Delay
, July 2016,
...
Le Faucheur, F., Ed. Wu, L. Davie, B. Davari, S. Vaananen, P. Krishnan, R.
,
Cheval, P.
...
Bocci, M., Ed. Bryant, S., Ed. Frost, D., Ed. Levrau, L.
...
Villamizar, C. "Use of Multipath with MPLS and MPLS Transport Profile (MPLS-
TP)" RFC 7190 DOI 10.17487/RFC7190
...
Elwyn Davies
 ...
 for their thorough and thoughtful review
