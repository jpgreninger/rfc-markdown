...
In some cases, proving effective control over an identifier requires an attestation from a third
...
certificates corresponding to a Service Provider Code (SPC) as defined in
...
The ACME Authority Token Challenge type, "tkauth-01", is here specified for use with the
...
Authority Token
used in the ACME challenge, as well as the binding (see Section 3.3), and those
...
authorizes ACME clients to receive Authority Tokens is given in Section 5.
 ...
 convey to the Token Authority the identifier type and value that will be
...
regularly refreshed. Definitions of a tkauth-type
 specify how they manage the freshness of
...
For example, imagine a case where a Token Authority for DNS names knows that a client is
...
some other entity. It is
purpose.

 that the ACME account fingerprint be used for this
...
signature string defined by a JSON Web Signature (JWS)
...
order to satisfy the requirement for replay prevention, the JWT
...
is being used for multiple challenges over time within its set lifetime.
 ...
 for the payload of the JWT to

 contain a "jti" element and

MUST

MUST

 also contain a new JWT claim, "atc", for Authority Token Challenge,
...
values in the "ACME Identifier Types" registry, as defined by
. The identifier type and
...
), which would be the value for the "tkvalue" element that the Token

, the "tktype" identifier type could be the TNAuthList (as
...
Optionally, the "atc" claim may contain a fourth boolean element, "ca". If set to "true", the "ca"
...
subordinate delegations from the issued certificate (using
...
the ACME server. Implementations compliant with this specification
interface for Authority Token acquisition as described below, though other interfaces
supported as well.
...
. This specification assumes that Token Authority URIs are known to clients
...
Note that ":id" here is a placeholder for an actual account identifier. The body of the POST
request
...
scope of the Authority Token it would like to receive from the Token Authority.
 ...
 contain the Authority Token Challenge element (the key "atc", colon, and its value)
...
IANA has added a new claim in the "JSON Web Token Claims" registry, as defined in
follows:
...
Per the guidance in
transactions used to request and acquire Authority Tokens
...
practices identified in
 ...
 use TLS, and similarly, the HTTPS REST

 use TLS. These measures are
...
MUST
 ...
 contain a field that identifies to the CA which

The capture of Authority Tokens by an adversary could enable an attacker to acquire a certificate
from a CA. Therefore, all Authority Tokens
ACME client requested the token from the Token Authority; here, that is the fingerprint specified
in Section 4. All Authority Tokens must specify an expiry (of the token itself as proof for a CA, as
opposed to the expiry of the name), and for some applications, it may make sense for that expiry
to be quite short. ACME services relying on Authority Tokens
 issue certificates with
a longer expiry than the expiry of the Authority Token. Any protocol used to retrieve Authority
Tokens from a Token Authority
acquiring an Authority Token. The details of this protocol are out of the scope of this
specification.

 use confidentiality to prevent eavesdroppers from
...
fingerprint object would permit other keys if, due to concerns about algorithmic agility, a more
robust algorithm were required at a future time. Future specifications can define new keys for
...
Automated Certificate Management Environment (ACME) Authority Token" RFC
9448 DOI 10.17487/RFC9448
,
rfc9448>

J. Peterson "TNAuthList Profile of
...
T. McGarry "Managing, Ordering, Distributing, Exposing, and

Peterson, J.
Registering Telephone Numbers (MODERN): Problem Statement, Use Cases, and
Framework" RFC 8396 DOI 10.17487/RFC8396
,
editor.org/info/rfc8396>
.
...
Ben Kaduk
 ...
 for contributions to this problem statement
