...
Today's virtualized environments place additional demands on the MAC address tables of Top-of-Rack (ToR) switches that connect to the servers. Instead of just one MAC address per server link, the ToR now has to learn the MAC addresses of the individual VMs (which could range in the hundreds per server). This is needed because traffic to/from the VMs to the rest of the physical network will traverse the link between the server and the switch. A typical ToR switch could connect to 24 or 48 servers depending upon the number of its serverfacing ports. A data center might consist of several racks, so each ToR switch would need to maintain an address table for the communicating VMs across the various physical servers. This places a much larger demand on the table capacity compared to non-virtualized environments.
...
- Source Port: It is recommended that the UDP source port number be calculated using a hash of fields from the inner packet -one example being a hash of the inner Ethernet frame's headers. This is to enable a level of entropy for the ECMP/loadbalancing of the VM-to-VM traffic across the VXLAN overlay. When calculating the UDP source port number in this manner, it is RECOMMENDED that the value be in the dynamic/private port range 49152-65535 [RFC6335].
...
+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ | Outer Destination MAC Address | Outer Source MAC Address | | Outer Source MAC Address | +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ |OptnlEthtype = C-Tag 802.1Q | Outer.VLAN Tag Information | +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ | Ethertype $=0 \times 0800$ |
...
| :--- | :--- |
| ![](https://cdn.mathpix.com/cropped/c873f9bb-c80a-4bb7-911d-2461d7620817-12.jpg?height=33&width=1354&top_left_y=863&top_left_x=339) |  |
| UDP Length | UDP Checksum |
| +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ |  |

VXLAN Header:
+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ $|\mathrm{R}| \mathrm{R}|\mathrm{R}| \mathrm{R}|\mathrm{I}| \mathrm{R}|\mathrm{R}| \mathrm{R} \mid$ Reserved | +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ | VXLAN Network Identifier (VNI) | Reserved | +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
Original Ethernet Payload (Note that the original Ethernet Frame's FCS is not included) +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
Outer Ethernet Header:
![](https://cdn.mathpix.com/cropped/c873f9bb-c80a-4bb7-911d-2461d7620817-13.jpg?height=422&width=1371&top_left_y=784&top_left_x=333)

Outer IPv6 Header:

![](https://cdn.mathpix.com/cropped/c873f9bb-c80a-4bb7-911d-2461d7620817-13.jpg?height=796&width=1371&top_left_y=1282&top_left_x=333)

Outer UDP Header:

| ![](https://cdn.mathpix.com/cropped/c873f9bb-c80a-4bb7-911d-2461d7620817-14.jpg?height=37&width=1371&top_left_y=292&top_left_x=335) |
| :--- |
|  |
| UDP Length |
|  |
...
VXLAN Header:
+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ $|R| R|R| R|I| R|R| R \mid$ Reserved | +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ | VXLAN Network Identifier (VNI) | Reserved | +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
![](https://cdn.mathpix.com/cropped/c873f9bb-c80a-4bb7-911d-2461d7620817-16.jpg?height=1646&width=1335&top_left_y=249&top_left_x=328)
...
![](https://cdn.mathpix.com/cropped/c873f9bb-c80a-4bb7-911d-2461d7620817-18.jpg?height=721&width=1391&top_left_y=253&top_left_x=333)
