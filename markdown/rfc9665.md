...
RFC 9665
Service Registration Protocol for DNS-Based Service
Discovery

Abstract

The Service Registration Protocol (SRP) for DNS-based Service Discovery (DNS‑SD) uses the
standard DNS Update mechanism to enable DNS‑SD using only unicast packets. This makes it
...
Appendix A.  Using Standard Authoritative DNS Servers Compliant with RFC 2136 to Test SRP

Requesters

Appendix B.  How to Allow SRP Requesters to Update Standard Servers Compliant with RFC
...
DNS‑SD clients can then discover the set of service instances of a particular type that are
available. They can then select an instance from among those that are available and obtain the
information required to use it. Although DNS‑SD using the DNS protocol can be more efficient
and versatile than using mDNS, it is not common in practice because of the difficulties associated
...
The existing practice for updating DNS zones is either to enter new data manually or to use DNS
Update

. Unfortunately, DNS Update requires either:
...
the requester. We reason that mDNS
advertise services
authentication as a part of its protocol rather than in the service advertisement.

, relying on whatever service is advertised to provide

 allows arbitrary hosts on a single IP link to
...
This leads us to the disappointing observation that this protocol is not a mechanism for adding
...
Finally, SRP adds the concept of a "lease"
[RFC8415]
requester does not renew the lease before it has elapsed, the registration is removed. The claim
on the name can have a longer lease so that another requester cannot immediately claim the
...
Section 10 of the DNS‑SD specification
...
their services directly discoverable by peers attached to that same local link.
 ...
 briefly discusses ways that servers can

[RFC6763]

 also allows clients to discover services by using the DNS protocol over
[RFC1035]

[RFC6763]
DNS‑SD
traditional unicast
. This can be done by having a system administrator manually
...
[RFC8766]
 ...
 provides one way to automatically populate the DNS namespace
...
Strictly speaking, fully qualified domain names end with a dot ("."). In DNS zone files and other
similar contexts, if the final dot is omitted, then a name may be treated incorrectly as relative to
some other parent domain. This document follows the formal DNS convention, ending fully
...
qualified domain names with a dot. When this document mentions domain names such as
"local." and "default.service.arpa.", the final dot is part of the domain name; it is not a period
...
. An SRP registrar is most likely an
...
[RFC6763]
Section 11 of the DNS‑SD specification
...
. If this process does not produce a default
...
described in Section 6.1 of the DNS Push Notification specification
...
, some simplifications are available. Instead of being

[RFC7228]

SRP requesters on CNNs are expected to receive, from the network, a list of SRP registrars with
which to register. It is the responsibility of a CNN supporting SRP to provide at least one registrar
address and port. It is the responsibility of the registrar supporting a CNN to handle the updates
appropriately. In some network environments, updates may be accepted directly into a local
"default.service.arpa." zone, which has only local visibility. In other network environments,
...
The DNS-Based Service Discovery specification
 describes the details of what each of
these RR types mean, with the exception of the KEY RR, which is defined in the specification for
how to store Diffie-Hellman Keys in the DNS
...
As described above, full-featured devices are responsible for knowing the domain in which to
register their services. Such devices
domain by the operator of the device. However, such devices
discovery as described in Section 11 of the DNS‑SD specification

 optionally support configuration of a registration

 support registration domain
...
necessary, as mentioned in Section 3.1.2.
...
[RFC2136]
 where the update process could involve many update attempts. The requester might
first attempt to add a name if it doesn't exist; if that fails, then in a second message the requester
might update the name if it does exist but matches certain preconditions. Because the Service
Registration Protocol described in this document uses a single transaction, some of this
...
The DNS protocol, including DNS updates, can operate over UDP or TCP. For UDP updates from
CNN devices, reliable transmission must be guaranteed by retransmitting when a DNS UDP
message is not acknowledged in a reasonable interval. Section 4.2.1 of the DNS specification
[RFC1035]
describing common DNS implementation errors
Guidelines document
DNS.

 provides some guidance on this topic, as does Section 1 of the IETF document

 also provides useful guidance that is particularly relevant to
...
needs to send more than one SRP Update to the SRP registrar, it
...
the requester
while an outstanding SRP Update is in flight, the SRP registrar
Update for that change until the previous SRP Update has completed.

 send any subsequent SRP Updates. If a configuration change occurs
 defer sending a new SRP
...
The requester generates a public/private key pair (Section 6.6). This key pair
stable storage; if there is no writable stable storage on the SRP requester, the SRP requester
be preconfigured with a public/private key pair in read-only storage. This key pair
 be
unique to the device. A device with rewritable storage
...
requester on the device
result of a "factory reset") and to generate a new key.

 provide a mechanism to erase the key (for example, as the

 retain this key indefinitely. When
...
proactively when regenerating a key or only in the event that the SRP update produces a name
conflict.
...
update are set using the EDNS(0) Update Lease option
 ...
 contain the same public key. The update is signed using SIG(0), using the private
...
key, the flags field in the KEY RR

. Because the KEY RR used in SIG(0) is not a zone-signing
...
The KEY record in Service Description updates
SRP registrar
...
records are not used when computing the SIG(0) signature.
 ...
 be omitted for brevity; if it is omitted, the
 behave as if the same KEY record that is given for the Host Description is
...
RCODE
this case, the SRP requester

, indicating that the desired name is already owned by a different SIG(0) key. In

 choose a new name or give up.
...
There is no specific requirement for how the SRP requester should choose a new name.
...
SHOULD
device may be temporarily disconnected or powered off -- disappearing from the network for a
few days -- it makes a claim on its name that lasts much longer.

Therefore, even if a device is disconnected from the network for a few days, and its services are
not available for that time, no other device can come along and claim its name the moment it
disappears from the network. In the event that a device is disconnected from the network and
...
[RFC2782]
 ...
 requires that the target hostname in the

Although the original SRV specification
RDATA of an SRV record not be compressed in DNS queries and responses, an SRP requester
...
Therefore, SRP registrars

 support compression of SRV RR targets.
...
Note that this document does not update the SRV specification
 compress SRV record targets. The requirement to accept compressed SRV
servers still
records in updates only applies to SRP registrars. SRP registrars that are also authoritative DNS
servers still
Multicast DNS

 compress SRV record targets in DNS responses. We note also that
...
servers that the code only compresses SRV targets in the case where a requester is generating an
SRP Update.
...
the name in reserve for when the SRP requester is once again able to provide the service.
 ...
 contain exactly one Host Description Instruction that contains exactly one

 be set to the same value it had previously; this holds

 also be zero. Otherwise, it
...
To support this, when removing a hostname, an SRP registrar
...
In some use cases, a requester may need to remove a specific service instance without removing
its other services. For example, a device may shut down its remote screen access (_rfb._tcp)
service while retaining its command-line login (_ssh._tcp) service. This can be accomplished in
one of two ways:

1.

To simply remove a specific service instance, the requester sends a valid SRP Update with a
Service Description Instruction (Section 3.3.1.2) containing a single "Delete All RRsets From
A Name" update to the service instance name. The SRP Update
Discovery Instructions (Section 3.3.1.1) consisting of "Delete An RR From An RRset" updates
[RFC2136]
 that delete any Service Discovery PTR records whose target is the service instance
name. However, even in the absence of such Service Discovery Instructions, the SRP
registrar
MUST
instance name.
...
Because both the removal of the old service and the add of the new service consist of a valid
Service Discovery Instruction and a valid Service Description Instruction, the update as a
whole is a valid SRP Update and will result in the old service being removed and the new
...
one added; or, to put it differently, the SRP Update will result in the old service being
...
. Since the removal of the old service and the addition of
...
Because the SRP Update is a DNS update, it
would be the Question Section in a DNS query or response) that indicates the zone to be
updated. Every delete and update in an SRP Update
the SRP Update.
...
MUST
 ...
 contain a single entry in the Zone Section (what

 be within the zone that is specified for
...
◦
if the Service Discovery Instruction is a "Delete An RR From An RRSet" instruction, that
...
SRV and TXT records describing that service. An "Add to an RRset" instruction for the
KEY record here is allowed but not implicit.
...
are being used (Section 7.1 of the DNS‑SD specification
...
An RRset" update for the corresponding TXT RR, and the target of the SRV RR
hostname given in the Host Description Instruction in the SRP Update, or

MUST

 be at least one "Add To
MUST
...
MUST
 ...
 correctly handle compressed names in the SRV target.
...
[RFC3927]
For example, if an IPv4 link-local address
 is
...
MUST
 ...
 contain exactly one Host Description Instruction. Multiple Service

An SRP Update
Discovery updates and Service Description updates may be combined into a single SRP Update
along with a single Host Description update, as described in Section 3.2.3. A DNS Update message
that contains any additional adds or deletes that cannot be identified as Service Discovery,
Service Description, or Host Description Instructions is not an SRP Update. A DNS update that
contains any prerequisites is not an SRP Update.

 include an EDNS(0) Update Lease option

An SRP Update
MUST
specified in the Update Lease option
...
[RFC9664]
 be less than or equal to the KEY-LEASE time. A DNS

. The LEASE time

MUST

When an SRP registrar receives a DNS Update message that is not an SRP update, it
the update as normal DNS Update
checks, if supported. Otherwise, the SRP registrar
RCODE.
...
, including access control checks and constraint
...
Service Description Instructions, the KEY from the Host Description Instruction is used. If any
...
then the SRP registrar
 reject the SRP Update with a YXDomain RCODE indicating that the
...
been explicitly present. After the SRP Update has been applied, every Service Description that is
 have the same value as the KEY RR that is present in
updated
...
MUST
 ...
 be zero except for bit 7, which can be one in the case of a zone key. SRP requesters

MUST

The IETF specification for DNSSEC Resource Records
KEY RR
implementing this version of the SRP specification
zeroes. SRP registrars implementing this version of the SRP specification
...
MUST
 ...
 set the flags field in the KEY RR to all
MUST

 accept and store

 states that the flags field in the
...
SRP registrars
 treat the update instructions for a service type and all its subtypes as
...
[RFC3596]
 ...
 and for IPv6 in Section 2.5 of the later document updating DNS

 add a Reverse Mapping PTR record (described for IPv4 in Section 3.5 of
[RFC1035]
...
for IPv6
) that corresponds to the Host Description. This is optional: The reverse
mapping PTR record serves no essential protocol function. One reason to provide reverse
mappings is that they can be used to annotate logs and network packet traces. In order for the
registrar to do a reverse mapping update, it must be authoritative for the zone that would need
to be updated or have credentials to do the update. The SRP requester
mapping update if it has credentials to do so.
...
MAY
 ...
 apply additional criteria when accepting updates. In some networks, it

The SRP registrar
may be possible to do out-of-band registration of keys and only accept updates from
preregistered keys. In this case, an update for a key that has not been registered
...
requester to update records that refer to its Host Description. Implementations of traditional
DNS Update

 do not normally provide a way to enforce a constraint of this type.
...
). In order to avoid inconsistencies, SRP places restrictions on

[RFC2181]

Requesters sending SRP Updates
contained within an SRP Update.

MUST

 use consistent TTLs in all RRs within each RRset

SRP registrars
Update are the same. If they are not, the SRP update

MUST

 check that the TTLs for all RRs within each RRset contained within an SRP

MUST

 be rejected with a Refused RCODE.

Additionally, when adding RRs to an RRset (for example, when processing Service Discovery
records), the SRP registrar
is enforced is up to the implementation.

 use the same TTL on all RRs in the RRset. How this consistency

MUST

TTLs sent in SRP Updates are advisory: they indicate the SRP requester's guess as to what a good
TTL would be. SRP registrars may override these TTLs. SRP registrars
 ensure that TTLs
are reasonable: neither too long nor too short. The TTL
...
some additional bookkeeping is required. When an update is constructed by the SRP requester, it
 include an EDNS(0) Update Lease Option
. The Update Lease Option contains two
MUST
...
ends in case the registrar is temporarily unavailable or under heavy load.
 ...
 refresh them well before the lease
...
hostname and all services that reference it
 be removed at the same time: It is never valid
for a service instance to remain when the hostname it references has been removed. If the KEY
record for the hostname is to remain, the KEY record for any services that reference it
remain. However, the Service Discovery PTR record
...
there is no service instance on the target of the PTR record.
 ...
 be removed since it has no key
...
MUST
 ...
 also track a lease time per service instance. The reason being that a
...
some different service instance had previously been registered. In this case, when that service
 remove the service instance, and any associated
instance lease expires, the SRP registrar
...
has forgotten about them.
 ...
 be retained until the KEY lease on that service expires). This is
...
MUST
 ...
 include an EDNS(0) Update Lease option in the response. The requester

 check for the EDNS(0) Update Lease option in the response, and when deciding when to
 use the lease times from the Update Lease option in

The SRP registrar
MUST
renew its registration the requester
the response in place of the lease times that it originally requested from the registrar. The times
may be shorter or longer than those specified in the SRP Update. The SRP requester must honor
them in either case.
...
SHOULD
 ...
 assume that each lease ends N seconds after the update was first

SRP requesters
transmitted (where N is the granted lease duration). SRP registrars
...
requester thinks the lease has not yet expired. In addition, the SRP requester
 begin
attempting to renew its lease in advance of the expected expiration time, as required by the DNS
Update Lease specification
, to accommodate the situation where the clocks on the SRP
...
SRP registrars
authoritative servers that allow both SRP and non-SRP DNS updates
don't include leases, but they
SHOULD
MUST

 reject updates that do not include an EDNS(0) Update Lease option. DNS
 accept updates that
...
of the administrative domain of the registrar.
 ...
 be configured to reject updates from source addresses outside
...
. If the network
way-handshake validation
infrastructure allows it, an SRP registrar
 accept TCP Fast Open payloads if all such packets
...
. This would ordinarily be accomplished by

. For example, a stub router
...
example, imagine that you set up SRP service for "example.com". SRP requesters can now
register names like "www" or "mail" or "smtp" in this domain. In addition, SRP Updates using
FCFS Naming can insert names that are obscene or offensive into the zone. There is no simple
solution to these problems. However, we have two recommendations to address this problem:
...
names like "www", "mail", "smtp", and so on. Lists of names are generally available or can
...
Descriptions
 provide a means for constraining what behaviors are allowed for a
...
the DNS-over-TLS specification
...
6.6.  Required Signature Algorithm
 ...
 implement the ECDSAP256SHA256 signature algorithm. SRP

 implement the algorithms that are listed in Section 3.1 of the DNSSEC

MUST

For validation, SRP registrars
registrars
SHOULD
[RFC8624]
Cryptographic Algorithms specification
", "
numbered 13 or higher and that have a "
MUST
validation column of the table. SRP requesters
MUST NOT
lower than 13 is available for use in validating SIG(0) signatures.

, in the validation column of the table, that are
MAY
RECOMMENDED

" designation in the
 assume that any algorithm numbered
...
Because DNS‑SD SRP Updates can be sent off-link, the privacy implications of SRP are different
 also
from those for mDNS responses. SRP Requester implementations that are using TCP
SHOULD
 offer TLS support.
use DNS-over-TLS
...
over-TLS specification
 ...
 if available. SRP registrar implementations
...
SRP requesters that are able to use TLS
required to support TLS, whether to use TLS is entirely the decision of the SRP requester.

 fall back to TCP. Since all SRP registrars are

SHOULD NOT

 elect not to return KEY
Public keys can be used as identifiers to track hosts. SRP registrars
...
record can be validated. If the zone is not signed, the authoritative DNS server
return a negative response (either NXDOMAIN or no data).
...
SHOULD NOT
 ...
 be treated as more trustworthy than any other insecure DNS
...
MUST NOT
 ...
 treat them as having special significance, except that it may be necessary

 recognize names that end in "service.arpa." as
...
DNSSEC OK (DO) bit set (Section 3.2.1 of the DNSSEC specification
validation

 is a best current practice: Although validation is not required, a caching
...
correct information in the authority section that proves that the record is nonexistent.
 ...
 be forwarded, with one important exception: a query for a DS
MUST
...
However, to provide accurate authority information, a query for the DS record
 result
...
DNS server operators
 configure an authoritative DNS server for "service.arpa." for use with
...
the authority of the Internet Architecture Board (IAB)
registrars for "arpa.".
...
of "service.arpa." in the "arpa." zone
...
, has added a delegation
, following the guidance provided in Section 7 of
...
registry of the "Locally-Served DNS Zones" group
...
"service.arpa. Subdomain" registry
entries may be added either as a result of Standards Action (Section 4.9 of the IANA Guidelines)
, provided that the values
or with IESG Approval (Section 4.10 of the IANA Guidelines)
and their meanings are documented in a permanent and readily available public specification,
in sufficient detail so that interoperability between independent implementations is possible.

. The IETF has change control for this registry. New
...
Default domain for SRP Updates
...
. The following subsections contain tables with the fields required by Section
...
, similar to the Port Control Protocol
...
DNS‑SD Service Registration Protocol Anycast Address
...
Eastlake 3rd, D. "Storage of Diffie-Hellman Keys in the Domain Name System
(DNS)" RFC 2539 DOI 10.17487/RFC2539
editor.org/info/rfc2539>
...
T. Lemon "Special-Use Domain 'home.arpa.'" RFC 8375 DOI
...
S. Cheshire "DNS Push Notifications" RFC 8765 DOI 10.17487/
...
Cheshire, S.
Updates" RFC 9664 DOI 10.17487/RFC9664
editor.org/info/rfc9664>
...
T. Lemon "An EDNS(0) Option to Negotiate Leases on DNS
...
"Internet Architecture Board statement on the registration of special use names
in the ARPA domain"
correspondence-reports-documents/2017-2/iab-statement-on-the-registration-of-
...
IANA "Locally-Served DNS Zones" <https://www.iana.org/assignments/locally-
served-dns-zones>
...
M. Krochmal "Requirements for a Protocol to Replace the
Cheshire, S.
...
R. Penno "Port Control Protocol (PCP) Anycast Addresses" RFC
<https://www.rfc-editor.org/info/
...
doc/html/draft-cheshire-dnssd-roadmap-03>
...
J. Hui "Automatically Connecting Stub Networks to Unmanaged

 and
Infrastructure" Work in Progress Internet-Draft, draft-ietf-snac-simple-06
,
November 2024,

<https://datatracker.ietf.org/doc/html/draft-ietf-snac-simple-06>
.
...
IANA "service.arpa Subdomain" <https://www.iana.org/assignments/locally-
served-dns-zones/locally-served-dns-zones>
...
Steinberg, D.H.
Guide" O'Reilly Media, Inc. ISBN 9780596101008
,
...
S. Cheshire "Zero Configuration Networking: The Definitive

, December 2005.

Appendix A.  Using Standard Authoritative DNS Servers
...
Appendix B.  How to Allow SRP Requesters to Update Standard
...
Update
 but not SRP will fail because the zone being updated is "default.service.arpa."
...
support standard DNS Update
 because the RCODE will either be NotZone, NotAuth, or
...
MAY
 ...
 attempt to register itself using normal DNS updates

In this case, a requester
...
scope for SRP, and a requester that implements SRP
 if that
itself and only attempt to use backwards capability with normal DNS Update
fails. Although the owner name of the SRV record for DNS Update (_dns-update._udp) specifies
UDP, it is also possible to use TCP, and TCP
SHOULD

 first attempt to use SRP to register

 be required to prevent spoofing.
...
Toke Høiland-Jørgensen Jonathan Hui Esko Dijk Kangping Dong
 and

Thanks to
Keshavarzian
 for their thorough technical reviews. Thanks to
testing the document by doing an independent implementation. Thanks to
doing a nice developmental edit,
implementation at the Montreal Hackathon at IETF 102, and
the hackathon and afterwards. Thanks to
Thanks also to
Cowan
David Dong Joey Salazar Jean-Michel Combes
reviews. Thanks to
members who provided comments or simply took the time to review the document.

 for their detailed second last call reviews. Thanks to
Joerg Ott

Nathan Dyck Gabriel Montenegro Kangping Dong Martin Turon
...
Patrik Fältström Dhruv Dhody
,
,
 for their respective directorate

 for
 for doing an SRP requester proof-of-concept

Tamara Kemper

 for a really thorough second Last Call review.

 for a really detailed IESG review! Thanks also to the other IESG

Abtin

 as well for

 for reviewing during
