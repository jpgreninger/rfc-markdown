...
(TRILL): Explicit Congestion Notification (ECN)
Support
...
without having to drop packets. This can improve network efficiency through better congestion
 ...
 allows a forwarding element (such as
...
control without packet drops. The forwarding element can explicitly mark a proportion of
...
, it was recognized that tunnels and lower-layer protocols would need to support
...
encapsulate IP packets, including propagation of ECN from and to IP.
 ...
 gives guidelines on the addition of ECN to protocols like TRILL that often
...
A large part of the guidelines for adding ECN to lower-layer protocols
...
specification, because the method specified assures that, if an egress RBridge is ECN ignorant (so
...
The terminology and acronyms defined in
...
feature invoked in some further extension of the TRILL header after the extension flags word.
[RFC7780] [RFC7179]
...
. Nonetheless, the scheme is designed to safely propagate some form of
 ...
 behave as described in the relevant subsections below,
...
indicating that ECN is understood by the ingress RBridge), the ingress RBridge
the guidelines in
 to add a flags word to the TRILL header. For a non-
...
MUST
 ...
 implement some form of AQM according to the
...
critical congestion on a TRILL Data packet, it
 set the CCE flag (flags word bit 26). Note
that Classic ECN marking
variants in Section 4.1 use a combination of critical and non-critical congestion indications.

 only uses critical congestion indications, but the two
...
Note that a transit RBridge that supports ECN does not refer to the TRILL-ECN field before
...
extension flags word is present, the 3-bit ECN codepoint is assumed to be all zero bits.
 ...
 set the ECN field of the outgoing IP packet to the codepoint at the
...
frame indicates a Not-ECT, it can be seen that the RBridge
 drop the packet. Such packet
...
CCE Arriving TRILL 3-Bit ECN Codepoint Name
...
Arriving TRILL 3-Bit ECN Codepoint Name
...
An asterisk in Table 3 indicates a combination that is currently unused in all variants of ECN
marking (see Section 4) and therefore
...
marking that complies with the guidelines in
a second level of marking severity below CE.

[RFC9599]

, including cases where ECT(1) represents
...
of the standardized form of ECN in IP
...
. It is expected that the TRILL ECN wire protocol is generic enough to
...
document.
 ...
 is recognized by the use of a PCN-compatible Diffserv codepoint
...
TRILL-ECN (Explicit Congestion Notification)
...
Critical Congestion Experienced (CCE)
...
TRILL support of ECN is a straightforward combination of previously specified ECN and TRILL
with no significant new security considerations.

For general security considerations regarding adding ECN to lower layer protocols, see
and
...
,
(RBridges): Base Protocol Specification" RFC 6325 DOI 10.17487/RFC6325
2011,
...
Eastlake 3rd, D. Ghanwani, A. Manral, V. Li, Y.
C. Bestler "Transparent
Interconnection of Lots of Links (TRILL): Header Extension" RFC 7179 DOI
10.17487/RFC7179
...
IANA "TRILL Extended Header Flags" <http://www.iana.org/assignments/trill-
,
,
parameters/>
...
. L4S is one example of the ways TRILL ECN handling may evolve

. It is similar to the original ECN wire protocol for IP
...
drop by a "Classic" queue (contrary to the original requirement for ECN
...
the likelihood that the Classic queue drops packets is defined as the square of the likelihood
...
already described in Section 3.3 for Classic ECN
