...
additions and changes have been made in the Unicode Standard that aﬀect the values produced
by the algorithm IDNA2008 speciﬁes. IDNA2008 allows adding exceptions to the algorithm for
...
2008, and despite not being completed until 2010, is widely known as "IDNA2008". It is speciﬁed in
the series of documents listed in Section 2.1. The IDNA2008 standard includes an algorithm by
which a derived property value is calculated based on the properties deﬁned in the Unicode
Standard.

The derived property values that can be calculated are deﬁned in
summary to aid in the reading of this document. For deﬁnition of the terms, please see
[RFC5892]
...
Those that are allowed to be used in IDNs. Code points with this property
...
in certain contexts or problematic in others, require that it not be used in labels unless speciﬁc
other characters or properties are present. The abbreviated term CONTEXT is used to refer to
this value. As explained in
CONTEXTO.

, CONTEXT is in turn divided into CONTEXTJ and

RFC 5892 [RFC5892]

DISALLOWED:

Those that should clearly not be included in IDNs. Code points with this property
...
Those code points that are not designated (i.e., are unassigned) in the Unicode
...
Problems can also be created if the properties assigned to those code points are inconsistent
...
changes to the Unicode Standard did not warrant an update to
...
; however, the changes in the derived property
Unicode 6.0.0 and
values do not result in exceptions (as deﬁned in Section 2.6 of
RFC 5892 [RFC5892]
) that would
...
Further, in 2015, the Internet Architecture Board (IAB) issued
...
(U+08A1) that was introduced in
statement was
of code point sequences not normalizing as one might expect appear in
the discussion [IDNA7]

 to focus on the latter request. More details about the problem
a draft that was part of

. In February of that year, the

Unicode 7.0.0 [Unicode-7.0.0]

a statement [IAB2005-1]
...
similar issues. While the aﬀected code points remain PVALID in this document, identiﬁcation of
the problem resulted in a clariﬁcation of the review process for new Unicode versions. That
clariﬁcation, which reinforces the original review plan to capture issues like these, was published
as
. Any review of Unicode versions after 12.0.0 should be made according to
RFC 8753 [RFC8753]
versions after version 12.0.0 can be made.

; an objective of this document is to ensure that a proper review of such
...
• "Internationalized Domain Names for Applications (IDNA): Deﬁnitions and Document

Framework" [RFC5890]
other material that are needed for understanding other documents in the set.

, informally called "Defs" or "Deﬁnitions", contains deﬁnitions and

• "Internationalized Domain Names in Applications (IDNA): Protocol" [RFC5891]
...
interpreted in combination with the Bidi document (described below).
obsoletes
[RFC3491]
...
RFC 3491 [RFC3491]
 ...
 and, in particular, the use of the tables to which
...
, informally called "Tables", lists the categories and rules that identify the code
...
, informally called "Bidi", speciﬁes special rules for labels that contain characters

• "Internationalized Domain Names for Applications (IDNA): Background, Explanation, and

, informally called "Rationale", provides an overview of the protocol and
...
, informally called "Mapping", discusses the issue of mapping characters into other
...
example this.
 ...
 describes some changes made to
...
PVALID to DISALLOWED. The IETF concluded that no update to
needed based on the changes made in
. As a result, the derived
property value remained aligned with the Unicode Standard. Speciﬁcally, no exception was
added.
...
for "stored strings" and "query strings". The tables themselves are dependent on
[Unicode-3.2.0]
...
A number of variations on IDNA2003, sometimes presented as "updated IDNA2003" or the like,
which follow the principles of IDNA2003 as understood by the implementers but that use
Nameprep
tables that represent how the implementers believe
[RFC3491]

 would have evolved had the IETF not moved in the direction of IDNA2008 instead.
...
A mix between IDNA2003 and IDNA2008 where code points assigned to Unicode after
3.2.0 [Unicode-3.2.0]
speciﬁed in IDNA2008.

 have derived property value calculated according to the algorithm

Unicode

•

. Because that document speciﬁes diﬀerent proﬁles, there are several variations that

A mix between IDNA2003 and IDNA2008 according to the
[UTS-46]
...
UTS#46 is ultimately based on a normative table very much like the one used by
[RFC3454]

 but updated for each new version of Unicode.
...
points to include in such a subset.
 recommend to DNS
...
7.0.0. This was discussed extensively in the IETF and also by the IAB in
requesting the IETF to investigate the issue. Speciﬁcally, the IAB stated:

their statement [IAB2005-1]

On the same precautionary principle, the IAB recommends that the Internationalized
Domain Names for Applications (IDNA) Parameters registry
assignments/idna-tables/>
on a solution to this problem.

 not be updated to Unicode 7.0.0 until the IETF has consensus
...
Although the discussion about this speciﬁc code point resulted in acceptance of the derived
...
Category changed from Lo (Other_Letter) to Ll (Lowercase_Letter) to reﬂect their status as the
...
The U+111C9 SHARADA SANDHI MARK was added to
the IDNA2008 algorithm to the code point assigned the derived property value DISALLOWED.
The changes in the underlying properties in
...
The characters ZANABAZAR SQUARE VOWEL SIGN AI (U+11A07) and ZANABAZAR SQUARE
VOWEL SIGN AU (U+11A08) were added to
...
and 12.0.0, aﬀecting the code point U+111C9. Its derived property value thus changed from
DISALLOWED to PVALID. In situations like these, IDNA2008 allows for addition of rules to
, Section 2.7. If the code point is accepted, it might still be rejected if validated by
5892 [RFC5892]
...
) for this code point. This also ensures all sandhi marks are treated
...
12.0.0. Some changes to speciﬁc characters changed their derived property value, whereas other
...
. Separately from any
added to IDNA2008 after the review process explained in
...
changes in Unicode, the IETF might conclude that updates to
IDNA2008 documents might become necessary; such updates might include changes to the
algorithm speciﬁed in IDNA2008 as well as additional rules, categories, or other forms of tuning,
like the clariﬁcations in
...
"IDNA Rules and Derived Property Values" [IANA-IDNA]

 registry after the expert
...
Not following these recommendations can lead to various security issues. Speciﬁcally, allowing
...
M. Blanchet "Nameprep: A Stringprep Proﬁle for
,
...
C. Karp "Right-to-Left Scripts for Internationalized
Alvestrand, H., Ed.
Domain Names for Applications (IDNA)" RFC 5893 DOI 10.17487/RFC5893
2010,
...
P. Hoﬀman, Ed. "The Unicode Code Points and
...
Internet Architecture Board "IAB Statement on Identiﬁers and Unicode 7.0.0"
January 2015,
documents/2015-2/iab-statement-on-identiﬁers-and-unicode-7-0-0/archive/>
.
...
February 2015,
.
documents/2015-2/iab-statement-on-identiﬁers-and-unicode-7-0-0/>
...
.
www.iana.org/assignments/idna-tables-12.0.0/>
...
,
P. Faltstrom "IDNA Update for Unicode 7.0 and Later Versions"
,

Klensin, J. C.
Work in Progress Internet-Draft, draft-klensin-idna-5892upd-unicode70-05
October 2017,
<https://datatracker.ietf.org/doc/html/draft-klensin-idna-5892upd-
.
unicode70-05>
...
M. Blanchet "Preparation of Internationalized Strings
...
P. Hoﬀman "Mapping Characters for Internationalized Domain
,
.
...
, and
Sullivan, A. Thaler, D. Klensin, J.
O. Kolkman "Principles for Unicode Code
,
Point Inclusion in Labels in the DNS" RFC 6912 DOI 10.17487/RFC6912
.
...
P. Fältström "Internationalized Domain Names for Applications

Klensin, J.
(IDNA) Review for New Unicode Versions" RFC 8753 DOI 10.17487/RFC8753
2020,
...
The Security and Stability Advisory Committee "SAC084" SSAC Comments on
Guidelines for the Extended Process Similarity Review Panel for the IDN ccTLD
Fast Track Process
sac-084-en.pdf>
.
...
The Unicode Consortium "The Unicode Standard, Version 3.2.0" Mountain
...
View: The Unicode Consortium ISBN 0-201-61633-5
www.unicode.org/versions/Unicode3.2.0/>
.

,

The Unicode Consortium "The Unicode Standard, Version 5.2.0" Mountain

,

,
View: The Unicode Consortium ISBN 978-1-936213-00-9
...
The Unicode Consortium "The Unicode Standard, Version 6.0.0" Mountain

,

,
View: The Unicode Consortium ISBN 978-1-936213-01-6
...
The Unicode Consortium "The Unicode Standard, Version 7.0.0" Mountain

,
View: The Unicode Consortium ISBN 978-1-936213-09-2
.
...
,
View: The Unicode Consortium ISBN 978-1-936213-10-8
.
www.unicode.org/versions/Unicode8.0.0/>
...
,
View: The Unicode Consortium ISBN 978-1-936213-16-0
.
...
,
View: The Unicode Consortium ISBN 978-1-936213-19-1
.
www.unicode.org/versions/Unicode11.0.0/>
...
The Unicode Consortium "The Unicode Standard, Version 12.0.0" Mountain

,
View: The Unicode Consortium ISBN 978-1-936213-22-1
.
...
,
The Unicode Consortium "Unicode Technical Standard #46, Version 12.0.0"
...
Harald Alvestrand Marc Blanchet Martin Dürst Asmus Freytag Ted Hardie John

Thanks to
,
Klensin Erik Nordmark Pete Resnick Peter Saint-Andre Michel Suignard Andrew Sullivan
,
Suzanne Woolf

 for input to this document.
