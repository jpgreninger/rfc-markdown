...
RFC 9685
Listener Subscription for IPv6 Neighbor Discovery
Multicast and Anycast Addresses

Abstract

This document updates the 6LoWPAN extensions to IPv6 Neighbor Discovery (specified in RFCs
4861 and 8505) to enable a listener to subscribe to an IPv6 anycast or multicast address. This
document also updates the Routing Protocol for Low-Power and Lossy Networks (RPL) (specified
in RFCs 6550 and 6553) to add a new Non-Storing multicast mode and new support for anycast
addresses in Storing and Non-Storing modes. This document extends RFC 9010 to enable a
6LoWPAN Router (6LR) to inject the anycast and multicast addresses in RPL.
...
Multicast and Anycast Subscription
...
RPL uses Destination Advertisement Object (DAO) messages to establish Downward routes. DAO
...
The classical Neighbor Discovery (ND) protocol
...
or more 6LoWPAN Routers (6LRs) that serve it. To that effect,
...
Advertisement (NA) messages between the 6LN and the 6LR.
 ...
 changes the classical IPv6 ND model to proactively establish the
...
protocol and provides return reachability for that address.
 ...
 so that a generic Address Registration

 updates

"Routing for RPL (Routing Protocol for Low-Power and Lossy Networks) Leaves"
provides the router counterpart of the mechanism for a host that implements
 to inject
...
[RFC9010]
addresses, and there is no equivalent of
subscription of one or more multicast addresses.

 to specify the 6LR behavior upon the
...
node that already implements
support to subscribe the multicast addresses they listen to.

 for unicast reachability, it makes sense to extend that
...
[RFC8505]
 ...
 by adding the capability for the 6LN to
...
to implement the amended RFC that they
MUST

 also implement the amending RFC.
...
address may be redistributed.
 ...
 to provide a registration method (called "subscription" in

 that already protects the address registration in

 is agnostic to the routing protocol in which the
...
As opposed to unicast addresses, there might be multiple registrations from multiple parties for
...
[RFC8505]
 ...
 may be used for an ND proxy operation as specified in
...
of the sequence counter in the TID field of the EARO.
 ...
 or in the Routing in Fat Trees (RIFT) protocol
...
[RFC6550]
 ...
 is the routing protocol of choice and

In the case of LLNs, RPL
the unicast address advertised with
...
happens as specified in
, including loop avoidance and detection, though
in the multicast case, multiple copies might be generated.

 is redistributed in RPL. This specification also
...
implement
[RFC8505]
behaviors to enable the registration for multicast addresses as an extension to
...
This specification also Extends
 to add multicast ingress replication (IR)
...
Energy) Bluetooth
over the TSCH mode of IEEE 802.15.4 (6TiSCH)
[RFC4919] [RFC6282]

 or a Route-Over LLN such as the Wi-SUN
...
A leaf acting as a 6LN registers its unicast addresses to a RPL router acting as a 6LR using a Layer
2 unicast NS message with an EARO as specified in
...
the address as the leaves. As prescribed in
, the 6LR forwards a multicast
...
In the new RPL Non-Storing Mode of Operation with ingress replication multicast support that is
...
In a simpler variation, the 6LNs do not set the R flag and the Root floods all the multicast
...
is a multicast address. This specification Amends
 by allowing the advertisement of
...
for use in the 6CIO.
 ...
 by defining a new capability bit
...
[RFC6550]
 ...
 to mandate the use of the ROVR field as the indication of the
...
[RFC6550]
 ...
 with a new P-Field in the RPL Target Option (RTO).
...
[RFC6550]
freshest unicast route and remove the stale ones, e.g., in the case of mobility.
...
This specification Extends
 for anycast and multicast advertisements to require that the
...
in the TIO so they are comparable when both forms of advertisements are received.
 ...
 maps the Address Registration lifetime in the EARO and the Path Lifetime
...
Though it was implicit with
, this specification clarifies that the freshness comparison
...
For the intermediate routers, the packet appears as any source-routed unicast packet. The
...
anycast in RPL to enable load balancing. In contrast, a mobile target that is multihomed should
...
Down the DODAG, the RPL router
address in their DAO messages and forward it to one parent if there is no such child.

 copy one and only one of the children that advertised the
...
•
 ...
 be ignored by the
When the value of 3 is received in an RTO (see Section 6.6), this value
...
In the case of an EARO (see Section 7.1) or an EDAR (see Section 7.2), the message
dropped, and the receiving node
or remain silent.

 be
 either reply with a status of 12 "Invalid Registration"
...
that the Target Address is to be processed as unicast, multicast, or anycast.
 ...
 to add the 2-bit P-Field (see Section 6.5) to the RTO to indicate

Section 2.7 of [RFC4291]
)
...
EDAC messages to signal the type of registration.
 ...
 by adding the concept of a subscription for anycast and
...
specification adds a new P-Field that is placed in the EARO flags and is set as follows:
 ...
 defines the EARO as an extension to the ARO defined in
...
This is a 2-bit field. It is reserved and
...
is only used in DAC messages and has to be set to 0 in DAR messages.
...
This is a 6-bit field. It is reserved and
...
New ARO Statuses are introduced to indicate a "Registration Refresh Request" and an
...
have missed a registration or lost a registration state)
...
That asynchronous multicast NA(EARO)
address (ff02::1), and the Target
previously by this node to accept registrations.

 be set to the link-local address that was exposed

 be sent to the all-nodes link-scope multicast
...
5.2 of [RFC8505]
 for the lollipop mechanism used in
...
Section
 ...
 be incremented each time. The receiver 6LN
...
than the SEQUENCE_WINDOW; see
same request. The 6LN
are desynchronized (not comparable) or decreased, then the NA(EARO) is considered as a
new request and it

 process one and only one of the series of messages. If the TIDs

 consider that multiple NA(EARO)

) are in fact indicative of the
...
The multicast NA(EARO)
 be resent enough times for the TID to be issued with the
...
between retries is 1 second, and the number of retries is 3. To reach 255 at boot time, the
 either issue at least 4 NA messages, skip a TID value, or start with a value that is
sender
more than 252. The best values for the short period, the number of retries, and the TID initial
setting depend on the environment and
...
◦
the Registered Address is a multicast address (
...
If this occurs, then the message, NS(EARO) or EDAR,
node

 either reply with a status of 12 "Invalid Registration" or remain silent.

 be dropped, and the receiving
...
MUST
 ...
 also subscribe all the IPv6 multicast addresses that it listens to, and it
...
, which is implicitly registered by all nodes,
...
MAY
 ...
 set the R flag in the EARO to obtain the delivery of the multicast packets by the
...
MUST
 ...
 also subscribe all the IPv6 anycast addresses that it supports, and it
...
The 6LR
the Source Link-Layer Address Option (SLLAO)) also registered ff02::1
nodes link-scope multicast address.

 also consider that all the nodes that registered an address to it (as known by
 to the all-
...
The 6LR
 maintain a subscription state per tuple (IPv6 address, ROVR) for both anycast
 notify the 6LBR with an EDAR message, unless it
and multicast types of addresses. It
...
[RFC9010]
 ...
 by adding the following behavior:

•

Upon a subscription with the R flag and the P-Field both set to 1 in the EARO, if the scope of
[RFC7346]
the multicast address is above link-scope
, then the 6LR injects the address in the
...
, it is possible for a node to autoconfigure a pair of public and private keys and
...
The first hop router (the 6LR) may then validate a registration and perform source address
...
the same address. In that context, the method specified in
...
MUST
 ...
 be generated from that key pair as specified in
 then be installed in the 6LBR so it can recognize the address and
...
[RFC8928]
 be associated with the address before it is

 to enforce the right

The key pair
multicast address, so the node can follow the steps in

MUST

 then be provisioned in each node that needs to subscribe to the anycast or

[RFC8928]

 to subscribe to the address.
...
A 1-bit flag set to 1 to indicate that the Peer NSSI field is valid; it

 be set to 0 when the
 be set to 1 when the message is unicast and the sender has

MUST

message is not unicast and
...
6-bit flags that are reserved and that
...
If the boot time of the sender is updated to a newer time, any state that the receiver installed in
 reassess all the state it installed
the sender before the reboot is probably lost. The receiver
...
by the sender before it rebooted
...
SHOULD
 ...
 be stored in persistent memory by the sender and incremented when it may
The NSSI
...
in a single RPL Instance administratively. This means that a multicast address that needs to span
 use a scope of Realm-Local whereas a multicast address that needs to span a
a RPL DODAG
RPL Instance
 use a scope of Admin-Local as discussed in
Multicast Address Scopes".

Section 3 of [RFC7346]
...
"IPv6 Addressing of IPv4/IPv6 Translators"
 enables embedding IPv4 addresses in IPv6
...
Local.
 ...
 use a multicast address with the appropriate scope, Realm-Local or Admin-

MUST

 enables forming 232 multicast
...
anycast operations using a multicast MOP.
 ...
 to deploy one RPL Instance in any MOP (typically MOP 1)
...
Using separate RPL Instances for unicast traffic on the one hand and for anycast and multicast
...
There
that require multicast or anycast services. In Storing mode, there
the 6LRs that support the mixed mode to also form a DODAG to the Root.

 be enough density of the 6LRs that support the mixed mode to cover all the 6LNs
 be enough density of
...
industry standard that mandates it. In that case, all the routers
mode.
...
This specification Extends
sections in these documents also apply to this document. In particular, the link layer
...
[RFC8505]
(RAN) in the RPL. Using an extension of
allows a RPL-Unaware Leaf (RUL) to subscribe as well. As noted in
...
allowed to inject RPL messages.
 ...
 as opposed to RPL to subscribe the address
[RFC9010]
, this provides a
...
, this specification enables tracking the origin of the multicast
...
[RFC8928]
 ...
 to prevent a rogue node from registering a unicast address that it
...
multicast service as specified in
target that is multicast and the P-Field set to 0
processed as if the P-Field is set to 1.

 be considered as multicast and

Section 12 of [RFC6550]

. In MOP 3, an RTO that is received with a
...
[IANA.RPL]
 ...
 and "Routing Protocol for Low Power and Lossy Networks (RPL)"
...
. The initial allocations are as indicated
...
X flag: Registration for Unicast, Multicast, and Anycast Addresses
Supported
...
IANA "Mode of Operation" <https://www.iana.org/assignments/rpl>
...
IANA "RPL Target Option Flags" <https://www.iana.org/assignments/
...
D. Thaler "Unicast-Prefix-based IPv6 Multicast Addresses"
...
Droms, R. "IPv6 Multicast Address Scopes" RFC 7346 DOI 10.17487/RFC7346
August 2014,
...
Control (MAC) and Physical Layer (PHY) Specifications" DOI 10.1109/IEEESTD.
2021.9363693 IEEE Std 802.11-2020 <https://ieeexplore.ieee.org/document/
9363693>
...
Networks (WPAN)" IEEE Std 802.15.1-2005 DOI 10.1109/IEEESTD.2005.96290
...
IEEE "IEEE Standard for Low-Rate Wireless Networks" DOI 10.1109/IEEESTD.
2020.9144691 IEEE Std 802.15.4-2020 <https://ieeexplore.ieee.org/document/
,
9144691>
...
M. Richardson "Architecture and Framework for

,

IPv6 over Non-Broadcast Access" Work in Progress Internet-Draft, draft-
ietf-6man-ipv6-over-wireless-06
html/draft-ietf-6man-ipv6-over-wireless-06>
...
Thubert, P., Ed. Przygienda, T.
,

, and
Signaling" Work in Progress Internet-Draft, draft-thubert-bess-secure-evpn-
mac-signaling-04
<https://datatracker.ietf.org/doc/html/
draft-thubert-bess-secure-evpn-mac-signaling-04>

J. Tantsura "Secure EVPN MAC
...
Thubert, P., Ed. "IPv6 Neighbor Discovery Prefix Registration" Work in

,

,

Progress Internet-Draft, draft-ietf-6lo-prefix-registration-06
<https://datatracker.ietf.org/doc/html/draft-ietf-6lo-prefix-registration-06>
...
Hui, J.
,
(MPL)" RFC 7731 DOI 10.17487/RFC7731
editor.org/info/rfc7731>

,

.

R. Kelsey "Multicast Protocol for Low-Power and Lossy Networks
...
Przygienda, A., Ed. Head, J., Ed. Sharma, A. Thubert, P. Rijsman, B.
Afanasiev "RIFT: Routing in Fat Trees" Work in Progress Internet-Draft, draft-
,
...
S. Krishnan "Definition of new tags for relations between
RFCs" Work in Progress Internet-Draft, draft-kuehlewind-rswg-updates-tag-02
,
,
8 July 2024,
updates-tag-02>

<https://datatracker.ietf.org/doc/html/draft-kuehlewind-rswg-
...
Robert, H. Liu, B. Zhang, M.
, and
Progress Internet-Draft, draft-heile-lpwan-wisun-overview-00
, 3 July 2017,
<https://datatracker.ietf.org/doc/html/draft-heile-lpwan-wisun-overview-00>

C. Perkins "Wi-SUN FAN Overview" Work in
...
This work is a production of an effective collaboration between the IETF 6lo WG and the Wi-Sun
FAN WG. Thanks to all in both WGs who contributed reviews and productive suggestions, in
particular:
Falendysz Don Sturek Dario Tedeschi Saurabh Jain and Chris Hett
,
Dijk
Ginoza Zaheduzzaman Sarker Paul Wouters Roman Danyliw John Scudder Dirk Von Hugo
,
Murray Kucherawy Kyle Rose Scott Kelly
,
, and
comments during the IETF last call and IESG review cycle.

Carsten Bormann Paul Duffy, Klaus Hueske Adnan Rashid Rahul Jadhav Gene
, with special thanks to
,

 for his useful WGLC reviews and proposed changes. Also many thanks to
,
