...
RFC 9716
Mechanisms for MPLS Ping and Traceroute
Procedures in Inter-Domain Segment Routing
Networks

Abstract

The Segment Routing (SR) architecture leverages source routing and can be directly applied to
the use of an MPLS data plane. A Segment Routing over MPLS (SR-MPLS) network may consist of
multiple IGP domains or multiple Autonomous Systems (ASes) under the control of the same
organization. It is useful to have the Label Switched Path (LSP) ping and traceroute procedures
when an SR end-to-end path traverses multiple ASes or IGP domains. This document outlines
mechanisms to enable efficient LSP ping and traceroute procedures in inter-AS and inter-domain
SR-MPLS networks. This is achieved through a straightforward extension to the Operations,
...
Many network deployments have built their networks consisting of multiple ASes either for the
...
[RFC8402]
 describes BGP peering segments, and
 specifies SR with an MPLS data plane.
 describes centralized BGP Egress Peer Engineering, which will help in steering packets
...
ASBR1, ASBR2, ASBR3, ASBR4, ASBR5, ASBR6, ASBR7,
...
: PeerNode SID, PeerAdj SID, and
Identifiers (SIDs) configured and advertised via
...
[RFC8403]
 describes mechanisms to carry out MPLS ping and traceroute from a Path Monitoring
...
necessary static routes, which may not be feasible.
 ...
 describes an Echo-relay-based solution that is predicated on advertising a new Relay

[RFC7743]
Node Address Stack TLV containing a stack of Echo-relay IP addresses. These mechanisms can be
applied to SR networks as well. The mechanism from
 requires the return ping packet
...
traceroute procedures.
 ...
 to convey the reverse path. Three new sub-TLVs are defined for the Reply Path TLV
...
This document focuses on the inter-domain use case. The protocol extensions described may also
...
advertisements with an MPLS label bound to the prefix as described in
paths using a list of segments as described in
...
. SR networks statically assign the labels to nodes,

The Reply Path (RP) TLV is defined in
...
The reverse path can be built from the PMS/head-end by stacking segments for the reverse path.
 is used to carry the return path. Reply Mode 5 (Reply
The Reply Path TLV as defined in
. While using the procedures described in
via Specified Path) is defined in
this document, the Reply Mode is set to 5 (Reply via Specified Path), and the Reply Path TLV is
included in the echo request message as described in
constructed as per
the SR Path.

. This document defines three new sub-TLVs to encode

Section 4.1 of [RFC7110]

Section 4.2 of [RFC7110]

. The Reply Path TLV is
...
TLV 21 (Reply Path TLV). If these sub-TLVs appear in TLVs 1 or 16, appropriate error codes
MUST
be returned as defined in
...
2 octets. Carries value 46 (assigned by IANA from the "Sub-TLVs for TLV Types 1, 16, and

21" registry).

Length:

2 octets. Carries value 8. The length value excludes the length of the Type and Length
...
3 bits of Traffic Class (TC). If the originator wants the receiver to choose the TC value, it

MUST

 set the TC field to zero.

S:

1 bit Reserved. The S bit
reception.
...
1 octet of TTL. If the originator wants the receiver to choose the TTL value, it
...
MAY
 ...
 override the originator's values for these fields. This would be determined by
The receiver
...
2 octets. Carries value 8 when no optional SID is included or value 12 when the optional
...
1 octet. When the A-Flag (as defined in Section 4.4) is present, this specifies the SR

Section 3.1.1 of [RFC8402]

 or the Flexible Algorithm as defined in

. The SR Algorithm is used by the receiver to derive the label. When the A-Flag is

Algorithm as described in
[RFC9350]
unset, this field has no meaning and thus
ignored on receipt.

MUST

 be set to zero (MBZ) on transmission and

IPv4 Node Address:

4-octet IPv4 address representing a node. The IPv4 Node Address
...
Optional 4-octet field containing the labels TC, S, and TTL as defined in Section 4.1. When

the SID field is present, it

MUST

 be used for constructing the Reply Path.
...
2 octets. Carries value 20 when no optional SID is included or value 24 when the
...
16-octet IPv6 address of one interface of a node. The IPv6 Node Address

MUST

 be present. It should be a stable address belonging to the node (e.g., loopback address).
...
This flag indicates the presence of an SR Algorithm ID in the SR Algorithm field

applicable to various Segment Types.

Unused bits in the Flag octet
receipt.
...
utilized regardless of the availability of IP connectivity. The LSP ping initiator
Mode of the echo request to 5 (Reply via Specified Path), and a Reply Path TLV
the echo request message correspondingly. The Reply Path TLV
reverse direction encoded as an ordered list of segments. The first segment
the top segment in the MPLS header that the responder

 contain the SR Path in the
 correspond to
MUST
 use while sending the echo reply.
...
, when the Reply Mode is set to 5 (Reply via Specified Path), the echo
...
the Reply Mode 5 defined in
, an echo reply with the Return Code set to "Malformed
...
. If the echo request message contains a malformed Segment sub-TLV, such
the rules of
...
When a Reply Path TLV is received, the responder that supports processing it
segments in Reply Path TLV to build the echo reply. The responder
Forwarding Equivalence Class (FEC) validation procedures as described in
[RFC8287]
reply has to be sent out, the Reply Path TLV
out.

 and this document does not suggest any change to those procedures. When the echo
 be used to construct the MPLS packet to send
...
Section 5.2 of [RFC7110]

 check the reachability of the top label

 be constructed as described in
MUST

 be constructed from the segments
 add any other label. The S bit is set for the bottom label as

[RFC8029]
. An MPLS packet is constructed with an
 be constructed from the first segment of the Reply
 be constructed by following the order of the segments from

The echo reply message is sent as an MPLS packet with an MPLS label stack. The echo reply
message
echo reply in the payload. The top label
Path TLV. The remaining labels
the Reply Path TLV. The MPLS header of the echo reply
in the Reply Path TLV and
. The responder
per the MPLS specifications
in its own Label Forwarding Information Base (LFIB) before sending the echo reply. If the top
 send the appropriate Return Code and follow the
label is unreachable, the responder
procedures as per
. The exception case is when the responder does not
...
at all. Even if sent (by following a default route present on the responder, for example), the echo
 provide necessary log information in case of
reply might not reach the originator. The node
 choose to send Type-C/Type-D segments
unreachability. In certain scenarios, the head-end
...
the node sending the echo reply
with the IPv4/IPv6 addresses. If an optional MPLS SID is present in the Type-C/Type-D segments,
the SID
 be used to encode the echo reply with MPLS labels. If the MPLS SID does not match
...
The Reply Path Return Code is set as described in
5.3 of [RFC7110]
path that the echo reply message is required to follow.

Section
, the Reply Path TLV is included in an echo reply indicating the specified return

Section 7.4 of [RFC7110]

. According to

When the node is configured to dynamically create a return path for the next echo request, the
procedures described in Section 5.5
0x0006, and the same Reply Path TLV or a new Reply Path TLV
reply.

 be used. The Reply Path Return Code

 be included in the echo
...
Section 5.4 of [RFC7110]

Section 4.6 of [RFC8029]
The rules and processes defined in
 apply
...
reply
 be sent in the next echo request with the TTL incremented by 1. If the initiator node
...
message.
 ...
 be ended with an appropriate log
...
network, which can be inferred from the LSDB, it is
segment. However, implementations
...
should be converted to a Type-A segment based on the ABR's own SRGB. This is because
 ...
 safely use other approaches if they see benefits in

 to add a Type-C or a Type-D
...
downstream nodes in the path will not know what SRGB to use to translate the IP address to a
...
the traceroute message was received. A Reply Path Return Code of 0x0006
echo reply to indicate that the next echo request
...
initiator, it
 choose to send one such path in the Reply Path TLV. The Reply Path TLV is built
...
is not guaranteed to be uniform in the domain.
 ...
 use the return path from the Reply Path
...
and EPE-SID on top of the existing segments in the Reply Path TLV.
 ...
 always translate the Reply Path TLV to a label stack and build an MPLS

An ASBR that receives the echo request from a neighbor belonging to the same AS
 look at
the Reply Path TLV received in the echo request. If the Reply Path TLV consists of a Type-C/Type-
D segment, it
from its own SRGB. The ASBR
constructed Reply Path TLV in the echo reply.

 convert the Type-C/Type-D segment to a Type-A segment by deriving a label

 set the Reply Path Return Code to 0x0006 and send the newly
...
head-end node/PMS that initiates the traceroute procedure
sent Reply Path TLV in the echo request message in every subsequent echo request.

 continue to send the previously
...
procedures. If such an ASBR is encountered in the forward path, dynamic return path building
 set the Return
procedures will fail. In such cases, an ASBR that supports this document
...
links or security-vulnerable links to prevent spoofing attacks.
 ...
 that an operator deploy security
 on inter-domain

 deploy appropriate filter policies as described in
...
Appropriate filter policies
the network. In the event of such a security breach, the network devices
to prevent denial-of-service attacks as described in

 be applied at the edges to prevent attackers from getting into
 have mechanisms

 will be applicable for this document.
...
Section 4.1 of RFC 9716

IPv4 Node Address with an optional SID for SR-MPLS

Section 4.2 of RFC 9716

IPv6 Node Address with an optional SID for SR-MPLS

Section 4.3 of RFC 9716
...
Use Reply Path TLV from this echo reply for building the next echo
request
...
S. Delord "Return Path Specified Label
...
Control (MAC) Security" IEEE Std 8021.AE-2018 DOI 10.1109/IEEESTD.
2018.8585421
...
Rosen, E. "Using BGP to Bind MPLS Labels to Address Prefixes" RFC 8277 DOI
,
10.17487/RFC8277
...
Filsfils, C., Ed. Previdi, S. Dawra, G., Ed. Henderickx, W.
"Interconnecting Millions of Endpoints with Segment Routing" RFC 8604 DOI
10.17487/RFC8604
...
PE4 for LSP ping is [N-ASBR4, EPE-ASBR4-ASBR1, N-PE1]. This example path provides the entire
...
[RFC8029]
. Every echo request packet head-end/PMS will include the appropriate
...
and P1 sends an echo reply using the return path. Note that implementations may choose to
...
If the packet needs to follow a return path specific to an algorithm (as defined in
...
us consider a path from PE1 to PE5 that goes through [PE1, ASBR1, ASBR4, ASBR6, ASBR8, PE5].
...
ASBR8, the EPE-SID from ASBR8 to ASBR6 is added to the Reply Path TLV. While visiting nodes in
...
. This traceroute doesn't need any changes to the Reply Path

. P1 sends an echo reply with the same Reply Path TLV with
...
its own SRGB. When ASBR4 receives the echo request, it should form this Reply Path TLV using its
...
,
 ...
 for suggesting the use of the generic Segment sub-TLV. Thanks to

Bruno Decraene

Thanks to
Adrian Farrel Huub van Helvoort Dhruv Dhody
comments. Thanks to
Mirsky
extent.

Gregory
 for the detailed review, which helped improve the readability of the document to a great

,
 for suggesting the use of the Reply Path TLV. Thanks to
