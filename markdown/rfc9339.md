...
may receive from R2 on that same link.
 ...
 routing protocols originates a
...
This document describes extensions to OSPF LLS
...
) links. It is not applicable for broadcast or Non-Broadcast
...
Reverse Metric: Unsigned integer of 2 octets that carries the value or offset of RM to replace or
...
MUST
 ...
 use the RM value to derive the metric for the link to the advertising router in its

A router receiving a Hello packet from its neighbor that contains the Reverse Metric TLV on a
link
...
modified its metric in response to receiving a Reverse Metric TLV from its neighbor
...
create collision scenarios. The following guidelines are
...
topologies for which it desires to signal the RM). A router
...
In certain scenarios, the OSPF router may also require the modification of the TE metric being
advertised by its neighbor router towards itself in the inbound direction. Using similar
procedures to those described above, the Reverse TE Metric TLV
reverse TE metric for router links. The neighbor
...
of RM). The rules for doing so are analogous to those given above for the Router-LSA.
 ...
 use the reverse TE metric value to derive
...
Routers that receive an RM advertisement
 log an event to notify system administration.
SHOULD
...
explicit user configuration, this
 immediately trigger the CSPF (Constrained Shortest Path
...
MUST NOT
 ...
 signal RM to neighbors by default and

 accept the RM from its neighbors by default. An implementation

An implementation
configuration option to enable the signaling of RM on specific links. An implementation
NOT
to accept the RM globally on the device, or per area, but an implementation
...
For the use case in Section 2.1, it is
 that the network operator limit the period of
...
IANA has registered code points from the "Link Local Signalling TLV Identifiers (LLS Types)"
registry in the "Open Shortest Path First (OSPF) Link Local Signalling (LLS) - Type/Length/Value
Identifiers (TLV)" registry group for the TLVs introduced in this document as follows:
...
extension described in this document. The purpose of using the reverse metric TLVs is to alter
the metrics used by routers on the link and influence the flow and routing of traffic over the
network. Hence, modification of the Reverse Metric and Reverse TE Metric TLVs may result in
traffic being misrouted. If authentication is being used in the OSPFv2 routing domain
[RFC7474]
the contents of the LLS block.
...
A router that is misbehaving or misconfigured may end up signaling varying values of RMs or
toggle the state of RM. This can result in a neighbor router having to frequently update its Router
LSA, causing network churn and instability despite existing OSPF protocol mechanisms (e.g.,
MinLSInterval, and
of frequent changes in RM signaling and ignore the RM (i.e., revert to using their provisioned
metric value) during such conditions.

 that implementations support the detection
...
The reception of malformed LLS TLVs or sub-TLVs
rate-limited to prevent Denial of Service (DoS) attacks.

SHOULD

 be logged, but such logging
...
Clos, C. "A study of non-blocking switching networks" The Bell System Technical
Journal, Vol. 32, Issue 2 DOI 10.1002/j.1538-7305.1953.tb01433.x
...
A. Lindem "OSPF Two-Part Metric" RFC 8042 DOI
...
M. Abrahamsson "IS-IS Routing with Reverse Metric"
...
• Jay Karthik
 ...
 for his contributions to the use cases and the review of the solution.

• Les Ginsberg Aijun Wang Gyan Mishra Matthew Bocci Thomas Fossati
...
• Acee Lindem
 ...
 for a detailed shepherd's review and comments.

• John Scudder

 for his detailed AD review and suggestions for improvement.

The document leverages the concept of RM for IS-IS, its related use cases, and applicability
aspects from
