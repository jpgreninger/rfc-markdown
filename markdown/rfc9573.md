...
RFC 9573
MVPN/EVPN Tunnel Aggregation with Common
Labels
...
upstream-assigned MPLS label that the ingress PE has bound to the packet's VPN.
 ...
 for each VPN, and each packet sent on the P2MP tunnel carries the
...
Essentially, for each ingress PE, the egress PE has a context-specific label space
...
, an SRGB is a local property of a node and identifies the set of
...
source ES and thus will not transmit the packet on its local Attachment Circuit to that ES. From
...
For example, all PEs could reserve a DCB [1000~2000], and they would all be provisioned so that
label 1000 maps to VPN 0, label 1001 maps to VPN 1, and so forth. Now, only 1000 labels instead
of 1,000,000 labels are needed for 1000 VPNs.
...
or a few context-specific label spaces that are independent of each PE. For example, if it is too
difficult to have a DCB of 10,000 labels across all PEs for all the VPNs/BDs/ESs that need to be
supported, a separate context-specific label space can be dedicated to those 10,000 labels. Each
separate context-specific label space is identified in the forwarding plane by a label from the DCB
(which does not need to be large). Each PE is provisioned with the label-space-identifying DCB
label and the common VPN/BD/ES labels allocated from that context-specific label space. When
...
sending traffic, an ingress PE imposes all necessary service labels (for the VPN/BD/ES) first, then
imposes the label-space-identifying DCB label. From the label-space-identifying DCB label, an
egress PE can determine the label space where the inner VPN/BD/ES label is looked up.

[RFC6514]

 assumes that certain MPLS labels
The MVPN/EVPN signaling defined in
...
[RFC7582]
 ...
 ("Multicast Virtual Private Network (MVPN): Using Bidirectional P-Tunnels"), when

Per
MP2MP tunnels are used for an MVPN, the root of the MP2MP tunnel is required to allocate and
Section 4 of [RFC6513]
advertise "PE Distinguisher Labels" (
). These labels are assigned from the
...
come from the same label space that the node uses to allocate its VPN-identifying labels.
 ...
 by this document that the PE Distinguisher Labels allocated by a particular node
...
For selective tunnels that instantiate S-PMSIs (see Sections 2.1.1 and 3.2.1 of
...
Inter-AS I-PMSI or EVPN per-region I-PMSI) tunnels
, labels need
...
In the remainder of this document, when we say that a BGP-MVPN/EVPN A-D route carries a DCB-
...
The protocol and procedures specified in this section
...
MUST
 ...
 be used when BIER or P2MP/MP2MP
...
or one of those few context-specific label spaces, and every PE that is part of the VPN/BD/ES is
 be assigned from the same label
aware of the assignment. The ES label and the BD label
space. If PE Distinguisher Labels are used
space as well.

 be allocated from the same label
...
the ESI, and then imposes the encapsulation for the transport tunnel.
 ...
 be attached to the route. The ID-Type in the EC is set to
...
route.
 ...
 place an entry for the label that is in the PTA or ESI Label EC in

MUST NOT

 carry both the DCB-flag and the Context-Specific Label Space ID

An x-PMSI/IMET route
EC. A received route with both the DCB-flag set and the Context-Specific Label Space ID EC
attached
Space ID EC is attached, the label in the PTA or ESI Label EC
assigned label from the label space of the source PE, and procedures provided in
[RFC7432] MUST

 be treated as withdrawn. If neither the DCB-flag nor the Context-Specific Label
 be treated as the upstream-
...
MUST
 ...
 all carry the Context-Specific Label Space ID EC,
...
Otherwise, a receiving PE

MUST

 treat the routes as if they were withdrawn.
...
[RFC6514]
 and specifies corresponding signaling and procedures. The first method
 from the regular per-platform

This document allows three methods (Section 3.3) of label allocation for MVPN PEs
EVPN PEs
(Option 1) is the equivalent of using common SRGBs
label space. The second method (Option 2) is the equivalent of using common SRGBs from a
third-party label space
...
. The third method (Option 3) is a variation of the second in that
...
regard.
 ...
 do not list any security concerns related to label
...
Rosen, E. Wijnands, IJ. Cai, Y.
(MVPN): Using Bidirectional P-Tunnels" RFC 7582 DOI 10.17487/RFC7582
2015,

<https://www.rfc-editor.org/info/rfc7582>

A. Boers "Multicast Virtual Private Network
...
Zhang, Z. Przygienda, A. Sajassi, A.
Work in Progress Internet-Draft, draft-ietf-bier-evpn-14
,
...
Stephane Litkowski Ali Sajassi
The authors thank
comments on, and suggestions for this document.
