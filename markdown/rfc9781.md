...
RFC 9781
A Concise Binary Object Representation (CBOR) Tag
...
[RFC8392]
 ...
 is always wrapped in a CBOR Object Signing

[RFC8392]
. Among other things, COSE provides end-to-end data

A CBOR Web Token (CWT) as specified by
[STD96]
and Encryption (COSE) envelope
origin authentication and integrity protection employed by
...
"A path for transferring data between two entities or components that ensures
...
).) Examples include conveyance via PCIe (Peripheral Component
...
definition of CWT Claims Sets was provided in
Definition Language (CDDL)
the structurally similar use of JWT Claims Sets
...
format tokens to Entity Attestation Tokens (EATs)
points.

, using its ability to also describe in the same definition

, without any protective wrapper (such

. This employs the Concise Data

 using its predefined extension
...
communication channels. As these are Claims, the Claims Sets defined in
...
. Especially in some resource-constrained
...
Channel is presumed to be used for conveying such UCCS. The following sections elaborate on
...
considerations to JWTs
states:

 using the algorithm "none". Section 3.2 of RFC 8725
...
the need to "Use Appropriate Algorithms" (Section 3.2 of RFC 8725
) includes choosing
...
such a Secure Channel setup protocol is the TLS
 handshake; the TLS record protocol
...
MUST
 ...
 come with additional instruction leaflets and security considerations.
...
In the scope of RATS Claims, the receiver
establishment of the Secure Channel. Furthermore, the channel
communication between the communicating RATS roles. For data confidentiality
receiving side
...
authentication and authorization will influence whether the sender can disclose the UCCS.
 ...
 be authenticated as well. This is achieved if the sender and receiver

 authenticate the sender as part of the
...
[TPM2]
.
Attesting Environment in a hardened environment, such as a TEE
...
the secure channel no longer protect the UCCS, which is now subject to the same security
...
) in UCCS over a privacy-preserving Secure Channel allows a
...
document as the specification reference.
 ...
 as defined in Section 9.2 of RFC 8949
...
recipient can authenticate the party that assembled the claims and created the CWT".
 ...
 apply. The security considerations of
...
application protocol (e.g., TLS and the protocol identified by ALPN)
...
security requirements on them. Similar security considerations to those described in
...
Using key material in accordance with any usage restrictions such as freshness or algorithm
restrictions.
...
IANA "CBOR Web Token (CWT) Claims" <https://www.iana.org/assignments/
cwt>
...
Trusted Computing Group "Trusted Platform Module 2.0 Library" Version 184
March 2025,
specification/>
...
an easy and unambiguous way to express structures for protocol messages and data formats
that use CBOR or JSON.
...
[RFC8392]
 ...
 does not define CDDL for CWT Claims Sets.

The CDDL model in Figure 1 shows how to use CDDL for defining the CWT Claims Set defined in
[RFC8392]
 Claims
sets by disabling feature "cbor" and enabling feature "json".

. These CDDL rules have been built such that they also can describe
...
Specifications that define additional Claims should also supply additions to the $$Claims-Set-
Claims socket, e.g.:
...
The above definitions, concepts, and security considerations also define a JSON-encoded Claims-
...
The example CWT Claims Set from
enclosing it with a tag number 601:

Appendix A.1 of [RFC8392]

 can be turned into a UCCS by
...
useful review.
 ...
 suggested some improvements to the CDDL.
