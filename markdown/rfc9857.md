...
RFC 9857
Advertisement of Segment Routing Policies Using
BGP - Link State

Abstract

This document describes a mechanism used to collect Segment Routing (SR) Policy information
that is locally available in a node and advertise it into BGP - Link State (BGP-LS) updates. Such
information can be used by external components for path computation, reoptimization, service
...
SR Policy architecture details are specified in
...
. An SR Policy comprises one or more
...
One example of a controller is the stateful Path Computation Element (PCE)
...
. As illustrated in the figure below, the PCC is not a Label Switching

Section 5.5 of [RFC4655]
Router (LSR) in the routing domain, thus the headend nodes of the SR Policies may not
implement the PCEP. In this case, a general mechanism to collect the SR Policy states from the
...
network to obtain a global view of the state of all SR Policy paths in the network.
 ...
 to report the SR Policy information to the PCE nodes. An
...
. Using the same protocol to collect SR Policy and state
...
protocols for network topology information collection. This document describes a mechanism to
distribute SR Policy information (both SR-MPLS and SRv6
...
[RFC9552]
 ...
 that is originating the advertisement of SR Policy information
...
[BGP-LS-TE-PATH]
 ...
 introduces further extensions to
...
The "Link-State Network Layer Reachability Information (NLRI)" defined in
...
The format of the Link-State NLRI is defined in
...
An additional NLRI Type known as "SR Policy Candidate Path NLRI" (value 5) is defined for the
...
Specifies the component that owns the SR Policy state in the advertising node.

An additional Protocol-ID "Segment Routing" (value 9) is introduced by this document that
MUST

 be used for the advertisement of SR Policies.
...
[RFC9552]
 ...
 and used as specified further in this
...
The Local Node Descriptors TLV

MUST

 include at least one of the following Node Descriptor TLVs:
...
, which identifies the headend node of
...
headend node is the BGP-LS Producer:
 ...
 include the following Node Descriptor TLVs when the
...
, which contains a valid BGP Identifier of the headend

•

Autonomous System (TLV 512)
(ASN) (or AS Confederation Identifier
node of the SR Policy.

[RFC9552]

, which contains the Autonomous System Number
, if confederations are used) of the headend
...
BGP Confederation Member (TLV 517)
confederation member (i.e., Member-AS Number); if BGP confederations are used, it
contains the headend node of the SR Policy.

, which contains the ASN of the

[RFC9086]

•

Other Node Descriptors as defined in
Policy. The determination of whether the IGP Router-ID (TLV 515) contains a 4-octet OSPF
Router-ID or a 6-octet ISO System-ID is to be done based on the length of that sub-TLV as the
Protocol-ID in the NLRI is always going to be "Segment Routing".

 to identify the headend node of the SR
...
MUST NOT
 ...
 include its identifiers in the Node Descriptor TLV in the NLRI as the

MUST

 only carry the identifiers of the SR Policy headend.

The Local Node Descriptors TLV
node is the BGP-LS Producer and it has this information about the headend (e.g., as part of its
topology database):

 include the following Node Descriptor TLVs when the PCE
...
, which contains the ASN (or AS Confederation

, if confederations are used) of the headend node of the SR Policy.
...
Other Node Descriptors as defined in
Policy. The determination of whether the IGP Router-ID (TLV 515) contains a 4-octet OSPF
Router-ID or a 6-octet ISO System-ID is to be done based on the length of that sub-TLV since
the Protocol-ID in the NLRI is always going to be "Segment Routing".
...
the instantiation of this path as specified in
points to be used are listed in Section 8.4.

Section 2.3 of [RFC9256]

. The protocol-origin code

Flags:

1-octet field with the following bit positions defined. Other bits
...
BGP-LS Attribute defined in
NLRI type.

 and are associated with the SR Policy Candidate Path
...
its attributes for the SR Policy candidate path. The TLV
BSID value for reporting as described in
...
rest are ignored.
 ...
 also optionally contain the Specified
...
semantics are described in detail in
the originator and

 be ignored by a receiver.

Section 6.2 of [RFC9256]
...
Indicates the data plane for the BSIDs and if a BSID is a 16-octet SRv6 SID (when set)

or 4-octet SR/MPLS label value (when clear).
...
as shown in the figure below, the TC, S, and TTL (total of 12 bits) are RESERVED and
 be set
to 0 by the originator and
...
[RFC8986]
 ...
 or the structure of the SID. Therefore, the SR Binding SID sub-TLV
...
Endpoint behavior
SHOULD NOT
SID TLV defined in Section 5.2
...
early draft versions of this specification.
 ...
 be used for the signaling of an SRv6 Binding SID. The use
...
attributes for the SR Policy candidate path. The TLV
SRv6 BSID value for reporting as described in
...
ignored by a receiver.
 ...
 be cleared by the originator and
...
The SRv6 Endpoint Behavior TLV (1250) and the SRv6 SID Structure TLV (1252)
...
TLV and the SRv6 SID Structure TLV.
 ...
 defines the SRv6 Endpoint Behavior
...
stated otherwise for individual flags. Other bits
be ignored by a receiver.
...
) for the SR Policy when set and

forwarding plane as specified in
...
protection of the active path as specified in
...
candidate path has not been evaluated), then this flag
and
...
path is not enabled for the "Drop-Upon-Invalid" behavior.
 ...
 for details. When clear, it indicates that the candidate
...
Refer to
 ...
 for steering into a transit policy using its BSID.
...
"Drop-Upon-Invalid" behavior. Refer to
...
Symbolic name for the SR Policy without a NUL terminator as specified in
...
RECOMMENDED
 ...
 that the size of the symbolic name be limited to

MAY

 choose to truncate long names to 255 bytes when signaling
...
terminator as specified in
symbolic name be limited to 255 bytes. Implementations
to 255 bytes when signaling via BGP-LS.

Section 2.6 of [RFC9256]

Symbolic name for the SR Policy candidate path without a NUL
. It is
...
candidate path that is either computed by the headend or delegated to a controller. The
...
The following bit positions are defined, and the other bits
and
...
One or more optional sub-TLVs

MAY

 be included in this TLV to describe other
...
TLV that is used to carry the affinity constraints
...
that is used to carry the SRLG values
...
Section 8.2.2 of [RFC9552]
 ...
 that have been excluded from the candidate
...
between their paths. The types of disjointness are described in
...
flags. The following flags are defined, and the other bits
MUST
...
4-octet value that is the group identifier for a set of disjoint paths.
 contain the entire PCEP ASSOCIATION Object as specified in
MAY
...
document) and not by any BGP Speaker as specified in
such that the update for a single SR Policy Candidate Path NLRI would exceed the supported
BGP message size by the implementation, then the PCEP ASSOCIATION Object
 be
...
(i.e., not determined to be malformed as per
) is used and the rest are
ignored.
...
MAY
 ...
 contain the entire PCEP ASSOCIATION Object as specified
paths. Alternatively, this field
Section 6.1 of [RFC8697]
in
 (including its optional TLVs) when PCEP is used for the signaling
of the SR Policy candidate path and where the BGP-LS Producer is unable to determine the
group identifier that can be accommodated in a 4-octet value (since PCEP supports multiple
methods of encoding an association identifier). Note that the parsing of the PCEP object is
expected to be performed only by the BGP-LS Consumer (hence, outside the scope of this
document) and not by any BGP Speaker as specified in
...
instances of this sub-TLV may be used to report different metric type uses.
 ...
 be used to report the metric margin or is bound to be used for
...
1-octet field that identifies the type of metric being used. Below is a list of metric
types introduced by this document along with references for each. Where the references are
...
that the metric types in this field are taken from the "BGP-LS SR Policy Metric Types" IANA
registry, which includes IGP Metric Types as well as metric types specific to SR Policy path
computation (i.e., the metric types are not from the "IGP Metric-Type" registry). Additional
metric types may be introduced by future documents. This document does not make any
assumptions about a smaller metric value being better than a higher metric value; that is
something that is dependent on the semantics of the specific metric type. This document uses
the words "best" and "worst" to abstract this aspect when referring to metric margins and
bounds.
...
Section 3 of [RFC5305]
 ...
 for IS-IS and is known as the default metric. This
...
[RFC5340]
 ...
 for OSPFv3 and is known as the
...
Section 3.7 of [RFC5305]
 ...
 for IS-IS as the TE default metric, in

Section

 for OSPFv2, and in

Section 4 of [RFC5329]
...
Section 4.1 of [RFC8570]

 for IS-IS and in

Section 4.1 of [RFC7471]
...
Section 4.3 of [RFC8570]

 for IS-IS and in

Section 4.3 of [RFC7471]
...
Section 4.4 of [RFC8570]

 for IS-IS and in

Section 4.4 of [RFC7471]
...
following bit positions are defined, and the other bits
MUST
...
The SR Segment List TLV is used to report a single SID list of a candidate path. Multiple instances
of this TLV may be used to report multiple SID lists of a candidate path.
...
Indicates that the SID list is associated with an explicit candidate path when set and
 be
 consider them all to

a dynamic candidate path when clear. All segment lists of a given candidate path
either explicit or dynamic. In case of inconsistency, the receiver
be dynamic.
...
1 octet that indicates the algorithm of the SIDs used in the SID list when the A-flag is

set. The algorithm values are from the "IGP Algorithm Types" IANA registry under the
...
associated with the specific SID list.
 ...
 be included as an ordered set of sub-
The SR Segment sub-TLV (defined in Section 5.7.1)
...
SID list is empty, the SR Segment List TLV
...
defines the encoding of the segment descriptors for each of those segment types to be used in the
Segment sub-TLV described previously in Section 5.7.1.
...
(Type G) IPv6 Prefix and Interface ID for link endpoints as Local, Remote pair for SR-
MPLS
...
The Segment is an SRv6 type and is specified simply as SRv6 SID. The format of its segment
descriptor is as follows:
...
4-octet value that carries the local IPv4 address associated with

the node's interface.

IPv4 Remote Interface Address:

4-octet value that carries the remote IPv4 address associated

with the interface on the node's neighbor. This is optional and
(e.g., when identifying point-to-point links).

MAY

 be set to 0 when not used
...
associated with the remote node. This is optional and
when identifying point-to-point links).

MAY

 be set to 0 when not used (e.g.,

Remote Node Interface ID:

4-octet value that carries the interface ID of the remote node

identified by the Remote Node Address. This is optional and
(e.g., when identifying point-to-point links).
...
16-octet value that carries the local IPv6 address associated with the

node's interface.

IPv6 Remote Global Address:

16-octet value that carries the remote IPv6 address associated

with the interface on the node's neighbor.
...
allocated to the specific SID list by the path computation entity. Only a single instance of this sub-
TLV is advertised for a given segment list. If multiple instances are present, then the first valid
one (i.e., not determined to be malformed as per
) is used and the rest
are ignored.
...
Bandwidth: 4 octets that specify the allocated bandwidth in unit of bytes per second in IEEE
...
given segment list. If multiple instances are present, then the first valid one (i.e., not determined
to be malformed as per
...
Policy candidate path's state and attributes are encoded in the BGP-LS Attribute field as SR Policy
State TLVs and sub-TLVs as described in Section 5. The SR Candidate Path State TLV as defined in
Section 5.3 is included to report the state of the candidate path. The SR BSID TLV as defined in
...
Sections 5.1 and 5.2 is included to report the BSID of the candidate path when one is either
specified or allocated by the headend. The constraints and the optimization metric for the SR
Policy candidate path are reported using the SR Candidate Path Constraints TLV and its sub-TLVs
as described in Section 5.6. The SR Segment List TLV is included for each SID list(s) associated
with the candidate path. Each SR Segment List TLV in turn includes an SR Segment sub-TLV(s) to
report the segment(s) and its status. The SR Segment List Metric sub-TLV is used to report the
metric values at an individual SID list level.

7.  Manageability Considerations

The existing BGP operational and management procedures apply to this document. No new
procedures are defined in this document. The considerations as specified in
this document.
...
SR Candidate Path State

SR Candidate Path Name
...
SR Segment List Metric

SR Affinity Constraint

SR SRLG Constraint

SR Bandwidth Constraint
...
SR Bidirectional Group Constraint
...
Configuration (CLI, YANG model via NETCONF, etc. In PCEP or
...
allocation policy of Expert Review
[RFC9552]
specified in
...
. There is also an additional guideline for the designated experts to

 using the guidelines for designated experts as
...
"BGP-LS SR Segment Descriptor Types" registry for a new segment type. However, this does not
...
Table 6: BGP-LS SR Segment Descriptor Type Code
Points
...
model. See
 for details. The security considerations of the base BGP-LS specification as
described in
...
. SR operates within a trusted SR

The BGP-LS SR Policy extensions specified in this document enable TE and service programming
use cases within an SR domain as described in
domain
, and its security considerations also apply to BGP sessions when carrying SR
...
controllers/applications in a secure manner within this trusted SR domain. The general guidance
for BGP-LS with respect to isolation of BGP-LS sessions from BGP sessions for other address-
families (refer to the security considerations of
) may be used to ensure that the SR
Policy information is not advertised to an External BGP (EBGP) peering session outside the SR
domain by accident or error.

[RFC9552]

Additionally, it may be considered that the export of SR Policy information, as described in this
document, constitutes a risk to the confidentiality of mission-critical or commercially sensitive
...
"Advertisement of Traffic Engineering Paths using BGP Link-State" Work in
Progress Internet-Draft, draft-ietf-idr-bgp-ls-te-path-02
, 11 November 2024,
<https://datatracker.ietf.org/doc/html/draft-ietf-idr-bgp-ls-te-path-02>
...
for Traffic Engineering Over MPLS" RFC 2702 DOI 10.17487/RFC2702
,
September 1999,
...
Segment Routing Policies in BGP" RFC 9830 DOI 10.17487/RFC9830
,
2025,
...
Dhruv Dhody Mohammed Abdul Aziz Khalid Lou Berger Acee

,
The authors would like to thank
Lindem Siva Sivabalan Arjun Sreekantiah Dhanendra Jain Francois Clad Zafar Ali Stephane
Litkowski Aravind Babu Mahendra Babu Geetanjalli Bhalla Ahmed Bashandy Mike Koldychev
Samuel Sidor Alex Tokar Rajesh Melarcode Venkatesswaran Lin Changwang Liu Yao Joel
Halpern
to thank
The authors would like to thank
...
,
 for their reviews and valuable comments. The authors would also like
 for her shepherd review and helpful comments to improve this document.
...
Susan Hares
 ...
 for his AD review and helpful suggestions to
...
The following people have contributed substantially to the content of this document and should
