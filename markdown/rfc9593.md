...
This approach generally works well when there is no ambiguity in selecting authentication
...
If the initiator doesn't support this extension, it ignores the received notification as an unknown
status notify.
...
MAY
 ...
 choose not to send an actual list of the supported authentication
...
MAY
 ...
 use it to send the SUPPORTED_AUTH_METHODS

other reason, then the responder
notification. Otherwise, the initiator
purpose by sending an empty IKE_INTERMEDIATE request. The initiator
...
used for authentication of this particular initiator. If these payloads are sent, they
identical to the IDi/IDr payloads sent later in the IKE_AUTH request.

 start the IKE_INTERMEDIATE exchange for this sole
 also indicate its
...
If the responder has sent any CERTREQ payload in the IKE_SA_INIT, then it
...
exchange, then the initiator
the Announcements to the CAs received in the IKE_SA_INIT response, or it
Announcements containing links to CAs.

 abort negotiation. Instead, the initiator
MAY
...
use. In this case the responder
MAY
initiator sends IDi/IDr in an IKE_INTERMEDIATE request, then it is
responder sends back the list of authentication methods in the response.

 send the list in any IKE_INTERMEDIATE exchange. If the
 that the
...
Note that sending the SUPPORTED_AUTH_METHODS notification and using information obtained
...
The Notify payload format is defined in
...
describe the semantics of the announcements for these methods. Implementations
...
), and for this reason there is no point to announce it via this
...
content of this field
 ...
 be ignored and treated as zero.
...
This format is applicable for the authentication methods "RSA Digital Signature" (1), "DSS Digital
...
[X.690]
 ...
 and identifies the signature algorithm (see
...
identified via AlgorithmIdentifier ASN.1 object.
...
to go with secure password authentication, they
...
In the situation when peers are going to use Multiple Authentication Exchanges
MAY
...
Authentication Method" registry on
 for discussion of security properties of each
...
Ounsworth, M. Gray, J. Pala, M.
,

J. Klaussner "Composite Signatures
, and
For Use In Internet PKI" Work in Progress Internet-Draft, draft-ietf-lamps-pq-
,
composite-sigs-01
lamps-pq-composite-sigs-01>
...
J. Korhonen "Multiple Authentication Exchanges in the Internet

Eronen, P.
Key Exchange (IKEv2) Protocol" RFC 4739 DOI 10.17487/RFC4739
,
2006,
...
Appendix A.  Examples of Announcing Supported
...
Paul Wouters
 ...
 for his valuable comments and proposals. The
...
Daniel Van Geest
 ...
 contributed to the clarity of the document.
