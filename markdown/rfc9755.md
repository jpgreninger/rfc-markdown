...
This specification extends the Internet Message Access Protocol, specifically IMAP4rev1 (RFC
3501), to support UTF-8 encoded international characters in user names, mail addresses, and
message headers. This specification replaces RFC 6855. This specification does not extend
IMAP4rev2 (RFC 9051), since that protocol includes everything in this extension.
...
This specification forms part of the Email Address Internationalization protocols described in
the Email Address Internationalization Framework document
[RFC3629]
[RFC3501]
Headers"
charset. This specification creates two new IMAP capabilities to allow servers to advertise these
new extensions.

. It extends IMAP
 in headers, as described in "Internationalized Email
. It also adds a mechanism to support mailbox names using the UTF-8
...
This specification assumes that the IMAP server will be operating in a fully internationalized
...
the authenticated state.
 ...
 with the "UTF8=ACCEPT" option (defined in

[RFC5161]

[RFC3501]

 forbids the use of 8-bit characters in atoms or quoted-

The IMAP base specification
strings. Thus, a UTF-8 string can only be sent as a literal. This can be inconvenient from a coding
standpoint, and unless the server offers IMAP non-synchronizing literals
requires an extra round trip for each UTF-8 string sent by the client. When the IMAP server
supports "UTF8=ACCEPT", it supports UTF-8 in quoted-strings with the following ABNF syntax
[RFC5234]
:
...
If the server supports "UTF8=ACCEPT", the client
 use extended quoted syntax with any
IMAP argument that permits a string (including astring and nstring). However, if characters
outside the US-ASCII repertoire are used in an inappropriate place, the results would be the
same as if other syntactically valid but semantically invalid characters were used. Specific cases
where UTF-8 characters are permitted or not permitted are described in the following
paragraphs.
...
with the specific exception that they
U+0080 - U+009F), a delete character (U+007F), a line separator (U+2028), or a paragraph
separator (U+2029).

 comply with the Net-Unicode Definition (

MUST NOT

MUST

 accept UTF-8 in mailbox names, and

 accept UTF-8 in mailbox names and convert them to the appropriate internal

 contain control characters (U+0000 - U+001F and
...
MUST NOT
receives such a "SEARCH" command in that situation, it
"BAD" response (due to the conflicting charset labels). This also applies to any IMAP command or
extension that includes an optional charset label and associated strings in the command
arguments, including the MULTISEARCH extension. For commands with a mandatory charset
field, such as SORT and THREAD, servers
"BAD" response (due to the conflicting charset labels).

 reject charset values other than UTF-8 with a
...
UTF8=ACCEPT" command, the server
that includes any 8-bit character in message header fields.

 reject, with a "NO" response, an "APPEND" command

MUST

5.  "LOGIN" Command and UTF-8

This specification does not extend the IMAP "LOGIN" command
...
[RFC3501]
 in this respect. This document extends the media-message
[RFC9051]
.
...
When IMAP4rev1 and UTF8=ACCEPT has been enabled, the server
message/rfc822 when computing the body structure, but
[RFC3501]
...
MAY
 ...
 treat message/global like

 also treat it as described in

When IMAP4rev2 and UTF8=ACCEPT are in use, the server
[RFC9051]
.
...
). Because these are incompatible changes to IMAP,
convention (modified UTF-7
explicit server announcement and client confirmation are necessary: clients
 use the
"ENABLE UTF8=ACCEPT" command before using this server. A server that advertises
"UTF8=ONLY" will reject, with a "NO [CANNOT]" response
require UTF-8 support and is not preceded by an "ENABLE UTF8=ACCEPT" command.

, any command that might
...
IMAP clients that find support for a server that announces "UTF8=ONLY" problematic are
encouraged to at least detect the announcement and provide an informative error message to
the end user.
...
do so other than for systems that wish to receive email that requires SMTPUTF8 capabilities to
be sure that all components of those systems -- including IMAP and other clients selected by
...
). Consequently, such messages may
requirements of current email specifications (e.g.,
...
) and trying to minimize those costs while still
...
. Getting downgrade algorithms right, and
minimizing the risk of operational problems and harm to the email system, is tricky and
requires careful engineering. These two algorithms are well understood and carefully designed.

 use one of the standardized
...
[RFC3501]
the originals that the IMAP specification
...
verification. (It will be possible for some digital signatures to be verified, if they cover only parts
 ...
 did not anticipate. This brings up two
...
of the original message that are not affected in the creation of the surrogate.) Second, servers
...
[RFC3501]
extreme care to be sure that UIDVALIDITY
 behaves as the user would expect. Those
...
client upgraded from being non-"UTF8=ACCEPT"-aware
downloaded from the server.

 discard its cache of messages
...
usage and upgrading, the options that are actually available, and so on. It is impossible, at least
at the time of publication of this specification, to give good advice that will apply to all situations,
or even particular profiles of situations, other than "upgrade legacy clients as soon as possible".
...
responsibility of the server to comply with the IMAP base specification
 with respect to all header information transmitted over the
...
[RFC3629]
The security considerations of UTF-8
apply to this specification, particularly with respect to use of UTF-8 in usernames and
passwords. Otherwise, this is not believed to alter the security considerations of IMAP.

 and PRECIS Usernames and Passwords
...
Special considerations, some of them with security implications, occur if a server that conforms
...
A. Melnikov "Preparation, Enforcement, and Comparison of
...
C. Newman "IMAP Support for UTF-8" RFC 5738 DOI 10.17487/
...
This non-normative section discusses the reasons behind some of the design choices in this
specification.
...
The "UTF8=ONLY" mechanism simplifies diagnosis of interoperability problems when legacy
...
with IMAP4rev2 as defined by
and IMAP4rev2 with the same code.

 and making it simpler for clients to support IMAP4rev1

[RFC9051]

[RFC9051]

 provides roughly the same abilities as

IMAP4rev2
APPEND's UTF8 item. None of
learn whether a particular message was stored using the UTF8 data item. As of today, an IMAP
client cannot learn whether a particular message was stored using the UTF8 data item, nor
would it be able to trust that information even if IMAP4rev1 and 2 were extended to provide
that information.
...
For these reasons, it was judged best to revise
IMAP4rev2.

[RFC6855]

 and adopt the same syntax as

B.2.  FETCH BODYSTRUCTURE

 defines a new media type, message/global, which is substantially like message/rfc822

[RFC6532]
except that the submessage may (also) use the syntax defined in
[RFC9051]
usually compute once and store.

 define a FETCH item to return the MIME structure of a message, which servers
...
This document is an almost unchanged copy of
Chris Newman
a new email address for him. We cannot be sure that he would approve of the changes in this
document, so we did not list him as author, but do gratefully acknowledge his work on
[RFC6855] Jiankang Yao

. Sean has since changed jobs and the current authors do not have
...
The authors wish to thank the participants of the EAI working group for their
contributions to this document, with particular thanks to
Harald Alvestrand David
,
Black Randall Gellens Arnt Gulbrandsen Kari Hurtta John Klensin Xiaodong Lee
,
,
Charles Lindsey Alexey Melnikov Subramanian Moonesamy Shawn Steele Daniel
Taharlev

 for their specific contributions to the discussion.
