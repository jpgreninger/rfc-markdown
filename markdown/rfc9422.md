...
[SMTP]
 ...
 provides the ability to deliver messages to a system
...
[ABNF]
 ...
 notation and its core rules to
...
MUST
 ...
 be registered with the IANA, as described
...
commonly understood abbreviations, e.g., "MAX" for "maximum", is encouraged.
 ...
 be comprehensible, but also should be kept as short as possible. The use of
...
SHOULD
 ...
 begin with the name of

Limit names

SHOULD

 end with one or more terms that describe the type of limit.
...
checking responses, and in some cases it may be possible to send multiple transactions.
 ...
 is commonly used to improve performance, especially
...
For example, it may seem like it would be better to impose a limit on the number of successful
...
servers must accept a minimum of 100 RCPT TO commands
...
circumstances, e.g., after successful authentication
 or negotiation of a security layer
[STARTTLS]
.
...
MAY
 ...
 return updated limits any time the protocol requires clients to reissue the EHLO
...
MUST
 ...
 discard any previous limits in favor of those provided by the most recent EHLO.

Clients
This includes the case where the original EHLO provided a set of limits but the subsequent EHLO
did not; in this case, the client
...
keywords, are best handled by ignoring that limit; in this case, the client
...
way; in this case, that specification
affects the other limits.

 specify how an error in the value syntax of one limit
...
3.8.  Caching of Limit Settings between Sessions Involving the Same Client
and Server SMTP

MAY

 cache limits determined during one session and use them to optimize their behavior
Clients
...
All that said, decades of operational experience with the SMTP "SIZE" extension
...
Use of the LIMITS extension to provide client-specific information - as opposed to general server
...
The syntax of the limit value, if the limit has one. This syntax
...
[ABNF]
registrations created by this specification in Section 4 above. Use of ABNF
...
mechanisms associated with SMTP
specification more generally and offer advice accordingly.

, Section 3 above, and the provision of this
...
Freed, N. "SMTP Service Extension for Command Pipelining" STD 60 RFC 2920
...
Klensin, J. "Simple Mail Transfer Protocol" Section 2.2 RFC 5321
...
Siemborski, R., Ed.
Authentication" RFC 4954 DOI 10.17487/RFC4954
...
A. Melnikov, Ed. "SMTP Service Extension for
...
Klensin, J. "Simple Mail Transfer Protocol" Section 4.5.3.1 RFC 5321
2008,
...
Klensin, J. "Simple Mail Transfer Protocol" Section 4.5.3.1.8  RFC 5321
2008,
...
Klensin, J. Freed, N.
,
Declaration" STD 10 RFC 1870 DOI 10.17487/RFC1870
www.rfc-editor.org/info/rfc1870>
...
The concept for this extension came from, and was developed by, Ned Freed and most of this
specification, including substantially all of the technical details, was written by him.
Unfortunately, he became ill and eventually passed away in May 2022 without being able to
complete the document and manage it through IETF Last Call. His contributions to the Internet,
work in the IETF, and the personal style that made both possible are irreplaceable and greatly
missed. With the support of the community, John Klensin picked the document up, responded to
some additional feedback, and got the document into what is believed to be a finished state. In
the interest of preserving this as Ned's document, a few comments that proposed additional
features and options were set aside for future work rather than our having to guess at whether
Ned would have approved of them.

The acknowledgments below are divided into two parts, those written by Ned and those
associated with input to the document after his passing.
...
A lot of people have helped make this specification possible. The author wishes to thank
Claus Assmann Laura Atkins Alex Brotman Richard Clayton Dave Crocker Viktor
,
Dukhovni Arnt Gulbrandsen Jeremy Harris Todd Herr Mike Hillyer Matthias Leisi John
,
Klensin Valdis Klētnieks John Levine Alexey Melnikov Keith Moore Michael Peddemors
,
Hector Santos George Schlossnagle Rolf E. Sonneveld
,
...
Additional helpful suggestions were received when the draft was requeued in the last part of
2022 and thereafter. Reviews and suggestions from
Hightower Murray S. Kucherawy Barry Leiba John Levine
especially helpful in identifying errors and suggesting clarifying some issues with the
document without changing Ned's fundamental issues or very much of his text.

Alex Brotman Dave Crocker Gene
...
IETF Last Call comments (and comments immediately before it started) from people not
listed above that resulted in document changes were received from
 (for
IANA),
