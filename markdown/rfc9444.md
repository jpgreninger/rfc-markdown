...
[RFC8555]
 ...
 defines a protocol that a certification authority (CA) and an applicant can use to
...
client uses the ACME protocol to request certificate issuance from the server. This document
...
The following terms are defined in "DNS Terminology"
...
[RFC8499]
The following definition for "subdomain" is taken from "DNS Terminology"
...
A domain is an ancestor domain of a subdomain if it contains that subdomain and has fewer
...
[RFC8555], Section 6.3
 ...
 introduces the following term which is used in this document:
...
Server replies with an order object that contains a set of links to authorization object(s) and
a finalize URI.

3.

Client sends POST-as-GET request(s) to retrieve the authorization object(s), with the
downloaded authorization object(s) containing the identifier that the client must prove
that they control, and a set of links to associated challenge objects, one of which the client
must fulfill.
...
name (type: "dns"). The domain name
appear in a certificate."

 be encoded in the form in which it would

MUST

• [RFC8555], Section 7.4

newOrder request: "The CSR
...
The ACME base document
identifiers may be defined and registered in the IANA
example,
identifier type.
...
[RFC8738]
 ...
 only specifies the "dns" identifier type. Additional

[ACME-Identifier-Types]

 registry. For

 specifies the "ip" identifier type. This document is only relevant for the "dns"
...
subdomain.
 ...
 does not mandate that the identifier in a
...
) defines the authorization object. This document defines a new
...
authorization object, then the server
 include the wildcard field with a value of true, as
per
...
) defines the identifier object for newAuthz requests. This
...
If the server is willing to allow a single authorization for the subdomains and there is not an
...
MUST
 ...
 be able to fulfill a challenge against the ancestor domain.
...
If not specified, then the assumed default value is false. If an ACME server supports authorization
...
The example illustrates the client completing a DNS challenge by publishing a DNS TXT
...
authorization resource to see when it is finalized. Refer to
more details.
...
This document specifies enhancements to ACME
 that optimize the protocol flows for
...
Therefore, all Security Considerations in ACME in the following areas are equally applicable to
ACME for Subdomains:
...
[RFC8555], Section 7.5.2
 and can deactivate an account using the
...
The
 definition of a subdomain is reproduced in Section 2. When comparing domains
...
The CA policy considerations listed in
include, but are not limited to:

[RFC8555], Section 10.5

 are equally applicable here. These
...
[RFC8555], Section 10.5
 ...
 for more CA policy considerations.
...
IANA "ACME Identifier Types" <https://www.iana.org/assignments/
...
IANA "ACME Validation Methods" <https://www.iana.org/
...
of Publicly-Trusted Certificates" <https://cabforum.org/baseline-requirements-
documents/>
...
J. Postel "The Domain Naming Convention for Internet User

 and

Su, Z.
Applications" RFC 819 DOI 10.17487/RFC0819
,
editor.org/info/rfc819>
.
...
Shoemaker, R.B. "Automated Certificate Management Environment (ACME) IP
Identifier Validation Extension" RFC 8738 DOI 10.17487/RFC8738
,
2020,
