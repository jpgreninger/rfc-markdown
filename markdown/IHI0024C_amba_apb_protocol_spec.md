...
\hline Date & Issue & Confidentiality & Change \\
\hline 25 September 2003 & A & Non-Confidential & First release for v1.0 \\
\hline 17 August 2004 & B & Non-Confidential & Second release for v1.0 \\
\hline 13 April 2010 & C & Non-Confidential & First release for v2.0 \\
\hline
\end{tabular}
...
\section*{ARM AMBA Specification Licence}
...
\section*{4. THE RELEVANT AMBA SPECIFICATION IS PROVIDED "AS IS" WITH NO WARRANTIES EXPRESS, IMPLIED OR STATUTORY, INCLUDING BUT NOT LIMITED TO ANY WARRANTY OF SATISFACTORY QUALITY, MERCHANTABILITY, NONINFRINGEMENT OR FITNESS FOR A PARTICULAR PURPOSE.}
...
\section*{Confidentiality Status}
...
\section*{Contents \\ AMBA APB Protocol Specification}
Preface
...
\section*{List of Tables \\ AMBA APB Protocol Specification}
Change history ..... ii
...
\section*{List of Figures \\ AMBA APB Protocol Specification}
...
\begin{tabular}{|l|l|}
\hline italic & Highlights important notes, introduces special terminology, denotes internal cross-references, and citations. \\
\hline bold & Highlights interface elements, such as menu names. Denotes signal names. Also used for terms in descriptive lists, where appropriate. \\
\hline monospace & Denotes text that you can enter at the keyboard, such as commands, file and program names, and source code. \\
\hline monospace & Denotes a permitted abbreviation for a command or option. You can enter the underlined text instead of the full command or option name. \\
\hline monospace italic & Denotes arguments to monospace text where the argument is to be replaced by a specific value. \\
\hline monospace bold & Denotes language keywords when used outside example code. \\
\hline
\end{tabular}
```
...
\section*{Feedback on this product}
...
\subsection*{1.1 About the APB protocol}
...
\subsection*{1.2.1 AMBA 2 APB Specification}
...
\subsection*{1.2.2 AMBA 3 APB Protocol Specification v1.0}
...
\subsection*{1.2.3 AMBA APB Protocol Specification v2.0}
...
\subsection*{2.1 AMBA APB signals}

Table 2-1 APB signal descriptions

\begin{table}
\captionsetup{labelformat=empty}
\caption{Table 2-1 lists the APB signals.}
\begin{tabular}{|l|l|l|}
\hline Signal & Source & Description \\
\hline PCLK & Clock source & Clock. The rising edge of PCLK times all transfers on the APB. \\
\hline PRESETn & System bus equivalent & Reset. The APB reset signal is active LOW. This signal is normally connected directly to the system bus reset signal. \\
\hline PADDR & APB bridge & Address. This is the APB address bus. It can be up to 32 bits wide and is driven by the peripheral bus bridge unit. \\
\hline PPROT & APB bridge & Protection type. This signal indicates the normal, privileged, or secure protection level of the transaction and whether the transaction is a data access or an instruction access. \\
\hline PSELx & APB bridge & Select. The APB bridge unit generates this signal to each peripheral bus slave. It indicates that the slave device is selected and that a data transfer is required. There is a PSELx signal for each slave. \\
\hline PENABLE & APB bridge & Enable. This signal indicates the second and subsequent cycles of an APB transfer. \\
\hline PWRITE & APB bridge & Direction. This signal indicates an APB write access when HIGH and an APB read access when LOW. \\
\hline PWDATA & APB bridge & Write data. This bus is driven by the peripheral bus bridge unit during write cycles when PWRITE is HIGH. This bus can be up to 32 bits wide. \\
\hline PSTRB & APB bridge & Write strobes. This signal indicates which byte lanes to update during a write transfer. There is one write strobe for each eight bits of the write data bus. Therefore, PSTRB[n] corresponds to PWDATA[(8n + 7):(8n)]. Write strobes must not be active during a read transfer. \\
\hline PREADY & Slave interface & Ready. The slave uses this signal to extend an APB transfer. \\
\hline PRDATA & Slave interface & Read Data. The selected slave drives this bus during read cycles when PWRITE is LOW. This bus can be up to 32-bits wide. \\
\hline PSLVERR & Slave interface & This signal indicates a transfer failure. APB peripherals are not required to support the PSLVERR pin. This is true for both existing and new APB peripheral designs. Where a peripheral does not include this pin then the appropriate input to the APB bridge is tied LOW. \\
\hline
\end{tabular}
\end{table}

\subsection*{2.1.1 Data buses}
...
\subsection*{3.1 Write transfers}

This section describes the following types of write transfer:
- With no wait states
- With wait states.

\subsection*{3.1.1 With no wait states}

Figure 3-1 shows a basic write transfer with no wait states.

\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/9883c7dd-f852-4e06-a2d6-75f88394cab5-17.jpg?height=471&width=798&top_left_y=640&top_left_x=1138}
\captionsetup{labelformat=empty}
\caption{Figure 3-1 Write transfer with no wait states}
\end{figure}
...
\subsection*{3.1.2 With wait states}
...
\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/9883c7dd-f852-4e06-a2d6-75f88394cab5-18.jpg?height=472&width=1112&top_left_y=229&top_left_x=822}
\captionsetup{labelformat=empty}
\caption{Figure 3-2 Write transfer with wait states}
\end{figure}
...
\begin{table}
\begin{tabular}{|l|l|l|l|}
\hline 31 & 2423 & 1615 & 87
\end{tabular} 0
\captionsetup{labelformat=empty}
\caption{Figure 3-3 Byte lane mapping}
\end{table}
- Note _ __

For read transfers the bus master must drive all bits of PSTRB LOW.

\subsection*{3.3 Read transfers}

Two types of read transfer are described in this section:
- With no wait states
- With wait states.

\subsection*{3.3.1 With no wait states}
...
\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/9883c7dd-f852-4e06-a2d6-75f88394cab5-20.jpg?height=473&width=798&top_left_y=724&top_left_x=1138}
\captionsetup{labelformat=empty}
\caption{Figure 3-4 Read transfer with no wait states}
\end{figure}

\subsection*{3.3.2 With wait states}
...
\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/9883c7dd-f852-4e06-a2d6-75f88394cab5-20.jpg?height=465&width=1112&top_left_y=1931&top_left_x=822}
\captionsetup{labelformat=empty}
\caption{Figure 3-5 Read transfer with wait states}
\end{figure}

\subsection*{3.4 Error response}
...
\subsection*{3.4.1 Write transfer}

Figure 3-6 shows an example of a failing write transfer that completes with an error.

\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/9883c7dd-f852-4e06-a2d6-75f88394cab5-21.jpg?height=521&width=955&top_left_y=1194&top_left_x=979}
\captionsetup{labelformat=empty}
\caption{Figure 3-6 Example failing write transfer}
\end{figure}

\subsection*{3.4.2 Read transfer}
...
\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/9883c7dd-f852-4e06-a2d6-75f88394cab5-22.jpg?height=532&width=1120&top_left_y=403&top_left_x=816}
\captionsetup{labelformat=empty}
\caption{Figure 3-7 Example failing read transfer}
\end{figure}

\subsection*{3.4.3 Mapping of PSLVERR}
...
\subsection*{3.5 Protection unit support}
...
\section*{Normal or privileged, PPROT[0]}
...
\section*{Secure or non-secure, PPROT[1]}
...
\section*{Data or Instruction, PPROT[2]}
...
\begin{table}
\captionsetup{labelformat=empty}
\caption{Table 3-1 Protection encoding}
\begin{tabular}{|l|l|}
\hline PPROT[2:0] & Protection level \\
...
\subsection*{4.1 Operating states}

Figure 4-1 shows the operational activity of the APB .

\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/9883c7dd-f852-4e06-a2d6-75f88394cab5-26.jpg?height=770&width=815&top_left_y=384&top_left_x=1106}
\captionsetup{labelformat=empty}
\caption{Figure 4-1 State diagram}
\end{figure}
...
\hline \multirow{3}{*}{} & \multicolumn{4}{|r|}{Table A-1 Issue A} \\
\hline & & Change & Location & Affects \\
\hline & & First release & - & - \\
\hline \multicolumn{5}{|r|}{Table A-2 Differences between issue A and issue B} \\
\hline Change & Location & & & Affects \\
\hline \multirow[t]{5}{*}{APB signal PREADY added} & • & Table 2-1 on page 2-2. & & All revisions \\
\hline & & - Write transfers on page 3-2 & & \\
\hline & & - Read transfers on page 3-5 & & \\
\hline & & - Error response on page 3-6 & & \\
\hline & & - Operating states on page 4-2 & & \\
\hline \multirow[t]{2}{*}{APB signal PSLVERR added} & & - Table 2-1 on page 2-2. & & All revisions \\
\hline & & - Error response on page 3-6 & & \\
\hline
\end{tabular}
...
\begin{table}
\captionsetup{labelformat=empty}
\caption{Table A-3 Differences between issue B and issue C}
\begin{tabular}{|l|l|l|}
\hline Change & Location & Affects \\
\hline Section added listing the changes made to this specification at each revision of the document. & APB revisions on page 1-3 & - \\
\hline APB signal PPROT added & \begin{tabular}{l}
- Table 2-1 on page 2-2 \\
- Protection unit support on page 3-8
\end{tabular} & All revisions \\
\hline APB signal PSTRB added & \begin{tabular}{l}
- Table 2-1 on page 2-2 \\
- Write strobes on page 3-4
\end{tabular} & All revisions \\
\hline
