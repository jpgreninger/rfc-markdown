...
RFC 9671
Sieve Email Filtering: Extension for Processing
...
Sieve Extension for Processing Calendar Attachments
...
. However, sometimes the enclosed iCalendar
...
, including use of the "Usage:" label

This document uses terminology and concepts from iCalendar
...
Sieve interpreters that implement this extension
for use with the capability mechanism.

MUST

 have an identifier of "processcalendar"
...
extension is not enabled, the :organizers argument
according to
...
an event is out of scope for this document.
 ...
 make any changes to the participant status of the recipient when

This action
to do so could result in unwelcome notifications being triggered for the recipient.

 remove alarms from calendar data before applying it to a calendar. Failure
...
If used in conjunction with the :organizers argument (Section 4.6), the implementation
process non-iTIP messages.

MUST NOT

If :allowpublic is omitted, the implementation
well-formed iTIP message and one of the recipient user's email addresses matches the Calendar
User Address (see
determined by the iTIP method (see

) of the intended target of the message, as

 process calendar data unless is it is a
...
identifier does not exist on any of the user's calendars), the implementation
...
The :updatesonly (Section 4.3) and :calendarid arguments are incompatible with each other. It is
an error if both arguments are used in the same "processcalendar" action.

4.5.  Delete Cancelled Argument

The optional :deletecancelled argument is used to tell the implementation that if it receives a
cancellation message, it
...
spec" of the URI.
 ...
 process calendar data unless is it is a well-formed iTIP message and
...
The "processcalendar" action can only be executed once per script. A script
MUST
...
It is critical that implementations correctly implement the behavior and restrictions described
throughout this document. Security issues associated with processing unsolicited calendar data
and methods for mitigating them are discussed in
...
The "processcalendar" extension
MUST NOT
flagged as spam and/or malicious. The "spamtest" and "virustest" extensions
the header test
used to make "processcalendar" conditional on "safe" content.

 (or
 if messages are scanned outside of the Sieve interpreter) can be

 process any calendar data enclosed in a message
...
The "processcalendar" extension
potentially malicious sender. The address and envelope tests
with the "extlists" extension
"processcalendar" conditional on the sender not being a member of that list.

 process calendar data received from a
[RFC5228]

) can be used to create a "deny list" and make
...
Similarly, the "processcalendar" extension
a known sender. The address and envelope tests
[RFC6134]
"extlists" extension
"processcalendar" conditional on the sender being a member of that list.

[RFC5228]
) can be used to create an "allow list" and make

 only process calendar data received from
...
SHOULD NOT
 ...
 process calendar data received from an

) and/or on whether one or more of the following passes or fails on
, DomainKeys Identified Mail (DKIM)

The "processcalendar" extension
untrustworthy sender. Trustworthiness may depend on whether the message has a valid
signature (see
the message: Sender Policy Framework (SPF)
Signatures
Conformance (DMARC)
. The mechanism by which a Sieve interpreter accesses the
results of such checks is outside the scope of this document, but if the results are available in
the message's header fields, the header test
...
If an attachment is found to be malicious, "processcalendar"
data.
...
The Calendaring and Scheduling Consortium "Calendar operator practices -
Guidelines to protect against calendar abuse" CC/R 18003:2019
standards.calconnect.org/csd/cc-18003.html>
...
K. Murchison "IANA Registry for Sieve Actions" RFC 9122 DOI
, June 2023,
...
Daboo, C. "Sieve Email Filtering: Spamtest and Virustest Extensions" RFC 5235
,
...
Kitterman, S. "Sender Policy Framework (SPF) for Authorizing Use of Domains
in Email, Version 1" RFC 7208 DOI 10.17487/RFC7208
...
Jenkins, N.
Data" RFC 8984 DOI 10.17487/RFC8984
,
info/rfc8984>

R. Stepanek "JSCalendar: A JSON Representation of Calendar
,
...
The authors would like to thank the following individuals for contributing their ideas and
