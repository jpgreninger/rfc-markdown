...
certificates.
 ...
 to provide alignment with the 2008 specification for Internationalized

[RFC5280]

An IDN in Unicode (native character) form contains at least one U-label
...
extensibility. RFC 8398
addresses.

 specifies the SmtpUTF8Mailbox for internationalized email
...
.
 ...
 both refer to the Punycode algorithm for
...
; the derived property values for characters in this

 are not allowed under this specification. For
...
[RFC8399]
 ...
 required conversion of A-labels to U-labels in order to process name
...
This section provides updates to several paragraphs of
is not replaced, then the original text and the replacement text are shown.

[RFC5280]

. For clarity, if the entire section
...
MAY
 ...
 specify a particular mailbox, all
...
(as with URIs). For example, ".example.com" indicates all the Internet mail addresses in
...
MAY
 ...
 specify all addresses at a particular
...
This update aligns with IDNA2008. Since all of
is not provided.

Section 7.2 of [RFC5280]

 is replaced, the OLD text
...
string.
 ...
 with the ACE prefix "xn--" added at the beginning of the
...
constraints, conforming implementations
...
MUST
 ...
 perform a case-insensitive exact match

 perform a

Implementations that have a user interface
Specifically, conforming implementations convert A-labels to U-labels for display
purposes.

SHOULD

 convert IDNs to Unicode for display.
...
Implementation consideration: There are increased memory requirements for IDNs. An
...
SHALL NOT
 ...
 perform the "ToASCII" label conversion specified in Section 4.1 of
 be considered a "stored string". That is, the AllowUnassigned
...
This update aligns with IDNA2008 and
...
otherName choice of GeneralName as specified in Section 3 of RFC 8398
that the UTF8 encoding of the internationalized email address
MUST NOT
Order-Mark (BOM)
SmtpUTF8Mailbox
...
. Note
 contain a Byte-
 to aid comparison. The email address local-part within the
...
IDNA2008, which is defined in
, and the
,
updates to these documents. Failure to use valid A-labels may yield a domain name that cannot
be correctly represented in the Domain Name System (DNS). In addition, the CA/Browser Forum
offers some guidance regarding internal server names in certificates

 ensure that IDNs are valid according to
,

[RFC5890] [RFC5891] [RFC5892] [RFC5893] [RFC5894]
...
An earlier version of this specification
 required conversion of A-labels to U-labels in
order to process name constraints for internationalized email addresses in SmtpUTF8Mailbox
other names. This led to implementation complexity and at least two security vulnerabilities.
Now, all IDNs are carried and processed as A-labels.
...
Zeilenga, K. "Lightweight Directory Access Protocol (LDAP): Internationalized
String Preparation" RFC 4518 DOI 10.17487/RFC4518
www.rfc-editor.org/info/rfc4518>
...
CA/Browser Forum "Internal Server Names and IP Address Requirements for
SSL: Guidance on the Deprecation of Internal Server Names and Reserved IP
Addresses provided by the CA/Browser Forum" Version 1.0
cabforum.org/internal-names/>
...
Datadog Security Labs "The OpenSSL punycode vulnerability (CVE-2022-3602):
Overview, detection, exploitation, and remediation"
<https://
securitylabs.datadoghq.com/articles/openssl-november-1-vulnerabilities/>
.
...
Housley, R. "Internationalization Updates to RFC 5280" RFC 8399 DOI 10.17487/
...
M. Suignard "Unicode Technical Standard #46: Unicode IDNA
Davis, M.
Compatibility Processing" Revision 31, The Unicode Consortium, Mountain
View
...
Wei Chuang
 ...
 for identifying the issue and a solution.

Thanks to
review and thoughtful comments.

Takahiro Nemoto John Klensin Mike Ounsworth
