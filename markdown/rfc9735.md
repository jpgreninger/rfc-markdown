...
RFC 9735
Locator/ID Separation Protocol (LISP) Distinguished
Name Encoding
...
) introduces two new numbering spaces: Endpoint Identifiers
...
. This document defines a termination character, an 8-bit
...
The variable-length string of characters are encoded as a NULL-terminated (0x00) US-ASCII
character set as defined in
US-ASCII range. A NULL character
the string.

, where UTF-8 has the characteristic of preserving the full

 appear only once in the string and
...
, is the length of the string in bits (including the NULL-terminated 0x00 octet).
messages
...
[RFC8060]
encodings
string in octets. The length field
NULL-terminated (0x00). If a NULL octet (0x00) appears before the end of the octet field, i.e., the
NULL octet (0x00) appears before the last position in the octet fields, then the string
accepted and the octets after the NULL octet (0x00)

), an explicit length field can be used to indicate the length of the ASCII
 still be

 include the NULL octet (0x00). The string
...
If the octet after the AFI field is the NULL octet (0x00), the string is a NULL string and
accepted. That is, an AFI 17 "Distinguished Name" encoded string
length.

MUST
 be at least 1 octet in
...
When performing DN-EID lookups, Map-Request messages
 carry an EID Mask-Len length
...
, a well-known format for a public-key hash can be encoded as a DN.
System, as in
[LISP-GEO]
...
different than registering IPv4 or IPv6 EIDs to the Mapping System. See
...
RECOMMENDED
 ...
 that each use case register their DNs with a unique Instance-ID. Any use

It is
cases that require different uses for DNs within an Instance-ID
...
LCAF; thus, the security considerations of
...
In a practical implementation of
 on LISP deployments, routers running as Proxy
...
The use of a DN here provides descriptive information about the role being registered and allows
...
, ETRs that plan to switch to using reliable
...
Rodriguez-Natal, A. Ermagan, V. Cabellos, A. Barkai, S.
"Publish/Subscribe Functionality for the Locator/ID Separation Protocol (LISP)"
,
RFC 9437 DOI 10.17487/RFC9437
<https://www.rfc-editor.org/info/
, August 2023,
rfc9437>
...
S. Hooda "LISP Site External Connectivity" Work in
...
Venkatachalapathy, B. Portoles-Comeras, M. Lewis, D. Kouvelas, I.
,
Cassar "LISP Map Server Reliable Transport" Work in Progress Internet-Draft,
,
draft-ietf-lisp-map-server-reliable-transport-05
...
Moreno, V.
Progress Internet-Draft, draft-ietf-lisp-vpn-12
,
...
providing deployment-experience samples.
 ...
 for moving this document through the process and
