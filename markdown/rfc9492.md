...
applications such as Segment Routing (SR) Policy and Loop-Free Alternates (LFAs) that also make
use of the link attribute advertisements have been defined. In cases where multiple applications
wish to make use of these link attributes, the current advertisements do not support application-
specific values for a given attribute, nor do they support indication of which applications are
using the advertised value for a given link. This document introduces link attribute
advertisements in OSPFv2 and OSPFv3 that address both of these shortcomings.
...
OSPF App-Specific Link Attributes
...
support of traffic engineering (TE) was introduced by
. Use of these extensions has been
, and
has been extended by
associated with deployments supporting TE over Multiprotocol Label Switching (MPLS) in the
presence of the Resource Reservation Protocol (RSVP), more succinctly referred to as RSVP-TE
[RFC3209]
.
...
. This has introduced ambiguity in that if a deployment includes a mix of RSVP-TE

In recent years, new applications have been introduced that have use cases for many of the link
attributes historically used by RSVP-TE. Such applications include SR Policy
 and LFAs
[RFC5286]
support and SR Policy support, for example, it is not possible to unambiguously indicate which
advertisements are to be used by RSVP-TE and which advertisements are to be used by SR Policy.
If the topologies are fully congruent, this may not be an issue, but any incongruence leads to
ambiguity.
...
. If both RSVP-TE and SR Policy are deployed in a network,
...
in the OSPFv2 TE Opaque Link State Advertisement (LSA)
LSA
and

[RFC5329]
. Additional RSVP-TE link attributes have been defined by
[RFC7471]
.

 and OSPFv3 Intra-Area-TE-
[RFC4203] [RFC7308]
,
,
...
. These LSAs were defined as generic containers for distribution of the

 for OSPFv2 and E-Router-LSAs
...
Section 14.
 ...
 and from the "OSPFv3 Extended-LSA Sub-TLVs" registry
...
To allow advertisement of the application-specific values of the link attribute, an Application-
Specific Link Attributes (ASLA) sub-TLV is defined. The ASLA sub-TLV is a sub-TLV of the OSPFv2
Extended Link TLV

 and OSPFv3 Router-Link TLV
...
be advertised by multiple applications. The ASLA sub-TLV
 be used for advertisement of the
...
MUST
Standard Application Identifier Bit Mask is not present, the SABM Length
...
the User-Defined Application Identifier Bit Mask is not present, the UDABM Length
set to 0.
...
Optional set of bits, where each bit represents a single standard application. Bits are defined
in the "Link Attribute Application Identifiers" registry, which is defined in
. Current
...
implementation
 ...
 be treated as if they are set to 0 on receipt. Bits that are not supported by an

 be ignored on receipt. Bits that are not

 be transmitted as 0 and
...
UDAs. Undefined bits that are transmitted
receipt. Bits that are not transmitted
MUST
are not supported by an implementation

MUST
 be treated as if they are set to 0 on receipt. Bits that
...
for all applications that use the link attributes advertised in the ASLA sub-TLV.

 be present and one or more application-specific bits
...
If the same attribute is advertised in more than one ASLA sub-TLV with the application listed in
the Application Identifier Bit Masks, the application
advertisement and ignore any subsequent advertisements of that attribute.

 use the first instance of

SHOULD

MAY

 be advertised associated with zero-length Application Identifier Bit Masks

 be used by standard applications and/or user-defined applications when no link attribute

Link attributes
for both standard applications and user-defined applications. Such link attribute advertisements
MUST
advertisements with a non-zero-length Application Identifier Bit Mask and a matching
Application Identifier Bit set are present. Otherwise, such link attribute advertisements
NOT
...
advertised in ASLA sub-TLVs are:
 ...
 state whether the new attributes support application-specific

 use the ASLA sub-TLV if
...
a backup path that does not share any SRLG with the protected link.
...
advertise the SRLG in the OSPFv3 Router-Link TLV, TLV type 12 is used.
 ...
 is used with TLV type 11. Similarly, for OSPFv3 to
...
(nominal or worst case), or loss.
 ...
 defines extended link metrics that are
...
sub-TLVs defined in
 ...
 is used with the following TLV types:
...
same format for the sub-TLVs defined in
...
[RFC7308]
 ...
 define the Administrative Group and Extended Administrative Group
...
[RFC3630]
 ...
 defines the TE Metric.

To advertise the TE Metric in the OSPFv2 Extended Link TLV, the same format for the sub-TLV
defined in
the TE Metric in the OSPFv3 Router-Link TLV, TLV type 22 is used.

 is used with TLV type 22. Similarly, for OSPFv3 to advertise

Section 2.5.5 of [RFC3630]

7.  Maximum Link Bandwidth

. Because it is an application-independent attribute, it

Maximum link bandwidth is an application-independent attribute of the link that is defined in
[RFC3630]
 be advertised in the
ASLA sub-TLV. Instead, it
Extended Link Opaque LSA in OSPFv2
Router-LSA Router-Link TLV in OSPFv3

[RFC7684]
 or as a sub-TLV of the Router-Link TLV in the E-
[RFC8362]
.

 be advertised as a sub-TLV of the Extended Link TLV in the
...
Bit Mask or by using a zero-length Application Identifier Bit Mask. The use of zero-length
Application Identifier Bit Mask is further discussed in Section 12.2.

9.  Local Interface IPv6 Address Sub-TLV

The Local Interface IPv6 Address sub-TLV is an application-independent attribute of the link that
is defined in
advertised in the ASLA sub-TLV. Instead, it
TLV inside the OSPFv3 E-Router-LSA

 be advertised as a sub-TLV of the Router-Link
MAY
[RFC8362]
.
...
format for the sub-TLV defined in
...
from the information carried in the OSPFv2 TE Opaque LSA
TE-LSA

 and OSPFv3 Intra-Area-
...
In the case of SR Policy, advertisement of application-specific link attributes does not indicate
...
specification defining this use
attribute advertisements and enablement for that application.

 define the relationship between application-specific link

MUST

This document allows the advertisement of application-specific link attributes with no
application identifiers, i.e., both the SABM and the UDABM are not present (see Section 5). This
supports the use of the link attribute by any application. In the presence of an application where
the advertisement of link attributes is used to infer the enablement of an application on that link
...
these applications. Further discussion of the associated issues can be found in Section 12.3.
...
new applications.
 ...
 make use of RSVP-TE LSA advertisements. This simplifies deployment of
...
to the restrictions specified in Section 5. If support for a new application is introduced on any
node in a network in the presence of such advertisements, the new application will use these
advertisements when the aforementioned restrictions are met. If this is not what is intended,
then existing link attribute advertisements
MUST
applications specified before a new application is introduced.

 be readvertised with an explicit set of
...
applications other than RSVP-TE
This results in duplicate advertisements for those attributes.

 be advertised using application-specific advertisements.
...
legacy advertisements for RSVP-TE. Attributes for applications other than RSVP-TE
...
is, however,
 to advertise all link attributes for RSVP-TE in the existing OSPFv2 TE
Opaque LSA
...
reservable link bandwidth and unreserved bandwidth,
[RFC3630]
sub-TLV.

 use the OSPFv2 TE Opaque LSA
 be advertised in the ASLA
...
malformed, they are detected and do not facilitate a vulnerability for attackers to crash or
otherwise compromise the OSPF router or routing process. Reception of a malformed TLV or sub-
 be counted and/or logged for further analysis. Logging of malformed TLVs and sub-
TLV
 be rate-limited to prevent a denial-of-service (DoS) attack (distributed or
TLVs
...
impacting TE, which uses various link attributes for its path computation. This is similar in
nature to the impacts associated with, for example,
this document limit the scope to specific applications, the impact of tampering is similarly
limited in scope.

[RFC3630]

. As the advertisements defined in
...
The values defined in this document have been allocated using the IETF Review procedure as
described in
...
The "OSPFv2 Extended Link TLV Sub-TLVs" registry
 defines sub-TLVs at any level of
nesting for OSPFv2 Extended Link TLVs. IANA has assigned the following sub-TLV types in the
...
The "OSPFv3 Extended-LSA Sub-TLVs" registry
for OSPFv3 Extended LSAs. IANA has assigned the following sub-TLV types in the "OSPFv3
Extended-LSA Sub-TLVs" registry:

 defines sub-TLVs at any level of nesting
...
Changes to Section 5 have been introduced to clarify normative behavior in the presence of such
advertisements.
length UDABM as a means of advertising link attributes that can be used by any application.

 defines advertising link attributes with zero-length SABM and zero-
...
However, the text uses the word "permitted", suggesting that the use of such advertisements is
"optional". Such an interpretation could lead to interoperability issues and is not what was
intended.

The replacement text makes explicit the specific conditions when such advertisements
used and the specific conditions under which they
...
A subsection discussing the use of zero-length Application Identifier Bit Masks has been added
. See Section 12.2.
...
Previdi, S., Ed. Filsfils, C., Ed. Decraene, B. Litkowski, S. Horneffer, M.
Shakir "Source Packet Routing in Networking (SPRING) Problem Statement and
Requirements" RFC 7855 DOI 10.17487/RFC7855
...
The following acknowledgments are included in
...
Alvaro Retana
 ...
 for his detailed review and comments.

For this document, the authors would like to thank
