...
provisioning time. Although that approach is viable in many situations, it falls short in several
...
defined by
. The resource server can use these parameters to explicitly communicate to
...
The validation operations mentioned in steps 2 and 6 imply that the resource server has a way of
...
meets the security criteria for the requested resource.
 ...
 or is validated via introspection
...
Section 6 of [RFC9068]
 ...
 assumes access tokens are treated as opaque by clients. The token
...
authentication scheme's error parameter (from
schemes, such as those seen in

, which use the same error parameter:

[RFC6750]

[RFC9449]

) and other OAuth authentication
...
authentication event associated with the access token. As defined in Section 1.2 of
, the
...
The example in Figure 3 shows a challenge informing the client that the last active
...
The auth-params max_age and acr_values
...
resource, as described in
 ...
 include the scope attribute with the value necessary to access the protected

 both occur in the same challenge if the resource
...
The acr_values and max_age authorization request parameters are both
defined in Section 3.1.2.1. of
[OIDC]
authorization server behavior defined in
...
. This document does not introduce any changes in the

 for processing those parameters; hence, any
...
SHOULD
 ...
 include a value reflecting

 attempt to authenticate the user in a manner that satisfies the

 establishes that an authorization server receiving a request containing

 states that if a request includes the max_age parameter, the authorization server
...
authentication level cannot be met, the authorization server
...
fails and returns an unmet_authentication_requirements error as defined in
. The
...
requirements.
 ...
 consider the requested acr value as necessary for successfully
...
, where the access token is encoded in JWT format and
...
, where the token is validated and decoded by sending it to an
...
MAY
 ...
 elect to use other encoding and validation
...
) are used to convey the time and context of the user-

 for details. The following is a conceptual example
...
metadata document, as defined in
[OIDCDISC]
Section 3 of
...
, the value acr_values_supported, as defined in
. The presence of acr_values_supported in the authorization server
...
MUST NOT
 ...
 be used to position OAuth as an authentication protocol. For the
...
other method. The resource server
 return a challenge without verifying the client presented
...
This specification registers the following values in the "OAuth Token Introspection Response"
registry
...
Lodderstedt, T. "OpenID Connect Core Error Code
unmet_authentication_requirements"
openid-connect-unmet-authentication-requirements-1_0.html>
...
This specification was developed within the OAuth Working Group under the chairpersonship of
Rifaat Shekh-Yusef
 with
Security Area Directors. Additionally, the following individuals contributed ideas, feedback,
corrections, and wording that helped shape this specification:
Pieter Kasselman Aaron Parecki Denis Pinkas Dima Postnikov

Caleb Baker Ivan Kanakarakis
...
Some early discussion of the motivations and concepts that precipitated the initial draft version
of this document occurred at the 2021 OAuth Security Workshop. The authors thank the
Guido Schmitz Steinar Noem
organizers of the workshop (
,
that is conducive to collaboration and community input.
