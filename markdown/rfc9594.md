...
RFC 9594
Key Provisioning for Group Communication Using
Authentication and Authorization for Constrained
Environments (ACE)

Abstract

This document defines how to use the Authentication and Authorization for Constrained
Environments (ACE) framework to distribute keying material and configuration parameters for
secure group communication. Candidate group members that act as Clients and are authorized to
join a group can do so by interacting with a Key Distribution Center (KDC) acting as the Resource
Server, from which they obtain the keying material to communicate with other group members.
While defining general message formats as well as the interface and operations available at the
KDC, this document supports different approaches and protocols for secure group
communication. Therefore, details are delegated to separate application profiles of this
document as specialized instances that target a particular group communication approach and
define how communications in the group are protected. Compliance requirements for such
application profiles are also specified.
...
different ways.
 ...
 or publish-subscribe (pub-sub) messaging

, and can be protected in
...
A.1 compiles the requirements that application profiles are
referred to by an identifier that starts with "REQ". Instead, Appendix A.2 compiles the
requirements that application profiles
starts with "OPT".

 fulfill; these are referred to by an identifier that

 to fulfill; these are
...
6.1 and 6.2 of
 ...
 for data encoding. However, using JSON
...
Information Format (AIF)
[RFC6749]
for entities in the considered architecture is defined in OAuth 2.0
...
[RFC9237]
 ...
 to express authorization information. The terminology

 and in the Authorization
...
The identifier of a group as a text string encoded as UTF-8
...
The identifier of a node as a text string encoded as UTF-8

with the semantics of URI path segments (see
...
A profile of the ACE framework as per
...
Authorization Server (AS): As per the AS defined in the ACE framework
...
ASKDCClientDispatcherGroupmembersRFC 9594
...
This exchange between the Client and AS
...
Once this exchange is completed, the joining node
...
MUST
KDC, such as the DTLS transport profile of ACE
ACE

 or the OSCORE transport profile of
...
The joining node and the KDC maintain the secure communication association to support
...
, the Client asks the AS for the authorization to join the group through the
As defined in
...
Communications between the Client and the AS
...
messages and "application/cwt" for the third message, which are defined in the ACE framework.
 ...
 be secured according to what is defined by
...
CASKDCGroupMembers/AuthorizationRequestAuthorizationResponse--(1)<TokenTransferRequestTokenTransferResponse\JoinRequest--Grouprekeying(optional)JoinResponseDispatcherSecuregroupcommunication(1)DefinedintheACEframeworkRFC 9594

Key Provisioning for Group Communication

September 2024

The transport profile of ACE also defines a number of details, such as the communication and
...
and
 contain the following parameters, which, if included,
as specified below.
...
Figure 4 shows an example in CDDL notation

[RFC8610]

 where scope uses AIF.
...
ClientASKDCAuthorizationRequest:POST/tokenAuthorizationResponse:2.01(Created)TokenTransferRequest:POST/authz-info|TokenTransferResponse:2.01(Created)|RFC 9594

Key Provisioning for Group Communication

September 2024

Figure 5 shows an example in CDDL notation where scope uses the textual format with the
group name and role identifiers encoded as CBOR text strings.

REQUIRED

 for application profiles of this specification to specify the exact format and
...
guidelines in
 ...
 to register its specific instance of "Toid"
...
If the application profile uses the textual format, it
use for abbreviating the role identifiers (OPT1).

MAY

 additionally specify CBOR values to
...
, other additional parameters can be included if necessary.
...
. Note that the 'expires_in' parameter
...
Additionally, when included, the following parameter
...
a scope claim (for example, see 'scope' registered in
...
access token is issued, the Authorization Server
 use the extended format of scope
...
MAY
 ...
 additionally contain other claims that the transport profile of ACE or other

When receiving an Authorization Request from a Client that was previously authorized and for
 reply with that token. Note
which the AS still stores a valid non-expired access token, the AS
...
, since it allows asking the KDC
...
[RFC9200]
 ...
 ask for this information from the KDC through the same Token Transfer
 have Content-Format "application/ace+cbor" registered

MUST

 include the access token. The CBOR map
...
MUST
 ...
 be formatted as a CBOR map, which
 additionally include the following parameter,

MAY

The joining node
Request. In this case, the message
in
MUST
which, if included,
...
[RFC9200]
 ...
 have Content-Format "application/ace+cbor", and its payload
...
While storing the access token, the KDC
...
deleting the access token (e.g., upon its expiration, see
 ...
 store the 'kdcchallenge' value associated with the
...
The same 'kdcchallenge' value
 be reused several times by the Client to generate new PoP
...
different authentication credential. Therefore, it is
 that the KDC keeps storing
...
the field 'id' of each 'sign_info_entry' specifies the name or array of group names to which that
 omit the 'sign_info' parameter in the
'sign_info_entry' applies. As an exception, the KDC
...
specification
 ...
 define additional parameters to use within this exchange (OPT2).

MAY

Application profiles of this specification
parameter values for the signature algorithm and signature keys if 'sign_info' is not used (OPT3).

 define alternative specific negotiations of
...
[RFC9200]
 ...
 parameter of the request and response messages
...
This parameter allows the Client and the RS to exchange information about a signature
...
algorithm used in the groups identified by the 'gname' values. It is
...
on the value of 'sign_alg'. It is
 for application profiles to define the possible values
...
content depends on the value of 'sign_alg'. It is
the possible values and structure for the elements of this parameter (REQ5).

 for application profiles to define
...
those values also indicating the type of container to use for exchanging the authentication
 for application
credentials with the KDC (e.g., a chain or bag of certificates). It is
...
[RFC8610]
 ...
 of the 'sign_info' parameter is given below.
...
This format is consistent with every signature algorithm currently defined in
...
The 'kdcchallenge' parameter is an
the /authz-info endpoint at the RS, as defined in
...
Section 5.10.1 of [RFC9200]
 ...
 parameter of the response message returned from
...
Later on as a group member, the Client can also rely on the interface at the KDC to perform
...
payload and specify a Content-Format, those messages
 have Content-Format "application/
...
/ace-group/GROUPNAME : one such sub-resource to /ace-group is hosted for each group with
...
access token scope ('gname' in Section 3.1) coincide, the KDC
...
/ace-group/GROUPNAME/kdc-cred : the path of this resource is invariant once the resource is
...
this is required for the correct group operation. It is
define whether the KDC has such an associated authentication credential (REQ8).

 for application profiles to
...
The KDC is expected to fully provide the interface defined above. It is otherwise
...
profiles of this specification
 extend the KDC interface by defining additional handlers, as
...
REQUIRED
 ...
 for application profiles of this specification to register a Resource Type for the
...
REQUIRED
 ...
 for application profiles of this specification to define what specific actions (e.g.,
...
GET request to /ace-group/GROUPNAME/nodes/NODENAME in order to retrieve the current
...
REQUIRED
 ...
 for application profiles of this specification to categorize possible newly defined
...
Upon receiving a request from a Client, the KDC
from that Client. If this is not the case, the KDC
response.

MUST

 check that it is storing a valid access token

MUST

 reply with a 4.01 (Unauthorized) error

Unless the request targets the /ace-group resource, the KDC
access token for that Client such that:

MUST

 check that it is storing a valid
...
In case the KDC stores a valid access token but the verifications above fail, the KDC
with a 4.03 (Forbidden) error response. This response
defined in
ace+cbor".

, in which case the Content-Format

 reply
 be an AS Request Creation Hints, as

Section 5.3 of [RFC9200]
...
If the request includes unknown or unexpected fields, the KDC
continue processing the request. Application profiles of this specification
mandatory payload formats for specific error cases (OPT4).

MUST

 silently ignore them and
...
Some error responses from the KDC can convey error-specific information according to the
problem-details format defined in
"application/concise-problem-details+cbor". The payload of these error responses
CBOR map specifying a Concise Problem Details data item (see
...
in Section 11.6 of this document.
 ...
 include the Custom Problem Detail entry 'ace-groupcomm-error', which is registered
...
[RFC8610]
 ...
 of the 'ace-groupcomm-error' entry is given below.
...
The problem-details format (in general) and the Custom Problem Detail entry 'ace-groupcomm-
error' (in particular) are
 for Clients to support. A Client supporting the entry 'ace-
...
The handler expects a request with the payload formatted as a CBOR map, which
...
exact encoding of the group identifier
 be specified by the application profile (REQ13).
...
Note that the KDC only verifies that the node is authorized by the AS to access this resource.
...
ClientKDCGroupNameandURIRetrievalRequest:FETCH/ace-groupGroupNameandURIRetrievalResponse:2.05(Content)--RFC 9594
...
The handler expects a request with payload formatted as a CBOR map, which
following fields, which, if included,
MUST
...
For example, the array ["role1", "role2+role3"] indicates that the Client wishes to receive
...
combinations of roles encoded as CBOR arrays of role identifiers.
 ...
 of 'get_creds' is given in Figure 9; as an example, it uses node
...
representation of the Client's authentication credential. This parameter
MUST
...
send messages to one or more group members. It is
...
'client_cred_verify': encoded as a CBOR byte string. This parameter
...
carried in the 'client_cred' parameter. Application profiles of this specification
...
is generated (REQ15).
 ...
 of the specific application profile to define how the challenge N_S
...
specific encodings of this parameter
specification (OPT6).

 be defined in application profiles of this
...
resource at this specific URI. The KDC
 use this URI to send CoAP requests to the Client
...
determine which exact roles should be considered for the Client, it
Request) error response.

 reply with a 4.00 (Bad
...
•
 ...
 reply with a 4.00 (Bad Request) error response, which
...
The KDC checks the authentication credential to be valid for the group identified by
...
If this verification fails, the handler
The response
formatted as defined in Section 4.1.2. Within the Custom Problem Detail entry 'ace-
 be set to 2 ("Authentication
groupcomm-error', the value of the 'error-id' field
MUST
...
MUST
 ...
 specify the exact approaches used to verify the

MUST

If the PoP evidence does not pass verification, the handler
Request) error response. The response
...
("Invalid proof-of-possession evidence").
 ...
 have Content-Format "application/concise-
...
If an eligible authentication credential for the Client is neither present in the 'client_cred'
 that the handler
parameter nor retrieved from the stored ones at the KDC, it is
...
MAY
 ...
 be a CBOR map. For instance, the CBOR map can include a 'sign_info'
...
The handler associates the node identifier NODENAME with the access token and the secure
...
credential is specified in the 'client_cred' parameter, then the handler
 replace the old
...
application profile of this specification, then the KDC
material and securely distribute it to the current group members (see Section 6).

 generate new group keying
...
the current members of the group if the KDC manages those and the Client requested those.
 ...
 have response code 2.01 (Created) if the Client has been added to the list
The Join Response
...
the sub-resource associated with the Client, i.e., /ace-group/GROUPNAME/nodes/NODENAME.
 ...
 include the Location-Path CoAP Options, specifying the path to

MUST

The Join Response message
MUST
payload of the response is formatted as a CBOR map, which
with the values specified below:

 have Content-Format "application/ace-groupcomm+cbor". The
...
11.8 of this specification. Implementations
MUST
...
registry for the key type in question.
 ...
 verify that the key type specified by this
...
the group keying material changes. The application profile
...
Additionally, documents specifying a type of keying material
...
SHOULD
 ...
 contain the following fields with the values specified below:
...
unsigned integer. If the 'exp' parameter is included, this parameter
...
parameter. After such an expiration time, the Client
 use the group keying material
...
If a Client has a reliable way to synchronize its internal clock with UTC, and both the 'exp' and
 use the 'exp' parameter value as expiration
'exi' parameters are present, then the Client
MUST
...
'ace_groupcomm_profile': its value is encoded as a CBOR integer and
...
parameter in the "ACE Groupcomm Profiles" registry (REQ19).
 ...
 register an application profile identifier and the related value for this
...
MUST
 ...
 be present if 'get_creds' was present in the Join Request; otherwise, it
...
authentication credential. It is
 for application profiles to define the specific
...
SHOULD
 ...
 be present if 'creds' is also present; otherwise, it
...
MAY
 ...
 be omitted if the Client can rely on other means to unambiguously gain
...
members in the group). The i-th element of the array specifies the node identifier that the
...
summarized in Table 3. Application profiles of this specification
...
If the KDC has an authentication credential as required for the correct group operation, the
KDC's authentication credential
credentials of the group members. It is
REQUIRED
specific formats that are acceptable to use for the authentication credentials in the group
(REQ6).

 have the same format used for the authentication

 for application profiles to define the
...
in the 'kdc_cred' parameter. Application profiles of this specification
...
to in case the 'rekeying_scheme' parameter is not included in the Join Response (OPT9).
 ...
 define a default group rekeying scheme to refer
...
rekeying scheme to use in the group. Some simple rekeying schemes may not require specific
...
Section 6.1 of
[RFC7252]
 include GROUPNAME in the url-path. A default url-path is /ace-
...
The KDC
 use this URI to send one-to-many CoAP requests to the Client group members
...
After sending the Join Response, if the KDC has an associated authentication credential as
required for the correct group operation, then the KDC
...
possession of its own private key.
 ...
 store the N_C value specified in the
...
Application profiles of this specification
MUST
verify the PoP evidence in 'kdc_cred_verify' and
in which case (REQ21).

 specify the exact approaches used by the Client to
 specify which of those approaches is used

MUST

Application profiles of this specification
 specify the communication protocol that members
...
ClientKDCJoinRequest:POST/ace-group/GROUPNAMEJoinResponse:2.01(Created)Location-Path="/ace-group/GROUPNAME/nodes/NODENAME"RFC 9594
...
If not previously established, the Client and the KDC
communication association (REQ24). This can be achieved, for instance, by using a transport
profile of ACE. The join exchange
Client and the KDC
pairwise communications that must be protected.

 use that same secure communication association to protect further

 occur over that secure communication association. The

 first establish a pairwise secure
...
REQUIRED
 ...
 that the secure communication association between the Client and the KDC is
...
credentials of the group members, the Client is
 to send its own authentication
...
member of the group. If the verification fails, the KDC
response. The response
MUST

 reply with a 4.03 (Forbidden) error
 have Content-Format "application/concise-problem-details+cbor"
...
group members").
 ...
 be set to 0 ("Operation permitted only to
...
group.
 ...
 also include the parameters 'rekeying_scheme' and 'mgt_key_material' as

MAY

 also include the parameters 'ace_groupcomm_profile', 'exp', and 'exi', as

The payload
specified in Section 4.3.1. If the 'exp' parameter is included, the 'exi' parameter
...
ClientKDCKeyDistributionRequest:GET/ace-group/GROUPNAMEKeyDistributionResponse:2.05(Content)RFC 9594
...
◦
The arrays 'role_filter' and 'id_filter'
bool, [ ], [ ] ]. If the 'get_creds' parameter has such a format, the request
considered malformed, and the KDC

MUST NOT

MUST

 both be empty, i.e., in CDDL notation: [
...
combination of roles of group members
MUST
specification (REQ1, REQ6, REQ25).

 be specified by application profiles of this
...
SHOULD
 ...
 be omitted according to the same criteria
...
MAY
 ...
 enforce one of the following policies in order to handle possible node

The handler
identifiers that are included in the 'id_filter' element of the 'get_creds' parameter of the request
but are not associated with any current group member. Such a policy
...
The KDC retains authentication credentials of group members for a given amount of time
...
If the KDC adopts this policy, the application profile
...
its leaving, possibly on a per-member basis.
 ...
 also specify the amount of time
...
ClientKDCAuthenticationCredentialRequest:FETCH/ace-group/GROUPNAME/credsAuthenticationCredentialResponse:2.05(Content)--RFC 9594
...
Figure 17: Example of Authentication Credential Request-Response to Obtain the Authentication
...
ClientKDCAuthenticationCredentialRequest:GET/ace-group/GROUPNAME/credsAuthenticationCredentialResponse:2.05(Content)--RFC 9594
...
Figure 19: Example of Authentication Credential Request-Response to Obtain the Authentication
...
If all verifications succeed, the handler returns a 2.05 (Content) message containing the KDC's
...
Application profiles of this specification
to compute the PoP evidence to include in the 'kdc_cred_verify' parameter and
which of those approaches is used in which case (REQ21).

 specify the exact approaches used by the KDC
 specify
...
requests to this resource, then the application profile
 specify how external signature
...
credential from the 'kdc_cred' parameter and
specified in the 'kdc_cred_verify' parameter. In case of successful verification of the PoP
evidence, the Client
currently stored one.

 store the obtained KDC's authentication credential and replace the

 verify the proof-of-possession (PoP) evidence
...
Response (see Section 4.3.1). Application profiles of this specification
approaches used by the Client to verify the PoP evidence in 'kdc_cred_verify' and
which of those approaches is used in which case (REQ21).
...
GroupMemberKDCKDCAuthenticationCredentialRequestGET/ace-group/GROUPNAME/kdc-credKDCAuthenticationCredentialResponse:2.05(Content)--RFC 9594
...
Figure 22: Example of KDC Authentication Credential Request-Response to Obtain the
Authentication Credential of the KDC
...
The specific format and meaning of group policies
this specification (REQ20).

MUST

 be specified in application profiles of
...
ClientKDC--PoliciesRequest:GET/ace-group/GROUPNAME/policiesPoliciesResponse:2.05(Content)RFC 9594
...
ClientKDCVersionRequest:GET/ace-group/GROUPNAME/numVersionResponse:2.05(Content)RFC 9594
...
MUST
 ...
 reply with a 4.03 (Forbidden) error response. The response
...
the 'error-id' field
 ...
 be set to 0 ("Operation permitted only to group members").
...
same as defined in the response of Section 4.3.2. If the 'exp' parameter is included, the 'exi'
 also be included. If the parameter 'exi' is included, its value specifies the
parameter
...
The CBOR map can include additional parameters that specify the individual keying material for
the Client. The specific format of individual keying material for group members or of the
information to derive such keying material
 be defined in application profiles of this
...
also observable
 for the associated node. In case the KDC removes that node from the
...
In order to mitigate this, a node that supports the CoAP No-Response Option
...
When any of the following happens, a node
protect outgoing messages and

SHOULD

MUST

 stop using the stored group keying material to

 stop using it to decrypt and verify incoming messages.
...
After having failed to decrypt messages from another group member and having sent a Key
...
ClientKDCKeyDistributionRequest:GET/ace-group/GROUPNAME/nodes/NODENAMEKeyDistributionResponse:2.05(Content)RFC 9594
...
(REQ11). If the verification fails, the KDC
The response
formatted as defined in Section 4.1.2. Within the Custom Problem Detail entry 'ace-groupcomm-
error', the value of the 'error-id' field
roles").

 have Content-Format "application/concise-problem-details+cbor" and is

 be set to 1 ("Request inconsistent with the current
...
material for the requesting Client, the KDC
MUST
response. The response
and is formatted as defined in Section 4.1.2. Within the Custom Problem Detail entry 'ace-
groupcomm-error', the value of the 'error-id' field
keying material").
 ...
 reply with a 5.03 (Service unavailable) error

 be set to 4 ("No available individual
...
members or of the information to derive such keying material
...
ClientKDCKeyRenewalRequest:POST/ace-group/GROUPNAME/nodes/NODENAMEKeyRenewalResponse:2.04(Changed)RFC 9594
...
Note that there is a difference between the Key Renewal Request in this section and the Key
...
the 'kdcchallenge' parameter, which specifies a newly generated 'kdcchallenge' value. In such a
case, the KDC
 store the newly generated value as the 'kdcchallenge' value associated with
...
parameters used in the group. If that cannot be successfully verified, the handler
MUST
with a 4.00 (Bad Request) error response. The response
...
(REQ14). If the PoP evidence does not pass verification, the handler
 reply with a 4.00 (Bad
Request) error response. The response
...
MUST
 have Content-Format "application/concise-problem-

 be set to 3 ("Invalid proof-of-
...
ClientKDCAuthenticationCredentialUpdateRequest:POST/ace-group/GROUPNAME/nodes/NODENAME/credAuthenticationCredentialUpdateResponse:2.04(Changed)--RFC 9594
...
Additionally, after updating its own authentication credential, a group member
 send to the
...
), the KDC can optionally use it and check whether the Client is
...
The KDC can perform a group rekeying before the current group keying material expires, unless
...
has triggered the rekeying process, each rekeying message can be intended for one or multiple
 support at least
group members, hereafter referred to as target group members. The KDC
 support additional
the "Point-to-Point" group rekeying scheme described in Section 6.1 and
ones.
...
MUST
 ...
 have Content-Format "application/ace-groupcomm+cbor" and its
Each rekeying message
 include at least the information specified in
payload is formatted as a CBOR map, which
MUST
the Key Distribution Response message (see Section 4.3.2), i.e., the parameters 'gkty', 'key', and
'num' defined in Section 4.3.1. The CBOR map
SHOULD
If the 'exp' parameter is included, the 'exi' parameter
...
target group members if it is relevant for the used rekeying scheme.
 ...
 also include the parameters 'exp' and 'exi'.
 also be included. The CBOR map
MAY
MUST
...
used rekeying scheme in the context of the used application profile of this specification. As a
 define additional information to
particular case, an application profile of this specification
...
Consistently with the used group rekeying scheme, the actual delivery of rekeying messages can
...
The KDC
. Thus,
 make the /ace-group/GROUPNAME resource observable
...
Figure 34 provides an example of point-to-point group rekeying. In particular, the example
...
of a more efficient and scalable group rekeying scheme, e.g.,
[RFC2627]
...
KDCGroupGroupGroupkeyingkeyingkeyingmaterialmaterialmaterial(num=6)(num=6)(num=6)/grp-rek/grp-rek/grp-rek/grp-rekC1C2C3C4[TOBEEVICTED]||\Storedgroupkeyingmaterial(num=5)/RFC 9594
...
Rekeying messages can be protected at the application layer by using COSE
...
In this case, the KDC acts as a publisher and
...
Each group member, when joining the group, received from the KDC a URI in the
...
SHOULD
 ...
 be the same one used to protect communications in the
...
KDC:Groupkeyingmaterial(num=6)Groupkeying:Updatedadministrativematerial(num=6):keyingmaterialforC1andC2Updatedadministrative:keyingmaterialforC4::::::v/grp-mrek/grp-mrek/grp-mrek/grp-mrek/grp-rekC1C2C3C4[TOBEEVICTED]||\Storedgroupkeyingmaterial(num=5)/RFC 9594
...
Since the KDC is the only sender of rekeying messages, the AEAD nonce can be computed as
...
The protected header of the COSE_Encrypt0 object
...
The unprotected header of the COSE_Encrypt0 object
with the value of the Partial IV computed above.

MUST

 include the 'Partial IV' parameter
...
In particular, the following applies when computing the countersignature.
 ...
 be signed by the KDC. To this end, the KDC computes a
...
The protected header of the signing object
the used signature algorithm.

MUST

 include the parameter 'alg', which specifies

If the source authentication of messages exchanged in the group is also ensured by means of
signatures, then rekeying messages
 be signed using the same signature algorithm and
...
parameter.
 ...
 be provided in the Join Response through the 'kdc_cred' parameter,
...
parameter of the Join Response sent by KDC (see Section 4.3.1)
...
), the AS Request Creation Hints message from the Resource Server (see
...
extended scope consists of the same CBOR byte string enclosed by a CBOR tag
...
The usage of the extended scope format is not limited to application profiles of this specification
...
CBOR tag, a new CBOR tag should not be registered in order to avoid code point squatting.
 ...
 is used for identifying the scope semantics or to register a new CBOR
...
If the binary-encoded scope uses semantics associated with a registered CoAP Content-Format
...
Note that the media type "application/ace-groupcomm+cbor"
...
See the "ACE Groupcomm Key Types"
registry
...
The KDC is expected to support all the parameters above. Instead, a Client can support only a
...
'peer_roles': This parameter is relevant for a Client that has to know about the roles of other
...
Table 6: ACE Groupcomm Error Identifiers
 ...
 and the Custom Problem Detail entry
If a Client supports the problem-details format
...
determine what actions to take next. If the Concise Problem Details data item specified in the
...
transport profile of ACE used between the Clients and the KDC, e.g.,
...
[RFC9290]
When using the problem-details format defined in
privacy and security considerations from Sections 4 and 5 of

 for error responses, then the
[RFC9290]
...
Section 10 of [GROUP-OSCORE]
freshness, e.g., as described in
.
...
then, the KDC
 ...
 also renew the group keying material on a regular or periodical fashion.

MAY

Unless otherwise defined by an application profile of this specification, the KDC
 renew
...
Since the minimum number of group members is one, the KDC
joining an empty group with new keying material never used before in that group. Similarly, the
KDC
 also provide new group keying material to a Client that remains the only member
in the group after the leaving of other group members.

 provide even a Client
...
The KDC should renew the group keying material in case it has rebooted, even if it stores the
...
guidance on such mechanisms, see

Section 4.5.1 of [RFC7641]
.
...
[RFC7959]
 ...
 are used and the keying material is updated in the
...
This is a descriptive name that enables easier reference to the item. The name
...
CBOR Key:
 ...
 be
This is the value used as the CBOR map key of the item. These values
...
. Integer values from -256 to 255
...
This is the value used to identify the keying material. These values
...
CBOR abbreviation for the name of this application profile. These values
...
. Integer values from -256 to 255 are designated as
...
The value to be used to identify the error. These values

MUST

 be unique. The value can

[RFC8126]

be a positive integer or a negative integer. Different ranges of values use different registration
. Integer values from -256 to 255 are designated as "Standards Action With
policies
...
The value to be used to identify the group rekeying scheme. These values
...
doc/html/draft-ietf-cose-cbor-encoded-cert-11>
...
,
,
the Constrained Application Protocol (CoAP)" Work in Progress Internet-Draft,
draft-ietf-core-coap-pubsub-14
<https://datatracker.ietf.org/doc/
, 18 April 2024,
html/draft-ietf-core-coap-pubsub-14>
.

A. Keränen "A publish-subscribe architecture for
...
OSCORE)" Work in Progress Internet-Draft, draft-ietf-core-oscore-
groupcomm-22
core-oscore-groupcomm-22>
...
P. Van der Stok "Discovery of OSCORE

,

Groups with the CoRE Resource Directory" Work in Progress Internet-Draft,
draft-tiloca-core-oscore-discovery-16
datatracker.ietf.org/doc/html/draft-tiloca-core-oscore-discovery-16>
...
C. Muckenhirn "Group Key Management Protocol (GKMP)

,

Harney, H.
Specification" RFC 2093 DOI 10.17487/RFC2093
,
...
Harney, H.
Architecture" RFC 2094 DOI 10.17487/RFC2094
,
...
,
, and
Architectures" RFC 2627 DOI 10.17487/RFC2627
,
editor.org/info/rfc2627>
.

R. Agee "Key Management for Multicast: Issues and
, June 1999,
...
Richardson, M.
,
Object Representation (CBOR)" RFC 9277 DOI 10.17487/RFC9277
,
,
<https://www.rfc-editor.org/info/rfc9277>
.

C. Bormann "On Stable Storage for Items in Concise Binary
, August 2022,
...
parameters and a corresponding Content-Format, as per the guidelines in
...
specify a transport profile of ACE
Section 4.3.1.1).

[RFC9200]

 to use between the Client and the KDC (see
...
, future algorithms can be registered in the "COSE

[COSE.Algorithms]

 as specifying none or multiple COSE capabilities.
...
Bormann Roman Danyliw Martin Duke Thomas Fossati Vidhi Goel Rikard Höglund Ben
,
Kaduk Erik Kline Warren Kumari Watson Ladd Daniel Migault John Preuß Mattsson
Zaheduzzaman Sarker Jim Schaad Ludwig Seitz Göran Selander Cigdem Sengul Dave Thaler
Henry Thompson Peter van der Stok
,
...
VINNOVA and the Celtic-Next project CRITISEC, by the H2020 project SIFIS-Home (Grant
agreement 952652), and by the EIT-Digital High Impact Initiative ACTIVE.
