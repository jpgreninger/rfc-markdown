...
RFC 9727
api-catalog: A Well-Known URI and Link Relation to
Help Discovery of APIs

Abstract

This document defines the "api-catalog" well-known URI and link relation. It is intended to
facilitate automated discovery and usage of published Application Programming Interfaces
(APIs). A request to the api-catalog resource will return a document providing information
...
, "api-catalog", that is encoded as a URI reference to an
...
, "api-catalog", of which the target resource is the Publisher's
...
.
 ...
 be named "api-catalog" in a well-known location as described by
...
When used in an API catalog document, the "item"
resource that represents an API that is a member of the API catalog.

[RFC6573]

 link relation identifies a target
...
MUST
 ...
 include hyperlinks to API endpoints. It is
...
that metadata directly in the API catalog document, they
at the API endpoint URIs they have listed (see Appendix A.2 for an example).

 make that metadata available
...
Section 4.2 of [RFC9264]

 publish the API catalog document in the Linkset format application/
 include a profile parameter

The Publisher
linkset+json (
Section 5 of [RFC9264]
(
...
SHOULD
 value of "https://www.rfc-editor.org/info/
...
Section 12 of
 make additional formats available via content negotiation (
...
Appendix A.2.
 ...
 also implement a Linkset with, at minimum, hyperlinks to API endpoints; see
...
return of their existing format.
 ...
 support content negotiation at the /.well-known/api-catalog URI to allow for the
...
To account for this scenario, it is
...
MAY
 ...
 include a link to the API catalog of the third-party that is the domain authority for
...
SHOULD
 ...
 consider caching and compression techniques to reduce the network
...
RECOMMENDED
 ...
 to follow operational best practice when hosting API catalog(s),
...
catalog as part of their API release lifecycle. The Publisher
 decide to include metadata
...
as part of the API release lifecycle is
...
allocated an incorrect or outdated description.)
 ...
 to detect any errors in business
...
and adapted (ensuring compliance to
metadata.

) to reflect the nature of the chosen
...
Section 3 of [WELL-KNOWN]
 ...
 for defining Well-Known URIs are met as
...
SHALL
 ...
 be appended to the /.well-known/ path-prefix for "well-known

6.2.  Formats and Associated Media Types

A /.well-known/api-catalog location
linkset+json and
...
to ensure no tampering of the API catalog.
 ...
 be used, i.e., make /.well-known/api-catalog available exclusively over HTTPS,
...
SHOULD
 ...
 take into account the security considerations from
...
expose any vulnerability related to the APIs listed.
 ...
 perform a security and privacy review of the API catalog prior to

•

SHOULD

 enforce read-only privileges for external requests to .well-known/api-

The Publisher
catalog and for internal systems and roles that monitor the .well-known/api-catalog URI.
 only be granted to roles that perform updates to the API catalog
Write privileges
...
As with any Web offering, it is
mitigate abuse and prevent denial-of-service attacks on the API catalog endpoint.

 to apply rate-limiting measures to help

RECOMMENDED

For the public-facing APIs scenario, security teams
...
about any internal domains (e.g., https://internal.example.com).
 ...
 additionally audit the API catalog to

SHOULD

For the internal/private APIs scenario, the Publisher
...
Wilde, E.
for Link Sets" RFC 9264 DOI 10.17487/RFC9264
,
editor.org/info/rfc9264>
.

H. Van de Sompel "Linkset: Media Types and a Link Relation Type
 and
,
...
S. Willmott "API Discovery Format"
...
Ed. Quintero, M., Ed. Kistler, M., Ed. Handl, R., Ed.
"OpenAPI Specification v3.1.0"
latest> Latest version available at
...
Verborgh, R. Mannens, E. Van de Walle, R.
...
N. Evans, Ed. "WADG0001 WebAPI type extension" Draft
...
This example uses the Linkset format
[RFC8631]
:

[RFC9264]

 and the following link relations defined in
...
[RFC9264]
This example also uses the Linkset format
 and lists the API endpoints in an array of
...
each API; hence, the document targeted by the bookmark link should support this.
 ...
 link relation (to indicate they are an item in the catalog). The intent is that
...
Jan Algermissen Phil Archer Tim Bray Ben Bucksch Sanjay Dalal David Dong Erik

Thanks to
Kline Mallory Knodel Murray Kucherawy Max Maton Darrel Miller Mark Nottingham Roberto
,
,
Polli Joey Salazar Rich Salz Herbert Van De Sompel Orie Steele Tina Tsou Gunter Van de Velde
,
Éric Vyncke
...
,
Erik Wilde
 for their reviews, suggestions, and support.
