...
parent PCE coordinating multi-domain path computation functions between child PCEs, thus
 explains, in the context of ACTN,
making it the base for PCE applicability for ACTN. As
...
identified with the Multi-Domain Service Coordinator (MDSC).
 ...
 describe a hierarchy of stateful PCEs with the
...
minimizing the load of the most loaded link (MLL)
belonging to the VN identified by the VN association.

[RFC5541]

) could be applied for all the LSPs
...
VN. An LSP
than one VNAG object in a PCEP message, it
ignore the others.

 belong to more than one VNAG. If an implementation encounters more

 process the first occurrence, and it
...
PCEP speaker
[RFC8697]
the VNAG object in a PCEP message. As per
...
Error-value=1 (Association Type is not supported).
 ...
 include the VN Association Type (7) in the ASSOC-Type-List TLV before using
...
The Association Identifiers (VNAG IDs) for this Association Type are dynamic in nature and
...
configured Association Range to be set. Thus, the VN Association Type (7)
...
uniquely identified by the association parameters (
...
the instruction from their child PCE (PNC).
 ...
 be included as a part of the first PCRpt message. Figure 1 shows an example of
...
[RFC0020]
 ...
 be a string of printable ASCII

 characters (i.e., 0x20 to 0x7E), without
VN. It
...
Identifier
 be specified by the customer, set via an operator policy, or auto-generated by
the PCEP speaker.

MAY

The VIRTUAL-NETWORK-TLV
VNAG object without the VIRTUAL-NETWORK-TLV, it
Type=6 (Mandatory Object missing) and Error-value=18 (VIRTUAL-NETWORK-TLV missing) and
close the session.

 be included in VNAG object. If a PCEP speaker receives the
...
document, the PCEP speaker
invalid object) and Error-value=11 (Malformed object) and
...
Transport Layer Security (TLS)
 ...
 and apply to the VN Association Type. Hence, securing the PCEP session using
.
RECOMMENDED
...
MUST
 ...
 be allowed to mark LSPs that belong to the same VN. This could also be done
...
[PCE-PCEP-YANG]
 ...
 should support the association between LSPs
...
further specifies the operations when VN associations are used.
 ...
 describes the network operations when PCE is used for VN operations. Section 3
...
A. Farrel, Ed. "The Application of the Path Computation
...
D. Ceccarelli "Applicability of the Path Computation
