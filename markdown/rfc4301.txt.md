...
![](https://cdn.mathpix.com/cropped/8c8e8c62-1897-4d40-9b8b-804785886102-008.jpg?height=691&width=1049&top_left_y=592&top_left_x=466)
...
$$
\begin{aligned}
& (\text { T-start*256 })+\text { C-start }<=(t * 256)+c<=(\text { T-end*256 })+ \\
& \text { C-end }
\end{aligned}
$$
...
c. X. 500 distinguished name, e.g., [WaKiHo97], CN = Stephen T. Kent, $0=$ BBN Technologies, SP = MA, C = US
...
Remote's
next layer protocol = Mobility Header "port" selector = Mobility Header message type
...
Remote's
next layer protocol = 1 (ICMPv4) "port" selector = OPAQUE
...
| Selector | SPD Entry | PFP | Value in Triggering Packet | Resulting SAD Entry |
| :--- | :--- | :--- | :--- | :--- |
| ICMP type and code | a single type \& range of codes a single type \& ANY code ANY type \& ANY code OPAQUE | 0 <br> 0 <br> 0 <br> 0 | type "t" \& code "c" <br> type "t" \& code "c" <br> type "t" \& code "c" <br> type "t" \& code "c" | single type \& range of codes <br> single type \& ANY code <br> ANY type \& ANY code <br> OPAQUE |

a single type \& 0 not avail. discard packet range of codes
a single type \& 0 not avail. discard packet ANY code
ANY type \& 0 not avail. ANY type \& ANY code
OPAQUE
not avail. OPAQUE
a single type \& 1 type "t" \& "t" and "c" range of codes code "c"
a single type \& 1 type "t" \& "t" and "c" ANY code
ANY type \& ANY code
OPAQUE
code "c"
type "t" \& "t" and "c"
...
type "t" \& ***
code "c"
a single type \& 1 not avail. discard packet range of codes
a single type \& 1 not avail. discard packet ANY code
ANY type \&
ANY code
OPAQUE
1 not avail. ***
...
| Selector | SPD Entry |  | Value in Triggering PFP Packet | Resulting SAD Entry |
| :--- | :--- | :--- | :--- | :--- |
...
| *** Use of $\mathrm{PFP}=1$ with an OPAQUE value is an error and SHOULD be prohibited by an IPsec implementation. |  |  |  |  |
| ![](https://cdn.mathpix.com/cropped/8c8e8c62-1897-4d40-9b8b-804785886102-043.jpg?height=29&width=1049&top_left_y=900&top_left_x=565) |  |  |  |  |
...
![](https://cdn.mathpix.com/cropped/8c8e8c62-1897-4d40-9b8b-804785886102-052.jpg?height=1067&width=1299&top_left_y=442&top_left_x=360)
...
| IPv4 | <-- How Outer Hdr Relates to Inner Hdr --> |
| :--- | :--- |
| Header fields: | -------------------- |
| version | 4 (1) |
| header length | constructed |
| DS Field | copied from inner hdr (5) |
| ECN Field | copied from inner hdr |
| total length | constructed |
| ID | constructed |
| flags (DF,MF) | constructed, DF (4) |
| fragment offset | constructed |
| TTL | constructed (2) |
| protocol | AH, ESP |
...
(9) An implementation MAY choose to provide a facility to pass the DS value from the outer header to the inner header, on a perSA basis, for received tunnel mode packets. The motivation for providing this feature is to accommodate situations in which the DS code space at the receiver is different from that of the sender and the receiver has no way of knowing how to translate from the sender's space. There is a danger in copying this value from the outer header to the inner header, since it enables an attacker to modify the outer DSCP value in a fashion that may adversely affect other traffic at the receiver. Hence the default behavior for IPsec implementations is NOT to permit such copying.
...
![](https://cdn.mathpix.com/cropped/8c8e8c62-1897-4d40-9b8b-804785886102-060.jpg?height=1340&width=1444&top_left_y=592&top_left_x=320)
...
A) Choose a selector in $\mathrm{Cj}, \mathrm{Sjn}$, that has not yet been chosen when traversing the tree from the root to this node. If there are no selectors not yet used, continue to the next unfinished branch until all branches have been completed. When the tree is completed, go to step D.
...
4) Get next $\mathrm{Cj}^{\mathrm{j}}$ and go to 2 .
...
dod(6) internet(1) security(5) mechanisms(5) pkix(7) id-mod(0) id-pkix1-explicit(18) \} ;
...
![](https://cdn.mathpix.com/cropped/8c8e8c62-1897-4d40-9b8b-804785886102-096.jpg?height=181&width=467&top_left_y=829&top_left_x=459)
