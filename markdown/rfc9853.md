...
This document specifies a Return Routability Check (RRC) subprotocol for use in the context of
the Connection ID (CID) construct for the Datagram Transport Layer Security (DTLS) protocol
versions 1.2 and 1.3.

Implementations offering the CID functionality described in RFCs 9146 and 9147 are encouraged
to also provide the RRC functionality described in this document. For this reason, this document
updates RFCs 9146 and 9147.
...
mechanism has been specified in
...
[RFC9146]
. The presentation language used in this document is
described in
...
this extension is empty. A client offering the rrc extension
extension
connection_id extension, the server
client offering the connection_id extension
application using DTLS has its own address validation mechanism. The client and server
NOT

. If the client includes the rrc extension in its ClientHello but omits the

 use RRC unless both sides have successfully exchanged rrc extensions.

 include the rrc extension in its ServerHello. A

 also offer the rrc extension, unless the

 also offer the connection_id
...
"peer address update" procedure described in
[RFC9146]
...
currently associated with that CID value, the receiver
 perform a return routability
...
The return_routability_check message
currently active security context.

MUST

 be authenticated and encrypted using the
...
Future extensions to the RRC subprotocol may define new message types. Implementations
be able to parse and understand the three RRC message types defined in this document. In
addition, implementations
 be able to parse and gracefully ignore messages with an
unknown msg_type.
...
A receiver that observes the peer's address change
...
initiates the return routability check.
 ...
 stop sending any buffered application
...
MUST
 ...
 continue using the previously valid address, i.e., no switch to the new path
...
◦
 perform a basic return routability check on the observed new address, as described
MUST
in Section 5.1.

5.

If T expires, the peer address binding is not updated. In this case, the initiator
a basic return routability check on the observed new address, as described in Section 5.1.
...
The initiator
path_challenge to account for packet loss on the probed path.

 send multiple return_routability_check messages of type
...
◦
The transmission of subsequent path_challenge messages
the chance of loss.

 go into different transport packets. (Note that the DTLS

 be paced to decrease
...
The initiator
when CID is enabled on the sending direction) to add padding up to the anti-amplification
limit to probe if the Path MTU (PMTU) for the new path is still acceptable.

 use the record padding mechanism available in DTLS 1.3 (and in DTLS 1.2,
...
MUST NOT
 ...
 delay sending an elicited path_response or path_drop messages.
...
MUST
 ...
 send exactly one path_response or path_drop message for each valid
...
both directions.
 ...
 send the path_response or the path_drop to the address from which
...
MUST
 ...
 silently discard any invalid path_response or path_drop it receives.

Note that RRC does not account for PMTU discovery on the reverse path. If the responder wants
to do PMTU discovery using RRC, it should initiate a new path validation procedure.
...
Once a connection has been established, the client and the server exchange application payloads
...
unsuccessful RRC operations, as they could represent security-relevant events when they
...
would hinder the RRC functionality altogether. Therefore, when RRC is used in DTLS 1.2 and
...
not implement anti-replay protection (see
[RFC6347]
...
Figure 4). The following descriptions of these attackers are based on those introduced in QUIC
...
InspectunencryptedportionsInjectoff-pathReorderModifyunauthenticatedportionson-pathDelayDropManipulatethepacketizationlayerRFC 9853
...
newoldpathpathReceiver1Attacker?SenderRFC 9853
...
newoldpathpathReceiver........path-3.1path-response.challenge.NATXtimeout.2path-.challenge...Sender.....'RFC 9853
...
Figure 8, the receiver sends a path_challenge (1) on the old path, and the sender replies with a
...
newoldpathpathReceiverpath-4path-1responsechallengeNATANATB3path-2path-challengedropSenderRFC 9853
...
Note that this defense is imperfect, but this is not considered a serious problem. If the path via
...
SHOULD
 ...
 avoid using the same CID on multiple network paths, in

When using DTLS 1.3, peers
...
newoldpathpathReceiver1path-challengeoff-pathNATattackerpath-2responseSenderRFC 9853

DTLS Return Routability Check

March 2026

Note that DTLS 1.2 does not offer the ability to request new CIDs during the session lifetime since
...
the correlation privacy threat is a concern.
 ...
 refuse to use CIDs with DTLS 1.2 and switch to DTLS 1.3 if
...
. This registration procedure is "Expert
...
semantics for this field is defined in
[RFC9847]
.

Section 5 of [RFC8447]
...
, assignments made in the Private Use space are not
...
For widespread experiments, provisional registrations (
) are
available.
...
IANA "Transport Layer Security (TLS) Parameters" <https://
...
Identifier for DTLS 1.2" RFC 9146 DOI 10.17487/RFC9146
,
...
Preuß Mattsson, J. Selander, G.

C. Amsüss "Amplification Attacks Using
the Constrained Application Protocol (CoAP)" Work in Progress Internet-Draft,
draft-irtf-t2trg-amplification-attacks-05
datatracker.ietf.org/doc/html/draft-irtf-t2trg-amplification-attacks-05>
...
Tschofenig, H. Fossati, T. Richardson, M.
Profiles for the Internet of Things" Work in Progress Internet-Draft, draft-ietf-
uta-tls13-iot-profile-19
<https://datatracker.ietf.org/doc/html/
draft-ietf-uta-tls13-iot-profile-19>

D. Migault "TLS/DTLS 1.3
...
Becker Hanno Böck Joe Clarke Manuel Pégourié-Gonnard Marco Tiloca Martin Thomson Mike
,
,
Bishop Mike Ounsworth Mohamed Boucadair Mohit Sahni Rich Salz Russ Housley Sean
Turner
, and

Colin Perkins Deb Cooley Eric Rescorla Éric Vyncke Erik Kline Hanno
,
