...
[EVC]
 ...
 standard, which is formally designated as ISO/IEC International

, was published in 2020. One of MPEG's goals is to keep EVC's Baseline

The Essential Video Coding
Standard 23094-1
profile essentially royalty-free by using technologies published more than 20 years ago or
otherwise known to be available for use without a requirement for paying royalties, whereas
more advanced profiles follow a reasonable and non-discriminatory licensing terms policy. Both
the Baseline profile and higher profiles of EVC
[HEVC]
gains over High Efficiency Video Coding
configurations.

[EVC]
 and Advanced Video Coding

 are reported to provide coding efficiency
...
unit-based RTP payload formats of H.264 Video Coding
[RFC6190]
[RFC9328]
implementation complexity, it has similar properties to those earlier payload format
specifications. This is a conscious choice, as at least the RTP Payload Format for H.264 video as
...
described in
communities. Certain mechanisms described in
temporal scalability. EVC currently does not offer higher forms of scalability.

[RFC6190]

 is widely deployed and generally known in the relevant implementer

 were incorporated, as EVC supports
...
[EVC] [AVC] [HEVC]
 ...
 share a similar hybrid video codec

The codings described in
, and
design. In this document, we provide a very brief overview of those features of EVC that are, in
some form, addressed by the payload format specified herein. Implementers have to read,
understand, and apply the ISO/IEC standard pertaining to EVC
 to arrive at interoperable,
well-performing implementations. The EVC standard has a Baseline profile and a Main profile,
the latter being a superset of the Baseline profile but including more advanced features. EVC also
includes still image variants of both Baseline and Main profiles, in each of which the bitstream is
restricted to a single IDR picture. EVC facilitates certain walled garden implementations under
commercial constraints imposed by intellectual property rights by including syntax elements
that allow encoders to mark a bitstream as to what of the many independent coding tools are
exercised in the bitstream, in a spirit similar to the general_constraint_info of
...
HEVC-like quad-tree-blocks partitioning that allows a block to be divided horizontally and
vertically into four smaller square blocks. The Main profile adds two advanced coding
...
Improved Quantization and Transform (IQT) uses a different mapping or clipping function for
...
EVC uses a similar binary arithmetic coding mechanism as HEVC CABAC (context adaptive
binary arithmetic coding) and VVC. The mechanism includes a binarization step and a
probability update defined by a lookup table. In the Main profile, the derivation process of
syntax elements based on adjacent blocks makes the context modeling and initialization
process more efficient.

In-loop filtering

[VIDEO-
. In the Main profile, an Advanced Deblocking Filter (ADDB) can be used as an

The Baseline profile of EVC uses the deblocking filter defined in H.263 Annex J
CODING]
alternative, which can further reduce undesirable compression artifacts. The Main profile
also defines two additional in-loop filters that can be used to improve the quality of decoded
pictures before output and/or for Inter prediction. A Hadamard Transform Domain Filter
(HTDF) is applied to the luma samples before deblocking, and a lookup table is used to
determine four adjacent samples for filtering. An adaptive Loop Filter (ALF) allows signals of
up to 25 different filters to be sent for the luma components; the best filter can be selected
through the classification process for each 4x4 block. Similarly to VVC, the filter parameters of
ALF are signaled in the Adaptation Parameter Set (APS).
...
(without switching Multipoint Control Units (MCUs) involved), a CVS can be as long in
...
profiles are defined in Annex A of
...
. Formally, they are defined as a set of constraints that
...
syntax elements. These syntax elements, toolset_idc_h and toolset_idc_l (located in the SPS),
are bitmasks that allow encoders to indicate which coding tools they are using within the
menu of profiles offered by the profile that is also signaled. No decoder conformance point is
...
EVC supports random access mechanisms based on IDR and clean random access (CRA) access
units.
...
substantially all reference picture list manipulations available in HEVC are specified,
including explicit transmissions or updates of reference picture lists. Although for reference
...
defined as any rectangular region within a picture. They can be encoded to have coding
dependencies with other slices from the previous picture but not with other slices in the same
picture. No specific support for parallel processing is specified in this RTP payload format.

1.1.4.  NAL Unit Header

EVC maintains the NAL unit concept of
...
excluding the NAL unit header.
 ...
 with different parameter options. EVC also uses a
...
. In this document, the value 1 may be used
...
refer to Section 7.4.2.2 of

. Note that nal_unit_type_plus1

. If the value of NalUnitType is less than or
...
to a profile specified in Annex A of
 shall ignore (i.e., remove from the bitstream and
...
[EVC]
 ...
 for convenience. Section 3.1.2 provides definitions specific to this document.
...
for the hypothetical reference decoder in Annex C of the
...
of
 ...
 and follows the order of NAL units in the bitstream.
...
. Within the scope of this document, one RTP stream is utilized to transport an
...
This payload format uses the fields of the header in a manner consistent with that specification.
 ...
 (included as Figure 2 for convenience).
...
NAL units), the RTP timestamp
 be set to the RTP timestamp of the coded picture of the
...
messages as specified in
 ...
 use the RTP timestamp for the display process, even
...
header) of the contained NAL unit, as shown in Figure 3.
 ...
 (denoted as PayloadHdr), followed by a conditional 16-bit DONL field
...
An AP consists of a payload header, as defined in Table 4 of
...
MUST
 ...
 be equal to 0 for this specification.

MUST

 be the smallest value of TID of all the aggregated NAL units. The value of
...
SHOULD
 ...
 be chosen so that the resulting IP packet is smaller than the path MTU size so to
...
derived as equal to the value of the DONL field, and the variable Decoding Order Number (DON)
for an aggregation unit that is not the first aggregation unit in an AP-aggregated NAL unit is
...
derived as equal to the DON of the preceding aggregated NAL unit in the same AP plus 1 modulo
65536. Otherwise (where sprop-max-don-diff is equal to 0), the DONL field
 be present
in an aggregation unit that is the first aggregation unit in an AP.
...
Figure 8 presents an example of an AP that contains two aggregation units, labeled "NALU 1" and
...
integer number of consecutive octets of that NAL unit. Fragments of the same NAL unit
MUST
...
subset of another FU.
 ...
 be nested; i.e., an FU must not contain a
...
An FU consists of a payload header as defined in Table 4 of
...
MUST
 ...
 be equal to the fields F, TID, Reserve, and E, respectively, of the

MUST

 be equal to 57. The fields
...
MUST NOT
 ...
 be transmitted in one FU; i.e., the S-bit and E-bit
...
A receiver in an endpoint or a MANE
...
Informative note: There are multiple reasons to allow the absolute difference of the
...
RTP stream
 be different from the NAL unit decoding order. Otherwise (where sprop-
...
SHOULD
 ...
 be encapsulated in an AP together with one or more other

A NAL unit of small size
...
MUST
 ...
 be used for carrying precisely one NAL unit in an RTP packet.
...
to allow for proper inter-stream synchronization must be considered.
...
MUST
 ...
 ignore any parameter unspecified in this document.
...
When profile-id is not present, a value of 0 (i.e., the Baseline profile)
...
EVC bitstreams transported over RTP using the technologies of this document
...
approximately standard-definition television (SD TV) resolution and frame rates; see Annex A
of
...
When used to indicate properties of a bitstream, level-id
in the SPS.

MUST

 be derived from the level_idc
...
This parameter is a base64-encoding representation (
...
parameter set.
 ...
 be derived from toolset_idh_h and toolset_idc_l in the sequence
...
separated (',') list of base64-encoding representations (
) of the sequence
...
Informative note: The value of depack-buf-cap indicates the maximum possible
size of the de-packetization buffer of the receiver only, without allowing for
network jitter. When network jitter occurs, an appropriately sized jitter buffer
...
OPTIONAL
 ...
 parameters profile-id, level-id, toolset-id, max-recv-level-id, sprop-max-don-

The
diff, sprop-depack-buf-bytes, and depack-buf-cap, when present,
"a=fmtp" line of SDP. The "a=fmtp" line is expressed as a media type string, in the form of a
...
. For a particular media format (i.e., RTP payload type),

 parameters sprop-sps, sprop-pps, and sprop-sei, when present,

The
MUST
included in the "a=fmtp" line of SDP or conveyed using the "fmtp" source attribute as
specified in
 be both included in the "a=fmtp" line of SDP and
sprop-sps, sprop-pps, or sprop-sei
...
type, the parameters sprop-sps, sprop-pps, and sprop-sei
 be applied to each SSRC with
...
The above represents an offer for symmetric video communication using
 and its payload
specification at the main profile and level 3. Informally speaking, this offer tells the receiver of
...
. At the same time, if this offer were accepted "as is", the offer can expect that
...
incapable of handling evc at level 3 but is capable of decoding level 2. As EVC video codecs must
...
This section applies to all profiles defined in
associated still image profiles.

[EVC]

, specifically to Baseline, Main, and the
...
MUST
 ...
 structure its answer according to one of the following two options:
...
from that in the offer with a different value of level-id.
 ...
 contain a payload type number used in the offer for the media subtype
...
Answerer is able to receive. When dealing with EVC, the Offerer assumes that the Answerer
...
The parameters sprop-sps or sprop-pps, when present (included in the "a=fmtp" line of SDP
or conveyed using the "fmtp" source attribute, as specified in
), are
used for out-of-band transport of the parameter sets (SPS or PPS, respectively). The Answerer
 use either out-of-band or in-band transport of parameter sets for the bitstream it is
MAY
sending, regardless of whether out-of-band parameter sets transport has been used in the
Offerer-to-Answerer direction. Parameter sets included in an answer are independent of
those parameter sets included in the offer, as they are used for decoding two different
bitstreams: one from the Answerer to the Offerer, and the other in the opposite direction. In
case some RTP packets are sent before the SDP Offer/Answer settles down, in-band
parameter sets

 be used for those RTP stream parts sent before the SDP Offer/Answer.
...
The following rules apply to transport of parameter sets in the Offerer-to-Answerer
direction.
...
MUST
 ...
 include sprop-sps and/or sprop-pps. If none of these parameters are present

◦
An offer
in the offer, then only in-band transport of parameter sets is used.
◦
If the level to use in the Offerer-to-Answerer direction is equal to the default level in the
offer, the Answerer
 be prepared to use the parameter sets included in sprop-sps and
sprop-pps (either included in the "a=fmtp" line of SDP or conveyed using the "fmtp" source
attribute) for decoding the incoming bitstream, e.g., by passing these parameter set NAL
units to the video decoder before passing any NAL units carried in the RTP streams.
Otherwise, the Answerer
 ignore sprop-vps, sprop-sps, and sprop-pps (either included
in the "a=fmtp" line of SDP or conveyed using the "fmtp" source attribute), and the Offerer
MUST
...
The following rules apply to transport of parameter sets in the Answerer-to-Offerer
direction.

MAY

 include sprop-sps and/or sprop-pps. If none of these parameters are

◦
An answer
present in the answer, then only in-band transport of parameter sets is used.
◦
The Offerer
pps (either included in the "a=fmtp" line of SDP or conveyed using the "fmtp" source
attribute) for decoding the incoming bitstream, e.g., by passing these parameter set NAL
units to the video decoder before passing any NAL units carried in the RTP streams.

 be prepared to use the parameter sets included in sprop-sps and sprop-
...
When sprop-sps and/or sprop-pps are conveyed using the "fmtp" source attribute, as
specified in
parameter sets included in sprop-sps and/or sprop-pps and associate them with the source
given as part of the "fmtp" source attribute. Parameter sets associated with one source (given
 only be used to decode NAL units conveyed in
as part of the "fmtp" source attribute)
RTP packets from the same source (given as part of the "fmtp" source attribute). When this
mechanism is in use, SSRC collision detection and resolution
in
...
one or more of the optional parameters to the point where the implementation can cope. This
...
configuration parameters, including level-id,
Answerer
(payload type) completely. Note that this implies that the level-id for Offer/Answer in
multicast is not changeable.

 either maintain all configuration parameters or remove the media format

 be used symmetrically; that is, the
...
id and level-id declare the values used by the bitstream, not the capabilities for receiving
bitstreams. As a result, the following interpretation of the parameters
...
The following subsections define the use of the Picture Loss Indication (PLI)
Intra Request (FIR)
...
In accordance with this document, a sender
Reference Picture Selection Indication (RPSI); and a receiver
received SLI as a received PLI, ignoring the "First", "Number", and "PictureID" fields of the PLI.

 send Slice Loss Indication (SLI) or
MUST
...
all cases, the timing and congestion-control mechanisms of
 ...
 be to send an IDR picture and relevant parameter sets,
...
) and the security of the call-control stack chosen (that
...
content.
 ...
 includes a user data Supplemental Enhancement
...
[EVC]
 ...
 leaves the handling of this SEI message to the receiving system. In order to avoid
...
not less than all RTP streams combined. This condition can be satisfied by implementing
congestion-control mechanisms to adapt the transmission rate by implementing the number of
layers subscribed for a layered multicast session or by arranging for a receiver to leave the
...
pre-coded bitstream to be tailored for such adaptivity.

The key mechanism available in
units belonging to higher temporal sub-layers (i.e., those NAL units with a large value of TID)
until the sending bitrate drops to an acceptable range.

 is temporal scalability. A media sender can remove NAL
...
the profile) are required to be changed does it become necessary to terminate and restart the
RTP streams. This may be accomplished by using different RTP payload types.
...
NAL unit have been lost, because the trailing FUs are meaningless to most decoders. MANE can
also remove higher temporal scalable layers if the outbound transmission (from the MANE's
...
"Information technology -- General video coding -- Part 1: Essential video
coding" ISO/IEC 23094-1:2020
, October 2020,
57797.html>
...
Zhao, S. Wenger, S. Sanchez, Y. Wang, Y.-K.
Payload Format for Versatile Video Coding (VVC)" RFC 9328 DOI 10.17487/
RFC9328
...
,
ITU-T "Part 10: Advanced video coding" ITU-T Recommendation H.264
...
IS0/IEC "Information technology - Generic coding of moving pictures and
associated audio information - Part 1: Systems" ISO/IEC 13818-1:2013
...
ITU-T "Video coding for low bit rate communication" ITU-T
...
Large parts of this specification share text with the RTP payload format for VVC
Roman Chernyak
specification for their excellent work.

 is thanked for his valuable review comments. We thank the authors of that
