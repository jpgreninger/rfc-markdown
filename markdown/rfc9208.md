...
This document deﬁnes a QUOTA extension of the Internet Message Access Protocol (IMAP) (see
...
This document deﬁnes a couple of extensions to the Internet Message Access Protocol
[RFC9051]
extension is compatible with both IMAP4rev1

[RFC3501]
 for querying and manipulating administrative limits on resource usage (quotas). This
...
codes deﬁned in this document are not present in such servers (see Section 10 for more details),
and clients
"QUOTA=".

 rely on their presence in the absence of any capability beginning with

. Some responses and response

[RFC2087]

MUST NOT

Any server compliant with this document
the "QUOTA=RES-" preﬁx, as described in Section 3.1.

MUST

 also return at least one capability starting with

Any server compliant with this document that implements the SETQUOTA command (see Section
4.1.3)

 also return the "QUOTASET" capability.
...
This document also reserves all other capabilities starting with the "QUOTA=" preﬁx for future
...
Although the IMAP4 QUOTA extension speciﬁed in
...
Section 3.2) without a priori knowledge of the implementation.
 ...
 has seen deployment in servers, it

[RFC2087]

2.  Document Conventions

In protocol examples, this document uses a preﬁx of "C: " to denote lines sent by the client to the
server and "S: " for lines sent by the server to the client. Lines preﬁxed with "//" are comments
explaining the previous protocol line. These preﬁxes and comments are not part of the protocol.
Lines without any of these preﬁxes are continuations of the previous line, and no line break is
present in the protocol before such lines unless speciﬁcally mentioned.
...
Other capitalized words are IMAP keywords
document.

[RFC3501] [RFC9051]

 or keywords from this

3.  Terms

3.1.  Resource

A resource has a name, a formal deﬁnition.

3.1.1.  Name

The resource name is an atom, as deﬁned in
IANA.
...
with "QUOTA=RES-". A server compliant with this speciﬁcation is not required to support all
reported resource types on all quota roots.

 be advertised as a capability by prepending the resource name

MUST

3.1.2.  Deﬁnition

The resource deﬁnition or document containing it, while not visible through the protocol,
...
MUST
 ...
 be represented as a 63-bit unsigned integer. 0 indicates that the
The usage of a resource
resource is exhausted. Usage integers don't necessarily represent proportional use, so clients
 compare an available resource between two separate quota roots on the same or
MUST NOT
diﬀerent servers.

Limits will be speciﬁed as, and
prohibited.

MUST

 be represented as, an integer. 0 indicates that any usage is

Limits may be hard or soft; that is, an implementation
any command if the limit on a resource is or would be exceeded.

MAY

 choose, or be conﬁgured, to disallow

All resources that the server handles
consisting of the resource name preﬁxed by "QUOTA=RES-".

MUST

 be advertised in a CAPABILITY response/response code

The resources
,
ANNOTATION-STORAGE (Section 5.4)
 are deﬁned in this document.

STORAGE (Section 5.1) MESSAGE (Section 5.2) MAILBOX (Section 5.3)
...
Each mailbox has zero or more implementation-deﬁned named "quota roots". Each quota root
...
Quota root names need not be mailbox names, nor is there any relationship deﬁned by this
document between a quota root name and a mailbox name. A quota root name is an astring, as
 be treated as an opaque string by any clients.
deﬁned in
...
implementation of this memo
QUOTA (Section 4.2.1)
for a particular quota root. A
an implementation-dependent way, if the granularity of the underlying system demands it. A
client

 advise the client of such inherent limits, by generating
 advise the client of which resources are limitable

SHOULD
SETQUOTA (Section 4.1.3)

 command to fail if a limit cannot be set.

 also round a quota limit in
...
Implementation Notes: This means that, for example, under UNIX, a quota root may have a
MESSAGE (Section 5.2)
similarly,
ﬁlesystem space.

 quota always set due to the number of inodes available on the ﬁlesystem;
 may be rounded to the nearest block and limited by free
...
usage and limits in an untagged QUOTA response. (Names of quota roots applicable to a
particular mailbox can be discovered by issuing the GETQUOTAROOT command; see Section
4.1.2.) Note that the server is not required to support any speciﬁc resource type (as advertised in
the CAPABILITY response, i.e., all capability items with the "QUOTA=RES-" preﬁx) for any
...
REQUIRED
 ...
 untagged responses: QUOTAROOT, QUOTA
...
The GETQUOTAROOT command takes a mailbox name and returns the list of quota roots for the
...
Note that unlike other command/responses/response codes deﬁned in this document, support for
...
The resource limits for the named quota root are changed to the speciﬁed limits. Any previous
...
change the quota roots for any number of existing mailboxes in an implementation-deﬁned
manner.
...
If the implementation chooses to change the quota roots for some existing mailboxes, such
changes

 be announced with untagged QUOTA responses.
...
\Deleted ﬂag set. The DELETED status data item is only required to be implemented when the
...
space that can be reclaimed by performing EXPUNGE on the mailbox. The server
 return
the exact value; however, it is recognized that the server may have to do a non-trivial amount of
work to calculate it. If the calculation of the exact value would take a long time, the server
instead return the sum of the RFC822.SIZE of the messages with the \Deleted ﬂag set. The
...
ﬁrst string is the name of the quota root for which this quota applies.
...
This response occurs as a result of a GETQUOTAROOT command. The ﬁrst string is the mailbox
...
quota limits.
 ...
 be returned in the tagged NO response to an APPEND/
...
The OVERQUOTA response code
 also be returned in an untagged NO response in the
...
speciﬁed in COPY/MOVE/APPEND) or to the currently selected mailbox. (The EXTRA WG chose not
to address this deﬁciency due to syntactic limitations of IMAP response codes and because such
events are likely to be rare.) This form of the OVERQUOTA response codes
 be returned
if there is no mailbox selected and no command in progress that adds a message to a mailbox
(e.g., APPEND).

 delivery or COPY/MOVE/APPEND in another IMAP
...
The following resource types are deﬁned in this memo. A server supporting a resource type
advertise this as a CAPABILITY with a name consisting of the resource name preﬁxed by
"QUOTA=RES-". A server
types it supports.

 support multiple resource types and

 advertise all resource
...
MAY
 ...
 not be the same as the sum of the RFC822.SIZE of the messages. Some

"STORAGE" is the physical space estimate, in units of 1024 octets, of the mailboxes governed by
the quota root. This
 include metadata sizes for the messages and mailboxes, and other
implementations
implementations
 store messages in such a way that the physical space used is smaller, for
example, due to use of compression. Additional messages might not increase the usage. Clients
MUST NOT

 use the usage ﬁgure for anything other than informational purposes; for example, they
...
MUST NOT
divided by 1024 octets of the message, but it

MAY

 warn about such condition.

The usage ﬁgure may change as a result of performing actions not associated with adding new
messages to the mailbox, such as SEARCH, since this may increase the amount of metadata
included in the calculations.

When the server supports this resource type, it
data item.

MUST

 also support the DELETED-STORAGE status
...
MUST
 ...
 be indicated by the server by advertising the "QUOTA=RES-

A resource named the same was also given as an example in
more precise deﬁnition.
...
mailboxes the client has no access to.
 ...
 be an exact number; however, clients
...
MUST
 ...
 also support the DELETED status data item.
...
4314 and this document are implemented.
 ...
 rights required to execute quota-related commands when both RFC
...
Section 4 of [RFC4314]

 for conventions used in this table.
...
Non-terminals referenced but not deﬁned below are as deﬁned by
...
lowercase characters to deﬁne token strings is for editorial clarity only. Implementations
...
conﬁdential information and should not be divulged to unauthorized persons. In particular, no
...
mailboxes), a user that can consume or render unusable the resource can aﬀect the resources
available to the other users; this might occur, for example, by a user with permission to execute
the SETQUOTA setting, which sets an artiﬁcially small value.
...
added the "QUOTA=" preﬁx and the "QUOTASET" capability to the "IMAP Capabilities" registry with
this document as the reference.

IANA has added the following notes to the "IMAP Capabilities" registry:

The preﬁx "QUOTA=RES-" is reserved per RFC 9208, Section 9.1. See Section 9.2 of that document for
values that follow this preﬁx.

All other capabilities starting with the "QUOTA=" preﬁx are reserved for future IETF Stream
extensions to RFC 9208.

9.2.  IMAP Quota Resource Type Registry

IANA has created a new registry for IMAP quota resource types. The registration policy for the
"IMAP Quota Resource Types" registry is "Speciﬁcation Required"
...
a reference to a speciﬁcation that describes the quota resource type in more detail

Designated experts should check that the provided references are correct, the references describe
the quota resource type being registered in suﬃcient detail to be implementable, the syntax of
any optional commands/responses is correct (e.g., ABNF validates), and the syntax/description
complies with rules and limitations imposed by IMAP
. Designated experts
should avoid registering multiple identical quota resource types under diﬀerent names and
...
The number of mailboxes governed by the quota
root.
...
, and it aims to clarify the meaning of diﬀerent terms
that were used in that RFC. It also provides more examples, gives guidance on allowed server
behavior, deﬁnes an IANA registry for quota resource types, and provides initial registrations for
4 of them.
...
, this document deﬁnes two more commonly used resource types,

When compared with
adds an optional OVERQUOTA response code, and deﬁnes two extra STATUS data items
...
Daboo, C.
 and
Extension" RFC 5257 DOI 10.17487/RFC5257
,
,
editor.org/info/rfc5257>
.

R. Gellens "Internet Message Access Protocol - ANNOTATE
<https://www.rfc-
...
The editor of this document would like to thank the following people who provided useful
comments or participated in discussions that lead to this update of
Daboo Lyndon Nerenberg Benjamin Kaduk Roman Danyliw
, and

[RFC2087] John Myers Cyrus
:
Éric Vyncke
.
...
, and it borrows a lot of text from that RFC. Thus, the
...
document.
 ...
 wrote a lot of text in an earlier draft version that became the basis for this
