...
Active multihoming redundancy modes. This document builds on the existing redundancy
...
[RFC7432]
 ...
 defines the All-Active and Single-Active redundancy modes. All-Active

EVPN
redundancy provides per-flow load balancing for multihoming, while Single-Active redundancy
ensures service carving where only one of the Provider Edge (PE) devices in a redundancy
relationship is active per service.

Although these two multihoming scenarios are widely utilized in data center and service
provider access networks, there are cases where active/standby multihoming at the interface
...
flow load balancing across multiple PEs providing multihoming. This determinism is essential
...
This document defines the Port-Active redundancy mode as a new type of multihoming in EVPN
...
When a Customer Equipment (CE) device is multihomed to a set of PE nodes using the Link
[IEEE_802.1AX_2014]
Aggregation Control Protocol (LACP)
, the PEs must function as a single
LACP entity for the Ethernet links to form and operate as a Link Aggregation Group (LAG). To
achieve this, the PEs connected to the same multihomed CE must synchronize LACP
...
, covers the scenario where a CE is multihomed to multiple PE nodes, using a LAG to

 has been used for this synchronization. EVPN, as described in

[RFC7275]

•

A CE device connected to EVPN multihoming PEs
connected to the EVPN multihoming PEs in a redundancy group.

MUST

 have a single LAG with all its links

•

Identical LACP parameters
priority, and port key.

MUST

 be configured on peering PEs, including the system ID, port
...
Figure 1 shows an MC-LAG multihoming topology where PE1 and PE2 are part of the same
redundancy group providing multihoming to CE1 via interfaces I1 and I2. Interfaces I1 and I2
are members of a LAG running LACP. The core, shown as IP or MPLS enabled, provides a wide
range of L2 and L3 services. MC-LAG multihoming functionality is decoupled from those services
in the core, and it focuses on providing multihoming to the CE. In Port-Active redundancy mode,
only one of the two interfaces, I1 or I2, would be in forwarding, and the other interface would be
in standby. This also implies that all services on the active interface operate in active mode and
all services on the standby interface operate in standby mode.
...
It offers open-standards-based active/standby redundancy at the interface level rather than
VLAN granularity
...
It eliminates the need for ICCP and LDP
Network (VXLAN)
the network).
...
[RFC5036]
 ...
 (e.g., Virtual eXtensible Local Area

 or Segment Routing over IPv6 (SRv6)
...
This mode is agnostic of the underlying technology (MPLS, VXLAN, and SRv6) and associated
services (Layer 2 (L2), Layer 3 (L3), Bridging, E-LINE, etc.)

d.

It enables deterministic QoS over MC-LAG attachment circuits.
...
[RFC7432]
 ...
 and does not require any new protocol enhancements to

f.

It can leverage various Designated Forwarder (DF) election algorithms, such as modulo
[RFC7432]
...
It replaces legacy MC-LAG ICCP-based solutions and offers the following additional benefits:

◦
Efficient support for 1+N redundancy mode (with EVPN using BGP Route Reflector),
...
. The ESI can be auto-derived or manually assigned, and the access interface

 be assigned per access interface as described in
MAY
...
The Ethernet Segment (ES)
PEs for the specified access interface.

MUST

 be configured in Port-Active redundancy mode on peering
...
which PE keeps the port in active mode and which PE(s) keeps it in standby mode. Although
the DF election defined in
 is per [ES, Ethernet Tag] granularity, the DF election is
...
MUST
 ...
 keep the corresponding access interface in an up and forwarding active
...
Non-DF routers
comparable to the Single-Active redundancy mode described in
VLANs.

 implement a bidirectional blocking scheme for all traffic
...
MAY
 ...
 bring and keep the peering access interface attached to them in an
...
of Sync (OOS) instead of setting the interface to a down state. This approach allows for
better convergence during the transition from standby to active mode.

 set the LACP state to Out

MAY

g.

The primary/backup bits of the EVPN Layer 2 Attributes (L2-Attr) Extended Community
[RFC8214] SHOULD

 be used to achieve better convergence, as described in Section 4.1.

3.  Designated Forwarder Algorithm to Elect per Port-Active PE

The ES routes operating in Port-Active redundancy mode are advertised with the new Port Mode
Load-Balancing capability bit in the DF Election Extended Community as defined in
Additionally, the ES associated with the port utilizes the existing Single-Active procedure and
signals the Single-Active multihomed site redundancy mode along with the Ethernet A-D per-ES
...
). Finally, The ESI label-based split-horizon procedures

Section 8.3 of [RFC7432] SHOULD

 be employed to prevent transient echo packets
...
Election Capabilities" to use with the DF election algorithm in the DF algorithm field:
 ...
 defines a DF Election Extended Community and a bitmap (2 octets) field to encode "DF
...
Port Mode Designated Forwarder Election. This bit determines that the DF election
algorithm

 be modified to consider the port ES only and not the Ethernet Tags.
...
The default DF election algorithm, or modulo-based algorithm, as described in
updated by
, is applied here at the granularity of ES only. Given that the ES-Import
...
An application of Highest Random Weight (HRW) to EVPN DF election is defined in
and it
...
Section 3.2 of [RFC8584]
 describes computing a 32-bit Cyclic Redundancy Check (CRC) over the
...
The algorithm used to determine the DF and Backup Designated Forwarder (BDF) per
of [RFC8584]

 is repeated and summarized below using only (Es) in the computation:

Section 3.2

1.

DF(Es) = Si| Weight(Es, Si) >= Weight(Es, Sj), for all j. In the case of a tie, choose the PE whose
IP address is numerically the least. Note that 0 <= i,j < number of PEs in the redundancy
group.

2.

BDF(Es) = Sk| Weight(Es, Si) >= Weight(Es, Sk), and Weight(Es, Sk) >= Weight(Es, Sj). In the
...
DF(Es) is defined to be the address Si (index i) for which Weight(Es, Si) is the highest; 0 <= i <
N-1.
...
[RFC9785]
 is modified to consider the port only and not any associated Ethernet Tags. The Port
...
Ethernet A-D per EVI withdrawal does not influence the DF election.
 ...
 be set to 0 when advertising Port Mode Designated

[RFC8584] MUST

Upon receiving the AC-DF bit set (A=1) from a remote PE, it
Port Mode Designated Forwarder Election.

MUST

 be ignored when performing
...
The EVPN L2-Attr Extended Community defined in
...
When advertised, the L2-Attr Extended Community
the Control Flags field, and all other bits and fields
...
SHALL
 ...
 have only the P or B bits set in

•

A remote PE receiving the optional L2-Attr Extended Community in Ethernet A-D per ES
routes

 consider only the P and B bits and ignore other values.
...
P and B bits received
the Ethernet A-D per ES.

SHOULD

 be considered overridden by "parent" bits when advertised in
...
The remote ESI Label Extended Community
mode (Section 3).

[RFC7432]

 signals the Single-Active redundancy
...
multihoming capabilities. The services may include any L2 EVPN solutions such as EVPN Virtual
. Additionally, L3 services
Private Wire Service (VPWS) or standard EVPN as defined in
may be provided within a VPN context, as specified in
...
standby redundancy, as detailed in
...
. However, ICCP requires LDP to be enabled as a
...
Introducing a new capability necessitates unanimity among PEs. Without consensus on the new
...
may be subject to unfair load balancing, service disruption, and potential issues such as traffic
loss or duplicate traffic, as mentioned in the security sections of those documents.

.This fallback behavior could be exploited by an attacker
...
Aggregation" IEEE 802-1ax-2014 DOI 10.1109/IEEESTD.2014.7055197
2015,
...
Brissette, P. Sajassi, A. Burdet, LA., Ed. Drake, J.
for EVPN Designated Forwarder Election" RFC 9722 DOI 10.17487/RFC9722
2025,

J. Rabadan "Fast Recovery
, May
...
,
Based EVPN Designated Forwarder (DF) Election" RFC RFC9785 DOI 10.17487/
RFC9785
...
Martini, L. Salam, S. Sajassi, A. Bocci, M. Matsushima, S.
Chassis Communication Protocol for Layer 2 Virtual Private Network (L2VPN)
Provider Edge (PE) Redundancy" RFC 7275 DOI 10.17487/RFC7275
, June 2014,
,
...
Sajassi, A., Ed. Brissette, P. Uttaro, J. Drake, J. Boutros, S.
,
"EVPN Virtual Private Wire Service (VPWS) Flexible Cross-Connect (FXC)
Service" RFC 9744 DOI 10.17487/RFC9744
<https://www.rfc-
editor.org/info/rfc9744>
...
Anoop Ghanwani
 ...
 for his comments and suggestions and

Stephane Litkowski

Contributors

In addition to the authors listed on the front page, the following people have also contributed to
this document:
...
edward.leyton@verizonwireless.com
