...
Security Event Subject Identifiers
...
Section 1.2 of [RFC8417]
 ...
 ("Security Event Token (SET)"), subjects related to
[RFC7519]
...
To address this problem, this specification defines Subject Identifiers as JSON
...
Subject Identifiers are intended to be a general-purpose mechanism for identifying subjects
...
[RFC8259]
 ...
 whose contents may be used to identify a subject
...
conform to a specific Identifier Format and
name of that Identifier Format.

 contain a "format" member whose value is the
...
. Identifier Formats that are expected to be used broadly by a variety of parties
 ...
 have a unique name registered in the IANA "Security Event

MAY

 describe more members than are strictly necessary to identify a
...
Formats
 ...
 declare any rules regarding the "format" member.
...
contain any members prohibited or not described by its Identifier Format and
members required by its Identifier Format.

 match the rules specified for that member by
MUST NOT
...
controls that email address, the mailbox itself, or anything else that the transmitter and receiver
...
identified with an "acct" URI as defined in
agreement through which a user gets access to a service and gets a unique identity with the
service provider. Subject Identifiers in this format
the "acct" URI for the subject. The "uri" member is
...
MUST NOT
 ...
 contain a "uri" member whose value is
...
The Email Identifier Format identifies a subject using an email address. Subject Identifiers in this
 contain an "email" member whose value is a string containing the email address of
format
the subject, formatted as an "addr-spec" as defined in
member is
identify a mailbox to which email may be delivered, in accordance with
Identifier Format is identified by the name "email".

 be null or empty. The value of the "email" member
...
[RFC5322]
 ...
 is consistently treated as case-insensitive per
...
canonicalization method. Therefore, the recipient
 apply its own canonicalization
...
members, analogous to how subjects are identified using the JWT "iss" and "sub" Claims in
 ID Tokens. These members
OpenID Connect [OpenID.Core]
[RFC7519]
member and "sub" member defined by
"sub" member are
Format is identified by the name "iss_sub".

 follow the formats of the "iss"
, respectively. Both the "iss" member and the
 be null or empty. The Issuer and Subject Identifier
...
Identifiers in this format
the opaque string identifier for the subject. The "id" member is
or empty. The Opaque Identifier Format is identified by the name "opaque".

 contain an "id" member whose value is a JSON string containing
 be null
...
"phone_number".
 ...
 contain a "phone_number" member whose value is a string

. The "phone_number" member is
...
. Subject Identifiers in this format

The Decentralized Identifier (DID) Format identifies a subject using a DID URL as defined in
[DID]
for the DID Subject being identified. The value of the "url" member
MAY
Decentralized Identifier Format is identified by the name "did".

 contain a "url" member whose value is a DID URL
MUST

 be a bare DID. The "url" member is

 be null or empty. The

 be a valid DID URL and
...
Figure 9: Example: Subject Identifier for the Decentralized Identifier Format, Identifying a Subject
with a Bare DID
...
"uri".
 ...
 be null or empty. The URI Format is identified by the name
...
will recognize or support. Subject Identifiers in this format

 contain an "identifiers"
...
member whose value is a JSON array containing one or more Subject Identifiers. Each Subject
Identifier in the array
 be null or empty. It
MUST NOT
multiple Email Subject Identifiers) but
identified by the name "aliases".

 and
 contain multiple instances of the same Identifier Format (e.g.,

 identify the same entity. The "identifiers" member is

 contain exact duplicates. This format is
...
MUST NOT
 ...
 be nested, i.e., the "identifiers" member of an "aliases"
...
The JWT "sub" Claim is defined in
 as containing a string value;
therefore, it cannot contain a Subject Identifier (which is a JSON object) as its value. This
document defines the JWT "sub_id" Claim, in accordance with
common claim that identifies the JWT Subject using a Subject Identifier. When present, the value
of this claim
 be a Subject Identifier that identifies the subject of the JWT. The JWT "sub_id"
 be included in a JWT, whether or not the JWT "sub" Claim is present. When both the
Claim
JWT "sub" and "sub_id" Claims are present in a JWT, they
JWT has one and only one JWT Subject.

 identify the same subject, as a
...
rely on both claims to determine the JWT Subject. An implementation
...
understand the format of the first claim. For example, an implementation may attempt to use
...
MAY
 ...
 contain an "iss_sub" Subject Identifier. In this case, the JWT's "iss"

The JWT "sub_id" Claim
Claim and the Subject Identifier's "iss" member
Connect [OpenID.Core]
...
understood by both parties. Similarly, the JWT's "sub" Claim and the Subject Identifier's "sub"
 be different. For example, this may be used by an OpenID Connect client to
member
communicate the JWT Subject's local identifier at the client back to its Identity Provider.

 be different. For example, an
...
Figure 19: Example: JWT with an "iss_sub" Subject Identifier Where the JWT Issuer and JWT Subject
...
MUST NOT
 ...
 make assertions or declarations regarding the subject
...
SHOULD
 ...
 take care that they are only transmitting multiple identifiers together when it is known
...
that the recipient already knows that the identifiers are related (e.g., because they were
...
used within SETs. The considerations described in
Subject Identifiers are used within JWTs.

Section 6 of [RFC8417]

 also apply when Subject Identifiers are
...
a Subject Identifier. Where such properties are required, implementations
mechanisms provided by the containing format (e.g., integrity protecting SETs or JWTs using
JSON Web Signature (JWS)
stack (e.g., using TLS

) or at the transport layer or other layer in the application
...
The name of the Identifier Format, as described in Section 3. The name
...
A reference to the document or documents that define the Identifier Format. The reference
 specify the name, format, and meaning of each member that may occur
document(s)
...
fields would be used. URIs that can be used to retrieve copies of each document
included.
...
Sporny, M., Ed. Guy, A., Ed. Sabadello, M., Ed. Reed, D., Ed. Longley, D. Steele,
O.
www.w3.org/TR/did-core/>

C. Allen "Decentralized Identifiers (DIDs) v1.0"
...
Saint-Andre, P. "The 'acct' URI Scheme" RFC 7565 DOI 10.17487/RFC7565
2015,
...
M. Ansari "Security Event Token (SET)"
<https://www.rfc-editor.org/info/
...
The authors would like to thank the members of the IETF Security Events Working Group, as well
as those of the OpenID Shared Signals and Events Working Group, whose work provided the
original basis for this document. We would also like to acknowledge
Justin Richer Mike Jones
,

Aaron Parecki Denis Pinkas
,
,
, and other members of the working group for reviewing this document.
