...
[RFC6363]
 ...
 provides a framework to define Content Delivery Protocols (CDPs)
...
,
 ...
 only considers block FEC schemes defined in accordance with the FEC
). These codes require the
...
[RFC6363]
 ...
 in order to also support FEC codes based on a sliding encoding

This document updates
window (a.k.a., convolutional codes)
. This encoding window, either fixed or variable
...
(RLC).
 ...
 provides an example of such a FEC
...
This document leverages on
 and reuses its structure. It proposes new sections specific
...
3.  Summary of Architecture Overview
 ...
 equally applies to this FECFRAME extension and is not
The architecture of
...
The FECFRAME architecture is illustrated in Figure 1 for a block FEC scheme from the sender's
...
encoding purposes.
 ...
 requires a transport protocol providing an unreliable
...
Section 4.1 of [RFC6363]
 ...
 that are specific to block FEC codes are not
...
defining and interpreting it.
 ...
 contain information to identify
...
what FEC scheme designers need to consider.
 ...
 be detailed there. Appendix A provides non-normative hints about
...
The FEC Framework constructs FEC Repair Packets according to Figure 7 in
...
repeat the other steps of
(5) are:

 ("Receiver Operation"). The new steps (4) and
...
FEC schemes. The global formats of source data packets (i.e.,
[RFC6363]
packets (i.e.,
are not repeated here.

, Figures 7 and 8) remain the same with Sliding Window FEC codes. They

, Figure 6) and repair data
...
The FEC scheme requirements of
...
A full specification of the Sliding Window FEC code.
 ...
 precisely define the valid FEC-Scheme-Specific Information values,
This specification
...
source symbols), the specification
 uniquely define the values of the encoding symbol
...
Section 9 of [RFC6363]

This FECFRAME extension does not add any new security considerations. All the considerations
of
 apply to this document as well. However, for the sake of completeness,
the following goal can be added to the list provided in
Statement"):

Section 9.1 of [RFC6363]
...
Section 10 of [RFC6363]

11.  IANA Considerations

This document has no IANA actions.

A FEC scheme for use with this FEC Framework is identified via its FEC Encoding ID. It is subject
to IANA registration in the "FEC Framework (FECFRAME) FEC Encoding IDs" registry. All the
rules of

 apply and are not repeated here.

Section 11 of [RFC6363]
...
3GPP "Multimedia Broadcast/Multicast Service (MBMS); Protocols and codecs"
3GPP TS 26.346
.
...
Watson, M. Luby, M.
Block" RFC 5052 DOI 10.17487/RFC5052
editor.org/info/rfc5052>
...
L. Vicisano "Forward Error Correction (FEC) Building
...
Roca, V.
B. Teibi "Sliding Window Random Linear Code (RLC) Forward
...
The authors would like to thank Christer Holmberg, David Black, Gorry Fairhurst, Emmanuel
Lochin, Spencer Dawkins, Ben Campbell, Benjamin Kaduk, Eric Rescorla, Adam Roach, and Greg
Skinner for their valuable feedback on this document. This document being an extension of
[RFC6363]
, the authors would also like to thank Mark Watson as the main author of that RFC.
