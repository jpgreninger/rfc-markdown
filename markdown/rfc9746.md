...
RFC 9746
BGP EVPN Multihoming Extensions for Split-Horizon
Filtering
...
EVPN MH Split-Horizon Extensions
...
. SR-MPLS tunneling is specified in
...
, an ES may be multihomed (attached to
...
Multiprotocol Label Switching over Generic Network Encapsulation
...
In summary and as specified in
...
, each NVE tracks the IP address(es) of other NVEs
...
Similarly, certain IP tunnels (those that include an identifier for the source ES in the tunnel
...
possible. Also, Section 4.1 of
 defines an Ethernet option Type-Length-Value
...
filtering argument (Arg.FE2) (see
the service Segment Identifier (SID) received on an A-D per ES route from the egress PE.

, the ingress PE adds specific information to the SRv6 packet to enable

Section 6.1.1 of [RFC9252]
...
The ESI Label method is applicable for both All-Active and Single-Active configurations, whereas
...
will continue to adhere to the procedures specified in
...
connected to an ES advertise an A-D per ES route for that ES, including the extended community,
...
MUST NOT
 ...
 be used with encapsulations that support only one SHT

 be used by encapsulations that support the two SHTs in Table 1.
...
for a given EVI, all the NVEs
 revert to the behavior in
...
An SHT different than 00
ES route where the "Single-Active" and SHT bits are different than zero
[RFC7606]
.
...
MUST NOT
 ...
 be set if the "Single-Active" bit is set. A received A-D per
...
MUST
 ...
 have the same value in each Ethernet A-D per ES route that an NVE
...
As an example, egress NVEs that support IP-based MPLS tunnels, such as MPLSoGRE or
MPLSoUDP, will advertise A-D per ES routes for the ES along with the BGP Encapsulation
Extended Community, as defined in
...
. This extended community indicates the

[RFC9012]

MUST NOT

 use an SHT value other than 00 when advertising an A-D per ES route
: VXLAN (type 8), NVGRE (type 9),

An egress NVE
with the following tunnel encapsulation types from
...
SHT value different than 00, it

 apply the treat-as-withdraw behavior, per
...
encapsulation type 19 in the BGP Tunnel Encapsulation attribute
...
[RFC8365]
 ...
 regarding the value that is advertised in the ESI Label

•

The A-D per ES route(s) for an ES
...
local-bias method.
 ...
 have an ESI Label value of zero if the SHT value is 01.
...
MAY
 ...
 have an ESI Label value of zero for VXLAN or NVGRE
...
NVEs supporting this specification.
 ...
 and a non-upgraded NVE can be attached to the same ES as other
...
An NVE maintains an administrative SHT value for an ES, which is advertised alongside the A-D
...
This document assumes that an implementation of
...
one A-D per ES route for the ES with an SHT value of 00, it
default split-horizon method, as described in Table 1, irrespective of its administrative SHT.

 revert its operational SHT to the
...
NVE2 has an SHT value of 01, the NVE
Table 1 as its operational SHT, regardless of its administrative SHT.

 use the default split-horizon method specified in

MUST

All NVEs attached to an ES with an operational SHT value of 10
ESI Label. If the operational SHT value is 01, the ESI Label
MAY
...
MUST
 be zero. If the operational SHT

 advertise a valid, non-zero
...
, an NVE that supports multiple data plane encapsulations (e.g., VXLAN,

As specified in
NVGRE, MPLS, MPLSoUDP, Geneve) must indicate all supported encapsulations using BGP
Encapsulation extended communities as defined in
...
community or a BGP Tunnel Encapsulation Attribute.
 ...
 for all EVPN routes. This section
...
MAY
 ...
 advertise multiple A-D per ES routes for the same ES,

It is important to note that an NVE
...
, an NVE that advertises multiple encapsulations in the A-D per ES route(s) for

 advertise encapsulations that use the same split-horizon filtering method in the
...
However, an A-D per ES route for ES-z
encapsulations.
...
support a single split-horizon method. In this case, the SHT value
...
be 00 and the treat-as-withdraw behavior

 is applied in case of any other value.
...
support both split-horizon methods. In this case, the SHT value
...
an A-D per ES route with an SHT value of 01. The ESI Label value in this case
...
per split-horizon method
where:

 be advertised. For example, consider an ES-z with n RTs,
...
In this scenario, three groups of A-D per ES routes

MUST

 be advertised for ES-z:
...
, it is the responsibility of the operator of a given EVI to ensure that all of the
...
All the security considerations described in
...
for the 1-octet Flags field in the ESI Label Extended Community
...
Finally, IANA has created the "Split-Horizon Type" registry for the related field of the "EVPN ESI
Label Extended Community Flags". The registry has been populated with the following initial
values:
...
New registrations in the "EVPN ESI Label Extended Community Flags", "Multihoming
Redundancy Mode", and "Split-Horizon Type" registries will be made through the "IETF Review"
procedure defined in
(BGP) Extended Communities" registry group.

. These registries are located in the "Border Gateway Protocol
...
Boutros, S. Sajassi, A. Drake, J. Rabadan, J.
,
...
S. Aldrin "EVPN control

,

plane for Geneve" Work in Progress Internet-Draft, draft-ietf-bess-evpn-
,
geneve-08
evpn-geneve-08>
...
www.iana.org/assignments/bgp-tunnel-encapsulation>
...
Maino, F. Kreeger, L.
,
(VXLAN-GPE)" Work in Progress Internet-Draft, draft-ietf-nvo3-vxlan-gpe-13
<https://datatracker.ietf.org/doc/html/draft-ietf-nvo3-vxlan-
November 2023,
gpe-13>
.

U. Elzur "Generic Protocol Extension for VXLAN
...
review and useful comments. Thanks to
thorough review.

Anoop Ghanwani Gyan Mishra
