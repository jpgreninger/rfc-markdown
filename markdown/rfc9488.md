...
RFC 9488
Local Protection Enforcement in the Path
...
[RFC5440]
 ...
 and Fast Reroute Extensions to RSVP-TE

 utilizes flags, values, and concepts previously defined in RSVP-TE Extensions
. One such concept in PCEP is the

PCEP
[RFC3209]
Local Protection Desired (L) flag in the LSP Attributes (LSPA) object in
originally defined in the Session Attribute object in
...
calculation. Therefore, the L flag does not require the transit router to satisfy protection in order
...
Switched Path (LSP) via the LSPA object.
 ...
 extends support in PCEP for Segment Routing
...
Protocol - Link State (BGP-LS)
 and take the protection into consideration as a path
constraint.
...
Desired (L) flag and extends on it with the introduction of the Protection Enforcement (E) flag.
 ...
 by further describing the behavior of the Local Protection
...
MUST
 ...
 have protection eligibility on all links.
...
, the mechanism to signal protection enforcement in PCEP is the
...
However,
links protected with Fast Reroute as defined in
[RFC5440]
PREFERRED, leading to operational differences.

." Implementations that use PCEP
 have interpreted the L flag as either PROTECTION MANDATORY or PROTECTION

 does state that "[w]hen set, this means that the computed path must include
...
does not influence whether the link must be pruned during a path calculation. Furthermore, the
...
E (Protection Enforcement): This flag controls the strictness with which the PCE must apply
...
MUST
 ...
 consider the protection eligibility
...
eligibility as an UNPROTECTED PREFERRED constraint but
...
define protected status assumptions. When computing a Segment Routing path, it is
 that a PCE assume a Node SID is protected. It is also
RECOMMENDED
RECOMMENDED
...
PCRpt and/or PCUpd messages as per
 for
PCE-initiated LSPs. It is important to note that
[RFC5440]

 to be included in PCUpd messages for PCC-initiated and PCE-initiated LSPs.

 for PCC-initiated LSPs or as per

 permit the LSPA object
...
not respect the enforcement constraint.
 ...
 and ignores the E flag. Thus, a computed path might

 be set to 1 depending on local
...
message as it may be communicating with a PCE that does not support this document.
 ...
 ignore the E flag value received from the PCE in a PCUpd
...
MAY
 ...
 ignore the E flag value received from the PCC in a PCRpt message as it

 process the E flag value received from the PCE in a PCUpd
...
John Scudder
 ...
 for reviewing and providing very
