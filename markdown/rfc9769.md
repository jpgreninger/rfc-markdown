...
This document specifies interleaved modes for the Network Time Protocol (NTP). These new
modes improve the accuracy of time synchronization by enabling the use of more accurate
transmit timestamps that are available only after the transmission of NTP messages. These
enhancements are intended to improve timekeeping in environments where high accuracy is
critical. This document updates RFC 5905 by defining these new operational modes.
...
Software timestamps captured in the user space in the NTP implementation itself are the least
...
, i.e., that contains a transmit
the transmission. A packet that strictly follows
timestamp corresponding to the packet itself, is said to be in the basic mode.

 does not specify any mechanism for a server to
...
of extension fields was authentication with
...
Requests and responses cannot always be formed in the interleaved mode. It cannot be used
...
MAY
 ...
 restrict the interleaved mode to specific IP addresses and/or authenticated
...
Both servers and clients that support the interleaved mode
...
not guaranteed to be monotonically increasing, the server
...
timestamp if necessary, to prevent an incorrect interleaved response later.
 ...
 check that the transmit and

SHOULD

When the server receives a request from a client, it
...
A response in the interleaved mode
 contain the transmit timestamp of the response that
contained the receive timestamp matching the origin timestamp from the request. The server
can drop the timestamps after sending the response. The receive timestamp
...
mode), the server
basic mode. In any case, the server
able to respond in the interleaved mode to the next request from the client.

 respond in the interleaved mode. If it responds, it

 save the new receive and transmit timestamps to be
...
will enable an interleaved response to the subsequent request. The client
...
less accurate. The first exchange is in the basic mode followed by a second exchange in the
...
The client
 update its NTP state when an invalid response is received, so that the
...
offset and delay using the formulas from
timestamps. The first set is
delay. The corresponding timestamps from Figure 1 are written in parentheses.

 and one of two different sets of
 for clients that filter measurements based on the
...
MAY
 ...
 filter measurements based on the mode. The maximum number of dropped

Clients
measurements in the basic mode
 be limited in cases where the server does not support,
...
MAY
 ...
 limit the saving of the receive and transmit timestamps to requests that have an
...
support the interleaved mode. For this reason, it
 be disabled by default and enabled
...
Peers that have an association need to count valid packets received between their transmissions
...
conditions are met:
 ...
 send a Peer B a packet in the interleaved mode unless all of the following
...
If Peer A has no association with Peer B and it responds with symmetric passive packets, it does
...
A client that supports the interleaved mode
detect packets conforming to the interleaved mode. The client
timestamp with the transmit timestamp from the previous packet to detect lost packets. If the
difference is larger than a specified maximum (e.g., 1 second), the packet
for synchronization in the interleaved mode to avoid a large error in the measurement.

 check if the origin timestamp is not zero to

 also compare the origin
...
interleaved modes. These issues are described in
[SECNTP]
.
...
Clients and peers
clients (e.g., as a reference timestamp) to prevent off-path attackers from easily getting the origin
timestamp needed to make a valid response in the interleaved mode.

 leak the receive timestamp in packets sent to other peers or

SHOULD NOT

Clients using the interleaved mode
timestamps in their requests (i.e., provide a precision of 232 seconds) to make it more difficult
for off-path attackers to guess the origin timestamp in the server response.

 randomize all bits of receive and transmit

SHOULD

Unlike in the basic client/server mode, clients using the interleaved mode cannot set the origin
timestamp in their requests to zero (i.e., reset the NTP association with every request) to make it
more difficult to track them as they move between networks.

Attackers can force the server to drop its timestamps in order to prevent clients from getting an
interleaved response. They can send a large number of requests, send requests with a spoofed
source address, or replay an authenticated request if the interleaved mode is enabled only for
authenticated clients. Clients
 rely on servers to be able to respond in the interleaved
mode.

MUST NOT

Protecting symmetric associations in the interleaved mode against replay attacks is even more
difficult than in the basic mode. In both modes, the NTP state needs to be protected between the
reception of the last non-replayed response and transmission of the next request in order for the
request to contain the origin timestamp expected by the peer. The difference is in the
timestamps needed to complete a measurement. In the basic mode, only one valid response is
needed at a time and it is used as soon as it is received, but the interleaved mode needs two
consecutive valid responses. The NTP state needs to be protected at all times, so that the
timestamps that are needed to complete the measurement when the second response is received
will not be lost.
...
, and
,
Randomization" RFC 9109 DOI 10.17487/RFC9109
,
www.rfc-editor.org/info/rfc9109>

M. Lichvar "Network Time Protocol Version 4: Port
<https://
...
,
"The Security of NTP's Datagram Protocol" Cryptology ePrint Archive, Paper
2016/1006

<https://eprint.iacr.org/2016/1006>
...
The interleaved modes described in this document are based on the implementation written by
David Mills
 in the NTP project. The specification of the broadcast mode is based purely on this
implementation. The specification of the symmetric mode has some modifications. The client/
server mode is specified as a new mode compatible with the symmetric mode. It is a simplified
special case of the symmetric mode, analogously to how the basic client/server mode is a special
case of the basic symmetric mode.

Doug Arnold Roman Danyliw Reese Enghardt Daniel Franke
,
The authors would like to thank
,
,
Benjamin Kaduk Erik Kline Catherine Meadows Tal Mizrahi Steven Sommars Harlan Stenn
,
,
,
Kristof Teichel
