...
This specification defines a set of additional Capability Objects that provide information about
current downstream CDN (dCDN) utilization and specified usage limits to the delegating
...
Capability Object as well as the capability-type (a CDNI Payload Type). The section also defines
the Capability Objects per such type. Below, we define two additional Capability Objects.

 describes the FCI Capability Advertisement Object, which contains a CDNI

Note: In the following sections, the term "mandatory-to-specify" is used to convey
 be included when serializing a given capability object.
which properties
When mandatory-to-specify is defined as a "Yes" for an individual property, it
means that if the object containing that property is included in an FCI message,
...
associated with an FCI Capabilities advertisement. When defining a CapacityLimit, the meaning
of a limit might be ambiguous if the uCDN and dCDN are observing telemetry via different data
...
An identifier of a telemetry source. The ID string assigned to this Telemetry
 ...
 be unique across all Telemetry Source objects in the advertisement
...
At the time of this writing, the "CDNI Telemetry Source Types" registry is limited to a single type:
generic (see Table 3 in Section 3.2.1).
...
An identifier for this metric. This name
within the containing Telemetry Source. The name
...
MUST
 ...
 be unique among metric objects
 remain consistent for the same
...
The percentile calculation the data represents, i.e., 50 percentile would equate
to the median over the time-granularity. Lack of a data-percentile indicates that the data
MUST
...
The following shows an example of a Telemetry Capability Object, including two metrics for a
source, that is scoped to a footprint.
...
the dCDN. The limit object is scoped to the footprint associated with the FCI Capabilities
advertisement encompassing this object. Limits
uCDN will need to ensure that all limits are considered rather than choosing only the most
specific.

 be considered using a logical "AND": A
...
Specifies an identifier associated with a limit. This

MAY

 be used as a relational

identifier to a specific CapacityLimit Object. If specified, this identifier
...
A soft limit at which a uCDN

SHOULD

 reduce traffic before hitting the hard
...
MUST
 ...
 be less than the value of maximum-hard. If this value is not
...
Specifies the current usage value of the limit. It is
...
Below are listed the valid limit-type entries registered in the "CDNI Capacity Limit Types"
...
Reference to the "name" property of a metric defined within a Telemetry

Source of a Telemetry Capability object.
...
The authors would like to express their gratitude to the members of the Streaming Video
Technology Alliance
review.

 Open Caching Working Group for their guidance, contribution, and
