...
RFC 9747
Unaffiliated Bidirectional Forwarding Detection
(BFD) Echo
...
[RFC5880]
 ...
 provides a low-overhead, short-interval method for detecting faults on the
...
. If several consecutive BFD Echo packets are
...
handling them. While this may appear to contravene
...
[BBF-TR-146]
 ...
 describes a use case for the Unaffiliated BFD Echo.

[RFC5880]

 by defining a new method of BFD Echo-only operation which
...
would be looped back by the first hop. A full description of the updates to
in Section 3.
...
exception that the received state is not derived from BFD Control packets originating from the
 ...
 adhere to the BFD state machine specified in

Section 6.2 of [RFC5880]
...
DeviceADeviceBUnaffiliatedBFDEchoSessionUnaffiliatedBFDEchoBFD|packetsloopedBFDsupportedBFDnotsupportedRFC 9747

Unaffiliated BFD Echo

March 2025

remote system, but rather from packets that are generated by the local system and looped back
...
destination port 3785, as defined in
. The standard procedures for BFD Asynchronous
...
as for a BFD Echo packet as specified in
for the session
TTL or Hop Limit value of 254 (similar to
number of hops); otherwise, the received packets

. All Unaffiliated BFD Echo packets
 have a
 to verify against a configured

. An Unaffiliated BFD Echo packet follows the same encapsulation rules

 be sent with a TTL or Hop Limit value of 255. Received packets
...
Min RX Interval" fields, as defined in
,
prevent the potential exposure of uninitialized memory. It is
...
be ignored and
 ...
 be populated with a specific value to

[RFC5880] MUST

RECOMMENDED

 that these fields be

MUST NOT

MUST

The "Required Min Echo RX Interval" field, as defined in
specific value to prevent the potential exposure of uninitialized memory. It is
that this field be set to 0. However, this value
interval for Unaffiliated BFD Echo packets when in the Up state

 be populated with a
RECOMMENDED
 be ignored upon receipt. The transmission

[RFC5880] MUST

 be provisioned on device A.
...
, the Unaffiliated BFD Echo session begins with the

Similar to what's specified in
...
Failed)", as specified in
 ...
 set bfd.SessionState to Down and bfd.LocalDiag to 2 (Echo Function
...
, and the fields within the Unaffiliated BFD Echo packet are populated as
...
MUST
 ...
 be set to the provisioned local discriminator.
...
Discriminator" looped back from the remote system.
 ...
 initially be set to 0, and then

MUST

MUST

 be set to the value of "My
...
MUST
 ...
 be set to a specific value, with a suggested value of 1 second
...
MUST
 ...
 be set to a specific value, with a suggested value of 0.
...
, but does not require BFD Asynchronous or Demand
...
[RFC5880]
 ...
 with respect to its descriptions on the BFD Echo function as
...
The third and ninth paragraphs of
...
value of not less than one second (1,000,000 microseconds). This is intended to keep
 ...
 set bfd.RequiredMinRxInterval to a
...
received BFD Control traffic at a negligible level, since the actual detection function is
...
remote system contains a nonzero value in Required Min Echo RX Interval.
 ...
 be transmitted when bfd.SessionState is not Up. BFD Echo
MUST NOT
 be transmitted unless the last BFD Control packet received from the

BFD Echo packets
MAY
between transmitted BFD Echo packets
the remote system in Required Min Echo RX Interval, except as follows: [...]

 be transmitted when bfd.SessionState is Up. The interval

 be less than the value advertised by
...
the remote system contains a nonzero value in Required Min Echo RX Interval.
 ...
 be transmitted when bfd.SessionState is not Up, and BFD
...
the remote system in Required Min Echo RX Interval, except as follows: [...]
 ...
 be transmitted when bfd.SessionState is Up, and the interval
...
All operational considerations from
 apply. Since this mechanism leverages existing
...
BFD Echo work, although the configuration won't go beyond the scope of
...
, otherwise the Unaffiliated BFD Echo packets might not pass the

[RFC3704]

Section 5 of [RFC5880]

, BFD Echo packets may be spoofed. Specifically for

As described in
Unaffiliated BFD Echo, a DoS attacker may send spoofed Unaffiliated BFD Echo packets to the
loopback device, so some form of authentication
Unaffiliated BFD Echo packets in this document are also BFD Control packets, the
"Authentication Section" as defined in
be included within the Unaffiliated BFD Echo packet.

 be included. Considering the

 for a BFD Control packet is
...
uninitialized memory, all fields of the Unaffiliated BFD Echo packet
 be populated with a
...
Broadband Forum "TR-146: Subscriber Sessions" Broadband Forum Technical
Report, TR-146, Issue 1
tr-146-1-0-0.pdf>
...
Wang Éric Vyncke Adrian Farrel Tim Wicinski Dhruv Dhody Stephen Farrell Gunter Van de
,
Velde Gyan Mishra Brian Trammell Gorry Fairhurst Mahesh Jethanandani John Scudder
,
Murray Kucherawy
, and
comments.
...
Ketan Talaulikar Greg Mirsky Santosh Pallagatti Aijun

 for their careful reviews and very helpful
...
Jeff Haas
 ...
 for his guidance, insightful review, and very
...
Erik Auerswald
 ...
 for his insightful comments during the

The authors would like to acknowledge

Detao Zhao

 for the very helpful discussion.
