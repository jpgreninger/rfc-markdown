...
This document speciﬁes mechanisms to conﬁgure DDoS Open Threat Signaling (DOTS) clients
...
DOTS Server/Call Home Client Discovery
...
5.1.1.  Format of DOTS Reference Identiﬁer Option
...
5.2.1.  Format of DOTS Reference Identiﬁer Option
...
[RFC8811]
 ...
 speciﬁes an architecture in which a DOTS client
...
mitigation actions are required. Indeed, because the lack of a common method to coordinate a
real-time response among involved actors and network domains inhibits the eﬀectiveness of
DDoS attack mitigation, the DOTS signal channel protocol
for DDoS attack mitigation. With this approach, DOTS can reduce the impact of an attack and
lead to more eﬃcient defensive actions in various deployment scenarios, such as those discussed
in
ﬁltering rules by means of the DOTS data channel protocol

. Moreover, DOTS clients can instruct a DOTS server to install named

 is meant to carry requests
...
Figure 1: Basic DOTS Architecture
 ...
 speciﬁes that the DOTS client may be provided with a list of DOTS servers, each
[RFC8811]
...
This document speciﬁes methods for DOTS clients to discover their DOTS server(s). The rationale
...
The discovery methods can also be used by a DOTS server to locate a DOTS client in the context of
...
speciﬁcation accommodates all these deployment cases.

Considerations for the selection of DOTS server(s) by multihomed DOTS clients are out of this
document's scope; readers should refer to
...
. DOTS clients use those credentials for authentication purposes
[RFC8572]
following the rules documented in
...
refers to a DOTS-aware software module responsible for requesting attack

response coordination with other DOTS-aware elements.

DOTS server:

is a DOTS-aware software module handling and responding to messages from
...
refers to a DOTS client or server deployed in a Call Home
...
refers to the peer DOTS server (base DOTS operation) or to a peer Call Home
...
[DOTS-USE-CASES]
 ...
 reveals that it is unlikely that
...
resolution or DHCP, to provision the CPE acting as a DOTS client with the DOTS server(s).
 ...
 do involve Customer Premises

[DOTS-USE-CASES]

•

Resolving a DOTS server domain name oﬀered by an upstream transit provider provisioned
...
Some of the use cases may allow DOTS clients to have direct communications with upstream
DOTS servers, that is, no DOTS gateway is involved. Leveraging existing protocol behaviors
that do not require speciﬁc features on the node embedding the DOTS client may ease DOTS
...
to discover this preference.
 ...
 allows the DOTS server administrators to provision the preferred DOTS
...
provided during the service subscription (i.e., manual/local conﬁguration).

•

Multiple DOTS clients may be enabled within a network (e.g., an enterprise network).
Dynamic discovery needs to be deterministic from an operational standpoint.

•

Some of the use cases may involve a DOTS gateway that is responsible for selecting the
appropriate DOTS server(s) to relay requests received from DOTS clients.

Consequently, this document describes a uniﬁed discovery logic (Section 4) that involves the
...
and a consistent priority among these options. If some devices prefer manual conﬁguration over
dynamic discovery while others prefer dynamic discovery over manual conﬁguration, the result
will be a process where the operator must ﬁnd devices that are using the wrong DOTS server(s),
determine how to ensure the devices are conﬁgured properly, and then reconﬁgure the device
...
SHOULD
 ...
 support at least one of the three mechanisms below to determine a DOTS

All DOTS clients
server list. All DOTS clients
speciﬁc device, of the following ways to discover DOTS servers in order to facilitate the
deployment of DOTS in large-scale environments. For example, a CPE will support the ﬁrst two
mechanisms, a host within a LAN will support the last two mechanisms, or an application server
will support a local conﬁguration. More examples are discussed in Section 3. DOTS clients will
prefer information received from the discovery methods in the order listed below.

 implement all three, or as many as are practical for any

1.

Explicit Conﬁguration:

Local/Manual Conﬁguration:

A DOTS client will learn the DOTS server(s) by means of local

or manual DOTS conﬁguration (i.e., DOTS servers conﬁgured at the system level).
Conﬁguration discovered from a DOTS client application is considered a local
conﬁguration.

An implementation may give the user an opportunity (e.g., by means of conﬁguration ﬁle
options or menu items) to specify DOTS server(s) for each address family. These may be
speciﬁed either as a list of IP addresses or the DNS name of a DOTS server. When only
DOTS server IP addresses are conﬁgured, a reference identiﬁer must also be conﬁgured
for authentication purposes.

Automatic Conﬁguration (e.g., DHCP):

The DOTS client attempts to discover DOTS server(s)

names and/or addresses from DHCP, as described in Section 5.

2.

Service Resolution: The DOTS client attempts to discover DOTS server name(s) using service
resolution, as speciﬁed in Section 6.

3.

DNS-SD: DNS-based Service Discovery. The DOTS client attempts to discover DOTS server
name(s) using DNS service discovery, as speciﬁed in Section 7.
...
capabilities and/or suitable DNS conﬁguration.
...
On hosts with more than one interface or address family (IPv4/IPv6), the DOTS server discovery
...
expiry of a validity timer (e.g., DHCP lease, DHCP information refresh time, or DNS TTL)
associated with a discovered DOTS agent

•

expiry of the certiﬁcate of a peer DOTS agent currently in use
...
Some certiﬁcation authorities issue server certiﬁcates based on IP addresses, but
preliminary evidence indicates that such certiﬁcates are a very small percentage (less
than 1%) of issued certiﬁcates.

In order to allow for PKIX-based authentication between a DOTS client and server while
accommodating the current best practices for issuing certiﬁcates, this document allows DOTS
...
reference identiﬁer for authentication purposes.

Deﬁning the option to include a list of IP addresses would avoid depending on an underlying
...
The design assumes that the same peer DOTS agent is used for establishing both signal and data
channels. For more customized conﬁgurations (e.g., transport-speciﬁc conﬁguration and distinct
DOTS servers for the signal and data channels), an operator can supply only a DOTS reference
identiﬁer that will be then passed to the procedure described in Section 6.
...
(or Call Home DOTS server) will then use the address selection procedure speciﬁed in
Section 4.3
of [RFC8782]

 to identify the IP address of the peer DOTS server (or Call Home DOTS client).
...
DOTS reference identiﬁer and a list that includes both 2001:db8:122:300::1 and
...
The DHCPv6 DOTS Reference Identiﬁer option is used to conﬁgure the name of the DOTS server
...
Figure 3: DHCPv6 DOTS Reference Identiﬁer Option

The ﬁelds of the option shown in Figure 3 are as follows:

OPTION_V6_DOTS_RI (141, see Section 9.2).

Length of the dots-agent-name ﬁeld in octets.
...
A fully qualiﬁed domain name of the peer DOTS agent. This ﬁeld is formatted
...
FQDN "dots.example.com", and the resulting Option-length ﬁeld is 18.
...
The DHCPv6 DOTS Address option can be used to conﬁgure a list of IPv6 addresses of a DOTS
...
The ﬁelds of the option shown in Figure 5 are as follows:
...
Length of the DOTS ipv6-address ﬁelds in octets. This
[RFC4291]

Includes one or more IPv6 addresses

 be a multiple of 16.
MUST
 of the peer DOTS agent to
...
MAY
 ...
 request options OPTION_V6_DOTS_RI and OPTION_V6_DOTS_ADDRESS, as

DHCP clients
. As a convenience to
deﬁned in Sections 18.2.1, 18.2.2, 18.2.4, 18.2.5, 18.2.6, and 21.7 of
...
OPTION_V6_DOTS_ADDRESS), it

 use only the ﬁrst instance of that option.
...
MUST
 ...
 silently discard multicast and host loopback addresses

[RFC6890]

If the DHCP client receives and validates both OPTION_V6_DOTS_RI and
OPTION_V6_DOTS_ADDRESS, the content of OPTION_V6_DOTS_RI is used as the reference
), while the valid addresses included
identiﬁer for authentication purposes (e.g., PKIX
in OPTION_V6_DOTS_ADDRESS are used to reach the peer DOTS agent. In other words, the name
conveyed in OPTION_V6_DOTS_RI
 be passed to an underlying resolution library in the
...
than one name, the DHCP client
(
Section 10 of [RFC8415]
is also used as a reference identiﬁer for authentication purposes.

), the name is passed to a name resolution library. Moreover, that name

 use only the ﬁrst name. Once the name is validated
...
can be used as identiﬁers for authentication.
...
[RFC2132]
 ...
 DOTS Reference Identiﬁer option is used to conﬁgure a name of the peer
...
Figure 6: DHCPv4 DOTS Reference Identiﬁer Option

The values s1, s2, s3, etc. represent the domain name labels in the domain name encoding.

The ﬁelds of the option shown in Figure 6 are as follows:
...
Includes the length of the "Peer DOTS agent name" ﬁeld in octets.

Code:
Length:
Peer DOTS agent name:

The domain name of the peer DOTS agent. This ﬁeld is formatted as
...
The DHCPv4 DOTS Address option can be used to conﬁgure a list of IPv4 addresses of a peer
...
The ﬁelds of the option shown in Figure 7 are as follows:
...
Contains one or more IPv4 addresses of the peer DOTS agent to be used
...
OPTION_V4_DOTS_ADDRESS is a concatenation-requiring option. As such, the mechanism
 be used if OPTION_V4_DOTS_ADDRESS exceeds the maximum
speciﬁed in
...
OPTION_V4_DOTS_ADDRESS in a Parameter Request List option
...
only the ﬁrst instance of that option.
...
OPTION_V4_DOTS_ADDRESS, the content of OPTION_V4_DOTS_RI is used as the reference
identiﬁer for authentication purposes (e.g., PKIX
), while the valid addresses included
in OPTION_V4_DOTS_ADDRESS are used to reach the peer DOTS agent. In other words, the name
 be passed to an underlying resolution library in the
conveyed in OPTION_V4_DOTS_RI
...
MUST
passed to a name resolution library. Moreover, that name is also used as a reference identiﬁer
for authentication purposes.
...
A DNS domain name is retrieved for each combination of interface and address family. A
...
is used if no speciﬁc name has been conﬁgured.

2.

[RFC3958]
Retrieved DNS domain names are then used for S-NAPTR lookups
...
This speciﬁcation deﬁnes "DOTS" and "DOTS-CALL-HOME" as application service tags (Sections
9.4.1 and 9.4.2). It also deﬁnes "signal.udp" (Section 9.4.3), "signal.tcp" (Section 9.4.4), and
...
If no DOTS-speciﬁc S-NAPTR records can be retrieved, the discovery procedure fails for this
domain name (and the corresponding interface and IP protocol version). If more domain names
are known, the discovery procedure
MAY
immediately. However, before retrying a lookup that has failed, a DOTS client
MUST
period that is appropriate for the encountered error (e.g., NXDOMAIN, timeout, etc.).

 perform the corresponding S-NAPTR lookups
...
DNS-based Service Discovery (DNS-SD)
 provides generic solutions for discovering
services. DNS-SD deﬁnes a set of naming rules for certain DNS record types that they use for
...
Section 4.1 of [RFC6763]
structure:

 speciﬁes that a service instance name in DNS-SD has the following

<Instance> . <Service> . <Domain>

The <Domain> portion speciﬁes the DNS subdomain where the service instance is registered. It is
a conventional domain name, such as "example.com".

The <Service> portion of the DOTS service instance name
signal._tcp", "_dots-data._tcp", "_dots-call-home._udp", or "_dots-call-home._tcp".

MUST

 be "_dots-signal._udp", "_dots-

This document does not deﬁne any keys; the TXT record of a DNS-SD service is thus empty
Section 6 of [RFC6763]
(

).

Figure 10 depicts an excerpt of the DNS zone conﬁguration ﬁle listing record examples to
...
An attacker can get a domain name, get a domain-validated public certiﬁcate from a certiﬁcation
...
. In addition to the mitigations listed in
Section 22 of [RFC8415]
Section 22 of [RFC8415]
agent may be preconﬁgured with a list of trusted DOTS domain names. If such a list is
preconﬁgured, a DOTS agent will accept a DHCP-discovered name if it matches a name in that
list. Also, the DOTS agent has to check that the "DNS-ID" identiﬁer type within subjectAltName in
the server certiﬁcate matches a preconﬁgured name. If the DOTS agent is instructed to trust
subdomains of the names in that list as well (e.g., "*.example.com"), a DOTS agent will accept a
DHCP-discovered name that matches a name in the preconﬁgured list (e.g., "dots-1.example.com"
or "dots-2.example.com").

, a DOTS

Relying on an underlying resolution library to resolve a supplied reference identiﬁer has similar
security issues as those discussed in Section 8.2 (e.g., an active attacker may modify DNS
messages used to resolve the supplied reference identiﬁer and point the client to an attacker
server).

Supplying both an IP address and the reference identiﬁer makes it easier to use a mis-issued
certiﬁcate.
...
The DOTS client (or a Call Home DOTS server) constructs one reference identiﬁer for the DOTS
server (or a Call Home DOTS client) based on the domain name that is used for S-NAPTR lookup:
DNS-ID. If the reference identiﬁer is found (as described in
certiﬁcate's subjectAltName extension, the DOTS client should accept the certiﬁcate for the
server.
...
and that is validated using DNSSEC is thereby proved to be the authoritative data.
 ...
 uses cryptographic keys and digital signatures to
...
Since DNS-SD is a speciﬁcation for how to name and use records in the existing DNS system, it
has no speciﬁc additional security requirements over and above those that already apply to DNS
queries and DNS updates. For DNS queries, DNSSEC
SHOULD
information is important. For DNS updates, secure updates
generally be used to control which clients have permission to update DNS records.

 be used where the authenticity of
[RFC2136] [RFC3007] SHOULD
...
IANA has allocated the following service names from the registry available at:
...
assignments/s-naptr-parameters/>

 for application service tags and application protocol tags.
...
Lemon, T. and S. Cheshire "Encoding Long Options in the Dynamic Host
Conﬁguration Protocol (DHCPv4)" RFC 3396 DOI 10.17487/RFC3396
,
2002,
...
Cotton, M., Vegoda, L., Bonica, R., Ed., and B. Haberman "Special-Purpose IP
,
...
Boucadair, M., Reddy, T., and W. Pan "Multi-homing Deployment

,

Considerations for Distributed-Denial-of-Service Open Threat Signaling (DOTS)"
, 23 November
Work in Progress Internet-Draft, draft-ietf-dots-multihoming-05
2020,
.
...
Reddy, T., Boucadair, M., and J. Shallow "Distributed Denial-of-
Service Open Threat Signaling (DOTS) Signal Channel Call Home" Work in
Progress Internet-Draft, draft-ietf-dots-signal-call-home-13
...
Dobbins, R., Migault, D., Moskowitz, R., Teague, N., Xia, L., and K. Nishizuka

,

"Use cases for DDoS Open Threat Signaling" Work in Progress Internet-Draft,
...
Watsen, K., Farrer, I., and M. Abrahamsson "Secure Zero Touch Provisioning
...
,
Boucadair, M., Ed. and T. Reddy.K, Ed. "Distributed Denial-of-Service Open
...
Mortensen, A., Ed., Reddy.K, T., Ed., Andreasen, F., Teague, N., and R. Compton
...
Thanks to
(BRSKI) text used in previous draft versions of the speciﬁcation.

Brian Carpenter

 for the review of the Bootstrapping Remote Secure Key Infrastructure

Many thanks to

Russ White

 for the review, comments, and text contribution.

Thanks to

Dan Wing Pei Wei Valery Smyslov
...
Bernie Volz
 ...
 for the review of the DHCP section.
...
Zhen Cao Kyle Rose Nagendra Nainar
...
Barry Leiba Martin Duke Roman Danyliw Éric Vyncke
