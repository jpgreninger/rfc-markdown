...
7.3.  IS-IS Sub-Sub-TLV Codepoints for Application-Specific Link Attributes Registry
...
protocol in support of Traffic Engineering (TE) was introduced by
[RFC5307] [RFC6119] [RFC7308]
...
. This has introduced ambiguity in that if a
...
[RFC6119]
 ...
 prohibits the use of TLV 139 when it is possible to use TLV 138.
...
Undefined bits that are transmitted
receipt.
...
Bits that are not supported by an implementation
...
specified in the bit mask
 ...
 use the link attribute advertisements in the sub-TLV.
...
the bit mask
 use the legacy advertisements for the corresponding link found in TLVs
...
When multiple sub-TLVs for the same link are advertised, they
SHOULD
...
numbered Link State Protocol Data Unit (LSP)
MUST
the same attribute
...
MAY
 ...
 be used, and subsequent advertisements of

 be ignored.

MUST

For a given application, the setting of the L-flag
In cases where this constraint is violated, the L-flag

MUST

 be the same in all sub-TLVs for a given link.

MUST

 be considered set for this application.
...
Link attributes
for both standard applications and UDAs. Such link attribute advertisements
...
given link. Otherwise, such link attribute advertisements
...
RSVP-TE bit (R-bit)
 be set in the Application Identifier Bit Mask. If an advertisement of
maximum reservable link bandwidth or unreserved bandwidth is received with bits other than
the R-bit set, the advertisement
...
, this single TLV provides support for
...
occurrences of the same identifier type
requirement
...
MUST
 ...
 be present. TLVs that do not meet this

MUST

 be present. Multiple

Multiple TLVs for the same link

MAY

 be advertised.

When the L-flag is set in the Application Identifier Bit Mask, SRLG values
in the TLV. Any SRLG values that are advertised
advertised, the corresponding legacy TLV (see Section 3.2) can be identified, and the SRLG values
advertised in the legacy TLV
 be used by the set of applications specified in the Application
Identifier Bit Mask.

 be ignored. Based on the link identifiers
...
For a given application, the setting of the L-flag
cases where this constraint is violated, the L-flag

MUST

MUST

 be the same in all TLVs for a given link. In
 be considered set for this application.
...
enablement for those applications.
 ...
 define the relationship between ASLA advertisements and
...
applications.
 ...
 make use of legacy advertisements. This simplifies deployment of new
...
Advertise all legacy link attributes using ASLA advertisements with the L-flag clear and the
...
registration procedure is "Expert Review" as defined in
...
different from the document that assigns the codepoint, the encoding reference
MUST
...
Note: If a link attribute can be advertised both as a sub-TLV of TLVs advertising neighbor
...
.
Bit definitions
 be assigned such that all bits in the lowest available octet are allocated
...
Segment Routing Policy (S-bit)

Loop-Free Alternate (F-bit)
...
impacting TE as discussed in
scope to specific applications, the impact of tampering is similarly limited in scope.

. As the advertisements defined in this document limit the
...
presence of such advertisements. In particular, the text in
 used the word "permitted",
