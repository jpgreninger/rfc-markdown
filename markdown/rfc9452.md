...
RFC 9452
Network Service Header (NSH) Encapsulation for In
Situ OAM (IOAM) Data
...
IOAM, as defined in
, is used to record and collect OAM information while the packet
...
Service Header (NSH) encapsulation
[RFC7665]
Architecture

 for the Service Function Chaining (SFC)
[RFC9197]
.
...
. IOAM-Data-Fields are carried as NSH payload using a Next

The NSH is defined in
Protocol header that follows the NSH headers. An IOAM header containing the IOAM-Data-Fields
 follow the definitions corresponding to IOAM Option-Types
is added. The IOAM-Data-Fields
Section 4 of [RFC9197]
(e.g., see
). In an administrative domain where
...
which are also configured to serve as encapsulating and decapsulating nodes for IOAM. The
 ensure that SFC-aware nodes along the Service Function Path support IOAM;
operator
otherwise, packets might be dropped (see the last paragraph of this section as well as
of [RFC8300]
MUST
the IOAM headers that are relevant based on its configuration. See
deployment-related aspects of IOAM-Data-Fields.

). The IOAM transit nodes (e.g., a Service Function Forwarder (SFF))
...
. The O bit
. The "NSH Next Protocol" value (referred to as "NP" in the diagram above) is

 be handled following the
...
corresponding to the IOAM Option-Type (e.g., see
[RFC9326]
...
MAY
 ...
 be included within the NSH encapsulation. For example, if an
...
data payload. The applicability of the IOAM Active and Loopback flags
...
node (such as an SFF) that does not support the IOAM header, it
mechanisms to maintain and notify of such events are outside the scope of this document.

, packets with unsupported Next Protocol values
...
IOAM is considered a "per domain" feature, where the operator decides how to leverage and
configure IOAM according to the operator's needs. The operator needs to properly secure the
IOAM domain to avoid malicious configuration and use, which could include injecting malicious
Section 9
IOAM packets into a domain. For additional IOAM-related security considerations, see
of [RFC9197]
[RFC9451]
.

. For additional OAM- and NSH-related security considerations, see
...
,
Operations, Administration, and Maintenance (IOAM) Loopback and Active
...
Brockners, F., Ed. Bhandari, S., Ed. Bernier, D.
Operations, Administration, and Maintenance (IOAM) Deployment" RFC 9378
DOI 10.17487/RFC9378
.

<https://www.rfc-editor.org/info/rfc9378>

T. Mizrahi, Ed. "In Situ
...
Each IOAM Option-Type (e.g., trace, proof-of-transit, and edge-to-edge) would be specified by
a type, with the different IOAM-Data-Fields being TLVs within this the particular option type.
NSH MD Type 2 offers support for variable length metadata. The length field is 6 bits,
resulting in a maximum of 256 (26 x 4) octets.

2.

Encapsulation of IOAM-Data-Fields using the "Next Protocol" field.

Each IOAM Option-Type (e.g., trace, proof-of-transit, and edge-to-edge) would be specified by
its own "next protocol".

3.

Encapsulation of IOAM-Data-Fields using the "Next Protocol" field.

A single NSH protocol type code point would be allocated for IOAM. A "sub-type" field would
then specify what IOAM options type (trace, proof-of-transit, edge-to-edge) is carried.

The third option has been chosen here. This option avoids the additional layer of TLV-nesting
that the use of NSH MD Type 2 would result in. In addition, this option does not constrain IOAM
data to a maximum of 256 octets, thus allowing support for very large deployments.
...
Karthik Babu Harichandra Babu Akshaya Nadahalli Stefano Previdi Hemant Singh Erik
,
Nordmark LJ Wobker Andrew Yourtchenko Greg Mirsky
,
,
,
comments and advice.
...
The following people contributed significantly to the content of this document and should be
