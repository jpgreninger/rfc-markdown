...
RFC 9516
Active Operations, Administration, and Maintenance
...
Path (SFP). According to

, an SFP is the instantiation of SFC in the network and
...
provides a level of indirection between the entirely abstract SFCs and a fully specified, ordered
...
Section 4 of [RFC8924]
 ...
 is used as the SFC encapsulation. Following the analysis of SFC OAM in

This document defines how active Operations, Administration, and Maintenance (OAM), per the
, is implemented when the Network Service Header (NSH)
definition of active OAM in
[RFC8300]
, this
...
defects, which conforms to the stateless character of transactions in SFC NSH
that Performance Monitoring OAM, as required by
...
. The SFC Echo Request and Echo Reply can be used with
...
The SFC active OAM deployment scope is deliberately constrained, as explained in
[RFC8300]
...
[RFC7665]
 ...
 is used extensively throughout this document, and the

In this document, SFC OAM refers to an active OAM
Additionally, "Echo Request/Reply" and "SFC Echo Request/Reply" are used interchangeably.

 in an SFC architecture.
...
, SFC-specific means are needed to perform the FM OAM task in an SFC
As discussed in
...
Packets of SFC active OAM

SHOULD

 be fate sharing with the monitored SFC data in the
...
MUST
 ...
 support monitoring of the continuity of the SFP between any of its
...
MUST
 ...
 support Remote Defect Indication notification by the egress to the
...
MUST
 ...
 support connectivity verification of the SFP. The definitions of the
...
MUST
 ...
 support fault localization of the loss of continuity check within an SFP.

SFC OAM

MUST

 support an SFP tracing to discover the RSP.
...
RSPs can be discovered using the trace function discussed in
...
MUST
 ...
 have the ability to discover and exercise all available RSPs in the
...
another element within the service function chain.
 ...
 offers an approach for defect localization
...
MUST
 ...
 be able to trigger on-demand FM remotely with responses being directed
...
of this document.
 ...
 describes an example of a proxy function for an
...
SFC active OAM combines OAM commands and/or data included in a message that immediately
 be set to
follows the NSH. To identify the SFC active OAM message, the Next Protocol field
[RFC9451]
SFC Active OAM (0x07) (Section 9.1). The O bit in the NSH
. A
case when the O bit is clear and the Next Protocol field value is set to SFC Active OAM (0x07) is
considered an erroneous combination. An implementation
notification mechanism is outside the scope of this specification, note that it
limiting control. The packet
the processing of the OAM payload.

 be dropped. An implementation

 report it. Although the

 be set, according to

 have control to enable
...
OAM Control Packet (e.g., BFD Control packet
...
[RFC0792]
localize defects in the data plane. ICMP (
and MPLS
...
Header (Figure 2).
 ...
 for IPv4 and
 are examples of broadly used active OAM protocols based on the Echo
...
registry for flags. This specification defines all flags for future use. Flags
...
(Section 9.2.5), the value of the Return Subcode field
...
a four-octet field. It

MUST

 be filled in by the sender of the Echo Request and
...
Request
Sender's Handle field. In some use cases, an implementation
...
SFC Echo Reply message
 ...
 copy the value from the received SFC Echo Request.
...
SFC Echo Request/Reply, the value of the Length field in the SFC Active OAM Header
 be 16
MUST
...
The value of the Return Code field
receiver of said Echo Request
MUST
registry (Section 9.2.5) in the corresponding Echo Reply that it generates.

 be set to zero by the sender of an Echo Request. The
MUST
 set it to one of the values in IANA's "SFC Echo Return Codes"
...
and/or Echo Reply. In
, a variable-length Context Header has been defined to protect
...
The SFC Echo Request control packet
of the monitored SFP. The Echo Request
The NSH
Echo Type field's value in the SFC Active OAM Header
...
MUST
 ...
 be immediately followed by the SFC Active OAM Header defined in Section 4. The
 be set to the SFC Echo Request/Reply

MUST

 use the appropriate underlay network encapsulation

 set the O bit in the NSH, as defined in

[RFC9451]
.

The value of the Reply Mode field
...
that generated the Echo Request, information that sufficiently identifies the source
...
Request
 ...
 include the Source ID TLV (Figure 5).
...
a two-octet field. It contains the UDP port number of the sender of the SFC OAM
 be used as the destination UDP port number in

MUST

control message. The value of the field
...
a field that contains the IP address of the sender of the SFC OAM control message,
 be used as the destination IP address in the IP/

MUST

i.e., IPv4 or IPv6. The value of the field
...
in the IP/UDP encapsulation of the SFC Echo Reply is one of the IP addresses associated with the
 ...
 replicate an SFC Echo Reply but choose the destination IP

 be present in an SFC Echo
...
responder. The value of the Port Number field
...
the address family is present, the receiver
Reply message, including relevant TLVs, follows the IP/UDP headers immediately.

 use the first TLV and ignore the rest. The Echo

 be used as the destination UDP port number
...
An SFF that received the SFC Echo Request

MUST

 validate the packet as follows:

1.

If the SFC Echo Request is integrity protected, the receiving SFF first
authentication.
...
properly formatted. In that case, the SFF
MUST
...
mechanism.
 ...
 send an SFC Echo Reply with the
...
processing is stopped, the message is dropped, and the event
...
the system identified in the Source ID TLV (see Section 6.5).
 ...
 send an SFC Echo Reply with the Return Code set to 1 ("Malformed Echo Request

5.

If there are any TLVs that the SFF does not understand, the SFF
with the Return Code set to 2 ("One or more of the TLVs was not understood") and set the
Subcode to zero. Also, the SFF
TLVs, contains only the misunderstood TLVs.

 include an Errored TLVs TLV (Section 6.4.1) that, as sub-

 send an SFC Echo Reply
...
deemed properly formed, then the SFF at the end of the SFP
...
If the SFF is not at the end of the SFP and the NSH TTL value is 1, the SFF
 send an SFC
...
Subcode set to zero.
 ...
 send an SFC Echo Reply with the Return Code set to 0 ("No Error") and the
...
Request sender
 use TLVs to request that the corresponding Echo Reply be transmitted over
...
bidirectional and hybrid SFC, as defined in
. For example, it allows an
...
message expects the receiver to use for the corresponding SFC Echo Reply.
 ...
 be transmitted. Hence, the Reply Service Function Path TLV included in the SFC

 sufficiently identify the SFP that the sender of the Echo Request

MUST

When sending an Echo Request, the sender
MUST
...
of the identifier of the reverse SFP and an appropriate Service Index.
 ...
 set the value of the Reply Mode field to "Reply

If the NSH of the received SFC Echo Request includes the MAC Context Header, the packet's
authentication

 be verified before using any data, as defined in Section 6.4.
...
Function Path TLV is present, then the responding SFF
 send an Echo Reply with the Return
...
found") and include the Reply Service Function Path TLV from the Echo Request message.
 ...
 send an Echo Reply with the Return Code set to 7 ("Reply SFP was not
...
has the route to the initiator. In that case, it
SHOULD
("Unverifiable Reply Service Function Path"). The receiver
cannot determine whether the SFP's return path has the route to the initiator. When sending the
Echo Request, the sender
return path SFP to help the receiver find the viable return path.

 set the value of the Return Code field to 8
 drop the Echo Request when it

 choose a proper source address according to the specified
...
SHOULD NOT
 ...
 accept the SFC Echo Reply unless the received message passes the following
...
As with the ICMP Echo Request/Reply
...
the Return Code in the received Echo Reply equals 5 ("End of the SFP").
 ...
 and the MPLS Echo Request/Reply
...
[RFC9263]
Group paths. In that case, such an element
is meant to induce the SFC Echo Request to follow the same RSP as the monitored flow.
 ...
 also be used for the SFC OAM traffic. Doing so
...
Request"). The sender of an SFP Consistency Verification Reply (CVRep) message
 set the
...
Every SFF that receives a CVReq message
...
For the received CVReq, an SFF that supports this specification
message the information about SFs that are available from that SFF instance for the specified
SFP. The SFF
 include the SFF Information Record TLV (Figure 9) in the CVRep message.
...
MUST
 ...
 stop processing the SFF Information Record TLV and notify an operator. The

 include control of rate-limited messages. Specification of the

 be verified before using any data. If the verification fails, the
...
[RFC9015]
 ...
 and indicates the type of SF, e.g., firewall,
...
Each SFF in the SFP
 send one and only one CVRep corresponding to the CVReq. If only one
SF is attached to the SFF in the SFP, only one SF Information Sub-TLV is included in the CVRep. If
several SFs are attached to the SFF in the SFP, the SF Information Sub-TLV
...
same. Information about all SFs
SF
even appear more than once in an SFP with a different service index.

 be listed as separate SF Information Sub-TLVs in the CVRep message. The same SF can

 be included in the CVRep message. Information about each
...
required, using one of the Context Headers defined in
...
. Also, the security needs for the
using the IP Encapsulating Security Payload Header
 and MPLS LSP
SFC Echo Request/Reply are similar to those of ICMP ping
ping
...
messages going to the control plane to mitigate potential Denial-of-Service attacks.
 ...
 that implementations throttle the number of SFC Echo Request/Reply
...
underlay, an implementation
 have means to check that the source of the Echo Request is
part of the SFP.
...
packet may be used to obtain network information. Thus, implementations
...
, filling the gaps and extending
combination with the existing mechanisms discussed in
...
,
Network Service Header in Service Function Chaining" RFC 9015 DOI 10.17487/
RFC9015

<https://www.rfc-editor.org/info/rfc9015>

L. Jalil "BGP Control Plane for the
...
D. Wing "Integrity Protection for the Network
Boucadair, M. Reddy.K, T.
,
Service Header (NSH) and Encryption of Sensitive Context Headers" RFC 9145
...
Boucadair, M. "Operations, Administration, and Maintenance (OAM) Packet and
Behavior in the Network Service Header (NSH)" RFC 9451 DOI 10.17487/
RFC9451
...
Aldrin, S. Pignataro, C., Ed. Kumar, N., Ed. Krishnan, R.
,
"Service Function Chaining (SFC) Operations, Administration, and Maintenance
(OAM) Framework" RFC 8924 DOI 10.17487/RFC8924
...
Wei, Y., Ed. Elzur, U. Majee, S. Pignataro, C.
Service Header (NSH) Metadata Type 2 Variable-Length Context Headers" RFC
9263 DOI 10.17487/RFC9263
,
rfc9263>

D. Eastlake 3rd "Network
,
<https://www.rfc-editor.org/info/
...
The authors greatly appreciate the thorough review and the most helpful comments from
,
Wing Dirk von Hugo Mohamed Boucadair Donald Eastlake 3rd Carlos Pignataro
Brockners
on BGP control plane for NSH SFC. The authors express their appreciation to
his suggestion about the load-balancing scenario. The authors greatly appreciate the
thoroughness of comments and thoughtful suggestions by
improved the document.

 for his review and the reference to the work
 for
