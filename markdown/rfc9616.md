...
networks, it fails to select reasonable routes in some topologies involving tunnels or VPNs.
 ...
 does not mandate a specific algorithm for computing
...
In order to enable the computation of RTTs, a node A
...
local clock). It then
 record the value t1 in the Origin Timestamp field of the Neighbour
Table entry corresponding to A and the value t1' in the Receive Timestamp field of the Neighbour
Table entry.

 include, in every Hello that it sends, a
...
When B sends an IHU to A, it checks whether both timestamps are defined in the Neighbour
Table. If that is the case, then it
a timestamped Hello TLV (either a normally scheduled Hello or an unscheduled Hello, see
Section 3.4.1 of [RFC8966]
 include in the IHU both the Origin Timestamp and the
Receive Timestamp stored in the Neighbour Table.

 ensure that its IHU TLV is sent in a packet that also contains
...
received. Node A
 then verify that it contains both a Hello TLV with timestamp t2' and an
...
computed at roughly the same point in the network stack. Transmit timestamps
computed just before the packet is passed to the network stack (i.e., before it is subjected to any
queueing delays); receive timestamps
the network stack.

 be computed just after the packet is received from
...
[DELAY-BASED]
 when RTT information is being used for route selection. The algorithm is
...
value. Thus, some form of smoothing
 be applied in order to avoid instabilities due to
occasional outliers.

SHOULD

 use the exponential average algorithm, which is simple to implement

An implementation
MAY
and appears to yield good results in practice
...
. The algorithm is parameterised by
...
SHOULD
 ...
 be between 0.8 and 0.9; the value 0.836 is the
...
for input to the metric computation procedure (
...
linearly; above rtt-max, the constant value max-rtt-penalty is added to the nominal cost.
 ...
 use the mapping described in Figure 3, which is parameterised by
...
ms, rtt-max = 120 ms, and max-rtt-penalty = 150 are
...
fluctuate when a link's RTT is between rtt-min and rtt-max. Implementations
...
TLV
 be silently ignored. If the Length field is smaller than the expected 4 octets, then this
sub-TLV

 be ignored (and the remainder of the enclosing TLV processed as usual).
...
(the first 8 octets are interpreted as described above), and any extra data contained in this sub-
 be silently ignored. If the Length field is smaller than the expected 8 octets, then this
TLV
sub-TLV
...
Jonglez, B. Boutier, M.
,
10.48550/arXiv.1403.3488

, and
, March 2014,

J. Chroboczek "A delay-based routing metric" DOI
...
Mills, D. "DCN Local-Network Protocols" STD 44 RFC 891 DOI 10.17487/
,
RFC0891
, December 1983,
...
to this protocol. We are also grateful to
Matejka David Schinazi Pascal Thubert Steffen Vogel
...
Donald Eastlake, 3rd Toke Høiland-Jørgensen Maria
,
...
, who prompted the investigation that originally lead
