...
Ethernet VPN (EVPN) provides an extensible and ﬂexible multihoming VPN solution over an
...
eﬃcient inter-subnet connectivity among these Tenant Systems and end devices while
...
7.2.  Sending Data Traﬃc without an ARP Request
...
EVPN
 provides an extensible and ﬂexible multihoming VPN solution over an MPLS/IP
...
are scenarios for which there is a need for a dynamic and eﬃcient inter-subnet connectivity
...
forwarding. When two TSs belonging to two diﬀerent subnets connected to the same Provider
Edge (PE) wanted to communicate with each other, their traﬃc needed to be backhauled from the
PE all the way to the centralized gateway where inter-subnet switching is performed and then
sent back to the PE. For today's large multi-tenant Data Center (DC), this scheme is very ineﬃcient
...
in order to avoid ineﬃcient forwarding of tenant traﬃc (i.e., avoid backhauling and hair
pinning). When a PE with IRB capability receives tenant traﬃc over an Attachment Circuit (AC), it
cannot only locally bridge the tenant intra-subnet traﬃc but also locally route the tenant inter-
subnet traﬃc on a packet-by-packet basis, thus meeting the requirements for both intra- and
inter-subnet forwarding and avoiding non-optimal traﬃc forwarding associated with a
centralized L3 GW approach.

Some TSs run non-IP protocols in conjunction with their IP traﬃc. Therefore, it is important to
handle both kinds of traﬃc optimally -- e.g., to bridge non-IP and intra-subnet traﬃc and to route
inter-subnet IP traﬃc. Therefore, the solution needs to meet the following requirements:

R1:

The solution must provide each tenant with IP routing of its inter-subnet traﬃc and Ethernet

bridging of its intra-subnet traﬃc and non-routable traﬃc, where non-routable traﬃc refers
to both non-IP traﬃc and IP traﬃc whose version diﬀers from the IP version conﬁgured in IP
Virtual Routing and Forwarding (IP-VRF). For example, if an IP-VRF in an NVE is conﬁgured for
IPv6 and that NVE receives IPv4 traﬃc on the corresponding VLAN, then the IPv4 traﬃc is
treated as non-routable traﬃc.

R2:

The solution must allow IP routing of inter-subnet traﬃc to be disabled on a per-VLAN basis

on those PEs that are backhauling that traﬃc to another PE for routing.
...
the case of VLAN-bundle and VLAN-based service models (see
...
payload, as speciﬁed for VXLAN in
...
header in the payload) as speciﬁed for Generic Protocol Extension (GPE) in
GPE]
...
EVPN Route Type 5, i.e., IP Preﬁx route, as deﬁned in
...
Network Identiﬁer in VXLAN, or a Virtual Subnet Identiﬁer in NVGRE, etc., unless it is
stated otherwise.
...
, is the instantiation of an IP-VPN instance in a PE. A
, is the instantiation of an EVI in a PE. A MAC-VRF consists of

A tenant needing IRB services on a PE requires an IP-VRF table along with one or more MAC-VRF
tables. An IP-VRF, as deﬁned in
MAC-VRF, as deﬁned in
one or more bridge tables, where each bridge table corresponds to a VLAN (broadcast domain). If
service interfaces for an EVPN PE are conﬁgured in VLAN-based mode (i.e.,
[RFC7432]
tenant VLAN per EVI. However, if service interfaces for an EVPN PE are conﬁgured in VLAN-
aware bundle mode (i.e.,
...
), then there are several bridge tables per MAC-
...
Each bridge table is connected to an IP-VRF via an L3 interface called an "IRB interface". Since a
...
IP-VRF is identiﬁed by its corresponding Route Target and Route Distinguisher, and MAC-VRF is
also identiﬁed by its corresponding Route Target and Route Distinguisher. If operating in EVPN
...
This document deﬁnes and describes two types of IRB solutions -- namely, symmetric and
...
performance trade-oﬀs. Speciﬁcally,
...
IRB
 ...
 be collapsed into a single IP lookup in a hardware implementation.
...
lookup followed by a MAC lookup, as depicted in the following ﬁgure.
...
In symmetric IRB, as shown in Figure 2, the inter-subnet forwarding between two PEs is done
...
MUST
 ...
 be of type Ethernet. Since only MAC lookup is performed at the egress PE (e.g.,

In asymmetric IRB, as shown in Figure 3, the inter-subnet forwarding between two PEs is done
...
The following sections deﬁne the control and data plane procedures for symmetric and
asymmetric IRB on ingress and egress PEs. The following ﬁgure is used to describe these
...
conﬁgured on each IRB interface associated with its subnet and fall into one of the following two
options:
...
address through the use of the Default Gateway extended community as speciﬁed in
[RFC7432]
...
any form of MAC security, then the ﬁrst option (i.e., using an anycast MAC address) should be used
to ensure that the applications receive traﬃc from the same IRB interface MAC address to which
they are sending. If the second option is used, then the IRB interface MAC address
...
all the VLANs for that tenant. Furthermore, it simpliﬁes the operation as there is no need for
...
If option 1 is used, an implementation
auto-derivation is used, the anycast MAC
are deﬁned in
...
MAY
 ...
 choose to auto-derive the anycast MAC address. If

MUST

 be auto-derived out of the following ranges (which
...
Where the last octet is generated based on a conﬁgurable Virtual Router ID (VRID) (range 1-255).
If not explicitly conﬁgured, the default value for the VRID octet is '1'. Auto-derivation of the
...
In addition to IP anycast addresses, IRB interfaces can be conﬁgured with non-anycast IP
...
operating in asymmetric IRB mode as the non-anycast IP addresses are conﬁgured along with
...
header and sender hardware address in the payload). For example, in Figure 4, TS1 is conﬁgured
...
MACx, which is the anycast MAC address of that IRB interface. Traﬃc routed from IP-VRF1 to TS1
...
on the presence of the Label2 ﬁeld and the IP-VRF Route Target in the EVPN MAC/IP
Advertisement route. If the Label2 ﬁeld of the received MAC/IP Advertisement route for host H1 is
non-zero, and one of its Route Targets identiﬁes the IP-VRF, the ingress PE will use symmetric IRB
mode when forwarding packets destined to H1. If the Label2 ﬁeld is zero and the MAC/IP
Advertisement route for H1 does not carry any Route Target that identiﬁes the IP-VRF, the ingress
PE will use asymmetric mode when forwarding traﬃc to H1.
...
PE, PE1, supports both modes. The forwarding mode will depend on the mode conﬁgured in the
egress PE, PE2. That is:

1.

If PE2 is conﬁgured for symmetric IRB mode, PE2 will advertise TS4 MAC/IP addresses in a
MAC/IP Advertisement route with a non-zero Label2 ﬁeld, e.g., Label2 = Lx, and a Route Target
that identiﬁes IP-VRF1 in PE1. IP4 will be installed in PE1's IP-VRF1; TS4's ARP and MAC
information will also be installed in PE1's IRB interface ARP table and BT1, respectively. When
a packet from TS2 destined to TS4 is looked up in PE1's IP-VRF route table, a longest preﬁx
match lookup will ﬁnd IP4 in the IP-VRF, and PE1 will forward using the symmetric IRB mode
and Label Lx.

2.

However, if PE2 is conﬁgured for asymmetric IRB mode, PE2 will advertise TS4 MAC/IP
information in a MAC/IP Advertisement route with a zero Label2 ﬁeld and no Route Target
...
When a packet from TS2 to TS4 arrives at PE1, a longest preﬁx match on IP-VRF1's route table
...
asymmetric forwarding modes.
 ...
 for more information about interoperability between symmetric and

The choice between symmetric or asymmetric mode is based on the operator's preference, and it
is a trade-oﬀ between scale (which is better in the symmetric IRB mode) and control plane
simplicity (asymmetric IRB mode simpliﬁes the control plane). In cases where a tenant has hosts
for every subnet attached to all (or most of) the PEs, the ARP and MAC entries need to be learned
by all PEs anyway; therefore, the asymmetric IRB mode simpliﬁes the forwarding model and
...
The Length ﬁeld of the BGP EVPN Network Layer Reachability Information (NLRI) for an
EVPN MAC/IP Advertisement route
the IPv6 address is carried).

 be either 40 (if the IPv4 address is carried) or 52 (if

MUST

•

The Route Distinguisher (RD), Ethernet Segment Identiﬁer, Ethernet Tag ID, MAC Address
Length, MAC Address, IP Address Length, IP Address, and MPLS Label1 ﬁelds
[RFC7432]
...
The MPLS Label2 ﬁeld is set to either an MPLS label or a VNI corresponding to the tenant's IP-
VRF. In the case of an MPLS label, this ﬁeld is encoded as 3 octets, where the high-order 20 bits
contain the label value.

[RFC7432]

Just as in
Length, and IP Address ﬁelds are part of the route key used by BGP to compare routes. The rest of
the ﬁelds are not part of the route key.

, the RD, Ethernet Tag ID, MAC Address Length, MAC Address, IP Address
...
This route is advertised with one or more Encapsulation Extended Communities
...
advertised. This extended community speciﬁes the MAC address to be used as the inner
...
tenant's subnet and another corresponding to the tenant's IP-VRF.
 ...
 be advertised with two Route Targets, one corresponding to the MAC-VRF of the
...
If the MPLS Label2 ﬁeld is non-zero, it means that this route is to be used for symmetric IRB, and
...
ﬁeld, then the receiving PE installs the MAC address in the corresponding MAC-VRF and the (IP,
MAC) association in the ARP table for that tenant (identiﬁed by the corresponding IP-VRF Route
Target).
...
MAC/IP Advertisement route includes the MPLS Label2 ﬁeld but the receiving PE only supports
asymmetric IRB mode, then the receiving PE
 ignore the MPLS Label2 ﬁeld and install the
MUST
MAC address in the corresponding MAC-VRF and (IP, MAC) association in the ARP table for that
tenant (identiﬁed by the corresponding IP-VRF Route Target).
...
provisioned at all PEs participating in an EVPN IRB instance, PEs
...
That is, if a given host's (IP, MAC) association is unknown, and an ingress PE needs to send a
...
procedure, a new gleaning procedure
...
learn subnet B's preﬁx route via EVPN RT-5 advertised from PE2 and PE3, so it can route the
packet to one of the PEs that have subnet B locally attached. Once the packet is received at PE2
OR PE3, and the route lookup yields a glean result, an ARP request is triggered and ﬂooded across
...
an IP-VRF
 ...
 typically be achieved via provisioning-connected route redistribution to BGP.

 be triggered for host B's (IP, MAC) to be learned and
...
IP lookup in the associated IP-VRF table. The lookup identiﬁes the BGP next hop of the egress PE
...
This VNI value from the received label2
IP-VRF as all PEs in the NVO
 be conﬁgured with the same IP-VRF VNI for this mode of
operation. If the received label2 value does not match the locally conﬁgured VNI value, the
route

 be used, and an error message

 be the same as the locally conﬁgured VNI for the
...
PEs may be conﬁgured to operate in one of these two modes depending on the administrative
...
lookup needs to be performed. If the VPN MPLS label or VNI identiﬁes a MAC-VRF instead of an IP-
VRF, then the procedures in Section 6.4 for asymmetric IRB are executed.

The lookup in the IP-VRF identiﬁes a local adjacency to the IRB interface associated with the
...
The Length ﬁeld of the BGP EVPN NLRI for an EVPN MAC/IP Advertisement route
...
The RD, Ethernet Segment Identiﬁer, Ethernet Tag ID, MAC Address Length, MAC Address, IP
Address Length, IP Address, and MPLS Label1 ﬁelds
...
The MPLS Label2 ﬁeld

MUST NOT

 be included in this route.
...
with a second Route Target corresponding to the IP-VRF.
 ...
 always be advertised with the MAC-VRF Route Target. It
...
Using the MAC-VRF Route Target, it identiﬁes the corresponding MAC-VRF and imports the
MAC address into it. For asymmetric IRB mode, it is assumed that all PEs participating in a
tenant's VPN are conﬁgured with all subnets (i.e., all VLANs) and corresponding MAC-VRFs/
...
Using the MAC-VRF Route Target, the receiving PE identiﬁes the corresponding ARP table or
...
receiving PE is identiﬁed by all the MAC-VRF Route Targets for that tenant.
...
If the receiving PE receives the MAC/IP Advertisement route with the MPLS Label2 ﬁeld but the
 ignore the MPLS
receiving PE only supports asymmetric IRB mode, then the receiving PE
Label2 ﬁeld and install the MAC address in the corresponding MAC-VRF and (IP, MAC) association
in the ARP table or NDP cache for that tenant (with the IRB interface identiﬁed by the MAC-VRF).
...
ingress PE deduces that the packet must be inter-subnet routed. Hence, the ingress PE performs an
IP lookup in the associated IP-VRF table. The lookup identiﬁes a local adjacency to the IRB
...
PE's MAC-VRF/ bridge table, the packet encapsulation procedures follow that of
MPLS and

 for VXLAN encapsulations.
...
forwarding described in
the packet processing associated with the inter-subnet forwarding semantics is conﬁned to the
ingress PE for asymmetric IRB mode.

 for NVO networks. In other words, all
...
[RFC7432]
 ...
 provides a diﬀerent level of granularity for the EVPN label.
...
individual MAC address identiﬁcation, then no MAC lookup is needed in the egress PE for MPLS
...
VRF and IP-VRF tables on all participating NVEs be updated.
...
the MAC Mobility for IRB. In order to place the emphasis on the diﬀerences between L2-only and
...
2. TS sends a data packet without ﬁrst initiating an ARP request to the target NVE.

3. TS is a silent host and neither initiates an ARP request nor sends any packets.

Depending on the expected TS's behavior, an NVE needs to handle at least the ﬁrst option and
...
the scope of this document, and it is covered in
...
is conﬁned to a single Ethernet segment, and it is not used for host mobility (i.e., by deﬁnition,
host mobility is between two diﬀerent Ethernet segments). Therefore, when an IPv6 host is
conﬁgured with both a Global Unicast address (or a Unique Local address) and a link-local
...
[RFC7432]
addresses ﬁlled in (per Sections 5.1 and 6.1) along with the MAC Mobility extended community,
...
then withdraws its EVPN MAC/IP Advertisement route. Furthermore, it sends an ARP probe locally
...
traﬃc without ﬁrst initiating an ARP request.

The target NVE, upon receiving the ﬁrst data packet, learns the MAC address of the TS in the data
...
If EVPN-IRB NVEs are conﬁgured to advertise MAC-only routes in addition to MAC-and-IP EVPN
...
per
 by advertising an EVPN MAC/IP Advertisement route with only the MAC
address ﬁlled in along with the MAC Mobility extended community, with the sequence
...
an EVPN MAC/IP Advertisement route with both the MAC and IP addresses ﬁlled in along with
...
the MAC Mobility extended community, with the sequence number set to the same value as
...
previously advertised EVPN MAC/IP route with both the MAC and IP address ﬁelds ﬁlled in.
...
If an EVPN-IRB NVE is conﬁgured not to advertise MAC-only routes, then upon receiving the ﬁrst
...
source NVE. It uses the local MAC route to ﬁnd the corresponding local MAC-IP route and sends a
...
procedure of triggering a unicast ARP probe at the target PE
source PE broadcast ARP probing procedure described earlier for better convergence.

 also be used in addition to the
...
neither initiates an ARP request nor sends any data traﬃc. Therefore, neither the target nor the
...
with both the MAC and IP address ﬁelds ﬁlled in along with the MAC Mobility extended
...
When the source NVE receives the EVPN MAC/IP Advertisement route, it updates its IP-VRF table
...
route with both the MAC and IP address ﬁelds ﬁlled in.
...
This document deﬁnes one new BGP Extended Community for EVPN.

8.1.  EVPN Router's MAC Extended Community

A new EVPN BGP Extended Community called "EVPN Router's MAC" is introduced here. This new
extended community is a transitive extended community with a Type ﬁeld of 0x06 (EVPN) and a
Sub-Type ﬁeld of 0x03. It may be advertised along with the Encapsulation Extended Community
deﬁned in
...
it is sent with EVPN RT-2. The advertising PE
SHALL
Extended Community to a route. In case the receiving PE receives more than one EVPN Router's
MAC Extended Community with a route, it
and propagate the others.

 process the ﬁrst one in the list and not store

 only attach a single EVPN Router's MAC
...
9.  Operational Models for Symmetric Inter-Subnet
Forwarding
...
MAC-VRF for each tenant subnet (e.g., each VLAN) that is conﬁgured for extension via VXLAN or
...
and since a TS MAC address cannot be used for inter-subnet traﬃc, the ingress NVE's MAC address
...
subnets represented by MAC-VRF1, MAC-VRF2, and MAC-VRF3 across two NVEs. There are ﬁve TSs
...
NVE2. When TS1, TS5, and TS4 exchange traﬃc with each other, only the L2 forwarding (bridging)
part of the IRB solution is exercised because all these TSs belong to the same subnet. However,
when TS1 wants to exchange traﬃc with TS2 or TS3, which belong to diﬀerent subnets, both the
...
RD and Ethernet Segment Identiﬁer (ESI) per
...
two BGP Extended Communities. The ﬁrst BGP Extended Community identiﬁes the tunnel type,
and it is called "Encapsulation Extended Community" as deﬁned in
BGP Extended Community includes the MAC address of the NVE (e.g., MACx for NVE1 or MACy for
NVE2) as deﬁned in Section 8.1. The EVPN Router's MAC Extended Community
...
If the route carries the new EVPN Router's MAC Extended Community and if the receiving
...
MUST
 ...
 use the treat-as-withdraw approach
...
actual implementation may diﬀer. Let's consider the data plane operation when TS1 in subnet-1
(MAC-VRF1) on NVE1 wants to send traﬃc to TS3 in subnet-3 (MAC-VRF3) on NVE2.
...
lookup identiﬁes the IP-VRF table. Next, an IP lookup is performed for the destination TS
...
corresponding MAC-VRF (i.e., MAC-VRF3) and, after a lookup of MAC DA, is forwarded to the
destination TS (TS3) over the corresponding interface.

In this symmetric IRB scenario, inter-subnet traﬃc between NVEs will always use the IP-VRF VNI/
MPLS label. For instance, traﬃc from TS2 to TS4 will be encapsulated by NVE1 using NVE2's IP-
...
redundancy mode, the associated NVE needs to also advertise the subnets statically conﬁgured
on each TS.

The main diﬀerence between this solution and the previous one is the additional advertisement
corresponding to each subnet. These subnet advertisements are accomplished using the EVPN IP
Preﬁx route deﬁned in
associated TS (which is in an overlay address space) as their next hop. The receiving NVEs
perform recursive route resolution to resolve the subnet preﬁx with its advertising NVE so that
they know which NVE to forward the packets to when they are destined for that subnet preﬁx.

. These subnet preﬁxes are advertised with the IP address of their

[RFC9136]

The advantage of this recursive route resolution is that when a TS moves from one NVE to
another, there is no need to re-advertise any of the subnet preﬁxes for that TS. All that is needed is
...
to MAC-VRF1 on NVE2. TS1 has two subnet preﬁxes (SN1 and SN2), and TS3 has a single subnet
preﬁx (SN3). The MAC-VRFs on each NVE are associated with their corresponding IP-VRF using
their IRB interfaces. When TS4 and TS1 exchange intra-subnet traﬃc, only the L2 forwarding
(bridging) part of the IRB solution is used (i.e., the traﬃc only goes through their MAC-VRFs);
however, when TS3 wants to forward traﬃc to SN1 or SN2 sitting behind TS1 (inter-subnet traﬃc),
then both the bridging and routing parts of the IRB solution are exercised (i.e., the traﬃc goes
...
on NVE2; NVE2 then performs an IP lookup in its IP-VRF and ﬁnds an entry for SN1. The following
...
Note that in Figure 7, above, SN1 and SN2 are conﬁgured on NVE1, which then advertises each in
an IP Preﬁx route. Similarly, SN3 is conﬁgured on NVE2, which then advertises it in an IP Preﬁx
route.

9.2.1.  Control Plane Operation

Each NVE advertises a route type 5 (EVPN RT-5, IP Preﬁx route deﬁned in
subnet preﬁxes with the IP address of its TS as the next hop (Gateway Address ﬁeld) as follows:
...
IP Preﬁx Length = 0 to 32 or 0 to 128
...
It imports the IP preﬁx into its corresponding IP-VRF conﬁgured with an import RT that is one
...
When receiving the EVPN RT-2 advertisement, the receiving NVE imports the MAC/IP addresses of
the TS into the corresponding MAC-VRF and IP-VRF per Section 9.1.1. When both routes exist,
recursive route resolution is performed to resolve the IP preﬁx (received in EVPN RT-5) to its
...
The following description of the data plane operation describes just the logical functions, and the
actual implementation may diﬀer. Let's consider the data plane operation when a host in SN1
behind TS1 wants to send traﬃc to a host in SN3 behind TS3.
...
The Ethernet header of the packet is stripped, and the packet is fed to the IP-VRF, where an IP
lookup is performed on the destination address. This lookup yields the ﬁelds needed for
...
The security considerations for Layer 2 forwarding in this document follow those of
MPLS encapsulation and those of
describes additional considerations.

[RFC7432]
 for VXLAN or NVGRE encapsulations. This section
...
, with the exception of the application of routing protocols between CEs and PEs.

This document describes a set of procedures for inter-subnet forwarding of tenant traﬃc across
PEs (or NVEs). These procedures include both Layer 2 forwarding and Layer 3 routing on a packet-
by-packet basis. The security consideration for Layer 3 routing in this document follows that of
[RFC4365]
Contrary to
CEs and PEs but rather considers the CEs as TSs or VAs that do not run dynamic routing protocols.
This can be considered a security advantage, since dynamic routing protocols can be blocked on
the NVE/PE ACs, not allowing the tenant to interact with the infrastructure's dynamic routing
protocols.
...
The VPN scheme described in this document does not provide the quartet of security properties
mentioned in
and replay protection). If these are desired, they must be provided by mechanisms that are
outside the scope of the VPN mechanisms.

 (conﬁdentiality protection, source authentication, integrity protection,

[RFC4365]

In this document, the EVPN RT-5 is used for certain scenarios. This route uses an Overlay Index
that requires a recursive resolution to a diﬀerent EVPN route (an EVPN RT-2). Because of this, it is
worth noting that any action that ends up ﬁltering or modifying the EVPN RT-2 route used to
convey the Overlay Indexes will modify the resolution of the EVPN RT-5 and therefore the
forwarding of packets to the remote subnet.

11.  IANA Considerations

IANA has allocated Sub-Type value 0x03 in the "EVPN Extended Community Sub-Types" registry as
follows:
...
This document has been listed as an additional reference for the MAC/IP Advertisement route in
...
Krattiger, L., Ed. Sajassi, A., Ed. Thoria, S. Rabadan, J.
Interoperability Modes" Work in Progress Internet-Draft, draft-ietf-bess-evpn-
modes-interop-00
<https://datatracker.ietf.org/doc/html/draft-ietf-
.
bess-evpn-modes-interop-00>
...
Malhotra, N., Ed. Sajassi, A. Pattekar, A. Rabadan, J. Lingala, A.

J.
,
Drake "Extended Mobility Procedures for EVPN-IRB" Work in Progress Internet-
Draft, draft-ietf-bess-evpn-irb-extended-mobility-07
<https://
.
datatracker.ietf.org/doc/html/draft-ietf-bess-evpn-irb-extended-mobility-07>
...
Rosen, E. "Applicability Statement for BGP/MPLS IP Virtual Private Networks
(VPNs)" RFC 4365 DOI 10.17487/RFC4365
.
editor.org/info/rfc4365>
...
,
U. Elzur, Ed. "Generic Protocol Extension for
...
Neeraj Malhotra
Krattiger
Linda Dunbar Florin Balus Yakov Rekhter Wim Henderickx Lucy Yong
,
,
feedback and contributions.

 for their valuable comments. The authors would also like to thank
 for their

Sami Boutros Jeﬀrey Zhang Krzysztof Szarkowicz Lukas
,
