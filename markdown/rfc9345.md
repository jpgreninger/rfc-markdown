...
the server (a CA)
. This organizational separation makes the (D)TLS server
...
allows the server to use modern signature algorithms such as Ed25519
...
. Figure 1 shows the intended deployment architecture.
certificate
...
July 2023
 ...
 issue credentials with a validity period longer than the maximum validity period or that
NOT
...
[XPROT]
 ...
 that certificates in use by servers that support outdated protocols such as
...
). In order to reduce the risk of cross- protocol attacks on
...
). This prevents them from being used with other,
...
mechanisms with (D)TLS 1.3
 ...
 being one example). These mechanisms, however, incur per-
...
These two mechanisms can be complementary. A server could use delegated credentials for
...
, reduces the risk of key compromise but
...
). (All certificates compatible with TLS 1.3
...
. This is expected to be the same as the sender's

SignatureScheme is as defined in
CertificateVerify.algorithm (as described in Section 4.1.3).
When using RSA, the public key
...
rsa_pss_rsae_sha384, and rsa_pss_rsae_sha512.
 ...
 use the rsaEncryption OID. As a result, the
...
MUST NOT
 ...
 send a delegated credential; if the extension is not

If the extension is present, the server
present, the server
less than 1.3, the server
...
unsuitable for the connection.
 ...
 send a delegated credential. When a (D)TLS version negotiated is
 ignore this extension. An example of when a server could choose

MUST

 send the delegated credential as an extension in the CertificateEntry of its end-

The server
MUST
entity certificate; the client
certificate, and
...
MUST
 ...
 be of a type advertised by the client in the "signature_algorithms"

The algorithm field
extension of the ClientHello message, and the dc_cert_verify_algorithm field
 be of a type
advertised by the client in the SignatureSchemeList; otherwise, the credential is considered not
valid. Clients that receive non-valid delegated credentials
"illegal_parameter" alert.

 terminate the connection with an
...
4.1.2.  Client Authentication
 ...
 send a "delegated_credential" extension in the
...
indicated support in its CertificateRequest, then the server
...
MUST
 ...
 be of a type advertised by the server in the "signature_algorithms"

The algorithm field
extension of the CertificateRequest message, and the dc_cert_verify_algorithm field
 be of a
type advertised by the server in the SignatureSchemeList; otherwise, the credential is considered
not valid. Servers that receive non-valid delegated credentials
...
servers that receive non-valid delegated credentials
"illegal_parameter" alert.
...
the validity of the original certificates. The lifetime of the delegated credential should be set
...
hardware security modules.
 ...
 be used to protect it, including file system controls, physical security, or
...
they
 also cache the associated delegated credential and re-validate it. Failing to do so
...
forging an RSA signature over an arbitrary message
. The TLS 1.2 specification describes a
...
due to the complexity of this mitigation
. For (D)TLS 1.2 servers that support RSA key
...
Furthermore, server operators may use elliptic curve certificates for DC-enabled traffic, while
using RSA certificates without the DelegationUsage certificate extension for non-DC traffic; this
completely prevents such attacks.

Note that if a signature can be forged over an arbitrary credential, the attacker can choose any
value for the valid_time field. Repeated signature forgeries therefore allow the attacker to create
multiple delegated credentials that can cover the entire validity period of the certificate.
Temporary exposure of the key or a signing oracle may allow the attacker to impersonate a
server for the lifetime of the certificate.
...
Specification of basic notation" ISO/IEC 8824-1:2021
...
Encoding Rules (DER)" ISO/IEC 8825-1:2021
rec/T-REC-X.690>
...
Bleichenbacher, D. "Chosen Ciphertext Attacks against Protocols Based on RSA
Encryption Standard PKCS #1" Advances in Cryptology -- CRYPTO'98, LNCS vol.
...
N. Sullivan "An Analysis of TLS Handshake Proxying" IEEE
...
Tuecke, S. Welch, V. Engert, D. Pearlman, L.
Public Key Infrastructure (PKI) Proxy Certificate Profile" RFC 3820 DOI
...
Boeck, H. Somorovsky, J.
<https://
Threat (ROBOT)" 27th USENIX Security Symposium
www.usenix.org/conference/usenixsecurity18/presentation/bock>
.

C. Young "Return Of Bleichenbacher's Oracle
...
Jager, T. Schwenk, J.
J. Somorovsky "On the Security of TLS 1.3 and QUIC
Against Weaknesses in PKCS#1 v1.5 Encryption" Proceedings of the 22nd ACM
...
extension. The ASN.1 module makes imports from
