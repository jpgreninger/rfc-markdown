...
[RFC7340]
 ...
 is a token format based on JSON Web Token (JWT)
...
communications established via a protocol like SIP
Revisited (STIR) problem statement
...
automaton to determine whether and how to alert a called party to a call and whom is calling.
 ...
 declared securing the display name of callers
...
in scenarios where there may be non-authoritative users initiating and signing RCD, therefore
limiting a PASSporT and the RCD claims it asserts via certificate-level controls.
...
This document defines Rich Call Data (RCD), which is a PASSporT extension
 that
...
originates a call, a service provider acting on behalf of a caller, or when third-party services may
 has been
be authoritative over the RCD on behalf of the caller. In general, PASSporT
...
. There are many SIP-specific references and

 is with the SIP protocol
...
with "display-name" in SIP
presentation numbers often used in the From header field
header field
that is defined as part of the jCard definitions or extensions to that data.
...
, or an icon associated with the caller. The second category is a set of RCD
 describes the

, typically a textual description of the caller, or alternate
...
identity description in the previous paragraph),
...
allows for a direct or indirect party that (a) enforces the policies to approve or certify the
content, (b) creates a cryptographic digest that can be used to validate that data, and (c) applies a
constraint in the certificate to allow the recipient and verifier to validate that the specific
content of the RCD is as intended at its creation and its approval or certification.
...
There are two mechanisms that are defined to accomplish that for two distinct categories of
...
specification
called JWT Claim Constraints, defined in
...
. The second of the mechanisms uses the capability
[RFC8226]
...
and SIP identity header
where the signer is authoritative, and an "rcd" claim contents include a URI identifying external
resources. In this mode, an RCD Integrity or "rcdi" claim
 be included. This integrity claim
...
be comprehensively validated that it was received as intended by the signer of the PASSporT.
 ...
 procedures. The second mode is an extension of the first
...
the ability, in particular when delegating signing authority for PASSporT, for agreed or vetted
content to be included in or referenced by the RCD claim contents. The primary framework for
allowing the separation of authority and the signing of PASSporTs by non-authorized entities is
...
detailed in
third and fourth modes differ with the absence or inclusion of referenced external content using
URIs.

, although other cases may apply. As with the first and second modes, the
...
header field value of a SIP request
field value
, or a similar field in other PASSporT using protocols. This key
included once as part of the "rcd" claim value JSON object. The key syntax of "nam"
the display-name ABNF given in
the claim value

 be
 follow
. If there is no string associated with a display name,

 or alternatively of the P-Asserted-Identity header
...
), or alternatively of the Additional-Identity header field value

 be included once as part of the
...
contained in an "rcd" PASSporT or "rcd" claims. The use of the "apn" key is intended as an
alternative to conveying the presentation number in the "tel" key value of a jCard, in situations
where no other rich jCard data needs to be conveyed with the call. Only one "apn" key may be
present. "apn"
 be used when it is the intent of the caller or signer to display the alternate
presentation number even if "jcd" or "jcl" keys are present in a PASSporT with a "tel" key value.
...
[RFC3261]
When being used for SIP
field with a purpose parameter value of "icon" as described in
example:

, this claim key value is used to protect the Call-Info header
...
Note that
 extends the specific usage of "icon" in SIP in the context of the larger rich
...
, there are alternative ways of including photos and logos as HTTPS
and 5.1.5) and in
...
information. The "jcd" key is optional. Either a "jcd" or "jcl"
 appear in the "rcd" claim, but
not both.

MAY

The jCard object value for "jcd"
content, but that URI-referenced content
[RFC9796]
may extend this capability, but
and the integrity of the content referenced by URIs.

MUST

 be a jCard JSON object that

MAY

 have URI-referenced

 further reference URIs. Future specifications
MUST NOT
 constrains the security properties of RCD information

[RFC9796]

Note: Even though we refer to
for usage in "rcd" claims, using the Call-Info header field with RCD information in a
SIP request beyond the use of identity header as defined in this document is not
required. Identity header fields are generally encouraged for all transport of
authenticated and protected RCD information over Network-to-Network Interfaces

 as the definition of the jCard properties
...
(NNIs) between untrusted parties or over untrusted networks. The use of Call-Info
header fields to carry RCD information as defined in
 is suggested for use
in trusted networks relationships, generally for User-Network Interfaces (UNIs).
...
MUST
 ...
 use the media type for JSON text as application/json with a default

The "jcl" key value is an HTTPS URL that refers to a jCard JSON object
The web server
encoding of UTF-8
defined in
and properties used should follow the normative usage and formatting rules and procedures.
The "jcl" key is optional. The "jcd" or "jcl" keys
be mutually exclusive.

. This link may correspond to the Call-Info header field value

 with a type of "jcard". As also defined in

 only appear once in the "rcd" claim but
...
, the format of the jCard

 on a web server.

MUST

The jCard object referenced by the URI value for "jcl"
URI-referenced content, but that URI-referenced content
Future specifications may extend this capability, but
properties of RCD information and the integrity of the content referenced by URIs.

[RFC9796]

MAY
 further reference URIs.

 be a jCard JSON object that

 constrains the security
...
The "rcdi" claim is included for the second and fourth modes described in the integrity overview
 each appear once in a PASSporT, but if "rcdi" is included,
(Section 4). "rcdi" and "rcd" claims
the "rcd"
defined as follows.

 be present correspondingly. The value of the "rcdi" claim is a JSON object that is

MUST

MAY

The claim value of the "rcdi" claim key is a JSON object with a set of JSON key/value pairs. Each
"rcdi" claim key/value pair corresponds to each of the elements of the "rcd" claim object that
require integrity protection with an associated digest over the content referenced by the key
string. The individual digest of different elements of the "rcd" claim data and URI-referenced
external content is kept specifically separate to allow the ability to verify the integrity of only the
elements that are ultimately retrieved, downloaded, or rendered to the end user.

[RFC6901]

 with a minor additional rule to support URI references to external content
...
digest. RCD implementations
 support the hash algorithms SHA-256, SHA-384, and SHA-512.
...
support additional recommended hash algorithms in
algorithms with "Yes" in the "Recommended" column of the IANA registry. Hash algorithm
identifiers
character following the algorithm string
MUST
subsequent characters are the base64 encoded
...
the determination of the input string used to determine the digest.
 ...
 be a hyphen character ("-", %x2D). The
[RFC4648]

 use only lowercase letters, and they

 contain hyphen characters. The

 digest of a canonicalized and
...
object claim value. This document defines the use of JSON pointer
...
completely deterministic at three points in the process. First, it must be deterministic at the
certification point when the content is evaluated to conform to the application policy and when
the JWT Claim Constraints are applied to the certificate containing the digest. Second, when the
call is signed at the Authentication Service, there may be a local policy to verify that the
provided "rcd" claim corresponds to each digest. Third, when the "rcd" data is verified at the
verification service, the verification is performed for each digest by constructing the input digest
string for the element being verified and referenced by the JSON pointer string.
...
procedures of this deterministic JSON serialization are defined in
...
references externally linked content, there
 be a JSON pointer and digest entry for the
MUST
...
, zero-based array indices are used to reference the URI strings.
...
The use of a JSON pointer and integrity digest for the "jcd" claim key and value is optional. The
"jcd" value is the directly included jCard array; it can be protected by the signature and can be
constrained directly with JWT Claim Constraints. However, for data length reasons (as with "icn"
...
value in JWT Claim Constraints over the jCard data is
...
Each URI referenced in the jCard array string
key and digest value.

MUST

 have a corresponding JSON pointer string

6.1.4.  "jcl" Elements

In the case of the use of a "jcl" URI reference to an external jCard, the procedures are similar to
"jcd" with the minor modification to the JSON pointer, where "/jcl" is used to refer to the external
jCard array string. Also, any following numeric array indices added to the "jcl" (e.g., "/jcl/1/2/3")
are treated as if the external content referenced by the jCard were directly part of the overall
"rcd" claim JSON object. The following example illustrates a "jcl" version of the above "jcd"
example.
...
The "rcdi"
and each URI referenced in the referenced jCard array string
pointer string key and digest value.

 have a "/jcl" key value and digest value to protect the referenced jCard object,
...
The "permittedValues" for the "rcd" claim
 contain
...
use different sets of rich call data corresponding to different call scenarios.
 ...
 contain a single entry or optionally
...
Section 8
. It should be recognized that the "rcdi" set of digests is
...
authentication service being performed by a non-authoritative party. For example, this may be
used with delegate certificates [RFC9060] for the signing of calls by the calling party directly,
...
For the cases where both "rcd" and "rcdi" claims should always be included in the PASSporT, the
certificate JWT Claims Constraint extension
...
a single string that can contain information as defined in
...
certificate issuer using JWT Claim Constraints in the certificate. When the signer of the PASSporT
...
match the contents of the allowed strings and is
 to be included by the certificate
issuer.
...
MAY
 ...
 appear in any PASSporT claims object as optional elements. The
[RFC8225],
MUST

The "rcd" or "crn" claims
 also add a PASSporT extension ("ppt") value, defined in
creator of a PASSporT
Section 8.1
 contain
, of "rcd" to the header of a PASSporT. In that case, the PASSporT claims
at least one "rcd" or "crn" claim (or both). Any entities verifying the PASSporT claims defined in
this document are required to understand the PASSporT extension in order to process the
...
A verifier that successfully verifies a PASSporT that contains an "rcd" claim
...
It abides by JWT Claims Constraint rules defined in
[RFC9118]
...
criteria is not met, relying parties

MUST NOT

 use any of the claims in the PASSporT.
...
digest string that doesn't match a generated digest
verification of the content referenced by the JSON pointer.

 be considered a failure of the

MUST

If there is any issue with completing the integrity verification procedures for referenced
external content, including HTTP or HTTPS errors, the referenced content
not verified. However, this
SHOULD NOT
claims content that is directly included in the claims of the PASSporT.

 impact the result of base PASSporT verification for
...
dereference a URI from the "rcd" claim for display to the end user is
...
In the following PASSporT example, a jCard is linked via HTTPS URL using "jcl", and a jCard file
is served at a particular URL.
...
An example "rcd" PASSporT that uses "nam" and "icn" keys with "rcdi" for calling name and
...
the string should be used; otherwise, the string should be an empty string, e.g., "". The claims
"jcl" and "jcd"
reference rules defined in this document would lead a set of constraints that is too restrictive.

 be used with compact form because the integrity rules and URI
...
Future specifications may revisit this to propose a consistent and comprehensive way of
addressing integrity and security of information and to provide specific guidance for other
protocol usage.
...
parameter of a Call-Info header as defined by
...
itself, and as such its RCD
assures verification services that the calling party number is not spoofed. A third-party
PASSporT is intended to be used in cases when the originating side does not supply a display-
name for the caller, so instead some entity in the call path invokes a third-party service to
provide rich caller data for a call.

 be used when a call lacks a first-party PASSporT that
...
A very similar flow could be followed by an intermediary closer to the origination of the call.
...
parties. When those elements are present, they
...
) used to sign STIR calls; it is instead a certificate that
identifies the source of the "rcd" data. How those credentials are issued and managed is outside
the scope of this document; however, the value of "iss"
 reflect the Subject of the certificate
...
contains to a call that should have no such assurance. Following the rules of
...
particular if there are multiple identity headers (as in the case of the inclusion of an "rcd" and
...
A set of "rcd" claims can be provided by either first-party providers that are directly authorized
to sign PASSporTs in the STIR ecosystem or third-party providers that are indirectly or delegated
...
This section documents SIP-specific usage for "rcd" PASSporTs in the SIP Identity header field
value. Other protocols using PASSporT may define their own guidance for "rcd" PASSporTs.

12.1.  Authentication Service Behavior for SIP Protocol

An authentication service creating a PASSporT containing an "rcd" claim
PASSporT extension ("ppt" value) of "rcd". Third-party authentication services following the
 include a PASSporT extension value of "rcd". If the PASSporT
behavior in Section 10.1
extension does contain an "rcd", then any SIP authentication services
 add a PASSporT
...
This document assumes that by default when using the SIP protocol, an authentication service
...
decide how it populates the value of the "nam" key, which
 also match or be determined by
...
equivalent to the From header field display-name value, it
...
claim types used.
 ...
 defines a Call-Info header field that

 be used as a source of RCD

MAY

RECOMMENDED

 to follow a trust framework that is out of scope of

Note also that, as a best practice, the accuracy and legitimacy of Rich Call Data information that
is included in the claims is
...
from the From header field value, if any, and
 use that as the string value for the "nam" key
MUST
when it recomputes the header and claims of the PASSporT object. Additionally, if there exists a
Call-Info header field as defined in
...
If the PASSporT is in full form with a PASSporT extension value of "rcd", then the verification
 extract the value associated with the "rcd" claim "nam" key in the object. If the
service
PASSporT signature is verified successfully, then the verification service
...
compare the string value of the "rcd" claim "nam" key value with the From header field value or
...
.
Similarly, "jcd", "jcl", "icn", "apn", or "crn" elements can be used optionally (based on local policy
[RFC9796]
for devices that support it) to populate a Call-Info header field following the format of
.
If PASSporT RCD claims types defined in the future are present, they should follow similar
...
required to support the security properties defined in
...
, which is the PASSporT extension supporting the Signature-based

In the case of
Handling of Asserted information using toKENs (SHAKEN) specification
...
For example, the PASSporT claims for the "shaken" PASSporT with "rcd" claims would be as
follows:
...
All of these data elements would benefit from the secure attestations provided by the STIR and
...
[RFC9118]
The use of JWT Claim Constraints, a mechanism defined in
,
...
As general guidance, the use of URLs and URIs that reference potentially dangerous or
intentionally harmful content should be considered in implementing this specification.
[RFC3986], Section 7
...
is important to note that when constraining PASSporTs to include specific claims or contents of
claims, it is also important to consider potential attacks by non-authorized signers that may
include other potential PASSporT claims that weren't originally vetted by the authorized entity
[RFC9118]
providing the delegate certificate. The use of JWT claims constraints (as defined in
)
for preventing the ability to include claims beyond the claims defined in this document may
...
Wendt, C.
M. Barnes "Personal Assertion Token (PaSSporT) Extension for
Signature-based Handling of Asserted information using toKENs (SHAKEN)"
,
RFC 8588 DOI 10.17487/RFC8588
<https://www.rfc-editor.org/info/
, May 2019,
rfc8588>
...
Housley, R. "Enhanced JSON Web Token (JWT) Claim Constraints for Secure
Telephone Identity Revisited (STIR) Certificates" RFC 9118 DOI 10.17487/
RFC9118
...
J. Peterson "SIP Call-Info Parameters for Rich Call Data" RFC
...
Alliance for Telecommunications Industry Solutions "Signature-based

Handling of Asserted information using toKENs (SHAKEN)" ATIS-1000074.v003
August 2022,
...
J. Peterson "Secure Telephone Identity Revisited (STIR) Out-of-
, February
...
3GPP "IP multimedia call control protocol based on Session Initiation

,

Protocol (SIP) and Session Description Protocol (SDP); Stage 3" 16.7.0 3GPP TS
24.229
...
Braun, F., Ed. "Subresource Integrity" W3C First Public Working
...
Ben Campbell Jack Rickard Jordan Simpson

David Hancock Robert Sparks Russ Housley Eric Burger Alec Fenichel

,
 for helpful suggestions, review, and comments.
