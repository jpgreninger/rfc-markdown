...
RFC 9485
I-Regexp: An Interoperable Regular Expression
Format

Abstract

This document specifies I-Regexp, a flavor of regular expression that is limited in scope with the
goal of interoperation across many different regular expression libraries.
...
I-Regexp supports the entire repertoire of Unicode characters (Unicode scalar values); both the I-
Regexp strings themselves and the strings they are matched against are sequences of Unicode
scalar values (often represented in UTF-8 encoding form
...
, but was unable to find a useful specification for regular
...
MUST
 ...
 conform to the ABNF specification in Figure 1.
...
As an additional restriction, charClassExpr is not allowed to match [^], which, according to this
...
implementation effort. Implementations

. Exceptions to this rule may be made for low-effort implementations that map I-

 document whether or not they are checking.
...
Boolean results as specified in
...
. Implementations that interpret I-Regexps
...
Note that the semantics of \d in XSD regular expressions is that of \p{Nd}; however, this would
...
Any I-Regexp is also an XSD regexp

[XSD-2]

, so the mapping is an identity function.
...
. XSD 1.1 is less widely implemented than XSD 1.0, and

; therefore, it is also included in
...
To obtain a valid regexp in Perl Compatible Regular Expressions (PCRE)
programming language's RE2 regexp library
...
3 ("UTF-8 definition" )
 ...
 applies to implementations. Particular note needs to be taken of the last
...
considerations in mind (e.g.,
). In any case, a checking implementation is still
.
RECOMMENDED
...
Peterson, D., Ed. Gao, S., Ed. Malhotra, A., Ed. Sperberg-McQueen, C. M., Ed.
,
,
P. Biron, Ed. "W3C XML Schema Definition Language
Thompson, H., Ed.
(XSD) 1.1 Part 2: Datatypes" W3C REC REC-xmlschema11-2-20120405 W3C REC-
...
Biron, P., Ed.
Edition" W3C REC REC-xmlschema-2-20041028 W3C REC-
...
Ecma International "ECMAScript 2020 Language Specification" Standard
ECMA-262, 11th Edition
content/uploads/ECMA-262.pdf>

<https://www.ecma-international.org/wp-
...
expressions for JSON" Work in Progress Internet-Draft, draft-ietf-jsonpath-
base-20
jsonpath-base-20>
...
"Perl-compatible Regular Expressions (revised API: PCRE2)" <http://pcre.org/
current/doc/html/>
...
,
commit 73031bb <https://github.com/google/re2>
...
Unicode, Inc. "Glossary of Unicode Terms" <https://unicode.org/
.
...
Discussion in the IETF JSONPATH WG about whether to include a regexp mechanism into the
JSONPath query expression specification and previous discussions about the YANG pattern and
Concise Data Definition Language (CDDL) .regexp features motivated this specification.

The basic approach for this specification was inspired by "The I-JSON Message Format"
[RFC7493]
.
