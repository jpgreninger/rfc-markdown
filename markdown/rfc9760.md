...
require that a receiving node as discussed in

IEEE 1588-2019 [IEEE1588-2019]
...
identities of the time sources in the network. This document describes clock roles and PTP Port
states using the optional alternative terms "timeTransmitter" instead of "master" and
"timeReceiver" instead of "slave", as defined in the
[IEEE1588-2019]
.
...
, Subclause 9.3), a mechanism that

 follow, sets up strict rules for all members of a PTP domain to

 be the active reference time source (Grandmaster). Although the

The "Best TimeTransmitter Clock Algorithm" (
all participating PTP Nodes
...
, Subclause 20.3). These constructs allow
...
A PTP timeTransmitter Clock that is not the Best timeTransmitter
...
Contains the properties of a given timeTransmitter Clock. The information
...
, a 64-bit number assigned to each PTP clock. This number
...
A mechanism in PTP for timeReceiver Clocks to negotiate unicast
...
[RFC8200]
 ...
 operate only in networks characterized by UDP
, as described by Annexes C and D of

This PTP Profile
[RFC0791]
A network node
MAY
instances in the same network node
...
different IP versions.
 ...
 include multiple PTP instances running simultaneously. IPv4 and IPv6
 operate in different PTP domains. PTP clocks that
...
The PTP system
Boundary Clocks, or neither, in any combination. PTP clocks
Ordinary Clocks, or Boundary Clocks. The Ordinary Clocks may be timeReceiver Only Clocks or
may be timeTransmitter capable.

 include switches and routers. These devices
MAY

 be Transparent Clocks,
MAY
 be Preferred timeTransmitters,
...
TimeTransmitter Clocks, Transparent Clocks, and Boundary Clocks
or two-step clocks. TimeReceiver Clocks
measurement method

MAY
 support both behaviors. The End-to-End delay

 be used.

 be either one-step clocks
...
wherever possible, the network
 be engineered so that the forward and reverse routes
...
MUST
 ...
 be sent as PTP event multicast messages (UDP port 319) to the PTP primary

Sync messages
IP address. Two-step clocks
(UDP port 320). Announce messages
...
address options are explained in
allotted by IANA; see the

 send Follow-up messages as PTP general multicast messages
 be sent as PTP general multicast messages (UDP port

"IPv6 Multicast Address Space Registry" [IPv6Registry]
.

, Annex D, Section D.3. These addresses are
...
Response PTP general messages. TimeTransmitter Clocks
...
Clocks.
 ...
 be sent as either multicast or unicast PTP event messages.
 respond to multicast Delay Request messages with multicast Delay
 respond to unicast Delay Request

MUST

SHOULD

 include support for multiple domains. The purpose is to support multiple
...
timeTransmitter Clocks, redundant sources of timing can be evaluated as an ensemble and/or
...
Whenever feasible, the underlying network transport technology
...
The Sync, Announce, and Delay Request default message rates
The Sync and Delay Request message rates
 be set to other values, but not less than once
...
other timeTransmitters.
 ...
 be changed from the default value. The Announce Receipt Timeout Interval

 each be once per second.
...
simultaneous Grandmasters if each active Grandmaster is operating in a different PTP domain.
 ...
 obey the standard Best TimeTransmitter Clock Algorithm as

. PTP systems using this PTP Profile
...
A PTP Port of a clock
value for the number of UTC leap seconds.

MUST NOT

 be in the timeTransmitter state unless the clock has a current

If a unicast negotiation signaling message is received, it
...
SHOULD
subsystems. TimeReceiver Clocks

 make use of information from all the timeTransmitters in their clock control

 be able to function in such networks even if they use
...
time from only one of the domains. TimeReceiver Clocks
presence of a rogue timeTransmitter. TimeReceivers
...
TimeReceivers
Acceptable timeTransmitter, then the timeReceiver
 synchronize to it. Note that IEEE
1588-2019 requires timeReceiver Clocks to support both two-step and one-step timeTransmitter
Clocks. See

 use an Acceptable TimeTransmitter Table. If a timeTransmitter is not an
MUST NOT

 be able to operate properly in the
...
Transparent Clock; therefore, timeReceivers
...
Profile.
 ...
 send Delay Request messages to the IP
...
Transparent Clocks
message. For example, a Transparent Clock
...
maintain separate clock rate offsets for each of the supported domains.
 ...
 change the transmission mode of an Enterprise Profile PTP

 change a unicast message to a multicast
...
Clocks
 ...
 support multiple simultaneous PTP domains. This will require them to
...
i.e., where the targetPortIdentity.clockIdentity attribute (defined in
have all bits set to 1,
used, they
specific PTP Node.

 be sent as a unicast message. Similarly, if any signaling messages are
 also be sent as unicast messages whenever the message is intended solely for a

 be used. Management messages intended for a specific clock,
) does not
...
Clocks operating in the Enterprise Profile
...
, Annex I.3. This variant of the Default Profile uses the End-
...
will not interoperate with Default Profile clocks as defined in
...
clocks in the other PTP Profiles obey the rules of the Enterprise Profile. These rules
...
IEEE "IEEE Standard for a Precision Clock Synchronization for Networked
Measurement and Control Systems" IEEE Std 1588-2019 DOI 10.1109/IEEESTD.
2020.9120376
...
Networked Measurement and Control Systems Amendment 2: Master-Slave
Optional Alternative Terminology" IEEE Std 1588g-2022 DOI 10.1109/IEEESTD.
2023.10070440
.
...
L. Bonebakker "Challenges deploying PTPv2 in a
global financial company" Proceedings of the IEEE International Symposium
on Precision Clock Synchronization for Measurement, Control and
Communication, pp. 1-6 DOI 10.1109/ISPCS.2012.6336634
<https://www.researchgate.net/publication/
260742322_Challenges_deploying_PTPv2_in_a_global_financial_company>
...
ITU-T "Time and phase synchronization aspects of telecommunication
<https://
networks" ITU-T Recommendation G.8271/Y.1366
www.itu.int/rec/T-REC-G.8271-202003-I/en>
...
IANA "IPv6 Multicast Address Space Registry" <https://iana.org/assignments/
...
K. Harris "Plugfest" Proceedings of the IEEE International
...
Richard Cochran Kevin Gross John Fletcher Laurent Montini
,
The authors would like to thank
,
and many other members of the IETF for reviewing and providing feedback on this document.
