...
metadata. The specifications below support relationships of the following forms:
 ...
 often need to be related to each other or to associated

[RFC5545]

Structured iCalendar:

iCalendar entities can be related to each other in some structured way,

for example, as parent, sibling, before, or after.

Grouped iCalendar:

iCalendar entities can be related to each other as a group. CATEGORIES are
...
Entities can be linked to other entities, such as vCards, through a URI and associated
...
[RFC5545]
 ...
 RELATED-TO property has no support for temporal relationships as
...
As such, the presence of a REFID property imparts no meaning to the component. It is merely a
...
category.
 ...
 is used as a free-form 'tagging' field.
...
referencing additional, external information relating to calendar components.
 ...
 lacks a general purpose method for
...
, which defines the generic concept of Web Linking, as well as its expression in the
...
tasks, for example, Calendaring Extensions to WebDAV (CalDAV) calendar collections

[RFC4791]
.

It is also often necessary to reference calendar components in other collections. For example, a
VEVENT might refer to a VTODO from which it was derived. The PARENT, SIBLING, and CHILD
...
When publishing events, it is useful to be able to refer back to the source of that information. The
...
stripping of inline data and to provide additional data about the attachment (size, filename, etc.).
 ...
 be treated as just another attachment. The ATTACH property

 to handle server-side management and
...
This allows for linking within a single collection of calendar components, and the value

MUST

 refer to another component within the same collection.

XML-REFERENCE:

In an XML environment, it may be necessary to refer to a fragment of an
...
Note that UID references may need updating on import. An example is data to be imported from
a file containing VTODO and VEVENT components, with a VTODO referring to VEVENT
...
relation types are added to the "Link Relations" registry, as specified in
...
: registered and extension. Registered

Section 2.1.1 of [RFC8288]
.

, are specified as unique URIs that
...
: FINISHTOSTART, FINISHTOFINISH, STARTTOFINISH, or
...
present in the predecessor entity and will refer to the successor entity.
 ...
 RELATED-TO property with one or more of these temporal relationships will be
...
The start of Task-A (which occurs after Task-B) controls the finish of
...
This indicates that the referenced calendar component is the first in a series

the referencing calendar component is part of.

RELTYPE=NEXT:

This indicates that the referenced calendar component is the next in a series
...
This establishes a reference from the current component to components with

a REFID property that matches the value given in the associated RELATED-TO property.

RELTYPE=CONCEPT:

This establishes a reference from the current component to components
...
This parameter
 ...
 be specified on all LINK properties and define the type of
...
This property specifies the length of the gap, positive or negative, between two
...
Section 3.3.6 of [RFC5545]

. :

This parameter is defined by the following notation, where dur-value is
...
This parameter
 ...
 be specified on the RELATED-TO property and defines the
...
VALUE=XML-REFERENCE indicates that the associated value references an
...
This property is used to specify formal categories or classifications of the calendar
...
The following is an example of this property. It points to a server acting as the source
...
When used in a component, the value of this property points to additional
...
, where the link target is carried in

[RFC8288]

The LINK property parameters map to
...
The following is an example of this property, which provides a reference to the
...
The following is an example of this property, which provides a reference to an entity
...
Example: The following is an example of this property, which provides a reference to a
...
The value of this property is free-form text that creates an identifier for associated
...
This property is used to represent a relationship or reference between one calendar

component and another. The definition here extends the definition in
[RFC5545]

 by allowing URI or UID values and a GAP parameter.
...
MAY
 ...
 be specified in any iCalendar component.

Description:

By default or when VALUE=UID is specified, the property value consists of the
...
To preserve backwards compatibility, the value type
or CHILD relationships are specified.

MUST

 be UID when the PARENT, SIBLING,
...
changed in a corresponding way. Similarly, if a PARENT calendar component is canceled or
...
All of the security considerations of
...
When the value is an XML-REFERENCE type, the targeted data is an XML document or portion
thereof. Consumers need to be aware of the security issues related to XML processing -- in
particular, those related to XML entities. See
reference may be invalid or become so over time.

Section 20.6 of [RFC4918]
...
Section 8.3.2 of [RFC5545]
. IANA has also added a reference to this document, where
defined in
[RFC5545]
the properties originally defined in
...
Data Types" registry defined in

Section 8.3.4 of [RFC5545]
.
...
Miles, A.
 ...
 and
Organization System Reference" W3C Recommendation REC-skos-
,
reference-20090818
reference-20090818>
...
S. Bechhofer "SKOS Simple Knowledge

<https://www.w3.org/TR/2009/REC-skos-
...
,
,
Framework" W3C Recommendation REC-xptr-framework-20030325
, 25 March
...
, and
Scheme" W3C WD WD-xptr-xpointer-20021219
www.w3.org/TR/2002/WD-xptr-xpointer-20021219>
...
Daboo, C. Quillaud, A.
WebDAV (CalDAV): Managed Attachments" RFC 8607 DOI 10.17487/RFC8607
June 2019,

K. Murchison, Ed. "Calendaring Extensions to
...
The author would like to thank the members of CalConnect, the Calendaring and Scheduling
Consortium technical committees, and the following individuals for contributing their ideas,
...
The author would also like to thank CalConnect and the Calendaring and Scheduling Consortium
