...
Multiprotocol Label Switched (MPLS) Data-Plane Failures" (usually referred to as LSP ping), are
encapsulated in IP packets with headers that include a Router Alert Option (RAO). In actual
...
Also, this document recommends the use of an IPv6 loopback address (::1/128) as the IPv6
destination address for an MPLS echo request message.
...
ping)
 detects data plane failures in MPLS Label Switched Paths (LSPs). It can operate
...
The reader is assumed be familiar with
...
, the IP header that encapsulates an MPLS echo request message must
 also says that the IP header that

According to
include a Router Alert Option (RAO). Furthermore,
...
Therefore, this document updates RFC 8029
message encapsulations and explains why RFC 7506

[RFC8029]

 to retire the RAO from both LSP ping
...
an RAO with the option value set to "Router shall examine packet"
...
[RFC8029]
 ...
 in that Requirement 3 is removed.
...
one can argue it decreases the reliability in many instances, due to the additional burden of
 in that mode 3 is removed.
processing the RAO. This document updates RFC 8029
...
RFC 7506
and Maintenance. This document explains why RFC 7506
Historic.

 defines the IPv6 Router Alert Option for MPLS Operations, Administration,
...
packets do not have the same behavior as prescribed in
addressed packet.

 for an IPv4 loopback
...
MPLS echo request message as follows:
 ...
 regarding the selection of an IPv6 destination address for an
...
misdirected to a host, it will be silently discarded.
 ...
 appear outside a host." Thus, the default behavior of
...
[RFC1812]
 ...
 forward, except over a loopback interface, any packet that has

SHOULD NOT

A router
a destination address on network 127. A router
network manager to disable these checks. If such a switch is provided, it
default to performing the checks.

MAY

 have a switch that allows the
...
The 127/8 range for IPv4 was chosen for a number of reasons.
 ...
 allocates the 127/8 as the "Internal host loopback address" and

[RFC1122]
RFC 1122
states: "Addresses of this form
...
packet is misdirected to a host, it will be silently discarded.
 ...
 appear outside a host." Thus, the default
...
MAY
 ...
 select the IPv6 destination address from
...
[RFC6790]
 ...
 be used. For example, the MPLS Entropy Label
[RFC6438]
...
Additionally, this specification updates
section with the following text:

Section 2.2 of [RFC8029]

 to replace the whole of the
...
SHOULD
 ...
 ignore RAO options when they arrive on incoming

Resulting from the removal of the Reply mode 3 "Reply via an IPv4/IPv6 UDP packet with Router
Alert" (see Section 2.2), this specification updates
following text:
...
Alert", then the IP header

 contain the Router Alert IP Option of value 0x0
...
MUST
 ...
 for IPv4 or 69
 in this case be the Router Alert label (1) (see
...
Furthermore, this specification updates
...
LSP Ping implementations that conform to this specification
 ignore RAO options when
...
The recommendations this document makes do not compromise security. Using the IPv6
loopback address ::1/128 strengthens security for LSP ping because it is standardized and has
...
that improved the readability of the document.
