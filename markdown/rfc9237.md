...
) to a device, a compact representation format is needed.
...
; CoAP is used for the explanatory examples as it is a good fit
...
often been called a "capability list") without concern to the subject for which the data item is
issued. As a consequence, AIF
 be used in a way that the subject of the authorizations is
...
the URI that identify the server ("authority" in
resource access (
 and
...
Uri-Path and Uri-Query options in CoAP). As a consequence, AIF
 be used in a way that it is
...
[RFC3986]
Section 6.2 of [RFC7252]

) are authenticated during REST
...
In this example, a device offers a temperature sensor /s/temp for read-only access, a LED
...
unambiguity of matching a given URI against a set of templates in an AIF data item.
 ...
 (for instance, to open up an authorization for many parameter
...
Section 2.2.
 ...
 (the Location-indicating mechanisms). (The concept is
...
Since the use of the extension defined in this section can be detected by the mentioning of the
...
[RFC7252]
, identified by the method code minus 1) is shown in
CDDL

 and the new ones from
...
For the information shown in Table 1 and Figure 3, a representation in Concise Binary Object
Representation (CBOR)
improvements are possible.

 is given in Figure 5; again, several optimizations and
...
populated with the following:
 ...
 for the media-type parameters Toid and Tperm,
...
. The designated expert will engage
...
The parameter names need to conform to
[KebabCase]
...
[RFC7252]
 ...
 apply when AIF is used with CoAP;
...
An implementation of the REST-specific model with dynamic resource creation (Section 2.3)
needs to carefully keep track of the dynamically created objects and the subjects to which the
Dynamic-X permissions apply -- both on the server side to enforce the permissions and on the
client side to know which permissions are available.
...
Jim Schaad Francesca Palombini Olaf Bergmann Marco Tiloca
comments that shaped the direction of this document.
were gaps in the media type specifications, and
review with further comments. Many thanks also to the IESG reviewers, who provided several
small but significant observations.
Area Director and indeed is responsible for much improvement in the document.
...
Loganaden Velvindron
 ...
 provided an extensive review as Responsible
