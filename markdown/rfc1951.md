...
A simple counting argument shows that no lossless compression algorithm can compress every possible input data set. For the format defined here, the worst case expansion is 5 bytes per 32 K -byte block, i.e., a size increase of $0.015 \%$ for large data sets. English text usually compresses by a factor of 2.5 to 3; executable files usually compress somewhat less; graphical data such as raster images may compress much more.
...
Each block is compressed using a combination of the LZ77 algorithm and Huffman coding. The Huffman trees for each block are independent of those for previous or subsequent blocks; the LZ77 algorithm may use a reference to a duplicated string occurring in a previous block, up to 32 K input bytes before.

Each block consists of two parts: a pair of Huffman code trees that describe the representation of the compressed data part, and a compressed data part. (The Huffman trees themselves are compressed using Huffman encoding.) The compressed data consists of a series of elements of two types: literal bytes (of strings that have not been detected as duplicated within the previous 32 K input bytes), and pointers to duplicated strings, where a pointer is represented as a pair <length, backward distance>. The representation used in the "deflate" format limits distances to 32 K bytes and lengths to 258 bytes, but does not limit the size of a block, except for uncompressible blocks, which are limited as noted above.
...
![](https://cdn.mathpix.com/cropped/9dbb146e-1d45-404f-a110-7cd9a71c8126-04.jpg?height=132&width=381&top_left_y=2285&top_left_x=335)
...
![](https://cdn.mathpix.com/cropped/9dbb146e-1d45-404f-a110-7cd9a71c8126-05.jpg?height=181&width=456&top_left_y=1048&top_left_x=339)
...
![](https://cdn.mathpix.com/cropped/9dbb146e-1d45-404f-a110-7cd9a71c8126-06.jpg?height=470&width=268&top_left_y=951&top_left_x=517)
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
|  | Bits | Dist | Code | Bits | Dist | Code | Bits | Distance |
    ...
    16: Copy the previous code length 3-6 times.
        The next 2 bits indicate repeat length
            ...
            Example: Codes 8, 16 (+2 bits 11),
                    ...
                    1 2 \text { code lengths of 8 (1 + 6 + 5)}
...
A compressor may limit further the ranges of values specified in the previous section and still be compliant; for example, it may limit the range of backward pointers to some value smaller than 32 K . Similarly, a compressor may limit the size of blocks so that a compressible block fits in memory.
...
[6] Hirschberg and Lelewer, "Efficient decoding of prefix codes," Comm. ACM, 33,4, April 1990, pp. 449459.
