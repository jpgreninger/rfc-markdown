...
RFC 9154
Extensible Provisioning Protocol (EPP) Secure
Authorization Information for Transfer

Abstract

The Extensible Provisioning Protocol (EPP) (RFC 5730) deﬁnes the use of authorization
information to authorize a transfer of an EPP object, such as a domain name, between clients
that are referred to as "registrars". Object-speciﬁc, password-based authorization information
(see RFCs 5731 and 5733) is commonly used but raises issues related to the security, complexity,
storage, and lifetime of authentication information. This document deﬁnes an operational
...
EPP Secure AuthInfo for Transfer
...
[RFC5730]
 ...
 deﬁnes the use of authorization

The Extensible Provisioning Protocol (EPP)
information to authorize a transfer of an EPP object, such as a domain name, between clients
that are referred to as "registrars". The authorization information is object speciﬁc and has been
deﬁned in "Extensible Provisioning Protocol (EPP) Domain Name Mapping"
...
encrypted (not hashed) passwords, which presents a signiﬁcant latent risk of password
...
This document deﬁnes an operational practice, using the EPP RFCs, that leverages the use of
...
where the domain name object as deﬁned in
 is used in this document for illustration
...
The EPP RFCs deﬁne the password-based

authorization information value using an XML schema "normalizedString" type, so they
don't restrict what can be used in any substantial way. This operational practice deﬁnes the
...
created, which results in long-lived authorization information. This speciﬁcation changes the
...
deﬁning an operational practice for storing the authorization information securely. The
...
documents. The example namespace preﬁxes used and their associated XML namespaces include
the following:
...
actors that are involved. This document will refer to these actors as "registrant", "registrar", and
 deﬁnes these terms formally for the Domain Name System (DNS). The terms
"registry".
...
[RFC8499]
 ...
 deﬁnes the registrant as "an individual or organization on whose behalf a
...
[RFC8499]
 ...
 deﬁnes the registrar as "a service provider that acts as a go-between for
...
registrant is registrar speciﬁc. In the EPP RFCs, the registrar is referred to as the "client", since
...
[RFC8499]
 ...
 deﬁnes the registry as "the administrative operation of a zone that allows
...
This document does not deﬁne a new protocol; rather, it deﬁnes an operational practice using
...
) use password-based authorization information to

[RFC5731]
. Other EPP objects that support password-based authorization information for transfer

The EPP RFCs (
support transfer with the <domain:pw> element
[RFC5733]
can use secure authorization information as deﬁned in this document. For authorization
information to be secure, it must be generated using a strong random value and have a short TTL.
The security of the authorization information is deﬁned in the following sections.

 and with the <contact:pw> element
...
4.1.  Secure Random Authorization Information
 ...
 be generated using a secure random value.

For authorization information to be secure, it
MUST
...
machine-generated value, the implementation
value of H. The lengths below are calculated using that value.

 use at least 128 bits of entropy as the
...
The authorization information
 only be set when a transfer is in process. This implies that
the authorization information has a TTL by which the authorization information is cleared when
the TTL expires. The EPP RFCs do not provide deﬁnitions for TTL, but since the server supports the
...
on proprietary registrar-speciﬁc criteria, which provides for a transfer-speciﬁc TTL tuned for the
particular circumstances of the transaction. The sponsoring registrar will be aware of the TTL,
and the sponsoring registrar
information is provided to the registrant.

 inform the registrant of the TTL when the authorization
...
The authorization information
cryptographic hash with at least a 256-bit hash function, such as SHA-256
with a per-authorization information random salt with at least 128 bits.

 be stored by the registry using a strong one-way
[FIPS-180-4]
...
An empty authorization information value
MUST
referred to as a "NULL" value. The representation of a NULL (undeﬁned) value is dependent
on the type of database used.

 be stored as an undeﬁned value that is
...
MUST NOT
 ...
 be stored by the losing registrar.
...
MUST
 ...
 only be stored by the gaining registrar as a "transient"

5.

The plain-text version of the authorization information
registrar or the registry, nor otherwise recorded where it will persist beyond the transfer
process.

MUST NOT

 be written to any logs by a

6.

All communication that includes the authorization information
[RFC5734]
...
stored with a NULL (undeﬁned) value. Based on the requirement to store the authorization
...
authorization information value is used as input in both the
the
information in the
5.4)

 and
 to deﬁne the unset state. The matching of the authorization
<transfer> request command (Section
...
<info> command (Section 5.3)

 and the

1.

Any input authorization information value
information value. For example, in
must not match an unset authorization information value that used <domain:null/> or
<domain:pw/>.

 match an unset authorization
 the input <domain:pw>2fooBAR</domain:pw>
...
An empty input authorization information value
information value.

MUST NOT

 match any set authorization

3.

A non-empty input authorization information value
the set authorization information value, which is stored using the same hash algorithm.

 be hashed and matched against
...
The gaining registrar optionally veriﬁes the authorization information with the <info>
...
information value. Examples of such extensions are found in
 ...
 disallow the passing of a non-empty authorization information

 allow the passing of an empty authorization
...
Example of passing an empty authorization information value in a domain name <create>
command
...
information. The registrar sets the authorization information by ﬁrst generating a strong,
...
setting it in the registry in the <update> command. The importance of generating strong
 ...
 allow the setting and unsetting of the authorization
...
authorization information values cannot be overstated: secure transfers are very important to
...
authorization information values as being not suﬃciently strong. Registrars, therefore, must be
...
information value needs to be set. The registrar
and if the TTL expires, the registrar will unset the authorization information.

 deﬁne a TTL, as described in Section 4.2,
...
When the registrar-deﬁned TTL expires, the sponsoring registrar
by unsetting the authorization information value and
 add back statuses like the
...
authorization information with an "eppcom:pwAuthInfoType" element can pass an empty
 and
authorization information value. Examples of such extensions are found in
...
. Setting an empty authorization information value unsets the authorization
 supports an explicit mechanism of unsetting the authorization
...
information value for veriﬁcation. The gaining registrar can pre-verify the authorization
...
<info> command. The registry compares the hash of the passed authorization information with
 ...
 allow the passing of a non-empty authorization
...
the hashed authorization information value stored for the object. When the authorization
information is not set or the passed authorization information does not match the previously set
value, the registry

 return an EPP error result code of 2202
...
command
 ...
 to verify the authorization information value:
...
The info response in object extensions, such as those deﬁned in
...
passed in the <info> command. The registry
authorization information is set or unset to the non-sponsoring registrar by not returning the
authorization information element in the response. The registry
...
information value. The registry
authorization information is unset by not returning the authorization information element.

 return any indication of whether the

 return an indication to the
...
Example of returning an empty authorization information value in a domain name info
response
set:

 to indicate to the sponsoring registrar that the authorization information is
...
MUST
 ...
 allow the passing of a non-empty
...
does not match the previously set value, the registry
[RFC5730]
transfer occurs immediately, the registry
("Command completed successfully")
MUST
pending"). The losing registrar
<poll> message.
...
. Whether the transfer occurs immediately or is pending is up to server policy. When the
 ...
 be informed of a successful transfer request using an EPP

, and when the transfer is pending, the registry

 return the EPP success result code of 1000
...
Example of passing a non-empty authorization information value in a domain name <transfer>
request command
...
authorization information. If the transfer request is not submitted within the
the transfer is canceled or rejected, the registrar
described in Section 5.2.

TTL (Section 4.2)
 unset the authorization information, as

 automatically unset the
...
the Secure Authorization Information Model deﬁned in this document by supporting incremental
transition steps. The transition steps are dependent on the starting point of the registry. Registries
may have diﬀerent starting points, since some of the elements of the Secure Authorization
...
The transition can be handled in the three phases deﬁned in Sections 6.1, 6.2, and 6.3.
...
. IANA has assigned the following URI in the "ns" subregistry within the
...
for the Extensible Provisioning Protocol (EPP)" registry as deﬁned in
...
Section 4.1 deﬁnes the use of a secure random value for the generation of authorization
information. The client
bits of entropy.

 choose a length and set of characters that result in at least 128
...
Section 4.2 deﬁnes the use of an authorization information TTL. The registrar
authorization information during the transfer process by setting the authorization information
at the start of the transfer process and unsetting the authorization information at the end of the
transfer process. The TTL value is left up to registrar policy, and the sponsoring registrar
inform the registrant of the TTL when providing the authorization information to the registrant.
...
MUST
 ...
 store the authorization information and the gaining registrar

Section 4.3 deﬁnes the storage and transport of authorization information. The losing registrar
MUST NOT
authorization information as a "transient" value during the transfer process, where the
authorization information
MUST
and with a per-authorization information random salt with at least 128 bits. All communication
that includes the authorization information
 be over an encrypted channel. The plain-text
MUST
authorization information

 be stored after the end of the transfer process. The registry
 store the authorization information using a one-way cryptographic hash of at least 256 bits
...
Section 4.4 deﬁnes the matching of the authorization information values. The registry stores an
unset authorization information value as a NULL (undeﬁned) value to ensure that an empty
input authorization information value never matches it. The method used to deﬁne a NULL
(undeﬁned) value is database speciﬁc.
...
"NIST Federal Information Processing Standards (FIPS) Publication 140-2" DOI
<https://csrc.nist.gov/publications/detail/ﬁps/
10.6028/NIST.FIPS.140-2
.
140/2/ﬁnal>
...
,
National Institute of Standards and Technology, U.S. Department of Commerce
"Secure Hash Standard, NIST Federal Information Processing Standards (FIPS)
,
Publication 180-4" DOI 10.6028/NIST.FIPS.180-4
.
...
Bauland Martin Casanova Scott Hollenbeck Benjamin Kaduk Jody Kolker Barry Leiba Patrick
Mevzek Matthew Pozun Srikanth Veeramachaneni
,
