...
RFC 9574
Optimized Ingress Replication Solution for Ethernet
VPNs (EVPNs)
...
Virtualization Overlay (NVO) network
...
. Network Virtualization Edge (NVE) and
...
In NVO networks where PIM-based trees cannot be used, ingress replication is the only option.
...
proxy Neighbor Discovery (ND) capabilities supported by EVPNs
...
, an NVE/PE builds a flooding list for BUM traffic based on the next hops of
...
specified in
the received EVPN Inclusive Multicast Ethernet Tag routes for the BD. While
...
solutions require some extensions to the BGP attributes used in
details.
...
solution
 ...
 be used in NVO and MPLS transport networks.
...
NVEs/PEs attached to multihomed TSs, as per
...
ingress replication signaling and procedures provided in
...
An EVPN Inclusive Multicast Ethernet Tag route
...
Regular NVE. Refers to an NVE that supports the procedures provided in
...
functions:
 ...
 and has no impact on the Customer
...
Multicast Ethernet Tag routes and attributes described in
its optimized ingress replication capabilities.

 so that an NVE/PE can signal

[RFC7432]

The Network Layer Reachability Information (NLRI) of the Inclusive Multicast Ethernet Tag route
[RFC7432]
format. The PMSI Tunnel Attribute's general format as provided in
from
shown in Figure 3.
...
) is used in this document; only a new tunnel type and new flags are specified, as
...
The Flags field in Figure 3 is 8 bits long as per
[RFC7902]
document defines the use of 4 bits of this Flags field:

[RFC7902]

, and the Leaf Information Required (L) flag was allocated by

. The Extension (E) flag was allocated by
[RFC6514]
...
The L flag (bit 7) is defined in

[RFC6514]

 and will be used only in the selective AR solution.
...
route
 ...
 be advertised with the T field set to 10 (AR-LEAF).
...
is common to all the EVIs on the PE (usually this is a loopback address of the PE).
 ...
 be set to an IP address of the advertising router that

MUST

SHOULD

 be set to the same IP address as the

◦
The Tunnel Identifier and Next Hop fields
Originating Router's IP Address field when the NVE/PE originates the route -- that is,
when the NVE/PE is not an ASBR; see
...
◦
The Next Hop address is referred to as the AR-IP and
for a given PE/NVE, unless the procedures provided in Section 8 are followed.

 be different from the IR-IP
...
MUST
 ...
 be set to Assisted Replication Tunnel. Section 11 provides the allocated

•

T (Assisted Replication type)

MUST

 be set to 01 (AR-REPLICATOR).
...
MUST
 ...
 be set to 0 for non-selective AR and

MUST

 be set to 1

 advertise a Replicator-AR route for
 advertise a Regular-IR route. The advertisement of the Replicator-AR route will

An NVE/PE configured as an AR-REPLICATOR for a BD
the BD and
...
will forward an IP-encapsulated BM frame in Assisted Replication forwarding mode if the outer
...
used for selective AR, and the fields of such a route are set as follows:
 ...
 in cases where the selective AR mode is used. An AR-LEAF

 send a Leaf A-D route
...
address used by the AR-LEAF in Regular-IR routes). The Next Hop address is set to the IR-IP,
 be the same IP address as the advertising router's IP address, when the NVE/
which
.
...
this Leaf A-D route is generated.
 ...
 is the "Route Type Specific" NLRI of the Replicator-AR route for which

•

The AR-LEAF constructs an IP-address-specific Route Target, analogously to
...
MUST
 ...
 include the PMSI Tunnel Attribute with Tunnel Type set to Assisted
...
PMSI Tunnel Attribute (Flags field) of the routes and
 signal the corresponding type (AR-
...
The AR-REPLICATOR role
an AR-enabled BD. This administrative option to enable AR-REPLICATOR capabilities
implemented as a system-level option as opposed to a per-BD option.

 be an administrative choice in any NVE/PE that is part of
...
An AR-REPLICATOR
route. The AR-REPLICATOR
ACs. If the Regular-IR route is advertised, the Assisted Replication Type field of the Regular-IR
route

 advertise a Regular-IR
 generate a Regular-IR route if it does not have local

 advertise a Replicator-AR route and
...
◦
If the destination IP address is the AR-REPLICATOR AR-IP address, the node
...
remote nodes in the BD. Some of those overlay tunnels
based on the BM flag received from the remote nodes in the BD.

 be flagged as non-BM receivers
...
◦
 forward the BM
If the destination IP address matches its AR-IP, the AR-REPLICATOR
...
received from the remote nodes in the BD.
 ...
 be flagged as non-U (unknown unicast) receivers based on the U flag
...
BM traffic to an AR-REPLICATOR that can replicate the traffic further on its behalf. It
...
The AR-LEAF role
enabled BD. This administrative option to enable AR-LEAF capabilities
as a system-level option as opposed to a per-BD option.

SHOULD

 be an administrative choice in any NVE/PE that is part of an AR-
...
In this non-selective AR solution, the AR-LEAF
Multicast Ethernet Tag route as described in
...
standpoint of ease of operation and troubleshooting of the BD.
 ...
 advertise a single Regular-IR Inclusive
...
reconfigured, the AR-LEAF
Regular-IR Inclusive Multicast Ethernet Tag routes as described in
happen in the following cases:

 use regular ingress replication based on the remote
...
MAY
 ...
 select more than one AR-REPLICATOR and do either per-flow or per-BD

◦
A single AR-REPLICATOR
 be selected for all the BM packets received on the AR-LEAF
...
load balancing.
◦
In the case of failure of the selected AR-REPLICATOR, another AR-REPLICATOR
selected by the AR-LEAF.
◦
When an AR-REPLICATOR is selected for a given flow or BD, the AR-LEAF
...
tunnel). The underlay destination IP address
 be the AR-IP advertised by the AR-
...
multicast IPv4 or IPv6 packets. The AR-REPLICATORs
plane packets to other overlay tunnels, since they will use the IR-IP address.

 change the selection of AR-REPLICATOR(s) dynamically due to an

 send service-level BM control plane packets, following the

 replicate these control
...
AR-LEAF nodes is
. Upon receiving a new Replicator-AR route where the AR-
...
MUST
 ...
 follow a data path implementation compatible with the following rules:
...
◦
If the AR-REPLICATOR-set is empty, the AR-LEAF
◦
If the AR-REPLICATOR-set is NOT empty, the AR-LEAF
...
MUST
 ...
 send the packet to flooding list #2.

 send the packet to flooding list
...
ingress replication as described in
. The RNVE does not signal any AR role and is
...
The solution is called "selective" because a given AR-REPLICATOR
...
The selective AR-REPLICATOR role
part of an AR-enabled BD. This administrative option
option as opposed to a per-BD option.

SHOULD

 be an administrative choice in any NVE/PE that is
 be implemented as a system-level
...
of the "selective" administrative option, an AR-REPLICATOR
 behave as a selective
...
The selective AR-REPLICATOR
for the following differences:

MUST

 follow the procedures described in Section 5.1, except

MUST

 have the L flag set to 1 when advertising the Replicator-AR
...
the AR-REPLICATOR AR-IP address, the node
...
▪
In any other case, the AR-REPLICATOR
MUST NOT
...
LEAF-set. In any other case, the AR-REPLICATOR
...
replication to its AR-LEAFs.
 ...
 replicate the BM traffic to remote
...
A selective AR-REPLICATOR data path implementation
rules:
...
Some of the overlay tunnels in the flooding lists
on the BM flag received from the remote nodes in the routes.

MAY

 be flagged as non-BM receivers based
...
any IP address of its selective AR-LEAF-set, the AR-REPLICATOR
packet to flooding list #2, skipping the AR-REPLICATOR-set. Non-BM overlay tunnels are
skipped when sending BM packets.
◦
If the destination IP address matches its IR-IP, the AR-REPLICATOR
#1 but
local ACs. This is the regular ingress replication behavior described in
overlay tunnels are skipped when sending BM packets.

 use flooding list
 skip all the overlay tunnels from the flooding list, i.e., it will only replicate to
. Non-BM
...
that the AR-REPLICATOR advertised in the Replicator-AR route) is not at the bottom of the
 copy the rest of the labels when forwarding them to the
stack, the AR-REPLICATOR
...
The selective AR-LEAF role
an AR-enabled BD. This administrative option to enable AR-LEAF capabilities
implemented as a system-level option as opposed to a per-BD option.
...
RECOMMENDED
 ...
 advertise a Regular-IR route if there are RNVEs in the BD. The selective
 advertise a Leaf A-D route after receiving a Replicator-AR route with L = 1. It
 that the selective AR-LEAF wait for a period specified by an AR-LEAF-join-
...
MUST
 ...
 send all the BM packets received on the ACs for a given BD to

 send a Leaf A-D route, including the route key and IP-address-

◦
The selective AR-LEAF
...
the new selective AR-REPLICATOR. The AR-REPLICATOR-activation-timer
 be the same
...
of selective and non-selective AR-LEAFs
SHOULD NOT
...
other AR-LEAFs that are not in its selective AR-LEAF-set.
 ...
 coexist in the same BD. If a non-selective
...
was selected; if one is found, flooding list #1
MUST
be used. Non-BM overlay tunnels are skipped when sending BM packets.

 be used. Otherwise, flooding list #2

MUST

•

When an AR-LEAF receives a BM packet on an overlay tunnel, it
...
The ability to signal and process these PFLs flags
 be an administrative choice. If a node
...
MAY
 ...
 be used along with the Assisted Replication solution.
...
In some cases, the AR-REPLICATOR node does not support more than one IP address to terminate
...
from the packets that must use Assisted Replication forwarding mode, the Replicator-AR
 advertise a different VNI/VSID than the one used by the Regular-IR route. For
route
...
when replicating to AR-LEAFs or RNVEs, the AR-REPLICATOR
source IP address when replicating to other AR-REPLICATORs.

 always use its IR-IP as the
...
When EVPNs are used for MPLSoGRE or MPLSoUDP, the ESI-label-based split-horizon procedure
provided in
...
horizon behavior in non-selective or selective AR.
 ...
 will not work for multihomed ESs defined on AR-LEAF nodes. Local bias is
...
bias
 also be followed. That is, traffic received with AR-IP as the IP DA will be treated as
...
[RFC8365]
regular local-bias rules
 and will not be forwarded to local ESs that are shared with
...
considerations related to the Leaf A-D route in
...
[RFC8365]
 ...
 apply to this document. The security
...
This document introduces the ability of the AR-REPLICATOR to forward traffic received on an
...
T. Morin "Registry and Extensions for P-Multicast Service

Rosen, E.
Interface Tunnel Attribute Flags" RFC 7902 DOI 10.17487/RFC7902
,
...
Rabadan, J., Ed. Sathappan, S. Nagaraj, K. Hankins, G.
"Operational Aspects of Proxy ARP/ND in Ethernet Virtual Private Networks"
RFC 9161 DOI 10.17487/RFC9161
rfc9161>
...
Neil Hart David Motz Dai Truong Thomas Morin Jeffrey Zhang
,
...
,
 ...
 for their valuable feedback and contributions. Also,

 for his thorough review, which improved the quality of the document

Contributors

In addition to the authors listed on the front page, the following people also contributed to this
