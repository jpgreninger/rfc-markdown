...
This document deﬁnes Path Computation Element Communication Protocol (PCEP) extensions
for grouping two unidirectional MPLS-TE Label Switched Paths (LSPs), one in each direction in
the network, into an associated bidirectional LSP. These PCEP extensions can be applied either
using a stateful PCE for both PCE-initiated and PCC-initiated LSPs or using a stateless PCE. The
PCEP procedures deﬁned are applicable to the LSPs using RSVP-TE for signaling.
...
Switching (MPLS) - Traﬃc Engineering (TE) Label Switched Paths (LSPs).

 speciﬁes extensions to PCEP to enable stateful control of MPLS-TE LSPs. It describes

[RFC8231]
two modes of operation: passive stateful PCE and active stateful PCE. In
...
June 2021
 ...
 introduces a generic mechanism for creating a grouping of LSPs. This grouping can
...
MUST
 ...
 support unidirectional, co-routed bidirectional, and associated bidirectional point-to-

The MPLS Transport Proﬁle (MPLS-TP) requirements document
TP
...
procedures for associated bidirectional LSPs are described in
 ...
 deﬁnes RSVP signaling extensions for binding forward and
...
This document deﬁnes PCEP extensions for grouping two unidirectional MPLS-TE LSPs into an
...
The procedures deﬁned are applicable to the LSPs using Resource Reservation Protocol - Traﬃc
Engineering (RSVP-TE) for signaling
...
. Speciﬁcally, this document deﬁnes two new

[RFC3209]

The procedure for associating two unidirectional Segment Routing (SR) paths to form an
associated bidirectional SR path is deﬁned in
document.
...
initiating the Bidirectional LSP Association, single-sided and double-sided, as deﬁned in
[RFC7551]
...
, in the single-sided case, the bidirectional tunnel is provisioned only on
...
are identiﬁed in the Bidirectional LSP Association Group TLV of their PCEP ASSOCIATION objects.
...
reverse LSP when it receives the RSVP-TE message to delete the forward LSP
 ...
 of the forward LSP Path message. The remote endpoint node then
...
As speciﬁed in
originating endpoint node is identiﬁed as the forward LSP of the single-sided initiated
bidirectional LSP.

, for fast reroute bypass tunnel assignment, the LSP starting from the
...
PCEP-speciﬁc LSP identiﬁers (PLSP-IDs) used are P1 and P2 on the originating endpoint node A
...
, in the double-sided case, the bidirectional tunnel is provisioned on
...
endpoint nodes. The forward and reverse LSPs are identiﬁed in the Bidirectional LSP Association
...
, for fast reroute bypass tunnel assignment, the LSP with the higher
 is identiﬁed as the forward LSP of the double-sided initiated
...
[RFC8537]
 ...
 for fast reroute bypass tunnel assignment is also applicable

3.4.  Summary of PCEP Extensions

The PCEP extensions deﬁned in this document cover the following modes of operation under the
stateful PCE model:
...
LSP. This is the passive stateful mode deﬁned in
...
long as it controls the LSP. This is the active stateful mode deﬁned in
...
well as the association group identifying the bidirectional LSP. This is the PCE-initiated mode
deﬁned in
...
network can be associated to form a bidirectional LSP without signiﬁcantly increasing the
...
are computed separately as triggered by two diﬀerent PCCs.
...
, LSPs are associated by adding them to a common association group. This

As per
document deﬁnes two new Association Types, called "Single-Sided Bidirectional LSP Association"
...
An LSP (forward or reverse)
Association.

MUST NOT

 be part of more than one Bidirectional LSP

•

The LSPs in a Bidirectional LSP Association
reverse directions.

MUST

 have matching endpoint nodes in the

•

The same tunnel (as deﬁned in
...
, the association group could be manually created by the

The Bidirectional LSP Association Types are considered to be both dynamic and operator
conﬁgured in nature. As per
...
association group are conveyed to the PCEP peer. The operator-conﬁgured Association Range
 be set for this Association Type to mark a range of Association Identiﬁers that are used for
MUST
operator-conﬁgured associations to avoid any Association Identiﬁer clash within the scope of the
...
Speciﬁcally, for the PCE-initiated bidirectional LSPs, these associations are dynamically created
...
object. This capability exchange for the Bidirectional LSP Association Types
using the Bidirectional LSP Association. Thus, the PCEP speaker
LSP Association Types in the ASSOC-Type-List TLV and
peer before using the Bidirectional LSP Association in PCEP messages.

 receive the same from the PCEP

 include the Bidirectional
...
The Bidirectional LSP Association Group TLV is an
...
Association or 5 for Double-Sided Bidirectional LSP Association).
 ...
 TLV for use with Bidirectional LSP
...
The value comprises of a single ﬁeld, the Flags ﬁeld (32 bits), where each bit represents a ﬂag
option.
...
When the Bidirectional LSP Association Group TLV is present, the R ﬂag
...
For co-routed LSPs, this TLV

MUST

 be present and the C ﬂag set.

•

For reverse LSPs, this TLV

MUST

 be present and the R ﬂag set.
...
The Bidirectional LSP Association Group TLV
appears more than once, only the ﬁrst occurrence is processed, and any others
ignored.

 be present more than once. If it
 be
...
Flags for the Bidirectional LSP Association Group TLV are deﬁned as follows.

R (Reverse LSP, 1 bit, bit number 31):

Indicates whether the LSP associated is the reverse LSP of
the bidirectional LSP. If this ﬂag is set, the LSP is a reverse LSP. If this ﬂag is not set, the LSP is
a forward LSP.
...
MUST
 ...
 be set for both the forward and reverse LSPs of a co-routed bidirectional LSP.

The C ﬂag is used by the PCE (both stateful and stateless) to compute bidirectional paths of the
forward and reverse LSPs of a co-routed bidirectional LSP.

The unassigned ﬂags (bit numbers 0-29)
received.

MUST

 be set to 0 when sent and
...
The PCEP procedure deﬁned in this document is applicable to the following three scenarios:
...
, Bidirectional LSP Associations can be created and updated by a

•

For a Single-Sided Bidirectional LSP Association initiated by the PCE, the PCE
...
a Double-Sided Bidirectional LSP Association initiated by the PCE, it
...
MUST
 ...
 report the forward and reverse LSPs in the Bidirectional LSP Association to
...
Single-Sided Bidirectional LSP Association on the originating endpoint node.
 ...
 create and update the forward and reverse LSPs independently for the
...
MAY
 ...
 create and update the forward LSP independently for the Double-Sided
...
, Bidirectional LSP Associations can also be created and updated by a

•

For a Single-Sided Bidirectional LSP Association initiated at a PCC, the PCC
PCRpt message to the PCE with both forward and reverse LSPs. For a Double-Sided
Bidirectional LSP Association initiated at the PCCs, both PCCs
...
MUST
 ...
 send a PCRpt message to

•

PCCs on the originating endpoint node
independently for the Single-Sided Bidirectional LSP Association.

MAY

 create and update the forward and reverse LSPs

•

PCCs on the endpoint nodes
Double-Sided Bidirectional LSP Association.

MAY

 create and update the forward LSP independently for the

•

PCCs establish and remove the association group on a per-LSP basis. PCCs
...
PCCs for the single-sided case
...
stateful PCE in order to avoid any race condition.
 ...
 delegate the forward and reverse LSPs independently to a
...
PCCs for the double-sided case
PCE would control the LSPs.

MAY

 delegate the forward LSPs to a stateful PCE, where the
...
group, thus enabling it to associate a common set of conﬁguration parameters or behaviors with
the request
. A PCC can request co-routed or non-co-routed forward and reverse paths
...
, the Bidirectional (B) ﬂag in the Request Parameters (RP) object is set

As deﬁned in
when the PCC speciﬁes that the path computation request is for a bidirectional TE LSP with the
same TE requirements in each direction. For an associated bidirectional LSP, the B ﬂag is also set
when the PCC makes the path computation request for the same TE requirements for the
forward and reverse LSPs.

Note that the B ﬂag deﬁned in a Stateful PCE Request Parameter (SRP) object
GMPLS]
...
, a PCEP-speciﬁc LSP Identiﬁer (PLSP-ID) is created by a PCC to uniquely

[RFC8231]

In the case of a Single-Sided Bidirectional LSP Association, the reverse LSP of a bidirectional LSP
created on the originating endpoint node is identiﬁed by the PCE using two diﬀerent PLSP-IDs,
...
MUST
 ...
 report all the existing Bidirectional LSP Associations to

. After the state synchronization, the PCE
...
support, the PCE speaker
value = 1 (Association Type is not supported).

 send PCErr with Error-Type = 26 (Association Error) and Error-
...
same TE tunnel (as deﬁned in
Sided Bidirectional LSP Association for a diﬀerent tunnel, the PCE speaker
Error-Type = 26 (Association Error) and Error-value = 15 (Tunnel mismatch in the association
group).

 belong to the
). If a PCE speaker attempts to add an LSP in a Single-
...
protocol" (Value 0)
Bidirectional LSP Associations deﬁned in this document, the PCE speaker
message with Error-Type = 26 (Association Error) and Error-value = 16 (Path Setup Type not
supported).

. If a PCEP speaker receives a diﬀerent PST value for the
...
A Bidirectional LSP Association cannot have both unidirectional LSPs identiﬁed as reverse LSPs
or both LSPs identiﬁed as forward LSPs. If a PCE speaker receives an LSP not complying to this
rule, the PCE speaker
= 17 (Bidirectional LSP direction mismatch).

 send PCErr with Error-Type = 26 (Association Error) and Error-value

MUST

A Bidirectional LSP Association cannot have one unidirectional LSP identiﬁed as co-routed and
the other identiﬁed as non-co-routed. If a PCE speaker receives an LSP not complying to this rule,
the PCE speaker
MUST
(Bidirectional LSP co-routed mismatch).

 send PCErr with Error-Type = 26 (Association Error) and Error-value = 18
...
(Endpoint mismatch in the association group).
 ...
 send PCErr with Error-Type = 26 (Association Error) and Error-value = 19

 have matching endpoint

MUST

MUST

The processing rules as speciﬁed in
Types deﬁned in this document.

Section 6.4 of [RFC8697]

 continue to apply to the Association

6.  Security Considerations

The security considerations described in
extensions deﬁned in this document as well.
...
session using Transport Layer Security (TLS)
 ...
 and apply to these Association Types. Hence, securing the PCEP
...
The mechanisms deﬁned in this document do not imply any control or policy requirements in
...
[RFC7420]
 ...
 describes the PCEP MIB; there are no new MIB objects deﬁned for LSP associations.

The PCEP YANG module

[PCE-PCEP-YANG]

 deﬁnes a data model for LSP associations.
...
The mechanisms deﬁned in this document do not add any new requirements on other protocols.
...
This document deﬁnes two new Association Types
new values in the "ASSOCIATION Type Field" subregistry
...
Single-Sided Bidirectional LSP Association
...
This document deﬁnes a new TLV for carrying additional information about LSPs within a
...
ﬁeld in the Bidirectional LSP Association Group TLV. New values are assigned by Standards
...
This document deﬁnes new Error-values for Error-Type 26 (Association Error). IANA has
...
15: Tunnel mismatch in the association
group
...
16: Path Setup Type not supported
...
18: Bidirectional LSP co-routed mismatch

RFC 9059

19: Endpoint mismatch in the association
group
...
R. Gandhi, Ed. "RSVP-TE Extensions for Associated
Zhang, F., Ed. Jing, R.
,
...
Gandhi, R., Ed. Shah, H.
Procedures for Co-routed Associated Bidirectional Label Switched Paths (LSPs)"
RFC 8537 DOI 10.17487/RFC8537
info/rfc8537>

J. Whittaker "Updates to the Fast Reroute
...
bidir-path-05
<https://tools.ietf.org/html/draft-ietf-pce-sr-bidir-
path-05>
...
,
"Requirements of an MPLS Transport Proﬁle" RFC 5654 DOI 10.17487/RFC5654
September 2009,
...
Lee, Y., Ed. Zheng, H., Ed. de Dios, O. Lopez, V.

,
Computation Element (PCE) Protocol Extensions for Stateful PCE Usage in
GMPLS-controlled Networks" Work in Progress Internet-Draft, draft-ietf-pce-
pcep-stateful-pce-gmpls-14
ietf-pce-pcep-stateful-pce-gmpls-14>
...
inputs to this document. The authors would also like to thank
Morton
authors would like to thank the following IESG members for their review comments and
suggestions:
Alvaro Retana

 for various discussions on association groups and
Mike Taillon Harish Sitaraman Al
 for reviewing this document and providing valuable comments. The

Barry Leiba Éric Vyncke Benjamin Kaduk Murray Kucherawy Martin Duke
