...
RFC 9142
Key Exchange (KEX) Method Updates and
...
3.2.1.  FFC Diﬃe-Hellman Using Generated MODP Groups

3.2.2.  FFC Diﬃe-Hellman Using Named MODP Groups
...
[RFC4253]
Secure Shell (SSH) is a common protocol for secure communication on the Internet. In
 be implemented. Over
SSH originally deﬁned two Key Exchange (KEX) Method Names that
...
" moving to
") of various key exchange mechanisms. Some
...
the ﬁrst key exchange is additionally used as the session identiﬁer, which is a unique
identiﬁer for this connection. It is used by authentication methods as a part of the data
that is signed as a proof of possession of a private key. Once computed, the session
identiﬁer is not changed, even if keys are later re-exchanged.
...
exchange algorithm name. However, some of them are implicit and deﬁned in the RFC that
deﬁnes the key exchange algorithm name.

Various RFCs use diﬀerent spellings and capitalizations for the hashing function and encryption
function names. For the purpose of this document, the following are equivalent names: sha1,
...
There are many possible symmetric ciphers available with multiple modes. The list in Table 1 is
intended as a representative sample of those that appear to be present in most SSH
implementations. The security strength estimates are generally available in
DES and AES, as well as Section 5.6.1.1 of
...
shared key being used has at most 80 bits of security strength, which may not be suﬃcient for
most users.
...
on chosen-preﬁx collision resistance.

. This paper shows that an on-
Transcript Collision attacks are documented in
path attacker does not tamper with the Diﬃe-Hellman values and does not know the connection
keys. The attack could be used to tamper with both I_C and I_S (as deﬁned in
[RFC4253]
cryptographic algorithm that the attacker knows how to break.

) and might potentially be able to downgrade the negotiated ciphersuite to a weak

[TRANSCRIPTION]

Section 7.3 of

These attacks are still computationally very diﬃcult to perform, but it is desirable that any key
...
[RFC6234]
 ...
 rather than the SHA-1 hash is strongly
...
have the same number of bits of security strength, but none are as yet deﬁned in any RFC for use
in a KEX for SSH.
...
sha384-*, which are deﬁned to use SHA2-384 (also known as SHA-384) deﬁned in
hash algorithm.
...
Elliptic Curve Cryptography (ECC) has families of curves for key exchange methods for SSH.
NIST prime curves with names and other curves are available using an object identiﬁer (OID)
with Elliptic Curve Diﬃe-Hellman (ECDH) via
[RFC5656]
.
exchanges are used with ECDH via
[RFC8731]

. Curve25519 and curve448 key

•

Finite Field Cryptography (FFC) is used for Diﬃe-Hellman (DH) key exchange with "safe
primes" either from a speciﬁed list found in
[RFC4419]
...
Integer Factorization Cryptography (IFC) using the RSA algorithm is provided for in
[RFC4432]
...
It is desirable that a minimum of 112 bits of security strength be selected to match the weakest of
...
[RFC5656]
 ...
 key exchanges for the named curves use a hashing function with a matching

For ECC, across all of the named curves, the minimum security strength is approximately 128 bits.
The
security strength. Likewise, the
...
. The nistp256, nistp384, and nistp521 (NIST prime curves) are provided in
[RFC7748]
...
same number of bits of security as the named curve.
 ...
 key exchanges use a hashing function that has more
...
suﬃcient to provide a consistent security strength for the 3des-cbc cipher.
...
provides approximately 112 bits of security. The 3des-cbc cipher itself provides at most 112 bits of
security, so the group14-sha256 key exchanges is suﬃcient to keep all of the 3des-cbc key, for 112
bits of security.
...
security, which is suﬃcient to protect aes192 with 192 bits of security.

1.2.3.  Integer Factorization Cryptography (IFC)

The only IFC algorithm for key exchange is the RSA algorithm speciﬁed in
...
matches the security strength. The use of a SHA-2 family hash with RSA 2048-bit keys has suﬃcient
...
algorithm strength. Guidance for these strengths can be found in Section 5.6.1.1 of
800-57pt1r5]
...
[RFC4253]
 ...
 in specifying how the use of data

This RFC also collects key exchange method names in various existing RFCs (
, and
,
[RFC4419] [RFC4432] [RFC4462] [RFC5656] [RFC8268] [RFC8308] [RFC8731]
...
, and
 ...
 deﬁnes the generation of a shared secret K (really the output of the KDF)
Section 7.2 of [RFC4253]
and an exchange key hash H. Each key exchange method uses a speciﬁed HASH function, which
must be the same for both key exchange and Key Derivation. H is used for key exchange integrity
across the SSH session as it is computed only once. It is noted at the end of
[RFC4253]
...
This process will lose entropy if the amount of entropy in K is larger than the internal
state size of HASH.
...
need are to be allowed ("
to "
implementation consensus are encouraged ("
...
" to increase interoperability and security.) Methods that are not "weak" and have

"). There needs to be at least one consensus

"). Methods that are newer or considered to
...
For this document, 112 bits of security strength is the minimum. Use of either or both of SHA-1 and
RSA 1024 bits at an approximate 80 bits of security fall below this minimum and should be
deprecated and moved to disallowed as quickly as possible in conﬁgured deployments of SSH. It
...
The ECC curves deﬁned for the key exchange algorithms above include the following: curve25519,
curve448, the NIST prime curves (nistp256, nistp384, and nistp521), as well as other curves allowed
for by
. There are key exchange mechanisms based on the Generic Security
Service Application Program Interface (GSS-API) that use these curves as well that have a "gss-"
preﬁx.

Section 6 of [RFC5656]

3.1.1.  curve25519-sha256 and gss-curve25519-sha256-*

Curve25519 is eﬃcient on a wide range of architectures with properties that allow higher-
performance implementations compared to the patented elliptic curve parameters purchased by
NIST for the general public to use as described in
[RFC5656]
methods use SHA2-256 (also known as SHA-256) deﬁned in
hash for use in both the KDF and session integrity. It is reasonable for both gss and non-gss uses of
 and
curve25519 key exchange methods. These key exchange methods are described in
[RFC8732]
sha256 key exchange method has multiple implementations and
gss-curve25519-sha256-* key exchange method
same performance and security characteristics as curve25519-sha256.

. The corresponding key exchange
[RFC6234]
...
. The curve25519-
 ...
 also be implemented because it shares the

. SHA2-256 is a reasonable
...
deﬁned in
. SHA2-512 is a reasonable hash for use in both the KDF and session integrity.
It is reasonable for both gss and non-gss uses of curve448 key exchange methods. These key
exchange methods are described in
agreement described in
implemented. The gss-curve448-sha512-* key exchange method
it shares the same performance and security characteristics as curve448-sha512.

[RFC8732]
. The curve448-sha512 key exchange method

 and are similar to the IKEv2 key

 also be implemented because
...
nistp521) as well as other curves to be deﬁned for the ECDH key exchange. At the time of this
writing, there are three named curves in this namespace that
in
speciﬁcally disabled by local security policy. In
other ECDH curves using OIDs is speciﬁed. These other curves

 be supported. They appear
 always be enabled unless
, the method to name

SHOULD
Section 6.1 of [RFC5656]

. If implemented, the named curves

Section 10.1 of [RFC5656]
...
deployed. The items listed as "

SHOULD

" where implementations may be more eﬃcient or widely

MAY

" in Table 8 are potentially less eﬃcient.
...
algorithm to use the same curve for both to maintain the same security strength in the
connection.
...
3.2.1.  FFC Diﬃe-Hellman Using Generated MODP Groups
 ...
 deﬁnes two key exchange methods that use a random selection from a set of pre-

[RFC4419]
generated moduli for key exchange: the diﬃe-hellman-group-exchange-sha1 method and the
diﬃe-hellman-group-exchange-sha256 method. Per
MODP group whose modulus size is equal to or greater than 2048 bits. MODP groups with a
modulus size less than 2048 bits are weak and
...
The diﬃe-hellman-group-exchange-sha1 key exchange method
...
The diﬃe-hellman-group-exchange-sha256 key exchange method
SHA2-256, which is reasonable for MODP groups less than 4096 bits.

MAY

 be used. This method uses
...
diﬃe-hellman-group-exchange-sha1

SHOULD NOT

diﬃe-hellman-group-exchange-sha256
...
The diﬃe-hellman-group14-sha256 key exchange method is deﬁned in
key exchange that has approximately 112 bits of security strength that matches 3des-cbc
symmetric cipher security strength. It is a reasonably simple transition from SHA-1 to SHA-2, and
given that diﬃe-hellman-group14-sha1 and diﬃe-hellman-group14-sha256 share a MODP group
and only diﬀer in the hash function used for the KDF and integrity, it is a correspondingly simple
transition from implementing diﬃe-hellman-group14-sha1 to implementing diﬃe-hellman-
group14-sha256. Given that diﬃe-hellman-group14-sha1 is being removed from mandatory to
implement (MTI) status, the diﬃe-hellman-group14-sha256 method
 be implemented. The
rest of the FFC MODP group from
...
The rsa1024-sha1 key exchange method is deﬁned in
with a SHA-1 hash. This key exchange does NOT meet security requirements. This method
NOT
...
[RFC4432]
 ...
 and uses an RSA 1024-bit modulus
...
phased out due to security concerns for SHA-1, as documented in
...
[RFC6234]
for use in SSH with many of the currently deﬁned key exchanges.

 is more secure than SHA-1. They have been standardized

Please note that at the present time, there is no key exchange method for Secure Shell that uses
the SHA-3 family of secure hashing functions or the Extendable-Output Functions
...
Section 6.2 of [RFC2409]

. The SSH group1 is deﬁned in

 as using the Oakley Group 2 provided in

Prior to the changes made by this document, diﬃe-hellman-group1-sha1 and diﬃe-hellman-
group14-sha1 were MTI. diﬃe-hellman-group14-sha1 is the stronger of the two. Group14 (a 2048-bit
MODP group) is deﬁned in
[RFC4253]
...
suggested that it should transition to deprecated ﬁrst and be disallowed at a later time. The
...
small hardware Internet of Things (IOT) devices that have insuﬃcient compute and memory
...
reasonable to retain the diﬃe-hellman-group14-sha1 exchange for interoperability with legacy
implementations. The diﬃe-hellman-group14-sha1 key exchange
be put at the end of the list of negotiated key exchanges.

 be implemented, but should

MAY

The diﬃe-hellman-group1-sha1 and diﬃe-hellman-group-exchange-sha1
...
diﬃe-hellman-group14-
sha1

diﬃe-hellman-group14-
sha256

diﬃe-hellman-group15-
sha512

diﬃe-hellman-group16-
sha512
...
diﬃe-hellman-group17-
sha512

diﬃe-hellman-group18-
sha512
...
MAY
 ...
 "Key Exchange Method Names" not otherwise mentioned in this
...
addition, the security considerations provided in
...
[RFC4251]
 ...
 and continue to apply. In

 apply. Note that Forward Secrecy is
...
A key exchange method is considered weak when the security strength is insuﬃcient to match the
symmetric cipher or the algorithm has been broken.

The 1024-bit MODP group used by diﬃe-hellman-group1-sha1 is too small for the symmetric
ciphers used in SSH.

MODP groups with a modulus size less than 2048 bits are too small for the symmetric ciphers used
in SSH. If the diﬃe-hellman-group-exchange-sha256 or diﬃe-hellman-group-exchange-sha1 key
...
The diﬃe-hellman-group14-sha1 algorithm is not yet completely deprecated. This is to provide a
...
IANA has added a new column to the "Key Exchange Method Names" registry
...
sha2-nistp521, and added references to
[RFC4462]
sha1-*, gss-group14-sha1-*, diﬃe-hellman-group-exchange-sha1, and diﬃe-hellman-group-
...
OK to Implement guidance entries for registrations that pre-date [RFC9142] are found in
Table 12 in Section 4 of [RFC9142].

Registry entries annotated with "
annotated with "
...
" are considered disallowed. Registry entries

" are deprecated and may be disallowed in the future.
...
,
C. Lonvick, Ed. "The Secure Shell (SSH) Transport Layer Protocol"
<https://www.rfc-editor.org/info/
...
,
Exchange (KEX) Groups for Secure Shell (SSH)" RFC 8268 DOI 10.17487/RFC8268
December 2017,
...
Velvindron, L.
Recommended Diﬃe-Hellman Modulus Size to 2048 Bits" RFC 8270 DOI 10.17487/
RFC8270

M. Baushke "Increase the Secure Shell Minimum
,
...
M. Baushke "Secure Shell (SSH) Key
...
Dang, Q. "Recommendation for applications using approved hash

,

algorithms" DOI 10.6028/NIST.SP.800-107r1
.
NIST.SP.800-107r1>
...
M. Kojo "More Modular Exponential (MODP) Diﬃe-Hellman
...
Ylonen, T.
,
4251 DOI 10.17487/RFC4251
.
rfc4251>

C. Lonvick, Ed. "The Secure Shell (SSH) Protocol Architecture" RFC
...
,
,
W. Simpson "Diﬃe-Hellman Group Exchange for the
Friedl, M. Provos, N.
,
Secure Shell (SSH) Transport Layer Protocol" RFC 4419 DOI 10.17487/RFC4419
March 2006,
...
Harris, B. "RSA Key Exchange for the Secure Shell (SSH) Transport Layer
Protocol" RFC 4432 DOI 10.17487/RFC4432
<https://www.rfc-
.
editor.org/info/rfc4432>
...
Hutzelman, J. Salowey, J. Galbraith, J.
, and
...
V. Welch "Generic Security Service

, May 2006,

,

J. Green "Elliptic Curve Algorithm Integration in the Secure Shell
...
S. Josefsson "Curve25519 and Curve448 for the Internet Key Exchange

Nir, Y.
Protocol Version 2 (IKEv2) Key Agreement" RFC 8031 DOI 10.17487/RFC8031
,
...
Sorce, S.
(GSS-API) Key Exchange with SHA-2" RFC 8732 DOI 10.17487/RFC8732
2020,

H. Kario "Generic Security Service Application Program Interface
, February
,
.
...
Authentication in TLS, IKE, and SSH" Network and Distributed System Security
Symposium (NDSS) DOI 10.14722/ndss.2016.23418
.
...
Miller Niels Moeller Matt Johnston Iwamoto Kouichi Simon Josefsson Dave Dugal Daniel
Migault Anna Johnston Tero Kivinen

Denis Bider Peter Gutmann Damien
...
Thanks to the following people for code to implement interoperable exchanges using some of
these groups as found in this document:
Dropbear. And thanks to
Poderosa implementations also adopting new Diﬃe-Hellman groups based on this document.

 for information about RLogin, Tera Term (ttssh), and
