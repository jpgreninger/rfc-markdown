...
RFC 9460
Service Binding and Parameter Specification via the
...
, or switch to an operationally preferable endpoint. It is highly desirable to
...
Providing an indication signaling that the "https" scheme should be used instead of "http" for
 (see
all HTTP requests to this host and port, similar to HTTP Strict Transport Security
Section 9.5).
...
The presentation format <RDATA> of the record (
...
ranges "a"-"z", "0"-"9", and "-". In ABNF
 ...
 appear at most once in the SvcParams. In presentation format,
...
format encoding.
 ...
 be parsed as specified above, and the decoded value
...
such keys
 ...
 use a comma-separated list (Appendix A.1).
...
the SvcParams, consuming the remainder of the record (so smaller than 65535 octets and
...
MUST
 ...
 reject the entire RRset and fall back to non-SVCB
...
[Attrleaf]
 ...
 be registered appropriately with IANA (see Section 11).
...
prepended. For schemes that specify a port (
), one reasonable possibility is
...
terms this behavior "Port Prefix Naming" and uses it in the examples throughout.
 ...
 specify additional underscore-prefixed labels to be
...
under its own owner name, as in ordinary CNAME processing (
...
TLS clients
 ...
 continue to validate TLS certificates for the original service name.
...
AliasMode, the recipient

MUST

SHOULD

 have the same mode. If an RRset contains a record in

 ignore any ServiceMode records in the set.
...
to ensure uniform load balancing.
 ...
 apply a random shuffle within a priority level to the records before using them,
...
). In AliasMode, the TargetName will be the
allowed (see, for example,
...
include SvcParams.
 ...
 ignore any SvcParams that are present. Zone-file parsers

 be equal to the owner name, as this would result in a
...
MAY
 ...
 be the owner of a CNAME record. Clients and recursive resolvers
...
MAY
 ...
 be the canonical name of a CNAME record, and the

To avoid unbounded alias chains, clients and recursive resolvers
number of SVCB aliases they will follow for each resolution request. This limit
i.e., implementations
...
MUST
 ...
 be able to follow at least one AliasMode record. The exact value of

 impose a limit on the total
 be zero,
MUST NOT
...
mapping, clients
 ...
 ignore any SvcParam that they do not recognize.

 define a protocol mapping that explains how
...
"self-consistent" if its SvcParams all comply with each other's requirements. Clients
any RR whose recognized SvcParams are not self-consistent and
MAY
...
not exist. This indication is advisory: clients encountering this indication
...
Otherwise, SVCB resolution has failed, and the list of available endpoints is empty.
...
and enable the optimizations discussed in Section 5.
 ...
 issue in parallel any other DNS queries that might be needed for

SHOULD

Once SVCB resolution has concluded, whether successful or not, if at least one AliasMode record
was processed, SVCB-optional clients
...
has A and/or AAAA records but no SVCB records.)
 ...
 append to the list of endpoints an endpoint
...
AAAA and/or A records for the selected TargetName and
...
If DNS responses are cryptographically protected (e.g., using DNSSEC or TLS
...
access to the SvcParams.
 ...
 abandon its attempt to reach the service, even if the client is SVCB-
...
MAY
 ...
 treat SVCB resolution failure as
...
SOCKS5
 have the option of using named destinations, in which case the client does not
...
not possible, SVCB-optional clients
clients
...
the proxy. When connecting using a SVCB record, clients
 provide the final TargetName and
port to the proxy, which will perform any required A and AAAA lookups.

 follow the standard SVCB
...
records in the Additional section for any TargetNames that are in the zone. If the zone is signed,
 also include DNSSEC records authenticating the existence or nonexistence of
the server
these records in the Additional section.

 return A, AAAA, and SVCB
...
process used for unknown RR types
Recursive resolvers that are aware of SVCB
 help the client to execute the procedure in
SHOULD
...
ordinarily follow (e.g., CNAME, DNAME
records
a failure response.

). Errors or anomalies in obtaining additional
MUST NOT

[DNAME]
 cause this process to terminate but

 themselves cause the resolver to send
...
Section 5.1.7 of [RFC6147]
(
 apply the same synthesis logic
...
would not improve performance when the above recommendation is implemented.
 ...
 extrapolate the AAAA synthesis logic to the IP hints in the SvcParams
...
MUST
 ...
 accomplish this by treating the entire SvcParams portion of the record as opaque,
...
Answer (e.g., RRSIG, NSEC, NSEC3).
 ...
 accompany each RRset in the Additional section
...
, "Unauthenticated RRs received and cached from ... the
...
appropriate." Recursive resolvers therefore
use in populating Additional section responses and
authenticated by DNSSEC.

 cache records from the Additional section for
 cache them for general use if they are
...
The EDNS Client Subnet (ECS) option
 allows recursive resolvers to request IP
...
Section 7.3.1 of [RFC7871]

, "Any records from [the Additional section]
...
[RFC7871], Section 7.2.1
 ...
 treat any records in the Additional section as having SOURCE PREFIX-LENGTH
MUST
...
client-side DNS cache. Responses in the Additional section of a SVCB response
 be placed
...
MAY
 ...
 try more than one option in parallel and
...
5.1.  Optimistic Pre-connection and Connection Reuse
 ...
 initiate a
If an address response arrives before the corresponding SVCB response, the client
connection as if the SVCB query returned NODATA but
...
Clients implementing this optimization
optimistic pre-connection, as per the guidance in

SHOULD

 wait for 50 milliseconds before starting
[HappyEyeballsV2]
.
...
If the client has an in-progress TCP connection to [2001:db8::2]:1234, it
...
When following the procedure in Section 4.2, recursive resolvers
early and produce a reply that omits some of the associated RRsets. This is
...
maximum response size is reached or when responding before fully chasing dependencies
...
As discussed in Section 3, clients
...
the client
 ...
 be able to fetch additional information that is required to
...
RR type $T, the follow-up query to the TargetName
...
(Section 9), which avoids Attrleaf label prefixes
 in order to improve compatibility with
...
Each new protocol mapping document
Protocol mappings
...
Protocol Negotiation (ALPN) protocol identifiers
supported by this service endpoint (the "SVCB ALPN set").

 and associated transport protocols

[ALPN]

[AltSvc]

, each ALPN protocol identifier is used to identify the application protocol
...
more alpn-ids. Zone-file implementations
...
(e.g., key1=\002h2) in the event that these characters are needed.
 ...
 be a comma-separated list (Appendix A.1) of one or

 disallow the "," and "\" characters in ALPN IDs
...
by nearly all clients and servers; this set
 be empty. To determine the SVCB ALPN set, the
...
MAY
 ...
 implement a fallback procedure, using a less-preferred transport if more-preferred
...
contain any supported protocols.
 ...
 attempt connection to a service endpoint whose SVCB ALPN set does not

To ensure consistency of behavior, clients
MAY
...
connection could have succeeded using a non-default ALPN protocol.
 ...
 reject the entire SVCB RRset and fall back to basic
...
SHOULD
 ...
 ensure that at least one RR in each RRset supports the default transports.

7.2.  "port"

The "port" SvcParamKey defines the TCP or UDP port that should be used to reach this alternative
endpoint. If this key is not present, clients

 use the authority endpoint's port number.
...
The presentation value of the SvcParamValue is a single decimal integer between 0 and 65535 in
...
If A and AAAA records for TargetName are locally available, the client
hints. Otherwise, clients
 perform A and/or AAAA queries for TargetName per Section 3,
 opt
and clients
...
performance.
 ...
 use the IP address in those responses for future connections. Clients

 use to reach the service.
...
addresses of the appropriate family in standard textual format
simpler parsing, this SvcParamValue
...
SHALL
 ...
 be a comma-separated list (Appendix A.1) of one or more IP
...
of addresses is invalid.
 ...
 pick addresses to use in a random order. An empty list
...
Happy Eyeballs
synthesize IPv6 addresses as specified in
AAAA resolution (Section 3). For best performance, server operators
"ipv6hint" parameter whenever they include an "ipv4hint" parameter.

 or ignore the "ipv4hint" key and wait for
SHOULD

. When only "ipv4hint" is present, NAT64 clients may
...
resolver is not compliant with the requirements in Section 4 and
 be included if
most clients are using compliant recursive resolvers. When TargetName is the service name or
the owner name (which can be written as "."), server operators
...
correctly for clients that ignore this SvcParamKey. Each SVCB protocol mapping
...
2.4.3), listed keys
 ...
 be a comma-separated list (Appendix A.1) of one or more valid

 appear more than once. For self-consistency (Section

 also appear in the SvcParams.
...
To enable simpler parsing, this SvcParamValue
...
list. Other automatically mandatory keys
...
MUST NOT
 ...
 appear in its own value-

 appear in the list either. (Including them
...
compatible RR type, specific to the "https" and "http" schemes. Clients
...
All the SvcParamKeys defined in Section 7 are permitted for use in HTTPS RRs. The default set of
...
using the "bad ports" list from
SvcParam.

 apply the same restriction to the "port"
...
The HTTPS RR parallels the concepts introduced in "HTTP Alternative Services"
...
to configure and maintain an additional delegation.
 ...
 used in SVCB, this construction enables offline DNSSEC
...
process described in Section 9.5, so domain owners
"_http".

 publish HTTPS RRs with a prefix of
...
verification, which would provide such assurance, are
modeled as an untrusted channel that might be controlled by an attacker, so Alt-Svc parameters
that cannot be safely received in this model
...
Shortening the TTL to compensate for incorrect caching is
impairs the performance of correctly functioning caches and does not guarantee faster
expiration from incorrect caches. Instead, server operators
...
based on a cached Alt-Svc response
...
example.com" sends an Alt-Svc field value of:
 ...
 retrieve any HTTPS records for the Alt-Svc alt-
...
SHOULD
 ...
 carry the same SvcParams as the origin unless a deviation is specifically
Alt-authorities
known to be safe. As noted in
 disallow any Alt-Svc connection
...
MUST NOT
 ...
 use an HTTPS RR response unless the client supports the TLS Server Name
...
). This supports the conservation of IP
...
similar to HSTS
lookup to determine if any HTTPS RRs exist for that origin. To do so, the client
...
ServiceMode HTTPS RRs (see Section 8), the client
 behave as if it has received an HTTP
...
incompatible ServiceMode RR does not trigger the redirect behavior.) Because HTTPS RRs are
 place any more trust in this
received over an often-insecure channel (DNS), clients
...
origin with an HTTPS RR, either directly or via the above redirect, such a client
 remove the
 rely on user recourse
user recourse option. Origins that publish HTTPS RRs therefore
...
[WebSocket]
For example, clients that support HTTPS RRs and implement
...
an HTTP proxy), clients that find a corresponding HTTPS RR
...
To avoid a delay for clients using a non-conforming recursive resolver, domain owners
 choose TargetName according to a
...
NOT RECOMMENDED
 ...
 avoid using a TargetName that is below a DNAME, as this is likely
...
). This could create an operational issue when the
...
This record would indicate that "https://simple.example" supports QUIC in addition to HTTP/1.1
...
This configuration is entirely compatible with the "apex aliasing" example, whether the client
...
Note that in the above example, the different CDNs have different configurations and different
...
[DoT]
preserving DNS transport (like DNS over TLS
...
registry
 ...
 have an entry in the "Uniform Resource Identifier (URI) Schemes"
 have a defined specification for use with SVCB.

 have an entry for its scheme under
...
SVCB/HTTPS RRs permit distribution over untrusted channels, and clients are
...
Section 2.1 of [AltSvc]
 for publishing and using SVCB and
...
MUST
 ...
 ensure that their DNS cache is partitioned for each local network, or flushed on
Clients
...
by forging erroneous responses, but this mechanism allows them to target particular protocols or
 ignore AliasMode "."
ports within a domain. Clients that might be subject to such attacks
records.
...
redirects.) As a mitigation, SVCB mapping documents
SHOULD
...
MUST
 ...
 ensure that the reference is stable and publicly available and that it specifies how to

The registration policy for new entries is Expert Review (
expert
convert the SvcParamValue's presentation format to wire format. The reference
 be any
individual's Internet-Draft or a document from any other source with similar assurances of
stability and availability. An entry
 specify a reference of the form "Same as (other key
MAY
...
, the following entry has been added to the DNS "Underscored and Globally Scoped
...
T. Pauly "Happy Eyeballs Version 2: Better Connectivity
...
Using Concurrency" RFC 8305 DOI 10.17487/RFC8305
...
Bagnulo, M. Sullivan, A. Matthews, P.
...
D. Wing "Discovery of the IPv6 Prefix Used for
...
Contavalli, C. van der Gaast, W. Lawrence, D.
...
Finch, T. Hunt, E. van Dijk, P. Eden, A.
,
,
<https://datatracker.ietf.org/doc/html/draft-ietf-dnsop-
, 8 July 2019,

DNS aliases (ANAME)" Work in Progress Internet-Draft, draft-ietf-dnsop-
aname-04
aname-04>

W. Mekking "Address-specific
...
Internet Systems Consortium "BIND v9.19.11 Configuration Reference:
...
A. Barth "HTTP Strict Transport Security (HSTS)"
...
Bellis, R. "A DNS Resource Record for HTTP" Work in Progress Internet-Draft,
...
doc/html/draft-bellis-dnsop-http-record-00>
...
Barr, D. "Common DNS Operational and Configuration Errors" RFC 1912 DOI
,
10.17487/RFC1912
.
...
The decoding algorithm allows char-string to represent any *OCTET, using quoting to group
...
decoding", because
 uses this algorithm to produce a <character-
...
to look in a different location for a service. However, there are several differences:
 ...
 can perform a function similar to that of the SVCB record, informing a client
...
SRV records cannot instruct the client to switch or upgrade protocols, whereas SVCB can
...
, this approach is extensible to cover Alt-Svc and Encrypted ClientHello use
...
, this approach is extensible to cover Alt-Svc and Encrypted ClientHello
...
, and wire format. The wire format uses hexadecimal (\xNN) for each
...
Figure 10: An "alpn" Value with an Escaped Comma and an Escaped Backslash in Two Presentation
Formats
...
Over the years, IETF participants have proposed a wide range of solutions to the "CNAME at the
zone apex" challenge, including
, and others. The authors are
,
grateful for their work to elucidate the problem and identify promising strategies to address it,
some of which are reflected in this document.

[HTTP-DNS-RR] [ANAME-DNS-RR]

,

Thank you to
Ian Swett Ralf Weber Jon Reed Martin Thomson Lucas Pardue Ilari Liusvaara
Tim Wicinski Tommy Pauly Chris Wood David Benjamin Mark Andrews Emily Stark Eric Orth
,
,
Kyle Rose Craig Taylor Dan McArdle Brian Dickson Willem Toorop Pieter Lexis Puneet Sood
Olivier Poitrey Mashooq Muhaimen Tom Carpay
,
suggestions on this document.

, and many others for their feedback and
