...
Purpose Labels (SPLs) are speciﬁed and documented.

This document applies that terminology change to the relevant IANA registry and also clariﬁes
...
4.  Clariﬁcation on Handling of the Entropy Label Indicator
...
RFC 7274
Labels but did not deﬁne consistent terminology.

[RFC7274]

 made some changes to the terminology used for MPLS Special-Purpose
...
At the time of writing the ﬁrst draft version of this document, the IETF was in the process of
allocating the very ﬁrst SPLs from the Extended SPL (eSPL) range
. This document
...
The range 0-1048575 of eSPLs is speciﬁed in RFC 7274
◦
...
would be needed to change this rule, and that RFC would need to deﬁne the ranges that
...
Multiprotocol Label Switching (MPLS) Label Values" code points
name space, there are two registries. One was called the "Special-Purpose MPLS Label Values"
registry
[eSPL]

. The other was called the "Extended Special-Purpose MPLS Label Values" registry
...
The diﬀerence in the name of the name space and the ﬁrst registry is only that the MPLS
...
changes the name of the ﬁrst registry to "Base Special-Purpose MPLS Label Values" but leaves the
name of the latter registry unchanged as "Extended Special-Purpose MPLS Label Values".

The following conventions will be used in speciﬁcations and when talking about SPLs.
...
registry do not need a name, as they are not available for allocation and
...
document updates
 ...
 by replacing those paragraphs as follows.
...
as reserved. Furthermore, values 0-6 and 8-15
following an XL; an LSR processing a packet with an XL at the top of the label stack
followed by a label with value 0-6 or 8-15
...
Values 0-15 of the "Extended Special-Purpose MPLS Label Values" registry are set aside
as reserved. Furthermore, an implementation
...
When inspecting a label stack to ﬁnd an Entropy Label Indicator (ELI -- label 7), a
...
SPLs. It does not aﬀect forwarding in the MPLS data plane, nor does it have any eﬀect on how
...
IANA "Special-Purpose Multiprotocol Label Switching (MPLS) Label

,

Values" <https://www.iana.org/assignments/mpls-label-values/>
...
We would like to thank the Routing Directorate reviewer,
insightful review, and

Tom Petch

 for pointing out several issues of clarity.

Eric Gray

, for a detailed, careful, and
