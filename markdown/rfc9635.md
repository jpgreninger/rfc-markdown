...
The Grant Negotiation and Authorization Protocol (GNAP) defines a mechanism for delegating
authorization to a piece of software and conveying the results and artifacts of that delegation to
the software. This delegation can include access to a set of APIs as well as subject information
...
a resource server. This specification also discusses discovery mechanisms that enable the client
instance to configure itself dynamically. The means for an authorization server and resource
server to interoperate are discussed in
...
uses URI as a generic term, since it aligns with the recommendations in
...
for long values, as per
part of the value.

. The \ character and leading spaces on wrapped lines are not
...
This document uses the term "mutual TLS" as defined by
is used to mean the same thing.

[RFC8705]

. The shortened form "MTLS"

For brevity, the term "signature" on its own is used in this document to refer to both digital
signatures (which use asymmetric cryptography) and keyed Message Authentication Codes
(MACs) (which use symmetric cryptography). Similarly, the verb "sign" refers to the generation of
either a digital signature or a keyed MAC over a given signature base. The qualified term "digital
signature" refers specifically to the output of an asymmetric cryptographic signing operation.
...
For example, a client can be a mobile application, a web application, a backend data
processor, etc.
...
AuthorizationResourceServerServerClientInstanceResourceEndOwner~~~~~~UserLegend:indicatesinteractionbetweenahumanandcomputerindicatesinteractionbetweentwopiecesofsoftware~~~indicatesapotentialequivalenceorout-of-bandcommunicationbetweenrolesRFC 9635
...
have frontend components that are installed on the end user's device as well as a backend
system that the frontend communicates with. If both of these components participate in the
...
time and with a specific duration of validity and/or to exercise some set of delegated rights to
...
GNAP defines its trust objective as follows: the RO trusts the AS to ensure access validation and
...
and GNAP aims to make common attacks impractical. This specification makes access tokens
opaque to the client and defines the request/response scheme in detail, therefore avoiding
extra trust hypotheses from this critical piece of software. Yet, the AS may further define
cryptographic attestations or optional rules to simplify the access of clients it already trusts,
due to past behavior or organizational policies (see Section 2.3).
...
proper validation of the proofing method is expected from the RS.
...
AS/RS: The AS promises to issue valid access tokens to legitimate client requests (i.e., after

carrying out appropriate due diligence, as defined in the GNAP). Some optional configurations
are covered by
...
techniques such as the Promise Theory
...
ContinueNeedInteractionPendingFinishInteraction(approve/deny)CancelRequestProcessingFinalizeFinalizedRevokeorFinalizeUpdateApprovedNoInteractionContinueRFC 9635
...
request moves to the pending state, and the AS returns a continuation response (Section 3.1)
along with any appropriate interaction responses (Section 3.3). If no such approval is
required, such as when the client instance is acting on its own behalf or the AS can determine
that access has been fulfilled, the request moves to the approved state where access tokens for
API access (Section 3.2) and subject information (Section 3.4) can be issued to the client
instance. If the AS determines that no additional processing can occur (such as a timeout or
an unrecoverable error), the grant request is moved to the finalized state and is terminated.
...
this request, the client instance can send a polling continuation request (Section 5.2) to the AS.
This returns a continuation response (Section 3.1) while the grant request remains in this
state, allowing the client instance to continue to check the state of the pending grant request.
If an interaction finish method (Section 2.5.2) is specified in the grant request, the client
instance can continue the request after interaction (Section 5.1) to the AS to move this request
to the processing state to be re-evaluated by the AS. Note that this occurs whether the grant
request has been approved or denied by the RO, since the AS needs to take into account the
full context of the request before determining the next step for the grant request. When other
...
assumed to have been completed. If the client instance sends a polling continuation request
...
(A) The end user interacts with the client instance to indicate a need for resources on behalf
...
EndUser~~~~ResourceOwner(RO)ABClient(1)ResourceInstanceServer(RS)2Authorization3Server(AS)456||(7)891011(12)13Legend:indicatesapossibleinteractionwithahumanindicatesaninteractionbetweenprotocolroles~~~indicatesapotentialequivalenceorout-of-bandcommunicationbetweenrolesRFC 9635
...
behalf of the current user, who acts as both the end user and the RO. Since the client instance is
capable of directing the user to an arbitrary URI and receiving responses from the user's
browser, interaction here is handled through front-channel redirects using the user's browser.
...
The redirection URI used for interaction is a service hosted by the AS in this example. The client
instance uses a persistent session with the user to ensure the same user that is starting the
...
(2) The client instance requests access to the resource (Section 2). The client instance
indicates that it can redirect to an arbitrary URI (Section 2.5.1.1) and receive a redirect from
the browser (Section 2.5.2.1). The client instance stores verification information for its
redirect in the session created in (1).

•

(3) The AS determines that interaction is needed and responds (Section 3) with a URI to send
the user to (Section 3.3.1) and information needed to verify the redirect (Section 3.3.5) in (7).
The AS also includes information the client instance will need to continue the request
(Section 3.1) in (8). The AS associates this continuation information with an ongoing request
...
ClientASEndInstanceUser1StartSession2RequestAccess3InteractionNeeded4RedirectforInteraction5AuthNRO6AuthZEnd7RedirectforContinuationUser8ContinueRequest9GrantAccess10AccessAPIRS||11APIResponseRFC 9635
...
(4) The client instance stores the verification and continuation information from (3) in the
...
(7) When the AS is done interacting with the user, the AS redirects the user back (Section
4.2.1) to the client instance using the redirect URI provided in (2). The redirect URI is
augmented with an interaction reference that the AS associates with the ongoing request
created in (2) and referenced in (4). The redirect URI is also augmented with a hash of the
security information provided in (2) and (3). The client instance loads the verification
information from (2) and (3) from the session created in (1). The client instance calculates a
hash (Section 4.2.3) based on this information and continues only if the hash validates. Note
that the client instance needs to ensure that the parameters for the incoming request match
those that it is expecting from the session created in (1). The client instance also needs to be
prepared for the end user never being returned to the client instance and handle timeouts
appropriately.

•

(8) The client instance loads the continuation information from (3) and sends the interaction
...
(9) If the request has been authorized, the AS grants access to the information in the form of
...
(1) The client instance requests access to the resource (Section 2). The client instance
indicates that it can display a user code (Section 2.5.1.3).

•

(2) The AS determines that interaction is needed and responds (Section 3) with a user code to
...
(3) The client instance stores the continuation information from (2) for use in (8) and (10).
The client instance then communicates the code to the user (Section 4.1.2) given by the AS in
(2).
...
ClientASEndInstance1RequestAccessUser2InteractionNeeded3DisplayUserCode4OpenURI5ROAuthN9ContinueRequest(A)610NotYetGranted(Wait)Code7AuthZ8Complete11ContinueRequest(B)EndUser12GrantAccess13AccessAPIRS||14APIResponseRFC 9635
...
(4) The user directs their browser to the user code URI. This URI is stable and can be
...
(6) The RO enters the code communicated in (3) to the AS. The AS validates this code against
a current request in process.
...
(8) When the AS is done interacting with the user, the AS indicates to the RO that the request
has been completed.
...
(10) If the access request has not yet been authorized by the RO in (6), the AS responds to the
...
(11) The client instance continues to poll the AS (Section 5.2) with the new continuation
information in (9).

•

(12) If the request has been authorized, the AS grants access to the information in the form of
access tokens (Section 3.2) and direct subject information (Section 3.4) to the client instance.
...
(1) The client instance requests access to the resource (Section 2). The client instance does
not send any interaction modes to the server, indicating that it does not expect to interact
with the RO. The client instance can also signal which RO it requires authorization from, if
known, by using the subject request field (Section 2.2) and user request field (Section 2.4). It's
also possible for the AS to determine which RO needs to be contacted by the nature of what
access is being requested.

•

(2) The AS determines that interaction is needed, but the client instance cannot interact with
the RO. The AS responds (Section 3) with the information the client instance will need to
...
combination of the user request (Section 2.4), the subject request (Section 2.2), the access
request (Section 2.1), and other policy information. The AS contacts the RO and authenticates
them.
...
(5) When the AS is done interacting with the RO, the AS indicates to the RO that the request
has been completed.
...
ClientASROInstance1RequestAccess2NotYetGranted(Wait)3AuthN6ContinueRequest(A)47NotYetGranted(Wait)AuthZ5Completed8ContinueRequest(B)9GrantAccess10AccessAPIRS||11APIResponseRFC 9635
...
(7) If the access request has not yet been authorized by the RO in (6), the AS responds to the
client instance to continue the request (Section 3.1) at a future time through additional
...
(8) The client instance continues to poll the AS (Section 5.2) with the new continuation
information from (7).
...
(1) The client instance requests access to the resource (Section 2). The client instance does
not send any interaction modes to the server.
...
ClientASInstance1RequestAccess2GrantAccess3AccessAPIRS||4APIResponseRFC 9635
...
(2) The AS determines that the request has been authorized based on the identity of the
client instance making the request and the access requested (Section 2.1). The AS grants
access to the resource in the form of access tokens (Section 3.2) to the client instance. Note
that direct subject information (Section 3.4) is not generally applicable in this use case, as
...
In this example flow, the client instance receives an access token to access an RS through some
valid GNAP process. The client instance uses that token at the RS for some time, but eventually
...
(2) The AS grants access to the resource (Section 3) with an access token (Section 3.2) usable
at the RS. The access token response includes a token management URI.
...
ClientASInstance1RequestAccess2GrantAccess3AccessResourceRS4SuccessResponse(TimePasses)5AccessResource6ErrorResponse7RotateToken8RotatedTokenRFC 9635
...
(6) The RS validates the access token and determines that the access token is expired. The RS
responds to the client instance with an error.

•

(7) The client instance calls the token management URI returned in (2) to rotate the access
token (Section 6.1). The client instance uses the access token (Section 7.2) in this call as well
as the appropriate key; see Section 6.1 for details.

•

(8) The AS validates the rotation request, including the signature and keys presented in (7),
and refreshes the access token (Section 3.2.1). The response includes a new version of the
access token and can also include updated token management information, which the client
...
ClientASEndInstanceUser1RequestAccess2InteractionNeeded3FacilitateInteraction4ROAuthN5AuthZEnd6SignalContinuationUser7ContinueRequest8GrantAccessRFC 9635
...
(2) The AS determines that interaction is needed and responds (Section 3) with appropriate
information for facilitating user interaction (Section 3.3).

•

(3) The client instance facilitates the user interacting with the AS (Section 4) as directed in
(2).
...
(6) When the AS is done interacting with the user, the AS returns the user to the client
instance and signals continuation.

•

(7) The client instance loads the continuation information from (2) and calls the AS to
continue the request (Section 5).

•

(8) If the request has been authorized, the AS grants access to the requested direct subject
...
the client instance requested; see Section 3.4 for details.

1.6.8.  Cross-User Authentication

In this scenario, the end user and RO are two different people. Here, the client instance already
knows who the end user is, likely through a separate authentication process. The end user,
operating the client instance, needs to get subject information about another person in the
system, the RO. The RO is given an opportunity to release this information using an
asynchronous interaction method with the AS. This scenario would apply, for instance, when the
end user is an agent in a call center and the RO is a customer authorizing the call-center agent to
access their account on their behalf.
...
Precondition: The end user is authenticated to the client instance, and the client instance has an
...
(1) The RO communicates a human-readable identifier to the end user, such as an email
address or account number. This communication happens out of band from the protocol,
such as over the phone between parties. Note that the RO is not interacting with the client
instance.
...
includes the RO's identifier in the sub_ids field of the subject information request (Section
2.2) and the end user's identifier in the user field (Section 2.4). The request includes no
...
EndROUser1IdentifyRO2ClientASROIDInstance3Req.4Res.5AuthN6AuthZ78FinishCompleted9Cont.10Subj.11InfoReturnROInfoRFC 9635

Grant Negotiation and Authorization Protocol (GNAP)

October 2024

interaction start methods, since the end user is not expected to be the one interacting with
the AS. The request does include the push-based interaction finish method (Section 2.5.2.2) to
allow the AS to signal to the client instance when the interaction with the RO has concluded.

•

(4) The AS sees that the identifiers for the end user and subject being requested are different.
...
(5) The AS contacts the RO and has them authenticate to the system. The means for doing this
are outside the scope of this specification, but the identity of the RO is known from the
Subject Identifier sent in (3).

•

(6) The RO is prompted to authorize the end user's request via the client instance. Since the
end user was identified in (3) via the user field, the AS can show this information to the RO
...
(8) The RO pushes the interaction finish message (Section 4.2.2) to the client instance. Note
that in the case the RO cannot be reached or the RO denies the request, the AS still sends the
interaction finish message to the client instance, after which the client instance can negotiate
next steps if possible.

•

(9) The client instance validates the interaction finish message and continues the grant
request (Section 5.1).
...
(11) The client instance can display or otherwise utilize the RO's user information in its
session with the end user. Note that since the client instance requested different sets of user
...
To start a request, the client instance sends an HTTP POST with a JSON
...
to be returned directly in the response from the AS.
...
directly or by interacting with the end user to determine their status as the RO.
Section 2.4.
...
complete.
 ...
 if interaction is supported. See Section 2.5.
...
Sending a request to the grant endpoint creates a grant request in the processing state. The AS
...
MUST
 ...
 be sent as a JSON object in the content of the HTTP POST request with Content-

The request
Type application/json. A key proofing mechanism
...
payload of a JWS wrapper, which is in turn sent as the message content.
 ...
 define an alternative content type, as
...
If the client instance is requesting one or more access tokens for the purpose of accessing an API,
 be an object (for a single
 include an access_token field. This field
the client instance
...
the access token to be used at the RS.
...
by the end user. If this field is included in the request, the AS
 include the same label in
the token response (Section 3.2).
tokens (Section 2.1.2);
OPTIONAL

 if used as part of a request for multiple access
...
that key's most recent rotation), and the access token
 be presented using the same key
...
MUST NOT
 ...
 be included more than once. If the request includes a flag value multiple

MUST

 return an invalid_flag error defined in Section 3.6.
...
If access is approved, the resulting access token is valid for the described resource. Since the
...
an array of objects as the value of the access_token parameter. Each object
...
or the values of the label field are not unique within the array, the AS
"invalid_request" error (Section 3.6).

 include the label field, and all values of these fields
...
The following non-normative example shows a request for two separate access tokens: token1
and token2.
...
All approved access requests are returned in the response structure for multiple access tokens
...
the RO, as defined by

[RFC9493] REQUIRED
.

 if Subject Identifiers are requested.

assertion_formats (array of strings):

include id_token for an OpenID Connect ID Token
Markup Language (SAML) 2 assertion
the "GNAP Assertion Formats" registry (Section 10.6).

[SAML2]

. Additional assertion formats can be defined in

REQUIRED

 if assertions are requested.

An array of requested assertion formats. Possible values
 and saml2 for a Security Assertion

[OIDC]

sub_ids (array of objects):

information is being requested. Each object is a Subject Identifier as defined by
identifiers in the sub_ids array
assume that subject information requests are about the current user and
...
SHOULD
 ...
 identify the same subject. If omitted, the AS
SHOULD
...
(Section 2.4). If this is determined positively, the AS
response (Section 3.4) as requested.

 return the RO's information in its
...
When sending a new grant request to the AS, the client instance
 identify itself by including
...
pending or the approved state, the AS can determine which client is associated with the grant by
...
during interaction, authorization, and management.
...
certificate). If the AS is not able to interpret or validate the class_id field, it
...
MUST
 ...
 prove possession of any presented key by the proofing mechanism
...
If the same public key is sent by value on different access requests, the AS
...
MUST NOT
 ...
 send a symmetric key by value in the key field of the request, as
...
class_id or display, the pre-registered values
runtime. Additional fields sent during a request but not present in a pre-registered client
 be added to the client's pre-registered record. See
instance record at the AS
additional considerations regarding client instance impersonation in Section 11.15.

 take precedence over any values given at

SHOULD NOT

MUST

A client instance that is capable of talking to multiple ASes
...
lieu of the client object. The instance identifier
through a grant response (Section 3.5) or
registration process at the AS.

 be obtained in another fashion, such as a static

 be assigned to a client instance at runtime
...
information to present to the RO during any interactive sequences.
 ...
 send that information in the "display" field. This field is a JSON object that declares
...
Display image to represent the client software. This URI
...
MAY
 ...
 be passed by value by using a data: URI
...
SHOULD
 ...
 use these values during interaction with the RO. The values are for informational
...
MUST NOT
 restrict display values to specific client instances, as identified by their keys

 be taken as authentic proof of the client instance's identity or
...
authenticate the client instance to the AS. The AS
 associate policies with the client instance
...
and created just for this series of requests. As such, the AS
...
page applications) and client software with many individual long-lived instances (such as mobile
 ...
 allow for client instances to make
...
applications) to generate key pairs per instance and use the keys within the protocol without
having to go through a separate registration step. The AS
 limit which capabilities are made
...
assertions, the client instance
instance

 pass this information by value or by reference (see Section 2.4.1).

 send that information to the AS in the user field. The client
...
SHOULD
 ...
 be validated by the AS. While the details of

Assertions presented by the client instance
...
If the identified end user does not match the RO present at the AS during an interaction step and
the AS is not explicitly allowing a cross-user authorization, the AS
...
such as an opaque identifier issued by the AS for this specific client instance, the AS
...
reference for the end user at this AS, the client instance
format of this string is opaque to the client instance.

 pass that reference as a string. The
...
If the AS does not recognize the user reference, it
3.6).

MUST

 return an unknown_user error (Section
...
as locale. A client instance
 declare an interaction mode it does not support. The client
instance
 send multiple modes in the same request. There is no preference order specified in
this request. An AS
3.3) in a request, depending on its capabilities and what is allowed to fulfill the request.

 respond to any, all, or none of the presented interaction modes (Section
...
cannot accept a redirect or push-style callback. This pattern is common for devices that have
robust display capabilities but expect the use of a secondary device to facilitate end-user
interaction with the AS, such as a set-top box capable of displaying an interaction URL as a QR
code.
...
In the following non-normative example, the client instance is indicating that it cannot start any
...
(Section 3.6) since the client instance will be unable to complete the request without
authorization:

MUST

 return an invalid_interaction error
...
Interaction start modes defined as objects
the object.

MAY

 define additional parameters to be required in
...
URI is usually communicated out of band to the RO through documentation or other messaging
...
has completed. This URI
MAY
the client instance. This URI

Indicates the URI that the AS will use to signal the client instance that interaction
 be hosted by or accessible to
 contain any fragment

 be unique per request and
MUST

 be an absolute URI and
...
interaction response, it
 use a unique callback URI to link to that ongoing state. The
allowable URIs and URI patterns
presented key information. The callback URI
interaction phase before redirect.
REQUIRED

SHOULD
 for redirect and push methods.

 be restricted by the AS based on the client instance's

 be presented to the RO during the
...
REQUIRED
 ...
 be generated by the client instance as a unique value for this request.
...
defined in Section 4.2.3. All requests to the callback URI
Section 4.2.

 use this nonce to allow the client to verify the connection between the pending

 be processed as described in
...
All interaction finish methods
continuing this grant request. This means that the interaction reference
AS and
interaction reference is returned to the client instance are specific to the interaction finish
method.

 be presented by the client as described in Section 5.1. The means by which the

 require presentation of an interaction reference for
...
Requests to the callback URI
4.2.1.

MUST

 be processed by the client instance as described in Section
...
If possible, the AS
the array supported by the AS. If none of the given locales are supported, the AS
default locale.

 use one of the locales in the array, with preference to the first item in
...
more continuation requests.
instance on this grant request. See Section 3.1.

REQUIRED

 if continuation calls are allowed for this client

access_token (object / array of objects):

A single access token or set of access tokens that the

client instance can use to call the RS on behalf of the RO.
...
REQUIRED
 ...
 if interaction is expected. See Section 3.3.
...
In the following non-normative example, the AS is returning a bearer access token (Section 3.2.1)
...
response with no Content-Type).
 ...
 be sent as a JSON object in the content of the HTTP response with Content-

MUST

 include the HTTP Cache-Control response header field
...
The URI at which the client instance can make continuation requests. This URI
 be an absolute URI. The client

 be stable at the AS. This URI
...
MAY
 ...
 use this value exactly as given when making a continuation request (Section 5).
...
The amount of time in integer seconds the client instance
...
seconds.
 ...
 be less than five seconds, and omission of the value

.
RECOMMENDED

 be interpreted as five
...
"continuation access token". The value of this property
specified in Section 3.2.1. This access token
the request and
MUST NOT
 contain the string bearer, and the key field
token
MUST NOT
token
 have a manage field. The client instance
access token in all requests to the continuation URI as described in Section 7.2.

 be bound to the client instance's key used in
 be a bearer token. As a consequence, the flags array of this access

 be an object in the format

 present the continuation

 be omitted. This access
...
REQUIRED
 ...
 if the grant request is in the pending state, as the field contains the
...
present the continuation access token in its continuation request.
 ...
 sign all continuation requests with its key as described in Section 7.3 and
...
MUST
 ...
 be in the approved state to include this field in the response.
...
The value of the access token as a string. The value is opaque to the client
 be limited to the token68 character set defined in
...
request (Section 2.1), if present.
included in the single access token request;
label was included in the request.

REQUIRED

 for multiple access tokens or if a label was

OPTIONAL

 for a single access token where no

manage (object):

Access information for the token management API for this access token. If
provided, the client instance
 manage its access token as described in Section 6. This
management API is a function of the AS and is separate from the RS the client instance is
...
defined in Section 8. If included, this
token. These rights
...
MUST
 ...
 reflect the rights associated with the issued access

 vary from what was requested by the client instance.
...
MAY
 ...
 use the access token past this time. Note that the access token
...
presented key. The key
client instance
 be able to dereference or process the key information in order to be able
...
in Section 11.38. It is
 that keys returned for use with access tokens be key
...
The URI of the token management API for this access token. This URI

absolute URI. This URI
value of the access token used to protect the URI. This URI
access token issued in a request.
...
.
 ...
 be an
 include the value of the access token being managed or the
SHOULD
...
management access token". The value of this property
specified in Section 3.2.1. This access token
the request (or its most recent rotation) and
the flags array of this access token
 be omitted. This access token
MUST
...
MUST
 ...
 be a bearer token. As a consequence,

 contain the string bearer, and the key field
 have a manage field. This access token
MUST
 present the

MUST

 have the same value as the token it is managing. The client instance
...
mechanism. If the bearer flag is present, the access token is a bearer token, and the key field
 be omitted. See Section 11.9 for additional considerations on the use of
in this response
bearer tokens.
...
resulting in the issuance of new access tokens. If this flag is omitted, the client instance can
...
and the key field is present with any value.
 ...
 reject any access token where the flags field contains the bearer flag

The following non-normative example shows a single access token bound to the client instance's
key used in the initial request. The access token has a management URI and has access to three
described resources (one using an object and two described by reference strings).
...
The following non-normative example shows a single bearer access token with access to two
described resources.
...
If the client instance requested a single access token (Section 2.1.1), the AS
with the structure for multiple access tokens.
...
members of which are distinct access tokens as described in Section 3.2.1. Each object
MUST
a unique label field, corresponding to the token labels chosen by the client instance in the
request for multiple access tokens (Section 2.1.2).
...
In the following non-normative example, two tokens are issued under the names token1 and
...
The AS
 refuse to issue one or more of the requested access tokens for any reason. In such
...
requested multiple access tokens (Section 2.1.2), the AS
token structure, even if only a single access token is granted. In such cases, the AS
with a structure for multiple access tokens containing one access token.

 respond with a single access
...
REQUIRED
 ...
 if the redirect interaction start
...
REQUIRED
 ...
 if the app interaction start mode is
...
REQUIRED
 ...
 if the user_code interaction start
...
client instance to verify the callback after interaction is completed.
...
methods expire, the client
 restart the interaction process for this grant request by
sending an update (Section 5.3) with a new interaction request field (Section 2.5).
omitted, the interaction response modes returned do not expire but
AS at any time.
...
MUST NOT
 ...
 respond with any interaction mode that the client instance did not indicate in

The AS
its request, and the AS
 respond with any interaction mode that the AS does not
...
MUST
 ...
 be in the pending state to include this field in the response.
...
which is a string containing the URI for the end user to visit. This URI
 be unique for the
request and
 contain any security-sensitive information such as user identifiers or
access tokens.
...
The URI returned is a function of the AS, but the URI itself
 be completely distinct from the
grant endpoint URI that the client instance uses to request access (Section 2), allowing an AS to
separate its user-interaction functionality from its backend security functionality. The AS will
...
is a string containing the URI for the client instance to launch. This URI
 be unique for the
request and
access tokens.
...
The client instance launches the URI as appropriate on its platform; the means for the client
instance to launch this URI are out of scope of this specification. The client instance
alter the URI in any way. The client instance
MAY
...
interaction in Section 4.1.4.
 ...
 attempt to detect if an installed application will
...
facilitate usability, this string
 consist only of characters that can be easily typed by the end
user (such as ASCII letters or numbers) and
manner (see Section 4.1.2). The string
MUST
attacker within the time it is accepted. The time in which this code will be accepted
short lived, such as several minutes. It is
characters in length.

MUST
 be randomly generated so as to be unguessable by an

 be processed by the AS in a case-insensitive

 that this code be between six and eight
...
The client instance
 communicate the "user_code" value to the end user in some fashion,
...
as an authentication factor for the RO.
 ...
 be stable, since the client
The URI that the end user is intended to enter the code into
...
user code, the client instance
the client instance is capable of communicating a long arbitrary URI to the end user, such as
through a scannable code, the client instance
for this purpose, instead of or in addition to the user code mode.

 instead use the "user_code_uri" mode (Section 2.5.1.4). If

 use the "redirect" mode (Section 2.5.1.1)
...
usability, this string
(such as ASCII letters or numbers) and
manner (see Section 4.1.3). The string
...
between six and eight characters in length.
 ...
 be processed by the AS in a case-insensitive
 be randomly generated so as to be unguessable by

 be short lived, such as several minutes. It is
...
The interaction URI that the client instance will direct the RO to. This URI

short enough to be communicated to the end user by the client instance. It is
that this URI be short enough for an end user to type in manually. The URI
the code value. This URI
...
MUST
 ...
 communicate the "code" to the end user in some fashion, such as
...
allow the end user to type or copy it to a secondary device without mistakes.
 ...
 also communicate the URI to the end user. Since it is expected that the
...
The URI returned is a function of the AS, but the URI itself
 be completely distinct from the
grant endpoint URI that the client instance uses to request access (Section 2), allowing an AS to
separate its user-interaction functionality from its backend security functionality. If the AS does
...
When the interaction is completed, the interaction component of the AS
...
the AS returns the approved information in the "subject" response field. The AS
 return the
...
assertion object described below.

REQUIRED

 if returning assertions.

updated_at (string):

Timestamp as a date string as described in

[RFC3339]

, indicating when the

 use this value to determine if it
identified account was last updated. The client instance
...
Subject Identifiers returned by the AS
 uniquely identify the RO at the AS. Some forms of
...
The client instance
 request or use any returned Subject Identifiers for communication
...
communication with the current user. To get such information, the client instance
...
MUST
 ...
 ensure that the returned subject information represents the RO. In most cases, the
The AS
AS will also ensure that the returned subject information represents the end user authenticated
 reuse Subject Identifiers for multiple different ROs.
interactively at the AS. The AS

SHOULD NOT

The "sub_ids" and "assertions" response fields are independent of each other. That is, a returned
assertion
the response. However, all Subject Identifiers and assertions returned
party.

 use a different Subject Identifier than other assertions and Subject Identifiers in
 refer to the same
...
MUST
 ...
 interpret all subject information in the context of the AS from which

The client instance
the subject information is received, as is discussed in Section 6 of
. For example, one
...
MAY
 ...
 define additional response properties in the "GNAP Subject

The grant request

MUST

 be in the approved state to return this field in the response.
...
, in JSON Web Token (JWT) compact format as a
...
, encoded as a single base64url string with no padding.
...
MAY
 ...
 also generate and return an instance identifier dynamically to the client

If desired, the AS
instance in the response to facilitate multiple interactions with the same client instance over
time. The client instance
the associated data values in the client field.

 use this instance identifier in future requests in lieu of sending

SHOULD

Dynamically generated client instance identifiers are string values that
client instance as secrets. Instance identifier values
...
A single ASCII error code defining the error. The value
...
MAY
 ...
 choose to only return the error as codes and provide the error as a

Alternatively, the AS
...
leading to prompting the RO again. Other fields

 include the continue field in the response along with the error, as
...
To facilitate the AS fulfilling this request, the client instance sends information about the actions
...
the requested access, the AS
immediately in its response (Section 3)

 move the grant request to the approved state and return results
...
The process of gathering authorization and consent in GNAP is left deliberately flexible to allow
for a wide variety of different deployments, interactions, and methodologies. In this process, the
AS can gather consent from the RO or apply the RO's policy as necessitated by the access that has
...
pattern is especially prevalent when the end user is sent to the AS for an interactive session,
...
means by which the client chooses which methods to follow are out of scope of this specification.
 ...
 initiate one or more of these interaction methods (Section 4.1) in order to
...
MUST
 ...
 use each interaction method once at most if a response can be detected.

The client instance
The AS
MUST
suitable timeouts to any interaction start methods provided, including user codes and redirection
URIs. The client instance

 handle any interact request as a one-time-use mechanism and

 apply suitable timeouts to any interaction finish method.
...
When interacting with an RO, the AS can use various strategies to determine the authorization of
...
While all of these cases are supported by GNAP, the details of their implementation and the
methods for determining which ROs or related policies are required for a given request are out
...
the AS
 ...
 reject attempts to use other interaction start modes. For example, if a user code has

 reject attempts to use the interaction start modes. If the client instance has
...
been successfully entered for a grant request, the AS will need to reject requests to an arbitrary
...
URI are out of scope for this specification.
 ...
 attempt to redirect the RO back to any client

 display an error to the RO and

MUST NOT

MUST NOT

 modify the URI when launching it; in particular, the client instance

 add any parameters to the URI. The URI

The client instance
MUST NOT
browser, though the URI
URI
 be protected by HTTPS, be
hosted on a server local to the RO's browser ("localhost"), or use an application-specific URI
scheme that is loaded on the end user's device.

 be accessible from an HTTP GET request, and it

 be reachable from the end user's

 be opened on a separate device from the client instance itself. The
...
that code at an associated URI. The client instance
 format the user code in such a way as to
...
RECOMMENDED
 ...
 that the AS choose from character values that are easily copied and typed
It is
...
instance or in the client software's documentation. As a consequence, these URIs
 be
short. The user code URI
usually opened on a separate device from the client instance itself. The URI
from an HTTP GET request, and it
MUST
RO's browser ("localhost"), or use an application-specific URI scheme that is loaded on the end
user's device.

 be reachable from the end user's browser, though the URI is

 be protected by HTTPS, be hosted on a server local to the
...
currently active request, the AS
...
When the RO enters this code at the user code URI, the AS
request that the code was associated with. If the AS does not recognize the entered code, the
interaction component
unrecognized code enter attempts, the interaction component
user indicating too many attempts and
input interactions. The user should be warned as such an error state is approached, if possible.

 display an error to the
 take additional actions such as slowing down the

 display an error to the user. If the AS detects too many

 uniquely identify the pending
...
directs the end user to enter that code at the URI. The client instance
...
This mode is used when the client instance is not able to facilitate launching a complex arbitrary
URI but can communicate arbitrary values like URIs. As a consequence, these URIs
short enough to allow the URI to be typed by the end user, such as a total length of 20 characters
 modify the URI when communicating it to the end user;
or fewer. The client instance
 add any parameters to the URI. The user code URI
in particular the client instance
MUST
separate device from the client instance itself. The URI
request, and it
("localhost"), or use an application-specific URI scheme that is loaded on the end user's device.
 ...
 be protected by HTTPS, be hosted on a server local to the RO's browser

 be accessible from an HTTP GET
...
When the RO enters this code at the given URI, the AS
request that the code was associated with. If the AS does not recognize the entered code, the
interaction component
unrecognized code enter attempts, the interaction component
user indicating too many attempts and
...
instance to continue, except for some limited error cases discussed below. If a finish method is
 instruct the RO to return to the client instance upon completion. In
not available, the AS
...
MUST
 ...
 create an interaction reference and associate that reference with the current

The AS
interaction and the underlying pending request. The interaction reference value is an ASCII
string consisting of only unreserved characters per
. The interaction
reference value
MUST
interaction reference

 be sufficiently random so as not to be guessable by an attacker. The
MUST

 be one-time-use to prevent interception and replay attacks.

Section 2.3 of [RFC3986]

MUST

 calculate a hash value based on the client instance, AS nonces, and the interaction

The AS
reference, as described in Section 4.2.3. The client instance will use this value to validate the
"finish" call.

 define a way to convey the hash and interaction reference
All interaction finish methods
...
continuation request. The AS
circumstances:

 follow the "finish" method in the following
...
MUST
 ...
 be able to process a request on the URI. If the URI is HTTP, the request

 parse the query parameters to extract the
When receiving the request, the client instance
hash and interaction reference values. The client instance
...
If the hash does not validate, the client instance
AS.

 send the interaction reference to the
...
making this call, as discussed in Section 11.34.
 ...
 protect itself against Server-Side Request Forgery (SSRF) attacks when

MUST

When receiving the request, the client instance
value as described in Section 4.2.3. If either fails, the client instance
...
received here.
 ...
 parse the JSON object and validate the hash
...
and injection attacks as discussed in Section 11.25. The AS
 always provide this hash, and
...
the "nonce" value sent by the client instance in the interaction finish field of the initial
...
If provided, the "hash_method" value
...
MUST
 ...
 be one of the hash name strings defined in the IANA

. If the "hash_method" value is not
...
any calls to the continuation URI, the client instance
 present the continuation access token
as described in Section 7.2 and present proof of the client instance's key (or its most recent
rotation) by signing the request as described in Section 7.3. The AS
...
Access tokens other than the continuation access tokens
requests. Conversely, continuation access tokens
...
MUST NOT
 ...
 be usable for continuation

 be usable to make authorized
...
MUST
 ...
 be able to tell from the client instance's request which specific ongoing request is
...
MUST NOT
period is indicated, the client instance
seconds. If the client instance does not respect the given wait period, the AS
too_fast error (Section 3.6).

 wait at least 5
 return the

 poll immediately and
...
The response from the AS is a JSON object of a grant response and
described in Section 3, as described in more detail in the subsections below.

MAY

 contain any of the fields
...
MUST
 ...
 include a continuation access token as well, and this token

If the AS determines that the client instance can make further requests to the continuation API,
the AS
 include a new continuation response (Section 3.1). The new continuation response
 be a new access token,
MUST
invalidating the previous access token. If the AS does not return a new continuation response,
the client instance
so, the AS

 make an additional continuation request. If a client instance does
...
For all requests to the grant continuation API, the AS
...
. That is to say, instead of returning the current status

 make use of long polling mechanisms
...
includes an interaction reference. The client instance
interact_ref in a POST request to the continuation URI.

 include that value as the field
...
return a too_many_attempts error (Section 3.6) and
moving it to the finalized state.

 invalidate the ongoing request by

SHOULD

MUST

If the grant request is in the approved state, the grant response (Section 3)
newly created access tokens (Section 3.2) or newly released subject information (Section 3.4). The
response
response

 contain a new continuation response (Section 3.1) as described above. The
...
If the grant request is in the pending state, the grant response (Section 3)
access tokens or subject information and
any interaction methods that have not been exhausted at the AS.

MAY

 contain a new interaction response (Section 3.3) to
...
response
continue field is included, it
by the client instance. The response

 contain a new continuation response (Section 3.1) as described above. If a

 include a wait field to facilitate a reasonable polling rate

 contain interaction responses (Section 3.3).
...
See Section 11.23 for considerations on polling for continuation without an interaction finish
method.
...
Since this response does not include a continue field, the client instance cannot continue to poll
the AS for additional updates and the grant request is finalized. If the client instance still needs
access to the resource, it will need to start with a new grant request.

5.3.  Modifying an Existing Request

The client instance might need to modify an ongoing request, whether or not tokens have
already been issued or subject information has already been released. In such cases, the client
instance makes an HTTP PATCH request to the continuation URI and includes any fields it needs
to modify. Fields that aren't included in the request are considered unchanged from the original
request.

A grant request associated with a modification request
When the AS receives a valid modification request, the AS
...
request, since the extent and context of the request could have changed.
 ...
 be in the approved or pending state.
 place the grant request into the
...
MAY
 ...
 include the access_token and subject fields as described in Sections

The client instance
2.1 and 2.2. Inclusion of these fields override any values in the initial request, which
...
MAY
 ...
 include the interact field as described in Section 2.5. Inclusion of this

The client instance
field indicates that the client instance is capable of driving interaction with the end user, and this
field replaces any values from a previous request. The AS
responses as described in Section 3.3, just like it would to a new request.

 respond to any of the interaction

MAY

MAY

 include the user field as described in Section 2.4 to present new

The client instance
assertions or information about the end user. The AS
...
or otherwise associated with this grant request.
 ...
 check that this presented user
...
October 2024
 ...
 include the client field of the request, since the client instance is
The client instance
assumed not to have changed. Modification of client instance information, including rotation of
keys associated with the client instance, is outside the scope of this specification.
...
MUST NOT
 ...
 include post-interaction responses such as those described in

Modification requests
issued from a continuation are considered new, separate access tokens. The AS
previously issued access tokens after a modification has occurred.

MUST NOT

 alter previously issued access tokens. Instead, any access tokens
...
approved state), the grant response (Section 3)
(Section 3.2) or newly released subject claims (Section 3.4). The response
continuation response (Section 3.1) as described above. If interaction can occur, the response
SHOULD

 contain any newly created access tokens
 contain a new
...
This continue field allows the client instance to make an eventual continuation call. Some time
...
As another example, the client instance initially requests read-only access but later needs to step
...
This allows the client instance to make an eventual continuation call. The client instance later
realizes that it now needs "write" access in addition to the "read" access. Since this is an
expansion of what it asked for previously, the client instance also includes a new interaction field
in case the AS needs to interact with the RO again to gather additional authorization. Note that
the client instance's nonce and callback are different from the initial request. Since the original
callback was already used in the initial exchange and the callback is intended for one-time use, a
new one needs to be included in order to use the callback again.
...
with a new request. Since the old access tokens are good for a subset of the rights requested here,
...
grant request is in the finalized state, it

 revoke all associated access tokens, if possible. The AS
...
in the following subsections. Other actions are undefined by this specification.
 ...
 call this URI to manage the access token with the rotate and revoke actions defined
...
the token management API. The client instance
token along with the value of the token management access token.

 present proof of the key associated with the
...
access token.
 ...
 validate the proof and ensure that it is associated with the token management
...
token management access token, or a combination of both.
 ...
 uniquely identify the token being managed from the token management URI, the
...
If the token is validated and the key is appropriate for the request, the AS
...
For successful rotations, the AS responds with an HTTP status code 200 (OK) with JSON-formatted
message content consisting of the rotated access token in the access_token field described in
Section 3.2.1. The value of the access token
access token used to access the management API. The response
management URI, and the value of this URI
instance to make the rotation call. The client instance
rotated access token.

 be different from the URI used by the client
 use this new URI to manage the

 be the same as the current value of the
...
response and
 ...
 be the same as the token before rotation.
...
If the AS is unable or unwilling to rotate the value of the access token, the AS responds with an
...
passed by value are always public keys.
 ...
 when doing key rotation.

The proofing method and parameters for the new key
the previous key.
...
MUST
 ...
 prove possession of both the currently bound key and the newly
The client instance
...
example.
 ...
 cover the value or reference of the new key, and the signature of the new key
...
Failure to present the appropriate proof of either the new key or the previous key for the access
 result in an invalid_rotation error code from
token, as defined by the proofing method,
the AS (Section 3.6).

MUST

An attempt to change the proofing method or parameters, including an attempt to rotate the key
 result in an invalid_rotation error code returned
of a bearer token (which has no key),
...
URI to indicate to the AS that the AS
...
The client instance makes an HTTP DELETE request to the token management URI, presenting
...
token), the AS
 invalidate the access token, if possible, and return an HTTP response code
204.
...
additional discussion of the presentation of public keys in Section 11.7.
 ...
 be presented in only one supported
...
.
 ...
 contain the alg (Algorithm) and kid (Key ID) parameters. The alg parameter

 be "none". The x5c (X.509 Certificate Chain) parameter
...
the request, with optional internal whitespace per
.
optionally removed.
OPTIONAL
...
. The PEM header and footer are

cert#S256 (string):

The certificate thumbprint calculated as per MTLS for OAuth

base64url encoding. Note that this format does not include the full public key.
...
October 2024
 ...
 be used to bind the request,
When the key is presented in GNAP, proof of this key material
...
instance's public key, and proof of that key
MUST
part of an access token response in Section 3.2.1, the proof of that key
client instance later presents the access token to the RS.

 be presented in that request. For a key used as
MUST
...
Keys referenced in this manner
for symmetric keys in Section 11.7. The key reference
private or shared symmetric key information.

MAY

 be shared symmetric keys. See the additional considerations

MUST NOT

 contain any unencrypted

Keys referenced in this manner

MUST

 be bound to a single proofing mechanism.
...
mechanism
password or a value derived from one. Symmetric keys
client instance to the AS.

 be used to supply the keys. Symmetric keys
...
MUST
 ...
 be passed by value from the

 be a human-memorable
...
Section 7.1, the access token
 be sent using the key and proofing mechanism defined by the
...
MUST
 ...
 be sent using the HTTP Authorization request header field and the
...
If the flags field contains the bearer flag, the access token is a bearer token that
...
Any keys presented by the client instance to the AS or RS
 be validated as part of the request
in which they are presented. The type of binding used is indicated by the proof parameter of the
key object in Section 7.1. Key proofing methods are specified either by a string, which consists of
the key proofing method name on its own, or by a JSON object with the required field method:
...
Individual methods defined as objects
object.

MAY

 define additional parameters as members in this
...
Proofing methods
method can be specified as an object with its parameters explicitly declared, such as:

 be defined as both an object and a string. For example, the httpsig
...
All key binding methods used by this specification
...
Definitions of key binding methods

 validate all components of the signed message to ensure that nothing

 enumerate how these requirements are fulfilled.

 cover all relevant portions of the

MUST

MUST

When a key proofing mechanism is bound to an access token, the key being presented
the key associated with the access token, and the access token
...
The key binding methods in this section
of GNAP, such as the extensions allowing the RS to make calls to the AS defined in
facilitate this extended use, "signer" and "verifier" are used as generic terms in the subsections

 be used by other components making calls as part
. To
...
below. In the core functions of GNAP specified in this document, the "signer" is the client
instance, and the "verifier" is the AS (for grant requests) or RS (for resource requests), as
appropriate.
...
the case of bearer tokens, the client instance's key.
 ...
 be proved in all continuation requests (Section 5). Token

MUST

In the following subsections, unless otherwise noted, the RS256 JSON Object Signing and
Encryption (JOSE) signature algorithm (defined in
...
Key proofing methods
Section 6.1.1. Key rotation mechanisms
MUST
simultaneously with the following attributes:

SHOULD

 define a mechanism to allow the rotation of keys discussed in

 define a way for presenting proof of two keys

•

The value of or reference to the new key material
Generally speaking, this amounts to using the existing key to sign the content of the message
that contains the new key.

 be signed by the existing key.

MUST

•

The signature of the old key
including the signature value of the old key under the coverage of the new key.

 be signed by the new key. Generally speaking, this means
...
When the proofing method is specified in string form, the signing algorithm
from the key material (such as using the JWS algorithm in a JWK formatted key), and the content
digest algorithm
...
. The covered components of the signature
...
The Content-Digest header as defined in

[RFC9530]
 calculate this field value and include the field in the
...
REQUIRED
 ...
 when the message request

message has content, the signer
request. The verifier
MUST
...
MUST
 ...
 include the tag signature parameter with the value gnap, and the verifier

The signer
verify that the parameter exists with this value. The signer
parameter with a timestamp of when the signature was created, and the verifier
that the creation timestamp is sufficiently close to the current time given expected network delay
 include the nonce parameter with a unique and unguessable
and clock skew. The signer
SHOULD
value. When included, the verifier
 determine that the nonce value is unique within a
MUST
reasonably short time period such as several minutes.

 include the created signature
...
If the signer's key presented is a JWK, the keyid parameter of the signature
kid value of the JWK, and the signing algorithm used
key's alg field of the JWK.

MUST
 be the JWS algorithm denoted by the

 be set to the

MUST

The explicit alg signature parameter
will be derived from either the key material or the proof value.

MUST NOT

 be included in the signature, since the algorithm
...
The verifier
message includes content, the verifier
Digest header. The verifier

MUST

MUST

 calculate and verify the value of the Content-

 validate the signature against the expected key of the signer.

 ensure that the signature covers all required message components. If the HTTP
...
policy and meets the requirements in this section.
 ...
 include multiple signatures, each with its own label. The verifier
...
When rotating a key using HTTP message signatures, the message, which includes the new public
key value or reference, is first signed with the old key following all of the requirements in
...
MUST
 ...
 include the Signature and Signature-Input values from the
...
The signer then creates a new signature using the new key, adding the signature input and value
...
MUST
 ...
 validate both signatures before processing the request for key rotation.
...
through the Client-Cert header field from a TLS reverse proxy as per
...
The verifier compares the TLS client certificate presented during MTLS negotiation to the
...
Since it is not possible to present two client authenticated certificates to a MTLS connection
...
The signer creates a JSON Web Signature (JWS)
...
REQUIRED
 ...
 if the key is presented in JWK format. This
...
The algorithm used to sign the request. The algorithm

MUST

 be appropriate to the

key presented. If the key is presented as a JWK, this
key. The algorithm
...
MUST
 ...
 be equal to the alg parameter of the
...
The HTTP URI used for this request. This value

MUST

 be an absolute URI, including
...
When the request is bound to an access token, the JOSE header
...
The hash of the access token. The value

MUST

 be the result of base64url encoding
...
object is the base64url encoding (without padding) of the SHA-256 digest of the bytes of the
...
MUST
 ...
 be validated against the expected key of the signer. If the HTTP message

When the verifier receives the Detached-JWS header, it
The signature
request contains content, the verifier
MUST
does, with no normalization or transformation of the request. All required fields
 match the corresponding portions of the
 be valid. All fields
present, and their values
HTTP message. For example, the htm field of the JWS header has to be the same as the HTTP verb
used in the request.

 calculate the hash of the content just as the signer

 parse and validate the JWS object.
...
Note that this proofing method depends on a specific cryptographic algorithm, SHA-256, in two
ways: 1) the ath hash algorithm is hardcoded and 2) the payload of the detached/attached
signature is computed using a hardcoded hash. A future version of this document may address
crypto-agility for both these uses by replacing ath with a new header that upgrades the algorithm
and possibly defining a new JWS header that indicates the HTTP content's hash method.
...
MUST
 ...
 be appropriate to the key presented.

If the key is presented as a JWK, this
.
be none.
REQUIRED
...
MUST
 ...
 be equal to the alg parameter of the key.
...
REQUIRED
 ...
 be the result of base64url encoding (with no

MUST

If the HTTP request has content (such as an HTTP POST or PUT method), the payload of the JWS
object is the JSON serialized content of the request, and the object is signed according to JWS and
. The signer presents the JWS as the content of the request
serialized into compact form
along with a content type of application/jose. The verifier
 extract the payload of the JWS
...
When the verifier receives an attached JWS request, it
The signature
MUST
 match the corresponding portions of
be present, and their values
the HTTP message. For example, the htm field of the JWS header has to be the same as the HTTP
verb used in the request.

 be validated against the expected key of the signer. All required fields
MUST
...
dimensions of access. Each object contains a
...
type property that determines the type of

REQUIRED

type (string):

The type of resource request as a string. This field

MAY

 define which other fields
...
The value of the type field is under the control of the AS. This field
exact byte match of the string value against known types by the AS. The AS
...
types protected by a single AS.
 ...
 do any collation or normalization of data types during comparison. It is

 ensure that
MUST

 be compared using an
...
(for example, a client instance asking for administrative-level access or access when the RO is
no longer online).
...
objects. The following non-normative example uses the same fictitious photo-api type definition
...
The access requested here is for read access to images on one server as well as write and delete
access for metadata on a different server (importantly, without requesting write or delete
access to images on the first server).
...
If this request is approved, the resulting access token's access rights will be the union of the
requested types of access for each of the two APIs, just as above.

8.1.  Requesting Resources by Reference

Instead of sending an object describing the requested resource (Section 8), access rights
MAY
...
This value is opaque to the client instance and
 be any valid JSON string; therefore, it could
...
such cases, the API designer choosing any such human-readable strings
...
other thing.
 ...
 include both object-type and string-type resource items. In this non-
...
optimize its calls to the AS before making a request, it
grant request endpoint to retrieve the server's discovery information. The AS
a JSON document with Content-Type application/json containing a single object with the
following fields:

 send an HTTP OPTIONS request to the
 respond with
...
request.
 ...
 with a scheme component (which

 match the URL the client instance used to make the discovery
...
methods. The values of this list correspond to the possible values for the interaction start field
of the request (Section 2.5.1) and
MUST
.
registry (Section 10.9).
OPTIONAL

 be values from the "GNAP Interaction Start Modes"
...
the interaction finish field of the request (Section 2.5.2) and
 be values from the "GNAP
Interaction Finish Methods" registry (Section 10.10).
...
section of the request (Section 7.1) and
Methods" registry (Section 10.16).

 be values from the "GNAP Key Proofing
MUST
.
OPTIONAL
...
formats. The values of this list correspond to possible values of the Subject Identifier field of
the request (Section 2.2) and
 be values from the "Subject Identifier Formats" registry
MUST
[Subj-ID-Formats] OPTIONAL
.
...
formats. The values of this list correspond to possible values of the subject assertion field of
the request (Section 2.2) and
(Section 10.6).

 be values from the "GNAP Assertion Formats" registry
...
The information returned from this method is for optimization purposes only. The AS
 deny
...
negotiate for access to that RS. The RS
parameters:

 additionally return the following
...
access: An opaque access reference as defined in Section 8.1.

MUST

action the client instance was attempting to take at the RS and
rights as well. Sent by the client as an access right in the grant request.

MAY

 be sufficient for at least the
 allow additional access

The client instance
token request. The client instance
...
MUST
 ...
 then use both the referrer and access parameters in its access

 check that the referrer parameter is equal to the URI of
...
If issued, the resulting access token would contain sufficient access to be used at both referenced
resources.
...
contextual methods for securely providing this endpoint to the client instance. Furthermore,
GNAP's design intentionally requires the client instance to only know the grant endpoint and not
additional parameters, since other functions and values can be disclosed and negotiated during
the grant process.

10.  IANA Considerations

IANA has added values to existing registries as well as created 16 registries for GNAP
REG]
 and populated those registries with initial values as described in this section.

[GNAP-

All use of value typing is based on data types in
...
SHOULD
 ...
 be specified in the definition. If a parameter is available in different types, each type
...
IANA has registered of the following scheme in the "HTTP Authentication Schemes" registry
[Auth-Schemes]
...
The request parameter's definition is sufficiently orthogonal to existing functionality
provided by existing parameters.

•

Registrations for the same name with different types are sufficiently close in functionality so
as not to cause confusion for developers.

•

The request parameter's definition specifies the expected behavior of the AS in response to
the request parameter for each potential state of the grant request.
...
Section 2.1.1 of RFC 9635

access_token

array of objects

Section 2.1.2 of RFC 9635
...
Section 2.2 of RFC 9635

Section 2.3 of RFC 9635

Section 2.3.1 of RFC 9635

Section 2.4 of RFC 9635

Section 2.4.1 of RFC 9635

interact

object

Section 2.5 of RFC 9635

interact_ref

string

Section 5.1 of RFC 9635
...
The flag specifies whether it applies to requests for tokens to the AS, responses with tokens
from the AS, or both.
...
Sections 2.1.1 and 3.2.1 of RFC 9635

durable

Response

Section 3.2.1 of RFC 9635
...
Section 7.1 of RFC 9635

string

Section 7.1.1 of RFC 9635
...
Any registration using an "object" type declares all additional parameters, their optionality,
and their purpose.

•

The start mode clearly defines what actions the client is expected to take to begin interaction,
what the expected user experience is, and any security considerations for this
communication from either party.

•

The start mode documents incompatibilities with other start modes or finish methods, if
applicable.

•

The start mode provides enough information to uniquely identify the grant request during
the interaction. For example, in the redirect and app modes, this is done using a unique URI
(including its parameters). In the user_code and user_code_uri modes, this is done using
the value of the user code.
...
Section 2.5.1.1 of RFC 9635

app

string

Section 2.5.1.2 of RFC 9635

user_code

string

Section 2.5.1.3 of RFC 9635

user_code_uri

string

Section 2.5.1.4 of RFC 9635
...
All finish methods clearly define what actions the AS is expected to take, what listening
methods the client instance needs to enable, and any security considerations for this
communication from either party.
...
Section 2.5.2.1 of RFC 9635

push

Section 2.5.2.2 of RFC 9635
...
All interaction hints clearly document the expected behaviors of the AS in response to the
hint, and an AS not processing the hint does not impede the operation of the AS or client
instance.
...
The response parameter's definition is sufficiently orthogonal to existing functionality
provided by existing parameters.
...
The response parameter's definition specifies grant states for which the client instance can
expect this parameter to appear in a response message.
...
Section 3.2.2 of RFC 9635

interact

object

Section 3.3 of RFC 9635

subject

object

Section 3.4 of RFC 9635

instance_id

string

Section 3.5 of RFC 9635
...
If the name of the registration matches the name of an interaction start mode, the response
parameter is unambiguously associated with the interaction start mode of the same name.
...
The proofing method provides sufficient coverage of and binding to the protocol messages to
which it is applied.

•

The proofing method definition clearly enumerates how all requirements in Section 7.3 are
fulfilled by the definition.
...
Section 7.3.2 of RFC 9635

jwsd

string

Section 7.3.3 of RFC 9635

jws

string

Section 7.3.4 of RFC 9635
...
The values in the discovery document are sufficient to provide optimization and hints to the
client instance, but knowledge of the discovered value is not required for starting a
transaction with the AS.
...
requests from the client instance to an RS, and any requests back to a client instance such as the
 ...
 to protect the contents of the request and response from manipulation and
...
This is particularly important in this specification for security artifacts such as nonces and for
...
interaction with the RO, and during any redirect back to the client instance. Without TLS
protection on these portions of the process, an attacker could wait for a valid request to start and
then take over the RO's interaction session.
...
client and the server (or the server only when using MTLS (Section 7.3.2)) have to validate the
[RFC9525]
,
other party's identity. Such a verification can be achieved via methods described in
but these are not enough to establish the identity of the client instance in many cases.
...
is no direct connection between the TLS key presentation and the message itself, other than the
...
client certificate into the forwarded request using the HTTP Client-Cert header field
giving the downstream system access to the certificate information. The TTRP has to be trusted to
...
provide accurate certificate information, and the connection between the TTRP and the
downstream system also has to be protected. The TTRP could provide some additional assurance,
for example, by adding its own signature to the Client-Cert header field using HTTP message
signatures
use GNAP's tag parameter value) but would be understood by the downstream service as part of
its deployment.

. This signature would be effectively ignored by GNAP (since it would not
...
generate a key pair on the device and present the public key, along with proof of holding the
...
an explicit prompt of the RO for approval during authorization, since such approval has already
been given. An AS doing such a process would need to take assurance measures that the different
instances are in fact correlated and authentic, as well as ensure that the expected RO is in control
...
integrated with each other and can be closely managed. It is particularly bad practice to allow an
...
client instances to present to RSs. As such, protecting the AS is central to any GNAP deployment.
...
Symmetric keys also have the expected advantage of providing better protection against
...
not validate the signature of all presented tokens would be susceptible to injection of a modified
...
valid. It also means that any party that is able to capture the token value in storage or in transit is
...
The RS also needs to ensure that sufficient portions of the message are covered by the signature.
...
being exposed to any credentials or information about the end user or RO as a requirement of
the delegation process. By using the variety of interaction mechanisms, the RO can interact with
the AS without ever authenticating to the client software and without the client software having
to impersonate the RO through replay of their credentials.

Consequently, no interaction methods defined in this specification require the end user to enter
...
in order to attempt to get the RO to approve access for the legitimate client instance.

A client instance needs to always be aware of which AS it is talking to throughout a grant process
...
compromised AS can be granted a valid access token on behalf of the RO. See Sections 4.5.5 and
5.5 of
 for details of one such attack, which has been addressed in this document
by including the grant endpoint in the interaction hash calculation. Note that the client instance
still needs to validate the hash for the attack to be prevented.
...
subsequently skip an explicit authentication event by the RO.
...
A special case exists where the AS is the generator of the assertion being presented by the client
...
given client to ask for certain kinds of information or use specific interaction mechanisms at
runtime.

It also is sensible to pre-register client instances when the software is acting autonomously,
without the need for a runtime approval by an RO or any interaction with an end user. In these
cases, an AS needs to rely on the trust decisions that have been determined prior to runtime to
determine what rights and tokens to grant to a given client instance.
...
An AS can also provide warnings and caveats to ROs during the authorization process, allowing
the user to make an informed decision regarding the software they are authorizing. For example,
if the AS has vetted the client software and this specific instance, it can present a different
authorization screen compared to a client instance that is presenting all of its information at
runtime.
...
An AS can also warn the RO about the provenance of the information it is displaying, allowing
the RO to make a more informed delegation decision. For example, an AS can visually
differentiate between a client instance that can be traced back to a specific developer's
registration and an instance that has self-asserted its own display information.
...
To avoid these issues, the AS can insist on the use of data: URIs, though that might not be
practical for all types of client software. Alternatively, the AS could pre-fetch the content of the
URI and present its own copy to the RO instead. This practice opens the AS to potential SSRF
attacks, as discussed in Section 11.34.
...
, a server should never use HTTP status code 307
...
request to the redirect target. In the HTTP standard
...
status code 302 (Found), user agents are allowed to keep a redirected POST request as a POST and
thus can resubmit the content.

, only status code 303 (See Other)
...
POST request). The AS checks the credentials and, if successful, may immediately redirect the RO
to the client instance's redirect URI. Due to the use of status code 307 (Temporary Redirect), the
RO's user agent now transmits the RO's credentials to the client instance. A malicious client
instance can then use the obtained credentials to impersonate the RO at the AS.
...
over to another component, be it the system browser, another application, or some action the RO
is instructed to take on another device. By using an interaction finish method, the client instance
can be securely notified by the AS when the interaction is completed and the next phase of the
protocol should occur. This process includes information that the client instance can use to
validate the finish call from the AS and prevent some injection, session hijacking, and phishing
attacks.
...
API while waiting for the RO to approve or deny the request. An attacker could take advantage of
this situation by capturing the interaction start parameters and phishing a legitimate user into
authorizing the attacker's waiting client instance, which would in turn have no way of
associating the completed interaction from the targeted user with the start of the request from
the attacker.

However, it is important to note that this pattern is practically indistinguishable from some
legitimate use cases. For example, a smart device emits a code for the RO to enter on a separate
device. The smart device has to poll because the expected behavior is that the interaction will
take place on the separate device, without a way to return information to the original device's
context.
...
single-page application) or in an associated session store on a backend server. In both cases,
...
token being bound to the expected interaction reference, but without hash calculation, the
...
(6) The AS completes the interact finish with interact reference IR1 and interact hash IH1
...
UserAttackerClientASInstance12345||6ABCD||E7FGRFC 9635
...
action the RO is instructed to take on another device. In order for the client instance to make its
continuation call, it will need to recall all of these protocol elements at a future time. Usually, this
means the client instance will need to store these protocol elements in some retrievable fashion.
...
security-sensitive information that would be valuable to an attacker, such as any token identifier,
...
All assertion processing needs to account for the security aspects of the assertion format in use.
...
For example, when SAML 2.0 assertions are used, the receiver has to parse an XML document.
...
problems with the processor. Therefore, any system capable of processing SAML 2.0 assertions
also needs to have a secure and correct XML parser. In addition to this, the SAML 2.0
specification uses XML Signatures, which have their own implementation problems that need to
be accounted for. Similar requirements exist for OpenID Connect ID Token, which is based on the
...
mix-up attacks (Section 11.12). Managing this binding is outside the scope of this specification,
but it can be managed either as a configuration element for the client instance or dynamically
through discovering the AS from the RS (Section 9.1).

The details of this attack, with additional discussion and considerations, are available in Section
3.2 of
...
client instance within GNAP. As discussed in
, the AS and RS can make use of stateless
access tokens with an internal structure and format. These access tokens allow an RS to validate
the token without having to make any external calls at runtime, allowing for benefits in some
deployments, the discussion of which is outside the scope of this specification.
...
In practice, an AS can hold onto an old token value for such limited purposes. For example, to
...
The most vulnerable place in this specification is the push-based post-interaction finish method
(Section 4.2.2), as the client instance is less trusted than the AS and can use this method to make
the AS call an arbitrary URI. While it is not required by the protocol, the AS can fetch other URIs
provided by the client instance, such as the logo image or home page, for verification or privacy-
preserving purposes before displaying them to the RO as part of a consent screen. Even if the AS
does not fetch these URIs, their use in GNAP's normal operation could cause an attack against the
end user's browser as it fetches these same attack URIs. Furthermore, extensions to GNAP that
allow or require URI fetch could also be similarly susceptible, such as a system for having the AS
fetch a client instance's keys from a presented URI instead of the client instance presenting the
key by value. Such extensions are outside the scope of this specification, but any system
deploying such an extension would need to be aware of this issue.
...
that supports the push-based interaction finish method can compare the callback URI in the
interaction request to a known URI for a pre-registered client instance, or it can ensure that the
URI is not on a blocklist of sensitive URLs such as internal network addresses. However, note that
because these types of calls happen outside of the view of human interaction, it is not usually
feasible to provide notification and warning to someone before the request needs to be executed,
as is the case with redirection URLs. As such, SSRF is somewhat more difficult to manage at
runtime, and systems should generally refuse to fetch a URI if unsure.
...
center. In such use cases, the client software used by the RO (the caller) and the end user (the
agent) are generally distinct, allowing the AS to differentiate between the agent's corporate
device making the request and the caller's personal device approving the request.
...
hardening and monitoring RS systems (beyond protection with access tokens) are out of the
scope of this document, but the use of GNAP to protect a system does not absolve the RS of
following best practices. GNAP generally considers that a breach can occur and therefore advises
to prefer key-bound tokens whenever possible, which at least limits the impact of access token
...
Returning a key with an access token should be limited to circumstances where both the client
and AS can be verified to be honest and when the trade-off of not using a client instance's own
...
mitigated or advise how the threats relate to GNAP.
 ...
 and either explain how these threats are
...
of these assets can reveal user actions to the host. If the AS presents the logo URI to the RO in a
browser page, the browser will fetch the logo URL from the authorization screen. This fetch will
tell the host of the logo image that someone is accessing an instance of the client software and
requesting access for it. This is particularly problematic when the host of the asset is not the
client software itself, such as when a content delivery network is used.

12.1.2.  Surveillance by the Authorization Server

The role of the AS is to manage the authorization of client instances to protect access to the user's
data. In this role, the AS is by definition aware of each authorization of a client instance by a
user. When the AS shares user information with the client instance, it needs to make sure that it
has the permission from that user to do so.

Additionally, as part of the authorization grant process, the AS may be aware of which RSs the
client intends to use an access token at. However, it is possible to design a system using GNAP in
which this knowledge is not made available to the AS, such as by avoiding the use of the
locations object in the authorization request.

If the AS's implementation of access tokens is such that it requires an RS callback to the AS to
validate them, then the AS will be aware of which RSs are actively in use and by which users and
clients. To avoid this possibility, the AS would need to structure access tokens in such a way that
they can be validated by the RS without notifying the AS that the token is being validated.
...
The AS is expected to store Subject Identifiers for users indefinitely, in order to be able to include
them in the responses to clients. The AS is also expected to store client key identifiers associated
with display information about the client, such as its name and logo.

The client is expected to store its client instance key indefinitely, in order to authenticate to the
AS for the normal functioning of the GNAP flows. Additionally, the client will be temporarily
storing artifacts issued by the AS during a flow, and these artifacts ought to be discarded by the
client when the transaction is complete.

The RS is not required to store any state for its normal operation, as far as its part in
implementing GNAP. Depending on the implementation of access tokens, the RS may need to
cache public keys from the AS in order to validate access tokens.
...
If the RO is different from the end user, there is an opportunity for the end user to cause
unsolicited messages to be sent to the RO if the system prompts the RO for consent when an end
user attempts to access their data.

The format and contents of Subject Identifiers are intentionally not defined by GNAP. If the AS
uses values for Subject Identifiers that are also identifiers for communication channels (e.g., an
email address or phone number), this opens up the possibility for a client to learn this
information when it was not otherwise authorized to access this kind of data about the user.
...
The biggest risk of correlation in GNAP is when an AS returns stable, consistent user identifiers
to multiple different applications. In this case, applications created by different parties would be
able to correlate these user identifiers out of band in order to know which users they have in
common.
...
Unrelated RSs also have an opportunity to correlate users if the AS includes stable user
identifiers in access tokens or in access token introspection responses.

In some cases, an RS may not actually need to be able to identify users (such as an RS providing
access to a company cafeteria menu, which only needs to validate whether the user is a current
employee), so ASes should be thoughtful of when user identifiers are actually necessary to
communicate to RSs for the functioning of the system.
...
is used at more than one AS, this could open up the possibility for multiple unrelated ASes to
correlate client instances. This is especially a problem in the common case where a client
instance is used by a single individual, as it would allow the ASes to correlate that individual
between them. If this is a concern of a client, the client should use distinct keys with each AS.
...
IANA "Named Information Hash Algorithm Registry" <https://www.iana.org/
...
Backman, A., Ed. Richer, J., Ed.
9421 DOI 10.17487/RFC9421
,
rfc9421>
...
M. Sporny "HTTP Message Signatures" RFC
...
Backman, A., Ed. Scurtescu, M.
Event Tokens" RFC 9493 DOI 10.17487/RFC9493
,
,
www.rfc-editor.org/info/rfc9493>
...
P. Jain "Subject Identifiers for Security
, December 2023,
...
Cantor, S., Ed. Kemp, J., Ed. Philpott, R., Ed.
Protocol for the OASIS Security Assertion Markup Language (SAML) V2.0" OASIS
Standard
core-2.0-os.pdf>

,
<https://docs.oasis-open.org/security/saml/v2.0/saml-

, March 2005,
.

E. Maler, Ed. "Assertions and
...
IANA "HTTP Authentication Schemes" <https://www.iana.org/assignments/
...
O. Oueidat "Security Analysis of Attack Surfaces on the Grant

,

Negotiation and Authorization Protocol" Master's thesis, Department of
...
IANA "Grant Negotiation and Authorization Protocol (GNAP)" <https://
www.iana.org/assignments/gnap>
.
...
F. Imbault "Grant Negotiation and Authorization Protocol

Richer, J., Ed.
Resource Server Connections" Work in Progress Internet-Draft, draft-ietf-gnap-
resource-servers-09
...
Helmschmidt, F. "Security Analysis of the Grant Negotiation and
,
Authorization Protocol" Master's thesis, Institute of Information Security,
,
University of Stuggart DOI 10.18419/opus-12203
<http://dx.doi.org/
10.18419/opus-12203>
...
Loreto, S. Saint-Andre, P. Salsano, S.
, and
...
G. Wilkins "Known Issues and Best
...
Lodderstedt, T. Richer, J.
,
Requests" RFC 9396 DOI 10.17487/RFC9396
editor.org/info/rfc9396>
...
B. Campbell "OAuth 2.0 Rich Authorization
<https://www.rfc-
...
Grassi, P. Richer, J. Squire, S. Fenton, J. Nadeau, E. Lefkovitz, N. Danker, J.
Choong, Y. Greene, K.
,
Federation and Assertions" NIST SP 800-63C DOI 10.6028/NIST.SP.800-63c
2017,
<https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.
800-63c.pdf>

M. Theofanos "Digital Identity Guidelines:
, and
,
...
IANA "Subject Identifier Formats" <https://www.iana.org/assignments/
,
...
time. GNAP is designed to allow the end user and the RO to be two different people, but it still
works in the optimized case of them being the same party.
...
OAuth 2.0 requires all clients to be registered at the AS and to use a client_id known to the AS
...
been coopted to mean a number of different things in different protocols, including flags for
turning special behavior on and off and the return of data apart from the access token. The
"resource" indicator (defined in
(as defined in

) expand on the "scope" concept in similar but different ways.

) and Rich Authorization Request (RAR) extensions
...
identifiers for the current user and structured assertions. GNAP makes no assumptions or
demands on the format or contents of the access token, but the RS extension allows a
...
human-centric authorization layer. In addition to this specification, GNAP supports various
...
The AS processes the request and determines that the RO needs to interact. The AS returns the
...
The client instance saves the response and redirects the user to the interaction start mode's
...
The AS retrieves the pending request by looking up the pending grant request associated with the
...
The client instance saves the response and displays the user code visually on its screen along
...
Note that the continuation URI and access token have been rotated since they were used by the
...
The client instance creates a request to the AS, identifying itself with its public key and using
...
The AS processes this and determines that the RO needs to interact. The AS determines that it can
...
The AS reaches out to the RO and prompts them for consent. In this example scenario, the AS has
an application that it can push notifications to for the specified account.
...
The AS retrieves the pending request based on the handle, determines that it has been approved,
and issues an access token.
...
2.0 system and wants to apply them to the new protocol. In OAuth 2.0, the client developer would
put their client_id and scope values as parameters into a redirect request to the authorization
endpoint.
...
The client_id can be used to identify the client instance's keys that it uses for authentication,
...
Conformant AS implementations of these profiles
specified in the profile and
implementations of these profiles
...
push finish method for the Secondary Device profile).
 ...
 implement additional features or profiles. Conformant client

 implement at least the features as specified, except

 implement at least the features as
...
Implementations conformant to the web-based redirection profile of GNAP
...
GNAP makes use of polymorphism within the
...
which allows for optimization and simplification in many common cases.
 ...
 structures used for the protocol.
...
For example, a resource request for a single access token is composed of an object of resource
...
The authors would like to thank the following individuals for their reviews, implementations,
and contributions:
Åke Axeland Aaron Parecki Adam Omar Oueidat Andrii Deinega Annabelle
,
Backman Dick Hardt Dmitri Zagidulin Dmitry Barinov Florian Helmschmidt Francis Pouatcha
,
,
George Fletcher Haardik Haardik Hamid Massaoud Jacky Yuan Joseph Heenan Kathleen
Moriarty Leif Johansson Mike Jones Mike Varley Nat Sakimura Takahiko Kawasaki Takahiro
,
Tsuchiya
, and
...
The authors would also like to thank the GNAP Working Group design team (
Dick Hardt Mike Jones
,
proposals to create the first draft version of this document.

, and the authors), who incorporated elements from the XAuth and XYZ
...
to integrate identity and authentication systems into the core protocol. Both
Dick Hardt
proposals that have been incorporated here. The authors would like to especially thank
Varley
protocol that fed into this standards work.

Mike
 and the team at SecureKey for feedback and development of early versions of the XYZ

 developed the use cases, diagrams, and insights provided in the XYZ and XAuth
...
Finally, the authors want to acknowledge the immense contributions of
 to the
content of this document. We thank him for his insight, input, and hard work, without which
GNAP would not have grown to what it is.
