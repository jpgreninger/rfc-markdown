...
RFC 9193
Sensor Measurement Lists (SenML) Fields for
Indicating Data Value Content-Format
...
binary Data Values, this document speciﬁes a pair of new SenML ﬁelds for indicating the content
...
SenML Data Content-Format Indication
...
open, and a Data Value (with SenML ﬁeld "vd") read from a Near Field Communication (NFC)
reader is sent in a single SenML Pack. The example is given in SenML JSON representation, so the
"vd" (Data Value) ﬁeld is encoded as a base64url string (without padding), as per
[RFC8428]

 can be used to send various kinds
...
Figure 1: SenML Pack with Unidentiﬁed Binary Data

The receiver is expected to know how to interpret the data in the "vd" ﬁeld based on the context,
...
this information in the form of a single unsigned integer. For instance,
...
listed in the "CoAP Content-Formats" registry
of [RFC7252]
...
Section 12.3
. Note that, at the time of writing, the structure of this registry only provides for zero
...
Figure 2: SenML Record with Binary Data Identiﬁed as CBOR

In this example SenML Record, the Data Value contains a string "foo" and a number 42 encoded
in a Concise Binary Object Representation (CBOR)
...
). The Data Value after base64 decoding is shown with CBOR diagnostic

 array. Since the example above uses
...
each speciﬁc Media-Type-Name used,
...
What SenML, as well as the new ﬁelds deﬁned here, guarantees is that a recipient implementation
knows when it needs to be updated to understand these ﬁeld values and the values controlled by
...
A registered label for representations (byte strings) prepared for interchange

[RFC1590] [RFC6838]

, identiﬁed by a Media-Type-Name.
...
, conventionally identiﬁed by the two names separated

A Media-Type-Name, optionally associated with parameters (

Content-Type:
[RFC2045]
HTTP and many other protocols, it is used in a Content-Type header ﬁeld.

, separated from the Media-Type-Name and from each other by a semicolon). In

Section 5 of

, as speciﬁed by Sections 16.6.1 and 18.6 of

A name registered in the "HTTP Content Coding Registry"

Content coding:
parameters]
transformation with semantics further speciﬁed in
. Confusingly, in
HTTP, content coding values are found in a header ﬁeld called "Content-Encoding"; however,
...
identiﬁed by (1) a numeric identiﬁer deﬁned in the "CoAP Content-Formats" registry
[IANA.core-parameters]
number), or (2) a Content-Format-String.

Section 12.3 of [RFC7252]

 (referred to as Content-Format
...
The string representation of a content format; either a Content-Format-
...
When a SenML Record contains a Data Value ﬁeld ("vd"), the Record
Format indication ﬁeld, using label "ct". The value of this ﬁeld is a Content-Format-Spec, i.e., one of
the following:
...
a Content-Format-String containing a Content-Type and zero or more content codings (see
below).

The syntax of this ﬁeld is formally deﬁned in Section 6.

The CoAP Content-Format number provides a simple and eﬃcient way to indicate the type of the
...
codings is also allowed. Both methods use a string value in the "ct" ﬁeld to keep its data type
consistent across uses. When the "ct" ﬁeld contains only digits, it is interpreted as a CoAP Content-
Format number.
...
). For example (using a content coding value of "deﬂate", as deﬁned
as in
):
in

Section 8.4 of [RFC9110]
Section 8.4.1.2 of [RFC9110]
...
speciﬁed, and the "identity" content coding is used -- no encoding transformation is employed.
...
The Base Content-Format ﬁeld, label "bct", provides a default value for the Content-Format ﬁeld
(label "ct") within its range. The range of the base ﬁeld includes the Record containing it, up to
(but not including) the next Record containing a "bct" ﬁeld, if any, or up to the end of the Pack
otherwise. The process of resolving (
adding its value with the label "ct" to all Records in this range that carry a "vd" ﬁeld but do not
already contain a Content-Format ("ct") ﬁeld.

) this base ﬁeld is performed by

Section 4.6 of [RFC8428]

Figure 4 shows a variation of Figure 2 with multiple records, with the "nfc-reader" records
resolving to the base ﬁeld value "60" and the "iris-photo" record overriding this with the "image/
...
The following examples are valid values for the "ct" and "bct" ﬁelds (explanation/comments in
parentheses):
...
"application/json@deﬂate" (JSON Content-Type with "deﬂate" as content coding -- equivalent
to "11050" CoAP Content-Format number)

•

"application/json@deﬂate@aes128gcm" (JSON Content-Type with "deﬂate" followed by
...
This speciﬁcation provides a formal deﬁnition of the syntax of Content-Format-Spec strings using
ABNF notation
adapted from various RFCs

, which contains three new rules and a number of rules collected and

[RFC9110] [RFC6838] [RFC5234] [RFC8866]
...
speciﬁes media types chosen by the attacker may expose vulnerabilities of handlers for these
...
, no CBOR labels nor Eﬃcient XML Interchange (EXI)
...
,
J. Reschke "HTTP Semantics" STD 97 RFC 9110
...
extension and
document with a separate mandatory-to-implement version of the ﬁelds.
Schaad
Tiloca

 asked for clarifying and using the term Content-Format-Spec.
...
, and
 ...
 provided helpful comments at Working Group Last Call.

 for the discussions leading to the design of this
